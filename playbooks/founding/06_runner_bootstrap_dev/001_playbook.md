...
**Prerequisites:**
- Vault items in `ZMB_CD_DEV` and `ZMB_CD_PROD`
...
Two distinct tags: `tag:worker` is this host, `tag:ci` is the ephemeral GitHub Actions node that deploys to it. Without the `tag:ci` → `tag:worker` grant the node advertises host keys but every CI connect fails with `tailnet policy does not permit you to SSH to this node`. A grant to `autogroup:member` does not cover CI — a tagged node carries no user identity, so it never matches a member rule.

Bootstrap the DEV bare-metal worker so CI can deploy the host-resident `agentsfleet-runner` daemon. After step 0 (human provisions server), remaining steps run without human interaction.
...
sudo tailscale up --auth-key "${TS_OAUTH_SECRET}?ephemeral=false&preauthorized=true" --advertise-tags=tag:worker --hostname zombie-dev-worker-ant --ssh
tailscale status
REMOTE
```
