// FleetHelp — commander.Help subclass that preserves agentsfleet's
// existing color scheme when commander renders --help. Wired by
// cli-tree.ts via `program.createHelp = () => new FleetHelp()`
...
export interface FleetHelpOptions {
...
export class FleetHelp extends Help {
  private readonly styleOpts: ResolvedStyleOpts;

  constructor({ stream, env }: FleetHelpOptions = {}) {
    super();
    this.styleOpts = {
...
export function styleTagline(text: string, { stream, env }: FleetHelpOptions = {}): string {
