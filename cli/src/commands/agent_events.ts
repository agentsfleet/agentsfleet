// `agentsfleet events <agent_id>` — newest-first paginated history print.
...
> =>
  Effect.gen(function* () {
    ...
    if (!flags.agentId) {
      return yield* Effect.fail(
        new ValidationError({
          ...
          suggestion:
            "usage: agentsfleet events <agent_id> [--actor=glob] [--since=2h] [--cursor=...] [--limit=N] [--json]",
        }),
      );
    }
    ...
    const path = `${wsAgentEventsPath(wsId, flags.agentId)}?${buildQuery(flags)}`;
    ...
    if (res.next_cursor) {
      yield* output.info(
        ui.dim(`  More: agentsfleet events ${flags.agentId} --cursor=${res.next_cursor}`),
      );
    }
  });
