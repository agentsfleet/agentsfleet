// `agentsfleet logs` — paginated event print for a specific fleet.
// Accepts `<fleet_id>` positional or `--fleet <id>` flag; `--limit`
...
const LOGS_LISTED = "Fleet event log" as const;
...
> =>
  Effect.gen(function* () {
    ...
    const fleetId = yield* requireFleetId(flags.fleetId);
