// Credential-add body resolution — split out of fleet_credential.ts to keep
...
// Compose the credential JSON from the typed custom-endpoint flags. Returns the
// same result-bag shape `parseDataObject` uses so the two body sources converge.
const typedProviderBody = (flags: CredentialAddFlags): ParsedData => {
...
// Resolve the credential `data` object from whichever input form the caller
// used: the typed custom-endpoint flags, or the generic `--data` blob.
export const resolveCredentialBody = (
