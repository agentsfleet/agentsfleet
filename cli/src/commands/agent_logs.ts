// `agentsfleet logs` — paginated event print for a specific agent.
// Accepts `<agent_id>` positional or `--agent <id>` flag; `--limit`
...
import { wsAgentEventsPath } from "../lib/api-paths.ts";
...
const USAGE = "logs requires --agent <id>";
...
const requireAgentId = (
  ...
  Effect.gen(function* () {
    ...
    const check = validateRequiredId(raw, "agent_id");
    if (!check.ok) {
      return yield* Effect.fail(
...
> =>
  Effect.gen(function* () {
    ...
    const agentId = yield* requireAgentId(flags.agentId);
    ...
    const path = `${wsAgentEventsPath(wsId, agentId)}?${qs.toString()}`;
    ...
    if (res.next_cursor) {
      yield* output.info(
        ui.dim(`  More: agentsfleet logs --cursor=${res.next_cursor}`),
      );
    }
  });
