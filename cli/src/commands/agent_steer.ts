...
import { wsAgentMessagesPath } from "../lib/api-paths.ts";
...
const steerTurnEffect = (
  ...
  Effect.gen(function* () {
    ...
    const post = yield* http.request<MessagesResponse>({
      path: wsAgentMessagesPath(wsId, agentId),
      method: "POST",
      body: { message },
      token,
    });
    ...
    const streamOutcome = yield* tailEventStream(wsId, agentId, post.event_id, token, streamGet, signal);
    ...
    if (streamOutcome.kind === STATUS_COMPLETE) {
      ...
      outcome = yield* pollEventTerminal(wsId, agentId, post.event_id, token, signal);
    }
    if (signal?.aborted) {
      return yield* failSteerInterrupted();
    }
    yield* renderOutcome(outcome, post.event_id, agentId);
  });
...
const renderOutcome = (
  ...
  Effect.gen(function* () {
    ...
    if (config.jsonMode) {
      ...
      yield* output.error(
        `event ${eventId} still in flight after ${SSE_FALLBACK_TIMEOUT_SECONDS}s — check: agentsfleet events ${agentId}`,
      );
    }
      ...
      if (outcome.status !== EVENT_STATUS.PROCESSED) {
        return yield* Effect.fail(
          new ConfigError({
            ...
            suggestion: `inspect: agentsfleet events ${agentId}`,
          }),
        );
      }
      return;
    ...
    return yield* Effect.fail(
      new ConfigError({
        ...
        suggestion: `retry, or inspect: agentsfleet events ${agentId}`,
      }),
    );
  });
...
> =>
  Effect.gen(function* () {
    ...
    if (!agentId) {
      return yield* Effect.fail(
        new ValidationError({ detail: "agent_id is required", suggestion: `usage: agentsfleet steer <agent_id> ${MESSAGE_PLACEHOLDER}` }),
      );
    }
        ...
        try: () =>
          runRepl({
            ...
            runTurn: async (line, signal) => {
              const turn = steerTurnEffect(wsId, agentId, line, token, streamGet, signal);
    ...
    if (singleMessage.trim().length === 0) {
      return yield* Effect.fail(
        new ValidationError({
          ...
          suggestion: `usage: agentsfleet steer <agent_id> ${MESSAGE_PLACEHOLDER}`,
        }),
      );
    }

    yield* steerTurnEffect(wsId, agentId, singleMessage, token, streamGet);
  });
