 ...
 * `OPENAI_COMPATIBLE_PROVIDER`): a self-managed credential whose JSON carries
 ...
 * already follows in `constants/billing.ts`). Every reader — the credential-add
 * command, the option validator, and the tests — imports from here (RULE UFS).
 *
 * The credential JSON field keys match the resolver's `S_API_KEY` / `S_BASE_URL`
...
// Credential JSON field names (verbatim with the server-side resolver).
export const CREDENTIAL_FIELD_PROVIDER = "provider" as const;
export const CREDENTIAL_FIELD_API_KEY = "api_key" as const;
export const CREDENTIAL_FIELD_BASE_URL = "base_url" as const;
export const CREDENTIAL_FIELD_MODEL = "model" as const;
