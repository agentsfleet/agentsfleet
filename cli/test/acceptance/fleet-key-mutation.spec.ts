 ...
 *
 * Prefix-scoped: the bound fleet is ACCEPTANCE_RUN_PREFIX-named and cleaned
...
interface FleetKeyRow {
  readonly fleet_key_id?: string;
}
interface FleetKeyListEnvelope {
  readonly items?: ReadonlyArray<FleetKeyRow>;
}

function listIncludesId(envelope: FleetKeyListEnvelope, id: string): boolean {
  const rows = Array.isArray(envelope.items) ? envelope.items : [];
  return rows.some((row) => row.fleet_key_id === id);
}

if (!isLive) {
  describe("fleet-key-mutation.spec.ts", () => {
    it.skip(`requires ${ACCEPTANCE_TARGET_ENV} to be an https URL`, () => {});
  });
} else {
  describe("fleet-key — mint → list → delete round-trip (seeded-credentials session)", () => {
    let apiUrl = "";
    let sessionJwt = "";
    let stateDir = "";
    ...
    beforeAll(async () => {
      ...
      const installed = await installPlatformOpsFleet({ env, runPrefix: ACCEPTANCE_RUN_PREFIX });
    ...
    afterAll(async () => {
      if (mintedKeyId) {
        try { await runFleetctl([CMD_AGENT_KEY, SUB_DELETE, mintedKeyId, FLAG_JSON], { env, stdin: "" }); }
        catch { /* best-effort key cleanup */ }
      }
      try { await cleanWorkspaceFleets(env, { runPrefix: ACCEPTANCE_RUN_PREFIX }); }
    ...
    it("list includes the minted key", async () => {
      ...
      const parsed = JSON.parse(result.stdout.trim()) as FleetKeyListEnvelope;
    ...
    it("delete removes the key and list no longer includes it", async () => {
      ...
      const parsed = JSON.parse(after.stdout.trim()) as FleetKeyListEnvelope;
