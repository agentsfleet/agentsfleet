/**
 * Credential-vault round-trip (live, seeded-credentials session).
 *
 * Walks the workspace credential vault end to end against the live DEV API:
 ...
 *
 * Every credential is prefix-scoped with ACCEPTANCE_RUN_PREFIX; afterAll
 ...
 * the invariant is "none of MY run's credentials remain".
 *
...
const KEY_CREDENTIALS = "credentials" as const;
...
interface CredentialListEnvelope {
...
function listIncludesName(envelope: CredentialListEnvelope, name: string): boolean {
  const rows = Array.isArray(envelope.credentials) ? envelope.credentials : [];
  return rows.some((row) => row.name === name);
}

/** No credential payload value (nor the JWT) may surface in any stream. */
function assertNoCredentialLeak(captured: RunResult, jwt: string): void {
  assertNoSecretLeak(captured, jwt);
  const merged = `${captured.stdout}\n${captured.stderr}`;
  for (const secret of SECRET_VALUES) {
    if (merged.includes(secret)) {
      throw new Error(
        `credential secret material leaked into captured stdout/stderr: ${captured.stdout}\n${captured.stderr}`,
      );
    }
  }
}
...
if (!isLive) {
  describe("credential-vault.spec.ts", () => {
    it.skip(`requires ${ACCEPTANCE_TARGET_ENV} to be an https URL`, () => {});
  });
} else {
  describe("credential-vault — round-trip (seeded-credentials session)", () => {
    let apiUrl = "";
    let sessionJwt = "";
    let stateDir = "";
    ...
    const roundTripName = credentialName("roundtrip");
    ...
    describe("happy-path round-trip", () => {
      it("add stores a named JSON credential", async () => {
        const result = await run([
          CMD_CREDENTIAL, SUB_ADD, roundTripName, FLAG_DATA, credentialPayload(), FLAG_JSON,
        ]);
      ...
      it("list --json contains the stored credential", async () => {
        const result = await run([CMD_CREDENTIAL, SUB_LIST, FLAG_JSON]);
        assert.equal(result.code, 0, `list exited ${result.code}: ${result.stderr}`);
        const parsed = parseJson<CredentialListEnvelope>(result.stdout, SUB_LIST);
        assert.ok(KEY_CREDENTIALS in parsed, `list missing ${KEY_CREDENTIALS}: ${result.stdout}`);
        assert.ok(Array.isArray(parsed.credentials), `${KEY_CREDENTIALS} not an array: ${result.stdout}`);
        assert.ok(
      ...
      it("show --json confirms existence without printing secret bytes", async () => {
        const result = await run([CMD_CREDENTIAL, SUB_SHOW, roundTripName, FLAG_JSON]);
        ...
        // planted secret (assertNoCredentialLeak already covers raw streams).
        for (const secret of SECRET_VALUES) {
      ...
      it("delete removes the credential", async () => {
        const result = await run([CMD_CREDENTIAL, SUB_DELETE, roundTripName, FLAG_JSON]);
      ...
      it("list --json no longer contains the deleted credential", async () => {
        const result = await run([CMD_CREDENTIAL, SUB_LIST, FLAG_JSON]);
        assert.equal(result.code, 0, `list exited ${result.code}: ${result.stderr}`);
        const parsed = parseJson<CredentialListEnvelope>(result.stdout, SUB_LIST);
        assert.ok(
          !listIncludesName(parsed, roundTripName),
          `deleted credential still present: ${result.stdout}`,
        );
      });
    ...
    describe("upsert guard", () => {
      const upsertName = credentialName("upsert");

      afterAll(async () => {
        await run([CMD_CREDENTIAL, SUB_DELETE, upsertName, FLAG_JSON]).catch(() => undefined);
      });

      it("first add stores; re-add without --force is skipped; --force overwrites", async () => {
        const first = await run([
          CMD_CREDENTIAL, SUB_ADD, upsertName, FLAG_DATA, credentialPayload(), FLAG_JSON,
        ]);
        ...
        const forced = await run([
          CMD_CREDENTIAL, SUB_ADD, upsertName, FLAG_DATA, credentialPayload(), FLAG_FORCE, FLAG_JSON,
        ]);
    ...
    describe("negative edges", () => {
      it("show of an unknown name reports exists:false and exits non-zero", async () => {
        const ghost = credentialName(UNKNOWN_NAME_SUFFIX);
        const result = await run([CMD_CREDENTIAL, SUB_SHOW, ghost, FLAG_JSON]);
      ...
      it("add without --data is rejected client-side (no network)", async () => {
        await runUnroutable([CMD_CREDENTIAL, SUB_ADD, credentialName("nodata"), FLAG_JSON]);
      });
    ...
    // residual credentials from other runs, so the invariant is "none of MY
    ...
    describe("post-teardown emptiness (prefix-scoped)", () => {
      ...
      it("list --json: no credential matches ACCEPTANCE_RUN_PREFIX", async () => {
        const result = await run([CMD_CREDENTIAL, SUB_LIST, FLAG_JSON]);
        assert.equal(result.code, 0, `list exited ${result.code}: ${result.stderr}`);
        const parsed = parseJson<CredentialListEnvelope>(result.stdout, "list-final");
        const rows = Array.isArray(parsed.credentials) ? parsed.credentials : [];
        ...
        assert.equal(
          mine.length,
          0,
          `expected zero credentials starting with ${ACCEPTANCE_RUN_PREFIX}; got ${JSON.stringify(mine)}`,
        );
      });
    });
  });
