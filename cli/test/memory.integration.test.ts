...
import { bufferStream, withAuthedStateDir, withFreshStateDir, type TestStream, stateDirEnv } from "./helpers-cli-state.ts";
...
const MEMORIES_PATH = `/v1/workspaces/${WS_ID}/fleets/${FLEET_ID}/memories`;
...
describe("memory — error shapes", () => {
  test("test_memory_unknown_fleet_error_shape: UZ-MEM-002 + fleet-listing suggestion, nonzero exit", async () => {
    await authedScope(async () => {
      const routes: MockRoutes = {
        [MEMORIES_ROUTE]: () =>
          jsonResponse(404, {
            error: { code: "UZ-MEM-002", message: "fleet not found" },
  ...
  test("missing auth: fresh state dir → existing auth-error path with login suggestion, nonzero exit", async () => {
    await withFreshStateDir(async () => {
      ...
      const code = await runCli(
        ["memory", "list", "--fleet", FLEET_ID],
        { stdout: out.stream, stderr: err.stream, env: { ...stateDirEnv(), AGENTSFLEET_API_URL: "http://127.0.0.1:9" } },
      );
      expect(code).not.toBe(0);
