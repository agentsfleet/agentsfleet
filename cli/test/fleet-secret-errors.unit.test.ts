...
describe("requireName validation (lines 102-106)", () => {
  test("secretAddEffectFromFlags fails with ValidationError when name is undefined", async () => {
    const captured: string[] = [];
    const exit = await baseProvide(
      secretAddEffectFromFlags({ name: undefined, data: '{"x":1}' }),
      captured,
    );
    ...
    if (Exit.isFailure(exit)) {
      ...
      if (err instanceof ValidationError) {
        expect(err.detail).toMatch(/secret name is required/i);
      }
    }
  });

  test("secretShowEffectFromName fails with ValidationError when name is undefined", async () => {
    const captured: string[] = [];
    const exit = await baseProvide(
      secretShowEffectFromName(undefined),
      captured,
    );
    expect(Exit.isFailure(exit)).toBe(true);
    if (Exit.isFailure(exit)) {
  ...
  test("secretDeleteEffectFromName fails with ValidationError when name is undefined", async () => {
    const captured: string[] = [];
    const exit = await baseProvide(
      secretDeleteEffectFromName(undefined),
      captured,
    );
    expect(Exit.isFailure(exit)).toBe(true);
    if (Exit.isFailure(exit)) {
  ...
  test("secretAddEffectFromFlags fails with ValidationError when name is empty string", async () => {
    const captured: string[] = [];
    const exit = await baseProvide(
      secretAddEffectFromFlags({ name: "", data: '{"x":1}' }),
      captured,
    );
    expect(Exit.isFailure(exit)).toBe(true);
    if (Exit.isFailure(exit)) {
...
describe("resolveDataSource @- sentinel paths (lines 122-131)", () => {
  test("fails with ValidationError when --data=@- and stdin is empty", async () => {
    ...
    try {
      const exit = await baseProvide(
        secretAddEffectFromFlags({ name: "mykey", data: "@-" }),
        captured,
      );
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
...
// ---------------------------------------------------------------------------
// secretListEffect — covers the non-empty secrets loop (lines 260-263)
// via direct Effect invocation (the integration test covers the empty path).
// ---------------------------------------------------------------------------

describe("secretListEffect non-empty list (lines 260-263)", () => {
  test("prints each secret row in human mode", async () => {
    const captured: string[] = [];
    ...
    const exit = await baseProvide(secretListEffect, captured, false, http);
    expect(Exit.isSuccess(exit)).toBe(true);
