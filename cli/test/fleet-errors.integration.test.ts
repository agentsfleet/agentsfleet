// Error-path integration tests for fleet.ts stop / resume / kill / delete.
// Covers the requireFleetId validation branch (invalid uuidv7 format)
...
const authedScope = <T>(fn: (stateDir: string) => Promise<T>): Promise<T> =>
  withAuthedStateDir({ workspaceId: WS_ID, sessionId: "sess_fleet_err" }, fn);

// ---------------------------------------------------------------------------
// requireFleetId — invalid format branch (lines 66-69 in fleet.ts)
// ---------------------------------------------------------------------------

describe("fleet id validation — invalid format", () => {
