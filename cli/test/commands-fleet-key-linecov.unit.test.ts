// Line-coverage backfill for commands/fleet_key.ts. The integration suite
// (fleet_key.integration.test.ts) drives the happy non-JSON paths end-to-end
...
import {
  ...
  type FleetAddArgs,
} from "../src/commands/fleet_key.ts";
...
const NO_AGENTS_MSG = "no fleet keys found";
...
const baseAddArgs: FleetAddArgs = {
  workspaceId: WS_ID,
  fleetId: WS_ID,
  name: "langgraph-bot",
...
describe("fleet add JSON mode", () => {
  test("prints the raw key response as JSON and skips the human table", async () => {
    const cap = newCapture();
    const response = { fleet_key_id: "fleet_key_001", key: "agt_raw", created_at: null };
    const exit = await Effect.runPromiseExit(
      provideAll(fleetAddEffectFromArgs(baseAddArgs), cap, {
        jsonMode: true,
        response,
      }),
    );
...
describe("fleet list JSON mode", () => {
  test("prints the list response as JSON and skips the human table", async () => {
    const cap = newCapture();
    const response = { items: [{ fleet_key_id: "fleet_a", name: "bot" }] };
    const exit = await Effect.runPromiseExit(
      provideAll(fleetListEffectFromArgs(WS_ID), cap, {
        jsonMode: true,
        response,
      }),
    );
...
describe("fleet list empty result", () => {
  test("emits the no-fleets notice instead of an empty table", async () => {
    const cap = newCapture();
    const exit = await Effect.runPromiseExit(
      provideAll(fleetListEffectFromArgs(WS_ID), cap, { response: { items: [] } }),
    );
    expect(Exit.isSuccess(exit)).toBe(true);
  ...
  test("treats a missing items field the same as an empty list", async () => {
    const cap = newCapture();
    const exit = await Effect.runPromiseExit(
      provideAll(fleetListEffectFromArgs(WS_ID), cap, { response: {} }),
    );
    expect(Exit.isSuccess(exit)).toBe(true);
    expect(cap.info).toContain(NO_AGENTS_MSG);
  });
...
describe("workspace context resolution without an explicit flag", () => {
  test("falls back to the active workspace when no --workspace is passed", async () => {
    ...
    const exit = await Effect.runPromiseExit(
      provideAll(fleetListEffectFromArgs(undefined), cap, {
        response: { items: [] },
...
describe("requireValidId rejection", () => {
  test("fleet delete rejects a malformed workspace id with a uuidv7 suggestion", async () => {
    const cap = newCapture();
    // The override workspace id is not a uuidv7 → requireValidId fails before
    // any HTTP request is issued.
    const exit = await Effect.runPromiseExit(
      provideAll(
        fleetDeleteEffectFromArgs(NOT_A_UUID, WS_ID, undefined),
        cap,
      ),
    );
    expect(Exit.isFailure(exit)).toBe(true);
...
describe("fleet delete JSON mode", () => {
  test("prints a deleted payload and skips the human success line", async () => {
    const cap = newCapture();
    const exit = await Effect.runPromiseExit(
      provideAll(fleetDeleteEffectFromArgs(WS_ID, WS_ID, undefined), cap, {
        jsonMode: true,
      }),
    );
    expect(Exit.isSuccess(exit)).toBe(true);
    expect(cap.json).toEqual([{ deleted: true, fleet_key_id: WS_ID }]);
    expect(cap.success).toEqual([]);
  });
});
