...
// requireAgentId, the cursor query-string append, and the JSON-mode early
...
import { logsEffectFromFlags } from "../src/commands/agent_logs.ts";
import { CliConfig } from "../src/services/config.ts";
import { HttpClient } from "../src/services/http-client.ts";
import { Output } from "../src/services/output.ts";
import { Workspaces } from "../src/services/workspaces.ts";
import { Credentials } from "../src/services/credentials.ts";
import { wsAgentEventsPath } from "../src/lib/api-paths.ts";
...
describe("logsEffectFromFlags — id validation guard", () => {
  test("fails with ValidationError when agentId is a malformed (non-uuidv7) id", async () => {
    const httpSpy: HttpSpy = { requestedPath: null };
    const outSpy: OutputSpy = { jsonPayloads: [], infoLines: [] };
    const exit = await runWith(
      logsEffectFromFlags({ agentId: INVALID_AGENTSFLEET_ID }),
      {
    ...
    if (Exit.isFailure(exit)) {
      ...
      if (err instanceof ValidationError) {
        expect(err.detail).toMatch(/invalid agent_id/i);
        expect(err.suggestion).toMatch(/logs requires --agent/);
      }
    }
...
describe("logsEffectFromFlags — JSON mode", () => {
  test("prints the raw response as JSON and returns early without an event stream", async () => {
    ...
    const exit = await runWith(
      logsEffectFromFlags({ agentId: VALID_AGENTSFLEET_ID }),
      {
...
describe("logsEffectFromFlags — cursor pagination", () => {
  test("appends a non-empty cursor to the events query string", async () => {
    ...
    const outSpy: OutputSpy = { jsonPayloads: [], infoLines: [] };
    const exit = await runWith(
      logsEffectFromFlags({ agentId: VALID_AGENTSFLEET_ID, cursor: CURSOR_TOKEN }),
      {
        config: configLayer(false),
        output: outputLayer(outSpy),
    ...
    expect(Exit.isSuccess(exit)).toBe(true);
    const base = wsAgentEventsPath(WORKSPACE_ID, VALID_AGENTSFLEET_ID);
