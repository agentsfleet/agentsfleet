// Parser-level unit tests for the agent subtree of buildProgram —
...
// covers the top-level + non-agent tree.
...
test("install accepts --from <path>", async () => {
  ...
  expect(calls[0]?.name).toBe("agent.install");
  expect(calls[0]?.frame.parsed.options.from).toBe("/tmp/skill");
});

test("agent update <id> accepts --from <path>", async () => {
  const { handlers, calls } = makeSpyTree();
  await dispatch(["agent", "update", VALID_ID, "--from", "/tmp/skill"], handlers);
  expect(calls[0]?.name).toBe("agent.update");
...
test("status [agent_id] dispatches with no positional (workspace-wide)", async () => {
  const { handlers, calls } = makeSpyTree();
  await dispatch(["status"], handlers);
  expect(calls[0]?.name).toBe("agent.status");
  expect(calls[0]?.frame.parsed.positionals).toHaveLength(0);
});

test("status <agent_id> dispatches with positional", async () => {
  const { handlers, calls } = makeSpyTree();
...
test("stop / resume / kill / delete each dispatch with required positional", async () => {
  for (const cmd of ["stop", "resume", "kill", "delete"]) {
    ...
    expect(calls[0]?.name).toBe(`agent.${cmd}`);
    expect(calls[0]?.frame.parsed.positionals[0]).toBe(VALID_ID);
  }
});

test("logs accepts --agent / --limit / --cursor", async () => {
  const { handlers, calls } = makeSpyTree();
  await dispatch([
    "logs",
    "--agent", VALID_ID,
  ...
  expect(calls[0]?.name).toBe("agent.logs");
...
test("events <id> accepts --actor / --since / --cursor / --limit", async () => {
  ...
  expect(calls[0]?.name).toBe("agent.events");
...
test("steer <id> <message> dispatches with two positionals", async () => {
  ...
  expect(calls[0]?.name).toBe("agent.steer");
...
test("credential add <name> accepts --data / --force", async () => {
  ...
  expect(calls[0]?.name).toBe("agent.credential.add");
...
test("credential show / list / delete each dispatch with the right shape", async () => {
  {
    ...
    expect(calls[0]?.name).toBe("agent.credential.show");
    expect(calls[0]?.frame.parsed.positionals[0]).toBe("openai");
  }
  {
    const { handlers, calls } = makeSpyTree();
    await dispatch(["credential", "list"], handlers);
    expect(calls[0]?.name).toBe("agent.credential.list");
  }
  {
    const { handlers, calls } = makeSpyTree();
    await dispatch(["credential", "delete", "openai"], handlers);
    expect(calls[0]?.name).toBe("agent.credential.delete");
    expect(calls[0]?.frame.parsed.positionals[0]).toBe("openai");
  }
});
