...
| `CliConfig` | `--api`/`--json`/`--no-open` + `AGENTSFLEET_API_URL` resolution |
...
- `AGENTSFLEET_TELEMETRY_DISABLED=1` — kill switch
- `DO_NOT_TRACK=1` — industry-standard signal (same effect)
- `~/.config/agentsfleet/telemetry.json` `consent: "denied"` — persistent
...
# AGENTSFLEET_TELEMETRY_DISABLED=1 and never need the override.
```

Tests in `test/` set `AGENTSFLEET_TELEMETRY_DISABLED=1` in `beforeAll` so they hit the analytics layer's noop branch without any network call. The dev-key override only matters when the suite is configured to make real PostHog requests (acceptance against `AGENTSFLEET_ACCEPTANCE_TARGET=https://...`).
