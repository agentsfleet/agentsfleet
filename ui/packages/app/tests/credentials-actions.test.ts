...
// ── Shared mocks ───────────────────────────────────────────────────────────
// These credential server actions are thin forwarders: each wraps
...
const { withTokenMock, createCredentialMock, deleteCredentialMock } = vi.hoisted(() => ({
  withTokenMock: vi.fn(),
...
describe("credential server actions — thin withToken forwarders", () => {
  it("createCredentialAction threads the token last and returns the wrapped result", async () => {
    createCredentialMock.mockResolvedValueOnce({ name: "openai" });
    const body = { name: "openai", data: { api_key: "sk-xxx" } };
    const r = await createCredentialAction("ws-1", body);
    expect(r).toEqual({ ok: true, data: { name: "openai" } });
    expect(createCredentialMock).toHaveBeenCalledWith("ws-1", body, "tok");
    expect(deleteCredentialMock).not.toHaveBeenCalled();
  });

  it("deleteCredentialAction threads the token last and returns the wrapped result", async () => {
    deleteCredentialMock.mockResolvedValueOnce(undefined);
    const r = await deleteCredentialAction("ws-2", "stripe");
    expect(r).toEqual({ ok: true, data: undefined });
    expect(deleteCredentialMock).toHaveBeenCalledWith("ws-2", "stripe", "tok");
    expect(createCredentialMock).not.toHaveBeenCalled();
  });

  it("createCredentialAction propagates a failure result from withToken untouched", async () => {
    ...
    const r = await createCredentialAction("ws-1", { name: "n", data: {} });
