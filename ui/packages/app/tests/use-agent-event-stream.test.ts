...
import { __resetRegistryForTests } from "@/lib/streaming/agent-stream-registry";
...
function mount(initial: EventRow[] = []) {
  return renderHook(() => useAgentEventStream(WS, ZID, initial));
}
...
describe("useAgentEventStream", () => {
  ...
  it("opens an EventSource against the same-origin stream URL on mount", () => {
    ...
    expect(FakeEventSource.instances[0]!.url).toBe(
      "/backend/v1/workspaces/ws_1/agents/zomb_1/events/stream",
    );
  });
