/**
 * multi-agent.spec.ts — pulse-cap contract.
 *
 * Seeds 6 agents in the fixture user's workspace (more than `PULSE_CAP=5`
 * in `AgentsList.tsx`). Asserts the listing renders all 6 rows but only
 ...
 * agents. A future change that lifts the cap regresses this assertion.
 *
 * Note on status: seedAgent creates rows that start in `active` status
...
test.describe("multi-agent pulse cap", () => {
  test(
    `${SEED_COUNT} active agents → exactly ${PULSE_CAP} pulse + 1 static glow + header counter`,
    async ({ page }) => {
      ...
      for (let i = 0; i < SEED_COUNT; i++) {
        await seedAgent(FIXTURE_KEY.regular, ws, { name: `pulse-${tag}-${i}` });
      }
      ...
      const pulsing = page.locator('a[href^="/agents/"][data-state="live"] [data-live]');
