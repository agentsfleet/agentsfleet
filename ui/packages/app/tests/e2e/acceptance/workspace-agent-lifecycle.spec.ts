/**
 * workspace-agent-lifecycle.spec.ts — multi-workspace + agent full life.
 *
 ...
 *   3. Install a agent via /agents/new (UI).
 *   4. Kill it (UI confirm dialog).
 *   5. Assert /agents marks the row failed (terminal).
 *
...
test.describe("multi-workspace + agent lifecycle", () => {
  test.setTimeout(FLOW_TIMEOUT_MS);

  test("operator switches workspace, installs, then kills the agent", async ({ page }) => {
    ...
    // fix lands, this branch graduates to clicking "Delete agent" →
    // confirming the dialog → asserting redirect to /agents and row
  ...
  test.afterEach(async () => {
    const secondary = await ensureSecondWorkspace(FIXTURE_KEY.regular, SECONDARY_NAME);
    await cleanWorkspaceAgents(FIXTURE_KEY.regular, secondary.id);
  });
});
