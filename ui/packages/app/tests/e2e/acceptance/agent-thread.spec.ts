/**
 * agent-thread.spec.ts — operator-facing chat surface renders against
 ...
 * /agents/[id], the thread surface mounts, the live-activity panel is
...
test.describe("agent thread surface", () => {
  test("renders the live-activity panel + composer for an authenticated user", async ({
    page,
  }) => {
    await signInAs(page, FIXTURE_KEY.regular);

    // Pick any existing fixture agent or seed a fresh one. Earlier specs
    // in this run may have left rows; we don't depend on a specific
    // identity, only that some agent exists for this workspace.
    const workspaceId = await getDefaultWorkspaceId(FIXTURE_KEY.regular);
    const existing = await listAgents(FIXTURE_KEY.regular, workspaceId);
    const agent =
      existing[0] ??
      (await seedAgent(FIXTURE_KEY.regular, workspaceId, {
    ...
    await expect(
      page.getByRole("heading", { name: new RegExp(agent.name, "i") }).first(),
    ).toBeVisible();
  ...
  test("survives a /dashboard ↔ /agents/[id] round-trip without unmounting the surface", async ({
    page,
  }) => {
    // Pins the registry behavior end-to-end: navigating away and back
    // to the same agent within the registry's idle window must NOT lose
