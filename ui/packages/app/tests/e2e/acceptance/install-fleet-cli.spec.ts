/**
 * install-fleet-cli.spec.ts — canonical install path.
 *
...
test.describe("install-fleet-cli", () => {
  test("agentsfleet install lands a row on /w/[workspaceId]/fleets with live state", async ({ page }) => {
    ...
    const result = await spawnFleetctl(["--json", "install", "--from", bundleDir], {
    ...
    const payload = JSON.parse(result.stdout) as { fleet_id: string; status: string };
