...
vi.mock("lucide-react", () => ({
  ActivityIcon: () =>
    React.createElement("svg", { "data-icon": "ActivityIcon" }),
...
import {
  CHARGE_TYPE,
  PROVIDER_MODE,
  type TenantBillingChargesResponse,
} from "@/lib/types";
...
describe("BillingUsageTab (test_billing_usage_ledger_and_empty)", () => {
  ...
  it("wires DataTable's stickyHeader on the usage ledger (its real consumer)", () => {
    ...
    const region = screen.getByRole("region", {
      name: /usage history, scrollable/i,
    });
  ...
  it("sorts every usage data column from its header arrow", () => {
      ...
      React.createElement(BillingUsageTab, {
        initialCharges: [
          ...
          charge({
            ...
            credit_deducted_nanos: 5_000_000,
          }),
        ],
        initialCursor: null,
        pageSize: 25,
    ...
    for (const name of ["Date", "Fleet and model", "Activity", "Amount"]) {
      ...
      if (name === "Amount")
        expect(screen.getAllByRole("row")[1]?.textContent).toContain("−$0.005");
    }
  });

  it("shows global table controls when the ledger fits one page", () => {
    render(
      React.createElement(BillingUsageTab, {
        initialCharges: [charge()],
        initialCursor: null,
  ...
  it("walks back down the trail to the first page", async () => {
    searchParamsRef.current = new URLSearchParams("c=tok_page2&cps=25");
    render(
      React.createElement(BillingUsageTab, {
        initialCharges: [charge()],
        initialCursor: null,
  ...
  it("changes rows per page through the URL and resets the cursor trail", async () => {
    searchParamsRef.current = new URLSearchParams("c=tok_page2&cps=25");
    render(
      React.createElement(BillingUsageTab, {
        initialCharges: [charge()],
    ...
    await waitFor(() => expect(routerPushMock).toHaveBeenCalled());
    const query = String(routerPushMock.mock.calls[0]?.[0]);
    expect(query).toContain("ps=50");
