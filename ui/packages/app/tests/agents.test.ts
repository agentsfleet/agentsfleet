...
// ── AgentConfig — delete flow ─────────────────────────────────────────────

describe("AgentConfig interactions", () => {
  it("delete flow: confirm dialog → DELETE call → push to /agents", async () => {
    fetchMock.mockResolvedValue({ ok: true, status: 204 });
    const { default: AgentConfig } = await import(
      "../app/(dashboard)/agents/[id]/components/AgentConfig"
    );
    const user = userEvent.setup({ delay: null });
    render(
      React.createElement(AgentConfig, {
        workspaceId: "ws_1",
        agentId: "zom_1",
        agentName: "platform-ops",
      }),
    );
    ...
    // InstallAgentForm hit). /agents is `force-dynamic` so refresh isn't
    // needed.
