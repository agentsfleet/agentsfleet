  ...
  default: ({
    ...
    open
      ? React.createElement(
          ...
          React.createElement(
            "button",
            { type: "button", onClick: () => onSubmit("first"), "data-testid": "first-submit" },
            "Submit",
          ),
          React.createElement(
            "button",
...
describe("useWorkspaceCreation", () => {
  it("requires the shared provider", async () => {
    const { useWorkspaceCreation } = await import(
      "../components/layout/WorkspaceCreationProvider"
    );
    expect(() => renderHook(() => useWorkspaceCreation({
      onSuccess: vi.fn(),
  ...
  it("ignores a rejected action after the provider unmounts", async () => {
    ...
    createWorkspaceActionMock.mockImplementationOnce(
      () => new Promise((_, rejectAction) => { reject = rejectAction; }),
    );
    const { useWorkspaceCreation, WorkspaceCreationProvider } = await import(
      "../components/layout/WorkspaceCreationProvider"
    );
    const onSuccess = vi.fn();
    const hook = renderHook(
      () => useWorkspaceCreation({ onSuccess }),
      { wrapper: WorkspaceCreationProvider },
    );
    act(() => { void hook.result.current.create(); });
  ...
  it("handles completion after the Strict Mode effect replay", async () => {
    ...
    act(() => { void hook.result.current.create(); });
    await act(async () => {
      release({ ok: true, data: { workspace_id: "ws_strict", name: "strict" } });
    });

    expect(onSuccess).toHaveBeenCalledWith({ workspace_id: "ws_strict", name: "strict" });
  });
...
describe("NoWorkspaceEmptyState", () => {
  ...
  it("navigates after creating the first workspace while attached", async () => {
    ...
    await waitFor(() => expect(routerPush).toHaveBeenCalledWith("/w/ws_first/fleets"));
  ...
  it("stays on the empty root when first-workspace creation finishes after Hide", async () => {
    ...
    await userEvent.click(screen.getByTestId("first-submit"));
    await waitFor(() =>
      expect(screen.getByTestId("create-first-workspace").getAttribute("aria-disabled")).toBe("true"),
    );
    await userEvent.click(screen.getByTestId("first-hide"));
    expect(document.activeElement).toBe(screen.getByTestId("create-first-workspace"));
    expect(screen.getByTestId("create-first-workspace").getAttribute("aria-disabled")).toBe("true");
    await userEvent.click(screen.getByTestId("create-first-workspace"));
    expect(screen.queryByTestId("create-dialog-open")).toBeNull();
    await act(async () => {
