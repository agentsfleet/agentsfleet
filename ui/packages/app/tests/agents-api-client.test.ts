...
// Pure API-client tests — these exercise lib/api/agents + lib/api/tenant_billing
// against a stubbed fetch, with no React/component mocks. The component and route
// tests for this domain live in agents-routes / agents-components / agents-install.
...
describe("lib/api/agents", () => {
  it("listAgents sends GET with bearer and parses the envelope", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      status: 200,
      json: async () => ({ items: [{ id: "zom_1" }], total: 1, next_cursor: null }),
    });
    const mod = await import("../lib/api/agents");
    const res = await mod.listAgents("ws_1", "tkn");
  ...
  it("installAgent sends POST body and returns the created agent", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      status: 201,
      json: async () => ({ agent_id: "zom_2", status: "active" }),
    });
    const mod = await import("../lib/api/agents");
    const body = {
      trigger_markdown:
    ...
    const res = await mod.installAgent("ws_1", body, "tkn");
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining("/v1/workspaces/ws_1/agents"),
      expect.objectContaining({
        method: "POST",
    ...
    expect(res.agent_id).toBe("zom_2");
  });

  it("installAgent surfaces API error status + code", async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 409,
      statusText: "Conflict",
  ...
  it("installAgent falls back to statusText when body is unparseable", async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 500,
  ...
  it("deleteAgent sends DELETE and returns void on 204", async () => {
    fetchMock.mockResolvedValue({ ok: true, status: 204 });
    const mod = await import("../lib/api/agents");
    const res = await mod.deleteAgent("ws_1", "zom_2", "tkn");
    expect(res).toBeUndefined();
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining("/v1/workspaces/ws_1/agents/zom_2"),
