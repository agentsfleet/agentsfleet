...
const { readPlatformAdminClaimMock, withTokenMock, listRunnersMock, createRunnerMock } = vi.hoisted(() => ({
...
vi.mock("@/lib/api/runners", () => ({ listRunners: listRunnersMock, createRunner: createRunnerMock }));

import { listRunnersAction, createRunnerAction } from "@/app/(dashboard)/admin/runners/actions";
