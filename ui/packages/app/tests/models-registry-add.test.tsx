...
describe("AddModelEntryDialog — known provider, new key", () => {
  ...
  it("enables Save for a local runtime with no key, and stores a body carrying no api_key", async () => {
    // A model server on the operator's own box authenticates nobody, so the
    // form must not demand a key it cannot have — the server exempts the same
    // set (`requiresApiKey`), and disabling Save here made operators type a
    // placeholder to satisfy the dashboard alone.
    const { user } = await renderDialog();
    const dialog = screen.getByRole("dialog");
    const save = () => within(dialog).getByRole("button", { name: /^save$/i });

    await user.type(within(dialog).getByLabelText(/^name$/i), "my-ollama");
    await user.type(within(dialog).getByLabelText(/^provider$/i), "ollama");
    // The model picker degrades to free text for a local runtime: the served id
    // is whatever was loaded, and the catalogue holds only a sentinel row.
    await user.type(within(dialog).getByLabelText(/^model$/i), "llama-3.3-70b-my-finetune");
    expect(save().hasAttribute("disabled")).toBe(false);
    // The key field announces itself as optional rather than merely accepting a blank.
    ...
    expect("api_key" in body.data).toBe(false);
    expect("base_url" in body.data).toBe(false);
  });

  it("keeps the model field free text for a local runtime whose floor row IS catalogued", async () => {
    // Production seeds one activation-floor row per local runtime, so the
    // catalogue is NOT empty for `ollama` — it holds the sentinel id `local`.
    // Without the free-text carve-out the picker becomes a constrained Select
    // offering that sentinel as the only model, which is not a model anyone
    // serves. Driving this with an empty catalogue would pass either way.
    catalogueState.models = [
      { id: "local", provider: "ollama", context_cap_tokens: 128000, input_nanos_per_mtok: 1, cached_input_nanos_per_mtok: 1, output_nanos_per_mtok: 1 },
    ];
    const { user } = await renderDialog();
    const dialog = screen.getByRole("dialog");

    await user.type(within(dialog).getByLabelText(/^name$/i), "my-ollama");
    // A populated catalogue makes Provider a Select, so pick rather than type.
    await user.click(within(dialog).getByLabelText(/^provider$/i));
    await user.click(await screen.findByRole("option", { name: /ollama/i }));
    const modelField = within(dialog).getByLabelText(/^model$/i);
    // A free-text input, not a combobox — and the sentinel is offered nowhere.
    expect(modelField.tagName).toBe("INPUT");
    expect(within(dialog).queryByRole("option", { name: "local" })).toBeNull();

    await user.type(modelField, "llama-3.3-70b-my-finetune");
    await user.click(within(dialog).getByRole("button", { name: /^save$/i }));
    await waitFor(() => expect(createSecretActionMock).toHaveBeenCalled());
    const [, body] = createSecretActionMock.mock.calls[0] as [string, { data: Record<string, unknown> }];
    expect(body.data.provider).toBe("ollama");
  ...
  it("a hosted provider with catalogue rows still gets a constrained picker", async () => {
    // The mirror negative: the free-text carve-out is scoped to local runtimes,
    // so a catalogued hosted provider keeps the Select that stops a typo'd model
    // from reaching PUT and 400ing.
    catalogueState.models = [
      { id: "claude-sonnet-5", provider: "anthropic", context_cap_tokens: 200000, input_nanos_per_mtok: 0, cached_input_nanos_per_mtok: 0, output_nanos_per_mtok: 0 },
    ];
    const { user } = await renderDialog();
    ...
    expect(within(dialog).getByLabelText(/^model$/i).tagName).not.toBe("INPUT");
  });

  it("replacing a held local-runtime credential also omits api_key", async () => {
    // The REPLACE arm is a separate call site from create, and it is the one a
    // re-save takes. Before this change it posted `api_key: ""` — a blank string
    // the vault stores and then reports as a key that is present.
    const ollamaSecret: Secret = {
      kind: "provider_key",
      name: "my-ollama",
      provider: "ollama",
      created_at: 1_777_507_200_000,
    };
    const { user } = await renderDialog([ollamaSecret]);
    const dialog = screen.getByRole("dialog");
    ...
    await waitFor(() => expect(replaceSecretActionMock).toHaveBeenCalled());
    expect(createSecretActionMock).not.toHaveBeenCalled();
    const [, , body] = replaceSecretActionMock.mock.calls[0] as [string, string, Record<string, unknown>];
    expect(body).toEqual({ provider: "ollama" });
  });

  it("still disables Save for a near-miss provider name that only looks local", async () => {
    // The exemption is exact-match on the server; a mirror that fuzzy-matched
    // would enable a keyless submit the API then refuses with UZ-PROVIDER-003.
    const { user } = await renderDialog();
    const dialog = screen.getByRole("dialog");

    await user.type(within(dialog).getByLabelText(/^name$/i), "not-really-local");
    await user.type(within(dialog).getByLabelText(/^provider$/i), "Ollama");
    await user.type(within(dialog).getByLabelText(/^model$/i), "llama-3.3-70b");
