...
// Mirrors the FakeEventSource pattern in tests/use-agent-event-stream.test.ts.
...
describe("agent-stream-registry — subscribe lifecycle", () => {
  it("opens a single EventSource per agentId regardless of subscriber count", () => {
...
describe("agent-stream-registry — server-rendered seed", () => {
...
describe("agent-stream-registry — refcount + idle release", () => {
  ...
  it("survives a same-agent revisit within the idle window — no new EventSource", () => {
    // Same-agent /dashboard ↔ /agents/[id] round-trip is the load-bearing
  ...
  it("opens a fresh EventSource on cross-agent subscription", () => {
    const a = subscribe(WS, Z_A, NO_SEED, () => {});
...
describe("agent-stream-registry — optimistic mutations", () => {
...
describe("agent-stream-registry — mutation edges", () => {
  it("reconcileOptimistic is a no-op for a agent with no active subscription", () => {
    reconcileOptimistic("never_subscribed", "temp_x", "evt_x");
    expect(getSnapshot("never_subscribed").events).toHaveLength(0);
  });

  it("markOptimisticFailed is a no-op for a agent with no active subscription", () => {
