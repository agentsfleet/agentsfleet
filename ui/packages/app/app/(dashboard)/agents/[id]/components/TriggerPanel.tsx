...
export default function TriggerPanel({
  ...
  if (list.length === 0) {
    return <EmptyTriggers agentId={agentId} />;
  }
          ...
          return (
            <AccordionItem key={key} value={key} className="px-4 last:border-b-0">
              ...
              <AccordionContent className="pb-4">
                <TriggerBody trigger={t} agentId={agentId} lastDeliveryAt={last} />
              </AccordionContent>
            </AccordionItem>
          );
        })}
...
function EmptyTriggers({ agentId }: { agentId: string }) {
  return (
    <Card className="bg-card" data-testid="trigger-panel-empty">
      <CardContent className="flex flex-col gap-3 py-4">
        ...
        <CopyUrlFallback url={webhookUrlFor(agentId)} source="none" />
      </CardContent>
    </Card>
  );
}
