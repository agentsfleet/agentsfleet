...
const eventsApi = vi.hoisted(() => ({ listAgentEvents: vi.fn() }));
...
describe("resolveLastDeliveries", () => {
  beforeEach(() => {
    eventsApi.listAgentEvents.mockReset();
  });
  ...
  it("fans out one events call per webhook + cron trigger and returns created_at", async () => {
    eventsApi.listAgentEvents.mockImplementation(async (_w, _z, _t, opts) => {
  ...
  it("records api keys as absent even when mixed with delivering triggers", async () => {
    eventsApi.listAgentEvents.mockResolvedValueOnce({
