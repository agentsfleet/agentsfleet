...
import { AGENTSFLEET_STATUS } from "@/lib/api/agents";
import type { Agent, AgentStatusSettable } from "@/lib/api/agents";
...
// Drives the per-agent lifecycle controls. The panel renders a state-aware
// set of actions:
//   - `active`              → Stop (graceful halt) + Kill (terminal)
//   - `paused` (auto-halt)  → Resume + Kill
//   - `stopped` (op halt)   → Resume + Kill
//   - `killed` (terminal)   → no actions (DELETE is offered in AgentConfig)
export default function KillSwitch({ workspaceId, agent }: KillSwitchProps) {
  const router = useRouter();
  const [pendingAction, setPendingAction] = useState<ActionConfig | null>(null);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [optimisticStatus, setOptimisticStatus] = useOptimistic(agent.status);
  ...
  async function handleConfirm(action: ActionConfig) {
    ...
    startTransition(async () => {
      ...
      const result = await setAgentStatusAction(workspaceId, agent.id, action.target);
      if (result.ok) {
