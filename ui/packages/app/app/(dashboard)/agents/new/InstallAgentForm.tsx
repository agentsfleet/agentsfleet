...
import { installAgentAction } from "../actions";
...
const schema = z.object({
  ...
  trigger_markdown: z.string().trim().superRefine((value, ctx) => {
    ...
    if (!frontmatter) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "TRIGGER.md needs YAML frontmatter between --- markers",
      });
      return;
    }
...
const DEFAULT_DAILY_BUDGET_DOLLARS = "1.0";
...
function topLevelFieldValue(frontmatter: string, field: string): string | null {
...
function buildManualTriggerMarkdown(skillMarkdown: string): string | null {
  const frontmatter = frontmatterBody(skillMarkdown);
  if (!frontmatter) return null;
  const name = topLevelFieldValue(frontmatter, "name");
  if (!name) return null;
  return [
    "---",
    ...
    "  tools: []",
    "  budget:",
    `    daily_dollars: ${DEFAULT_DAILY_BUDGET_DOLLARS}`,
    "---",
    "",
  ].join("\n");
}
...
export default function InstallAgentForm({ workspaceId }: Props) {
  ...
  function onSubmit(values: FormValues) {
    setApiError(null);
    const triggerMarkdown = values.trigger_markdown || buildManualTriggerMarkdown(values.source_markdown);
    if (!triggerMarkdown) {
      form.setError("source_markdown", {
        type: "manual",
        message: "SKILL.md frontmatter needs name: before a manual wake can be generated",
      });
      return;
    }
    startTransition(async () => {
      const result = await installAgentAction(workspaceId, {
        ...values,
        trigger_markdown: triggerMarkdown,
      });
      if (result.ok) {
        captureProductEvent(EVENTS.agent_created, { agent_id: result.data.agent_id });
    ...
    <Form {...form}>
      ...
      >
        ...
        <p className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          Leave <code className="font-mono">TRIGGER.md</code> blank to install with
          manual wake only; add cron or webhook rules later.
        </p>
        <Alert variant="info">
          <div>
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                ...
                manual wake with no external tools. If pasted, include{" "}
