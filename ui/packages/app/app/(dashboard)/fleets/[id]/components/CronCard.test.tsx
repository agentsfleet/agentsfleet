...
describe("CronCard", () => {
  it("renders the raw cron expression in the header", () => {
    render(<CronCard trigger={{ type: "cron", schedule: "*/15 * * * *" }} fleetId="agt_ax" />);
  ...
  it("shows the unparseable warning when the schedule is malformed", () => {
    render(<CronCard trigger={{ type: "cron", schedule: "not a cron" }} fleetId="agt_ax" />);
  ...
  it("links to the actor-filtered deliveries view", () => {
    ...
    expect(link.getAttribute("href")).toBe("/fleets/agt_ax?actor=cron:*");
  });
  ...
  it("uses the literal 'unparseable' fallback reason when the parser throws a non-Error", () => {
    ...
    try {
      render(
        <CronCard trigger={{ type: "cron", schedule: "*/15 * * * *" }} fleetId="agt_ax" />,
      );
