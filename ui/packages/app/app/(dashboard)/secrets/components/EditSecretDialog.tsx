...
import { SECRET_NAME_MAX, parseSecretDataObject } from "../lib/secret-data";

export type EditSecretDialogProps = {
  workspaceId: string;
  /** The secret being edited. Its name is the reference key Fleets resolve. */
  name: string;
  open: boolean;
...
/**
 * Edit a stored secret. Rotate (default) overwrites the secret value under
...
export default function EditSecretDialog({
  ...
  function onSubmit() {
    setError(null);
    const parsed = parseSecretDataObject(dataJson, DATA_REQUIRED);
    if (!parsed.ok) {
      setError(parsed.message);
      return;
    }
    const target = isRename ? newName.trim() : name;
    if (isRename && (target === "" || target.length > SECRET_NAME_MAX)) {
      setError(`New name must be 1–${SECRET_NAME_MAX} characters`);
      return;
    }
      ...
      if (!created.ok) {
        setError(
          presentErrorString({
            ...
            action: isRename ? "rename the secret" : "rotate the secret",
          }),
        );
        return;
      }
