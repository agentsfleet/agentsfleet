...
describe("CronCard", () => {
  ...
  it("shows the unparseable warning when the schedule is malformed", () => {
    render(<CronCard trigger={{ type: "cron", schedule: "not a cron" }} workspaceId="ws_1" fleetId="agt_ax" />);
  ...
  it("links to the actor-filtered deliveries view", () => {
    ...
    expect(link.getAttribute("href")).toBe("/w/ws_1/fleets/agt_ax?actor=cron:*");
  });
  ...
  it("uses the literal 'unparseable' fallback reason when the parser throws a non-Error", () => {
    ...
    try {
      render(
        <CronCard trigger={{ type: "cron", schedule: "*/15 * * * *" }} workspaceId="ws_1" fleetId="agt_ax" />,
      );
