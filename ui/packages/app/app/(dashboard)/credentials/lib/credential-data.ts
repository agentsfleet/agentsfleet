...
// credential name is at most CREDENTIAL_NAME_MAX chars. AddCredentialForm (zod)
// and EditCredentialDialog (plain state) both validate against this so the two
// can never drift on what they accept or the messages they show.

export const CREDENTIAL_NAME_MAX = 64;
export const CREDENTIAL_DATA_NOT_OBJECT =
  "Data must be a JSON object — strings, arrays, and scalars are rejected";
export const CREDENTIAL_DATA_EMPTY_OBJECT = "Object must have at least one field";
...
export type ParsedCredentialData =
 ...
 */
export function parseCredentialDataObject(raw: string, requiredMessage: string): ParsedCredentialData {
  ...
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    return { ok: false, message: CREDENTIAL_DATA_NOT_OBJECT };
  }
  if (Object.keys(parsed).length === 0) {
    return { ok: false, message: CREDENTIAL_DATA_EMPTY_OBJECT };
  }
