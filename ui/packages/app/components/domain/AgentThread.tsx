...
import { steerAgentAction } from "@/app/(dashboard)/agents/actions";
import { SteerComposer } from "./SteerComposer";
import { renderAgentMessage } from "./agentMessageRenderers";
 ...
 * `@assistant-ui/react` over `useAgentEventStream` + the `steerAgentAction`
 * Server Action; custom renderers under `agentMessageRenderers` paint the
 ...
 */
export function AgentThread({ workspaceId, agentId, initial }: AgentThreadProps) {
  const stream = useAgentEventStream(workspaceId, agentId, initial);
  // Pass the registry methods (each `useCallback([agentId])`-stable), not
  ...
  const runtime = useExternalStoreRuntime<AgentEvent>({
...
type StreamApi = ReturnType<typeof useAgentEventStream>;
type NewHandlerCtx = {
  workspaceId: string;
  agentId: string;
...
function useNewMessageHandler({
  ...
  return useCallback(
    async (msg: AppendMessage) => {
      ...
      try {
        const result = await steerAgentAction(workspaceId, agentId, text);
        if (result.ok) {
    ...
    [workspaceId, agentId, appendOptimistic, reconcileOptimistic, markOptimisticFailed],
  );
}
