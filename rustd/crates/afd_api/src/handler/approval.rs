...
/// awkward one.
fn read_reason(body: &Bytes) -> Result<&str, Refusal> {
    if body.is_empty() {
        return Ok("");
    }
    ...
    let reason = match request.reason {
        ...
        // An escaped note arrives owned, and this handler hands a borrow to the
        // store. Refusing here rather than allocating keeps the lifetime honest
        // and costs nothing a real operator would notice.
        Some(Cow::Owned(_escaped)) => return Err(Refusal::malformed(DETAIL_MALFORMED_JSON)),
        None => "",
    };
