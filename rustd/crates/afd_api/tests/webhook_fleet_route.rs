...
/// The GitHub route reads its binding BY SOURCE, so a fleet declaring another
/// provider's webhook first is measured on its GitHub trigger — the allow-list
/// and the secret this route was addressed for — and not on whichever trigger
/// the document lists first. The bare `/v1/webhooks/{fleet_id}` route names
/// no provider and asks for none.
#[tokio::test]
async fn the_github_route_asks_for_the_github_trigger_by_source() {
    let ingress = serving(signed::TRIGGER_GITHUB, FleetStatus::Active.as_str());
    let response = deliver(
        &ingress,
        EVENT_WORKFLOW_RUN,
    ...
    assert_eq!(response.status(), StatusCode::ACCEPTED);
    assert_eq!(
        ingress.sources_asked(),
        vec![Some("github".to_owned())],
        "the route names its own source when it reads the binding"
    );
}
