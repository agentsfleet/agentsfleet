...
use super::{dragonfly_calls_in, used_memory_in};
...
#[test]
fn test_dragonfly_calls_sum_every_command_family() {
    assert_eq!(dragonfly_calls_in(COMMANDSTATS), Some(42));
}

#[test]
fn test_a_reply_with_no_call_counts_is_refused_rather_than_read_as_zero() {
    assert_eq!(
        dragonfly_calls_in("# Commandstats\r\n"),
        None,
        "a server that answered nothing this parser knows did not serve zero commands"
    );
    assert_eq!(dragonfly_calls_in(""), None);
}
...
fn test_a_malformed_count_is_skipped_and_the_rest_still_read() {
    ...
    assert_eq!(dragonfly_calls_in(mixed), Some(7));
}
