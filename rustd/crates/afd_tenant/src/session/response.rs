...
use afd_datastore::session::SessionStatus;
...
impl Redeemed {
    ...
    pub(super) fn first(payload: afd_datastore::session::VerifyPayload) -> Self {
        Self::of(payload, false)
    }

    /// A repeat inside the replay window.
    pub(super) fn repeated(payload: afd_datastore::session::VerifyPayload) -> Self {
        Self::of(payload, true)
    }

    fn of(payload: afd_datastore::session::VerifyPayload, repeated: bool) -> Self {
        Self {
