...
pub use afd_datastore::session::SessionStatus;
...
impl Sessions {
    ...
    pub async fn approve(
        ...
        let outcome = self
            .store
            .approve(
                &afd_datastore::session::Approval {
...
fn session_ttl_millis() -> i64 {
    i64::try_from(afd_datastore::session::SESSION_TTL.as_millis()).unwrap_or(i64::MAX)
}
...
mod response;
pub use self::response::{Cancelled, Opened, Redeemed, Waiting};
