...
impl Fleets {
    ...
    pub async fn install(
        ...
        match self.finish(workspace, &id, now).await {
            Ok(()) => {
                // After the flip, because both writes hang off a fleet row that
                // must already exist, and best-effort, because a card that
                // could not be written is a gap the first delivery closes.
                self.request_declared_grants(workspace, &id, &declared, now)
                    .await;
