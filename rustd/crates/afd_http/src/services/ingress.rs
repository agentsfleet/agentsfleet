...
pub trait WebhookIngress: Send + Sync + std::fmt::Debug + 'static {
    ...
    /// The fleets in `workspace` attached to `channel` on `provider`.
    ///
    /// # Errors
    /// Reports a datastore that would not answer, a row this build cannot read,
    /// and a stored document that no longer parses.
    fn mention_subscribers(
        &self,
        workspace: &Uuid7,
        provider: &str,
        channel: &ChannelId,
    ) -> impl Future<Output = IngressResult<Vec<Subscriber>>> + Send;

    /// Admits one routed mention, at most once however often it arrives.
    ///
    /// # Errors
    /// Reports a database that would not record the acceptance.
    fn admit_mention(
...
impl WebhookIngress for Ingress {
    ...
    fn mention_subscribers(
        ...
        Self::mention_subscribers(self, workspace, provider, channel)
    }
