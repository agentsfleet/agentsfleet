#![expect(clippy::expect_used, reason = "tests inspect failures directly")]
...
fn invalid(body: &ImportBody) -> InvalidBundle {
    ...
    match error {
        ...
        crate::Error::Storage(_) => panic!("validation cannot touch storage"),
    }
}
...
fn test_bundle_import_rejects_hostile() {
    let cases = [
        ("../secret.txt", b"safe".as_slice(), InvalidBundle::UnsafeSupportPath),
        ("docs//note.md", b"safe".as_slice(), InvalidBundle::UnsafeSupportPath),
        ("key.txt", b"api_key: stolen".as_slice(), InvalidBundle::EmbeddedCredential),
    ];
    for (path, content, expected) in cases {
        let mut input = body();
        input.support_files.push(SupportFile { path: path.into(), content: content.to_vec() });
        assert_eq!(invalid(&input), expected);
    }

    let mut oversized = body();
    oversized.support_files.push(SupportFile { path: "large.bin".into(), content: vec![0; 64 * KIBIBYTE + 1] });
...
fn manifest_contains_hashes_not_support_bytes() {
    let mut input = body();
    input.support_files.push(SupportFile { path: "README.md".into(), content: b"review notes".to_vec() });
    let prepared = prepare(&input).expect("the bundle is valid");
    let encoded = serde_json::to_string(&prepared.support_manifest).expect("the manifest serializes");
