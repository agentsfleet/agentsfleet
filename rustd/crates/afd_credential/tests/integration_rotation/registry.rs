...
async fn a_tenant_that_never_configured_a_provider_is_composed_from_the_live_default() {
    ...
    // The values are the seeded row's, unmodified — a view that substituted a
    // built-in would look identical until the operator changed the default.
    assert_eq!(&*shown.provider, provider.as_str());
    assert_eq!(&*shown.model, model.as_str());
    assert_eq!(shown.context_cap_tokens, 64_000);
