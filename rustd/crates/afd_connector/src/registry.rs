...
///
/// Named in the registry so the events route dispatches on data and never on
/// a provider's name: a second chat connector that delivers mentions is one
/// more arm returning [`EventProducer::Mention`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EventProducer {
    /// A mention of the bot in a channel, answered in its thread.
    Mention,
}
