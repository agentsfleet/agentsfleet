...
#[cfg_attr(feature = "openapi", utoipa::path(
    ...
    description = concat!(
        "Idempotent — removing an id that is already gone, or one that names ",
        "another workspace's entry, still returns 204. The two are ",
        "indistinguishable on purpose: every statement is scoped by ",
        "workspace, so separating them would need an unscoped read whose only ",
        "effect is to confirm the id exists somewhere. Removal is permanent ",
        "and cannot disturb a fleet installed from the entry: the fleet ",
        "copied the bundle at install time and carries its own. Re-onboarding ",
        "the same bytes afterwards mints a new entry with a new id. Distinct ",
        "from `delete_platform_fleet_library`, which is operator-held. ",
    ),
    params(
