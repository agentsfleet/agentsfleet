...
#[cfg_attr(feature = "openapi", utoipa::path(
    ...
    description = concat!(
        ...
        "field that leads `data`. The first frame is `event: hello`, sent as ",
        "soon as the subscription attaches and before any activity. Its ",
        "`data` is `{\"kind\":\"hello\",\"fleet_ids\":[\"<fleet_id>\"],",
        "\"counters\":{}}` — the one fleet this connection carries. It uses ",
        "identifier 0 and does not advance the activity sequence, so a client ",
        "may report the stream live without waiting for a heartbeat. An ",
        "instance holding no pub/sub connection sends no `hello`. ",
        "The daemon brackets every run. ",
        "`event_received` carries `event_id`, `actor`, `event_type` and ",
        "`created_at` when a run's row is written. `event_complete` carries ",
        "the terminal row as `GET …/events` lists it, without `fleet_id` and ",
        "`workspace_id`. The completion adds `fleet_status` and ",
        "`pending_approvals`. It also carries `final_reply`, the reply text, ",
        "when that text is at most 64 KiB. Without it, read the event detail. ",
        "`gate_opened` and `gate_resolved` carry ",
...
#[cfg_attr(feature = "openapi", utoipa::path(
    ...
    description = concat!(
        ...
        "`fleet_id` added as the leading field. The frame kinds are the ones ",
        "`GET …/fleets/{fleet_id}/events/stream` documents. No publisher ",
        "payload carries `fleet_id` itself, so the tag is the frame's one ",
        "scope. The client routes each frame by that tag. A stalled client ",
