    ...
    if (!workspaceId) {
      return yield* Effect.fail(
        new ValidationError({
          ...
          suggestion: "usage: zombiectl workspace use <workspace_id>",
        }),
      );
    }
    ...
    if (!workspaceId) {
      return yield* Effect.fail(
        new ValidationError({
          ...
          suggestion: "usage: zombiectl workspace delete <workspace_id>",
        }),
      );
    }
    return yield* validateWorkspaceId(workspaceId);
    ...
    if (!known) {
      return yield* Effect.fail(
        new ConfigError({
          ...
          suggestion: "run `zombiectl workspace add` or `zombiectl workspace list`",
        }),
      );
    }
...
> = Effect.gen(function* () {
  ...
  yield* output.info(
    "Manage credentials at /credentials in the dashboard, or run: zombiectl zombie credential",
  );
});
