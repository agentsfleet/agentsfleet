...
/// acceptable (zombied is the single Redis consumer — see runner_fleet.md).
...
fn tryCandidate(hx: Hx, conn: *pg.Conn, runner_id: []const u8, zombie_id: []const u8) !?Acquired {
    const won = switch (try affinity.claim(conn, hx.alloc, zombie_id, runner_id, affinity.LEASE_TTL_MS)) {
        .taken => return null,
        .won => |w| w,
    };
