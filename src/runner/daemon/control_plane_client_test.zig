...
test "a hung control plane surfaces a transport error within the armed deadline" {
    ...
    try testing.expectError(error.RequestFailed, c.heartbeat(alloc, "agt_rtest", DEADLINE_PROBE_MS, null, null));
...
test "two verbs ride one pooled connection (keep-alive reuse)" {
    ...
    const first = try c.heartbeat(alloc, "agt_rtest", DEADLINE_PROBE_MS, null, null);
    defer first.deinit();
    try testing.expectEqual(.ok, first.value.status);
    const second = try c.heartbeat(alloc, "agt_rtest", DEADLINE_PROBE_MS, null, null);
    defer second.deinit();
