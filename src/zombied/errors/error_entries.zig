...
pub const ENTRIES = [_]Entry{
    ...
    e("UZ-AUTH-011", .bad_request, "Verification code did not match",
        "The 6-digit verification code did not match what the dashboard issued. " ++
        "Double-check the code shown in your browser and try again."),
    e("UZ-AUTH-012", .gone, "Login session already consumed",
        "This login session has already been consumed. Start over with `zombiectl login`."),
    e("UZ-AUTH-013", .gone, "Login session aborted",
        "This login session was aborted (too many wrong codes, explicit cancel, or replaced by a newer session). " ++
        "Start over with `zombiectl login`."),
    e("UZ-AUTH-014", .gone, "Login session not approved",
        "This login session has not been approved in the dashboard yet. " ++
        "Approve it in your browser before submitting a verification code."),
    e("UZ-AUTH-015", .conflict, "Login session already approved",
        "This login session has already been approved. Do not call /approve a second time."),
    e("UZ-AUTH-016", .bad_request, "Invalid CLI public key",
        "The supplied public_key is malformed. Expect base64url-encoded P-256 SubjectPublicKeyInfo."),
    e("UZ-AUTH-017", .bad_request, "Invalid token name",
        "token_name must be 1-64 characters of printable ASCII."),
    e("UZ-AUTH-018", .bad_request, "Invalid verification code shape",
        "verification_code must be exactly 6 ASCII digits."),
    e("UZ-AUTH-019", .bad_request, "Invalid ciphertext",
    ...
    e("UZ-RUN-001", .unauthorized, "Invalid runner token",
    ...
    e("UZ-RUN-007", .internal_server_error, "Sandbox establishment failed",
        "The runner could not establish the required sandbox (Landlock/cgroup/netns) for execution; the lease was refused fail-closed rather than run unconfined."),
    e("UZ-RUN-009", .forbidden, "Runner revoked",
        "This runner has been revoked by an operator and may no longer lease, renew, or report. Its in-flight work is reassigned to other healthy runners."),
    e("UZ-RUN-010", .conflict, "Lease exceeded max runtime",
        "The lease reached the hard maximum runtime and may not be renewed further; the run is terminated. The child is killed and the result, if any, is reported."),
    e("UZ-RUN-011", .conflict, "Lease lost",
