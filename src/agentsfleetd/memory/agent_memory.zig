...
const log = logging.scoped(.agent_memory);
...
/// Strategy for bounding a agent's hydration payload. `selective` pins the
...
/// Upsert one memory entry under `agent_id` — the ONLY `INSERT` into
...
/// Idempotent via `ON CONFLICT (key, agent_id) DO UPDATE`, so a retried or
...
pub fn storeEntry(
    ...
    _ = try conn.exec(
        ...
        \\ON CONFLICT (key, agent_id) DO UPDATE
        \\  SET content = EXCLUDED.content,
        \\      category = EXCLUDED.category,
        \\      updated_at = EXCLUDED.updated_at
    , .{ uid, id, key, content, category, agent_id, ts_ms });
}

/// Evict the entries beyond `max` for `agent_id` (the per-agent durable-set cap),
...
pub fn enforceCap(conn: *pg.Conn, agent_id: []const u8, max: usize) !u64 {
    const affected = try conn.exec(
        ...
        \\    OFFSET $2
        \\  )
    , .{ agent_id, @as(i64, @intCast(max)), CATEGORY_CORE });
    const n = affected orelse {
        log.warn("memory_cap_evict_count_unavailable", .{ .agent_id = agent_id });
        return 0;
    };
...
/// Delete the agent's `daily` rows with `updated_at` older than `cutoff_ms` —
/// `enforceCap`'s sibling in shape, call site, and posture: per-agent scoped
...
pub fn sweepExpiredDaily(conn: *pg.Conn, agent_id: []const u8, cutoff_ms: i64) !u64 {
    const affected = try conn.exec(
        \\DELETE FROM memory.memory_entries
        \\WHERE agent_id = $1::uuid
        \\  AND category = $2
        \\  AND updated_at < $3
    , .{ agent_id, CATEGORY_DAILY, cutoff_ms });
    const n = affected orelse {
        log.warn("memory_daily_sweep_count_unavailable", .{ .agent_id = agent_id });
        return 0;
    };
    return if (n < 0) 0 else @intCast(n);
}

/// Every memory entry for `agent_id`, newest first — the hydration read the
...
// Pure selection-policy tests live in the sibling `agent_memory_test.zig`
// (registered in `tests.zig`); storeEntry/enforceCap/listAll need a live
// Postgres and are exercised by `agent_memory_integration_test.zig`.
