...
test "integration: memory_runtime has no access to core.fleets (RULE CTX)" {
    ...
    // SELECT on core.fleets must be rejected with SQLSTATE 42501 (insufficient_privilege).
    const result = conn.query("SELECT id FROM core.fleets LIMIT 1", .{});
    try std.testing.expectError(error.PG, result);

    const pg_err = conn.err orelse {
        return error.ExpectedPgError;
    };
    // SQLSTATE 42501 = insufficient_privilege (permission denied for table fleets).
