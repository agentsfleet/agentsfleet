//! Integration tier for the single durable write adapter (`agent_memory.zig`):
//! `storeEntry` / `enforceCap` / `listAll` need a live Postgres and the
//! `memory_runtime` role. Mirrors `agent_memory_role_test.zig`'s harness —
//! `LIVE_DB=1` + `TEST_DATABASE_URL` (set by `make test-integration-db`);
//! self-skips otherwise. `memory.memory_entries` carries no FK to `core.agents`
//! (role isolation is the boundary), so a synthetic `agent_id` needs no seed.

const std = @import("std");
const common = @import("common");
const pg = @import("pg");
const base = @import("../db/test_fixtures.zig");
const adapter = @import("agent_memory.zig");
const protocol = @import("contract").protocol;
const PgQuery = @import("../db/pg_query.zig").PgQuery;

// Distinct synthetic agents per concern so concurrent/re-runs never collide.
...
const TestDb = struct {
    ...
    /// Wipe a agent's rows so a re-run starts clean (memory_runtime has DELETE).
    fn wipe(self: TestDb, agent_id: []const u8) void {
        _ = self.conn.exec("DELETE FROM memory.memory_entries WHERE agent_id = $1::uuid", .{agent_id}) catch |err|
            std.log.warn("wipe ignored: {s}", .{@errorName(err)});
    }

    fn count(self: TestDb, agent_id: []const u8) !usize {
        var q = PgQuery.from(try self.conn.query("SELECT COUNT(*) FROM memory.memory_entries WHERE agent_id = $1::uuid", .{agent_id}));
        defer q.deinit();
        const row = try q.next() orelse return 0;
        return @intCast(try row.get(i64, 0));
    }

    fn countCategory(self: TestDb, agent_id: []const u8, category: []const u8) !usize {
        var q = PgQuery.from(try self.conn.query("SELECT COUNT(*) FROM memory.memory_entries WHERE agent_id = $1::uuid AND category = $2", .{ agent_id, category }));
        defer q.deinit();
        const row = try q.next() orelse return 0;
        return @intCast(try row.get(i64, 0));
    }
...
/// v7-shaped from the agent's distinguishing tail + n (collision-free across
/// fixture agents); the globally-unique id column carries the same tail.
fn seedDailies(db: TestDb, agent_id: []const u8, n: usize) !void {
    _ = try db.conn.exec(
    ...
    , .{ agent_id, adapter.CATEGORY_DAILY, @as(i64, @intCast(n)) });
}
...
test "integration: enforceCap failure propagates as an error, deleting nothing" {
    ...
    // Inject a deterministic failure: the driver rejects a malformed agent_id
...
test "integration: one core fact survives a thousand daily pushes — durable and hydrated" {
    ...
    // The core fact is the single OLDEST row the agent owns — under the old
...
test "integration: numeric millis round-trip — updated_at reads back exactly; listAll newest-first" {
    ...
    // is drained before the next query on the same connection.
    {
        var q = PgQuery.from(try db.conn.query(
            "SELECT updated_at FROM memory.memory_entries WHERE agent_id = $1::uuid AND key = $2",
            .{ ZID_TS, "newer" },
        ));
        defer q.deinit();
...
test "integration: daily sweep deletes only aged daily rows — young daily and other agents untouched" {
    const alloc = std.testing.allocator;
    const db = (try TestDb.open(alloc)) orelse return error.SkipZigTest;
    defer db.close();
    db.wipe(ZID_SWEEP_A);
    ...
    // and B's rows (a different agent, same age) are out of scope.
...
test "integration: sweep failure propagates as an error, deleting nothing" {
    ...
    // a malformed agent_id errors before any bind. The capture handler's
...
test "integration: listAll is scoped per agent — no cross-agent bleed" {
    const alloc = std.testing.allocator;
    const db = (try TestDb.open(alloc)) orelse return error.SkipZigTest;
    defer db.close();
    db.wipe(ZID_ISO_A);
