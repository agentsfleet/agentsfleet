...
test "integration: memory_runtime has no access to core.agents (RULE CTX)" {
    ...
    const result = conn.query("SELECT id FROM core.agents LIMIT 1", .{});
    try std.testing.expectError(error.PG, result);

    const pg_err = conn.err orelse {
        return error.ExpectedPgError;
    };
    // SQLSTATE 42501 = insufficient_privilege (permission denied for table agents).
