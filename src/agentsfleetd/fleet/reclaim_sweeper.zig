...
//! lease path re-checks the per-agent affinity claim before any re-delivery.
//!
...
/// Per-agent per-sweep claim bound — keeps one pathological stream from
/// monopolizing a sweep pass; the next pass continues where this one stopped.
const SWEEP_CLAIM_LIMIT: usize = 10;
const SWEEP_INTERVAL_NS: u64 = @as(u64, @intCast(queue_consts.agent_reclaim_interval_ms)) * std.time.ns_per_ms;
...
pub fn run(pool: *pg.Pool, queue: *queue_redis.Client, alloc: std.mem.Allocator, shutdown: *std.atomic.Value(bool)) void {
    log.info(LOG_SWEEPER_STARTED, .{ .interval_ms = queue_consts.agent_reclaim_interval_ms, .min_idle_ms = queue_consts.agent_xautoclaim_min_idle_ms_int, .batch_limit = SWEEP_BATCH_LIMIT });
...
pub fn sweepOnce(pool: *pg.Pool, queue: *queue_redis.Client, alloc: std.mem.Allocator) !SweepStats {
    const agents = try fetchActiveAgents(pool, alloc);
    defer freeIds(alloc, agents);
    var consumer_buf: [queue_redis.CONSUMER_ID_BUF_LEN]u8 = undefined;
    const consumer_id = queue_redis.stableConsumerId(&consumer_buf);
    var stats = SweepStats{ .scanned_agents = @intCast(agents.len) };
    for (agents) |agent_id| {
        stats.reclaimed_entries += reclaimAgentStrays(queue, agent_id, consumer_id);
    }
    return stats;
}

/// Claim up to SWEEP_CLAIM_LIMIT idle-past-bound entries for one agent into
...
/// next pass retries.
fn reclaimAgentStrays(queue: *queue_redis.Client, agent_id: []const u8, consumer_id: []const u8) i64 {
    var reclaimed: i64 = 0;
    var i: usize = 0;
    while (i < SWEEP_CLAIM_LIMIT) : (i += 1) {
        var event = (redis_agent.xautoclaimAgent(queue, agent_id, consumer_id) catch |err| {
            log.warn("reclaim_claim_failed", .{ .agent_id = agent_id, .err = @errorName(err) });
            return reclaimed;
        }) orelse return reclaimed;
        defer event.deinit(queue.alloc);
        reclaimed += 1;
        log.info("reclaim_swept", .{ .agent_id = agent_id, .event_id = event.event_id, .actor = event.actor });
    }
    return reclaimed;
}

/// Active agents only: a paused/stopped agent's entries are deliberately
/// retained where they are — on resume the agent re-enters the candidate
/// scan and this sweep picks its strays up on the next pass.
fn fetchActiveAgents(pool: *pg.Pool, alloc: std.mem.Allocator) ![][]const u8 {
    const conn = try pool.acquire();
    defer pool.release(conn);
    var q = PgQuery.from(try conn.query(
        \\SELECT id::text FROM core.agents WHERE status = $1
        \\ORDER BY updated_at ASC LIMIT $2
    , .{ agent_config.AgentStatus.active.toSlice(), SWEEP_BATCH_LIMIT }));
    defer q.deinit();
