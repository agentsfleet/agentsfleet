...
//
// Rate note: arrange() seeds the suite's own registry rates, so the server
// prices every slice against them and the token_cost identity plus the strict
// non-zero arms assert unconditionally. Requires LIVE_DB; skipped when
...
// so the server's own rate resolution prices the wire deltas non-zero. The
// cache is process-global: page_allocator, per
