...
//   * hold then schedule: an unsatisfiable fleet stays unclaimed (the event is
...
const SOURCE_MD =
    ...
    \\---
    \\
    \\You are a placement test fleet.
;
...
/// against a fleet's required_tags). `labels_json` is a literal like '["gpu"]'.
...
/// Seed the fleet + session, then stamp its required_tags. `tags_literal` is a
/// Postgres TEXT[] literal like '{gpu}' or '{}'. base.seedFleet omits
...
/// Seed a second tagged fleet (AGENT2_ID) + its session + a fresh event, for
...
/// the fleet carrying `tags_json`, and one fresh event. Runners are seeded
...
fn seedBase(h: *TestHarness, conn: *pg.Conn, tags_json: []const u8) !void {
    ...
    try seedFleetWithTags(conn, tags_json);
    try publishEventFor(h, FLEET_ID);
}
...
fn cleanupAll(h: *TestHarness, conn: *pg.Conn) void {
    ...
    execIgnore(conn, "DELETE FROM core.fleet_events WHERE fleet_id IN ($1::uuid, $2::uuid)", .{ FLEET_ID, AGENT2_ID });
...
test "claim respects required tag subset: a [gpu] fleet leases only to a gpu runner" {
...
test "empty required_tags leases to any runner (back-compat with the global race)" {
    ...
    // [] ⊆ any labels ⇒ a label-less runner claims an untagged fleet.
    try std.testing.expect(try leasePresent(h, PLAIN_TOKEN));
}
...
test "unsatisfiable tags hold then schedule once a matching runner enrolls" {
    ...
    try seedBase(h, conn, "{gpu}");
    // Only a plain runner exists: the fleet's [gpu] is unsatisfiable → holds.
    try seedRunnerWithLabels(conn, PLAIN_RUNNER_ID, "plc-plain", PLAIN_TOKEN, "[]");
    try std.testing.expect(!try leasePresent(h, PLAIN_TOKEN));

    // A matching runner enrolls → the still-unclaimed fleet schedules. (The
...
test "concurrent: eligible runners race one tagged fleet — exactly one wins, ineligible never do" {
    const h = startHarness() catch |err| {
        if (err == error.SkipZigTest) return error.SkipZigTest;
        return err;
    };
    ...
    // One [gpu] fleet carrying exactly ONE event.
    try seedBase(h, conn, "{gpu}");
...
test "complex: two tagged fleets route only to their matching runner" {
    const h = startHarness() catch |err| {
        if (err == error.SkipZigTest) return error.SkipZigTest;
        return err;
    };
    ...
    // Z_GPU [gpu] (the default FLEET_ID) + Z_ARM [arm64] (AGENT2_ID), each
    // with its own event; one gpu runner and one arm runner.
    try seedBase(h, conn, "{gpu}");
    try seedSecondFleet(h, conn, "placement-arm-bot", "{arm64}");
    try seedRunnerWithLabels(conn, GPU_RUNNER_ID, "plc-gpu", GPU_TOKEN, "[\"gpu\"]");
    try seedRunnerWithLabels(conn, ARM_RUNNER_ID, "plc-arm", ARM_TOKEN, "[\"arm64\"]");

    // The gpu runner gets its fleet...
    try std.testing.expect(try leasePresent(h, GPU_TOKEN));
    ...
    // across to consume the other fleet's event.
