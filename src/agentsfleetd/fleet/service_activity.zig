...
//! `fleet:{id}:activity` — downstream Server-Sent-Events (SSE) is unchanged.
//!
...
//! a fleet it has no lease on. No fencing: a stale holder's cosmetic frames are
...
/// The lease fields a publish needs: which fleet's channel, and the event the
...
/// Publish every frame to the fleet's activity channel, reusing one Scratch
...
fn publishOne(
    ...
    switch (frame) {
        .tool_call_started => |b| activity_publisher.publishToolCallStarted(client, scratch, alloc, target.fleet_id, target.event_id, b.name, b.args_redacted),
        .fleet_response_chunk => |b| activity_publisher.publishChunk(client, scratch, target.fleet_id, target.event_id, b.text),
        .tool_call_completed => |b| activity_publisher.publishToolCallCompleted(client, scratch, target.fleet_id, target.event_id, b.name, b.ms),
        .tool_call_progress => |b| activity_publisher.publishToolCallProgress(client, scratch, target.fleet_id, target.event_id, b.name, b.elapsed_ms),
    }
}
