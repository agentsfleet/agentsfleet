...
//     to the hard cap, never past it — a wedged-but-emitting fleet terminates.
//   - a fleet with an expired affinity slot but NO prior runner_leases row:
//     a second runner claims it fresh; reclaimPriorActive finds nothing to
//     re-lease (the fleet is simply free), so no stale row is resurrected.
//
...
const FLEET_ID = "0195b4ba-8d3a-7f13-8abc-2b3e1e0dac01";
...
test "a free fleet with an expired slot but no prior lease is claimed fresh" {
    const h = try startHarness();
    defer h.deinit();
    const conn = try h.acquireConn();
    defer h.releaseConn(conn);
    ...
    // re-lease (the fleet was simply free); no stale row resurrected.
    const prior = try reclaim.reclaimPriorActive(conn, ALLOC, FLEET_ID);
