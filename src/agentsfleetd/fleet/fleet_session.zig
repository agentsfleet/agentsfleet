...
pub fn claimFleet(
    ...
    const row = try q.next() orelse {
        log.warn("fleet_event_loop.claim_not_found", .{
    ...
    log.info("fleet_event_loop.claimed", .{
        .fleet_id = fleet_id,
        .name = config.name,
...
pub fn clearExecutionActive(alloc: Allocator, session: *Self, pool: *pg.Pool) void {
    ...
    , .{session.fleet_id}) catch |err| log.warn(logging.EVENT_IGNORED_ERROR, .{ .err = @errorName(err) });
}
