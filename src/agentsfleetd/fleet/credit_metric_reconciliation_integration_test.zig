...
//
// Pricing note: arrange() raises the fixture pair's token rates above zero, so
// the reconciliation identity carries real money and the strict non-zero arm
// asserts unconditionally. The zero arms (replay, lost fence, failed settle)
// stay zero for their own reasons, which is the point.
...
/// same money. One table, one sum, no mirror.
...
test "integration: the wallet drain equals the ledger sum across a metered run" {
    ...
    // nothing means the fixture broke, not that the
