...
fn claimReportAndSettle(hx: Hx, runner_id: []const u8, lease: Lease, body: protocol.ReportRequest) !renewal_settle.SettleOutcome {
    ...
    // The outcome rides the claim so the runner's succeeded/failed tally lands
    // atomically with the lease's active→reported flip (exactly-once by the
    // claim guard — a fenced retry increments nothing).
    const succeeded = report_mapping.fromReport(body).succeeded();
    return renewal_settle.claimAndSettle(conn, body.lease_id, runner_id, now_ms, meter, succeeded);
}
...
fn finalize(hx: Hx, runner_id: []const u8, lease: Lease, body: protocol.ReportRequest) void {
    ...
    outbound.enqueueOutboundAnswer(hx, .{ .workspace_id = lease.workspace_id, .fleet_id = lease.fleet_id, .event_id = lease.event_id }, body.response_text);
