//! Read-side store for `core.agent_events` — operator-visible event
//! history. Two listing entry points:
//!
//!   listForAgent(...)    — per-agent history.
//!   listForWorkspace(...) — workspace aggregate, optional agent_id
//!                            filter for drill-down.
//!
//! Sorted newest-first by `(created_at DESC, event_id DESC)`. Filters
//! and cursor live in `agent_events_filter.zig`.
//!
...
const filter_mod = @import("agent_events_filter.zig");
...
// 42601 if glued straight onto `core.agent_events`).
const EVENTS_SELECT =
    \\SELECT agent_id::text, event_id, workspace_id::text, actor, event_type,
    ...
    if (filter.cursor) |c| {
        ...
        if (filter.actor_like) |al| {
            return queryRows(conn, alloc, EVENTS_SELECT ++
                ...
                \\LIMIT $6
            , .{ workspace_id, agent_id, parsed.created_at, parsed.event_id, al, lim });
        }
        return queryRows(conn, alloc, EVENTS_SELECT ++
            \\WHERE workspace_id = $1::uuid AND agent_id = $2::uuid
            \\  AND (created_at, event_id) < ($3, $4)
            \\ORDER BY created_at DESC, event_id DESC
            \\LIMIT $5
        , .{ workspace_id, agent_id, parsed.created_at, parsed.event_id, lim });
    }
    if (filter.since_ms) |since| {
        if (filter.actor_like) |al| {
            return queryRows(conn, alloc, EVENTS_SELECT ++
                ...
                \\ORDER BY created_at DESC, event_id DESC
                \\LIMIT $5
            , .{ workspace_id, agent_id, since, al, lim });
        }
        return queryRows(conn, alloc, EVENTS_SELECT ++
            \\WHERE workspace_id = $1::uuid AND agent_id = $2::uuid
            \\  AND created_at >= $3
            \\ORDER BY created_at DESC, event_id DESC
            \\LIMIT $4
        , .{ workspace_id, agent_id, since, lim });
    }
    if (filter.actor_like) |al| {
        return queryRows(conn, alloc, EVENTS_SELECT ++
            \\WHERE workspace_id = $1::uuid AND agent_id = $2::uuid
            ...
            \\ORDER BY created_at DESC, event_id DESC
            \\LIMIT $4
        , .{ workspace_id, agent_id, al, lim });
    }
    return queryRows(conn, alloc, EVENTS_SELECT ++
        \\WHERE workspace_id = $1::uuid AND agent_id = $2::uuid
        \\ORDER BY created_at DESC, event_id DESC
        \\LIMIT $3
    , .{ workspace_id, agent_id, lim });
}
...
pub fn listForWorkspace(
    ...
    if (agent_id_filter) |zid| {
        return listForAgent(conn, alloc, workspace_id, zid, filter);
    }
...
fn readRow(alloc: std.mem.Allocator, row: pg.Row) !EventRow {
    const agent_id = try alloc.dupe(u8, try row.get([]const u8, 0));
...
test {
    _ = @import("agent_events_store_test.zig");
}
