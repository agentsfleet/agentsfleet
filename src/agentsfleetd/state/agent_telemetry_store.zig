...
//!
//! Writes to `core.agent_execution_telemetry`. All queries use PgQuery (RULE FLS).
...
//! Cursor encode/decode lives in agent_telemetry_cursor.zig.
...
// without it we'd get "agent_execution_telemetryWHERE" (PG syntax error 42601).
const TELEMETRY_SELECT =
    \\SELECT id, tenant_id, workspace_id, agent_id, event_id,
...
pub fn insertTelemetry(
    ...
    _ = try conn.exec(
        ...
        \\  (uid, id, tenant_id, workspace_id, agent_id, event_id,
        \\   charge_type, posture, model,
        \\   credit_deducted_nanos,
        \\   token_count_input, token_count_output, wall_ms,
        \\   recorded_at)
...
fn queryRows(conn: *pg.Conn, alloc: std.mem.Allocator, comptime sql: []const u8, params: anytype) ![]TelemetryRow {
    ...
    while (try q.next()) |row| {
        ...
        const agent_id_s = try alloc.dupe(u8, try row.get([]const u8, 3));
        errdefer alloc.free(agent_id_s);
...
test {
    _ = cursor_mod;
    _ = @import("agent_telemetry_store_test.zig");
}
