...
// (it needs the private siblings). This file exercises the DB-backed surface:
// the debit boundary rules, the starter grant, and the platform pricing paths.
// Pricing depends on no clock, so every assertion here runs unconditionally.
...
test "integration: should fund a fresh tenant from the starter grant alone" {
    const db_ctx = (try base.openTestConn(ALLOC)) orelse return error.SkipZigTest;
    defer db_ctx.pool.deinit();
    defer db_ctx.pool.release(db_ctx.conn);
    ...
    // The starter grant is now the ONLY thing standing between a new tenant and
    // a charged slice — there is no trial window zeroing the rates behind it.
    // So a fresh tenant must hold exactly the grant, and it must be positive:
    // a zero here would mean new accounts are billed with nothing to bill.
    const row = (try tenant_billing.getBilling(db_ctx.conn, ALLOC, TENANT_ID)).?;
    defer ALLOC.free(@constCast(row.grant_source));
