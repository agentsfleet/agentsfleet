...
//!
//! The rate-cache populator keeps its own SELECT in model_rate_cache.zig (a
//! self-contained hot-path module) but shares this module's TABLE constant.
...
/// Public row: model_id (as `id`) + provider + rates. No uid (never exposed
/// publicly), no updated_at_ms (returned separately as the catalogue version).
...
/// The public catalogue plus the max updated_at_ms across the returned rows
...
const SELECT_ADMIN =
    \\SELECT uid::text, provider, model_id, context_cap_tokens,
...
pub fn listForAdmin(alloc: std.mem.Allocator, conn: *pg.Conn) ![]AdminRow {
    var q = PgQuery.from(try conn.query(SELECT_ADMIN ++ " " ++ TABLE ++ "\n ORDER BY provider, model_id", .{}));
    defer q.deinit();
...
/// The public catalogue: all rows ordered by model_id, or the single row whose
/// model_id == filter when given. Also returns the max updated_at_ms seen.
pub fn listForPublic(alloc: std.mem.Allocator, conn: *pg.Conn, filter: ?[]const u8) !PublicList {
    var rows: std.ArrayList(PublicRow) = .empty;
    errdefer rows.deinit(alloc);
    var max_updated_ms: i64 = 0;

    if (filter) |m| {
        var q = PgQuery.from(try conn.query(SELECT_PUBLIC ++ " " ++ TABLE ++ "\n WHERE model_id = $1", .{m}));
        defer q.deinit();
...
pub fn remove(conn: anytype, uid: []const u8) !?i64 {
    return conn.exec("DELETE FROM " ++ TABLE ++ " WHERE uid = $1::uuid", .{uid});
}
