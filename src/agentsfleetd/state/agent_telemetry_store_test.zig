// Integration tests for agent_telemetry_store.zig.

const std = @import("std");
const pg = @import("pg");
const PgQuery = @import("../db/pg_query.zig").PgQuery;
const store = @import("agent_telemetry_store.zig");
...
const AGENTSFLEET_A = "agent-telem-a";
const PLATFORM_MODEL = tenant_provider.PLATFORM_DEFAULT_MODEL;

fn seedStageRow(conn: *pg.Conn, workspace_id: []const u8, agent_id: []const u8, event_id: []const u8, recorded_at: i64) !void {
...
fn seedReceiveRow(conn: *pg.Conn, workspace_id: []const u8, agent_id: []const u8, event_id: []const u8, recorded_at: i64) !void {
    try store.insertTelemetry(conn, ALLOC, .{
        .tenant_id = base.TEST_TENANT_ID,
        .workspace_id = workspace_id,
        .agent_id = agent_id,
...
fn teardownTelemetry(conn: *pg.Conn, workspace_id: []const u8) void {
    _ = conn.exec("DELETE FROM core.agent_execution_telemetry WHERE workspace_id = $1", .{workspace_id}) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
}
...
test "insert_telemetry_idempotent_on_event_charge" {
    ...
    var q = PgQuery.from(try db_ctx.conn.query(
        "SELECT COUNT(*)::BIGINT FROM core.agent_execution_telemetry WHERE workspace_id = $1 AND event_id = $2",
        .{ WS_A, evt },
    ));
    defer q.deinit();
...
test "insert_telemetry_two_rows_per_event" {
    ...
    var q = PgQuery.from(try db_ctx.conn.query(
        "SELECT COUNT(*)::BIGINT FROM core.agent_execution_telemetry WHERE event_id = $1",
        .{evt},
    ));
    defer q.deinit();
    const row = (try q.next()).?;
...
test "insert_receive_has_null_tokens_and_wall_ms" {
    ...
    var q = PgQuery.from(try db_ctx.conn.query(
        ...
        \\FROM core.agent_execution_telemetry
