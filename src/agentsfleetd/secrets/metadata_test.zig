...
//
// Two credential gates read `isLocalRuntime` and both fail OPEN on a true —
// `handlers/tenant_provider_cap.zig` waives catalogue membership, and
// `state/secret_probe.zig` waives the non-empty `api_key` requirement. A name
// that matches when it should not therefore buys a hosted provider two
// exemptions at once, which is why exactness is tested as hard as membership.

test "every local runtime is recognised" {
    // Literal names, not a loop over the array the implementation scans: that
    // form cannot fail for ANY implementation that reads the list, so dropping a
    // provider from the list would keep it green. Membership is a behaviour to
    // pin, and pinning it only in the Python parity scrape leaves Zig blind.
    for ([_][]const u8{
        "litellm",   "llama.cpp", "llamacpp",
        "lm-studio", "lmstudio",  "ollama",
        "osaurus",   "sglang",    "vllm",
    }) |name| {
        try testing.expect(metadata.isLocalRuntime(name));
    }
    try testing.expectEqual(@as(usize, 9), metadata.LOCAL_RUNTIME_PROVIDERS.len);
}

test "a hosted provider is not a local runtime" {
    // These MUST keep enforcing catalogue membership AND a non-empty key: they
    // are billable under platform posture and dial a vendor that authenticates.
...
test "local-runtime matching is exact, never a prefix or case fold" {
    // A near-miss must not silently buy either exemption.
    for ([_][]const u8{ "", "vllm2", "xvllm", "VLLM", "Ollama", "llama.cp", "llama.cppx", "lm-studio " }) |name| {
        try testing.expect(!metadata.isLocalRuntime(name));
    }
}

test "the openai-compatible sentinel is not a local runtime" {
    // Both are exempt from both gates, but for different reasons and by
    // different branches — a custom endpoint dials a URL the tenant supplies,
    // a local runtime dials NullClaw's fixed localhost entry. Collapsing them
    // would let a local runtime smuggle a base_url past validateSecretEndpoint.
    try testing.expect(!metadata.isLocalRuntime(metadata.OPENAI_COMPATIBLE_PROVIDER));
}
