//! Fleet Redis stream operations.
//!
//! Operates on per-fleet streams (fleet:{fleet_id}:events) using
//! XREADGROUP / XAUTOCLAIM / XACK with the shared `fleet_lease` consumer
...
/// argument passed to `xackFleet`. Producer-side does NOT write a
...
/// Build the stream key for a fleet's event stream.
fn fleetStreamKey(buf: []u8, fleet_id: []const u8) ![]const u8 {
    return std.fmt.bufPrint(buf, "{s}{s}{s}", .{
        queue_consts.fleet_stream_prefix,
        fleet_id,
        queue_consts.fleet_stream_suffix,
    });
}
...
pub fn ensureFleetConsumerGroup(client: *redis_client.Client, fleet_id: []const u8) !void {
    ...
    const stream_key = try fleetStreamKey(&key_buf, fleet_id);
...
/// XREADGROUP on fleet:{id}:events reading the consumer's OWN Pending
...
/// double-leasing: the caller holds the fleet's affinity claim, so no other
...
pub fn xreadgroupFleetPending(
    ...
    defer resp.deinit(client.alloc);
    return decodeSingleFleetEvent(client.alloc, resp);
}
...
/// probes many fleets per poll and must not park on any single stream; the
...
/// XAUTOCLAIM one stale fleet event (idle past the comptime-bounded
...
/// ids). The claimed entry is then re-delivered by `xreadgroupFleetPending`
...
pub fn xautoclaimFleet(
    ...
    defer resp.deinit(client.alloc);
    return decodeAutoClaimFleetEvent(client.alloc, resp);
}
...
fn decodeSingleFleetEvent(alloc: std.mem.Allocator, value: redis_protocol.RespValue) !?FleetEvent {
...
fn decodeAutoClaimFleetEvent(alloc: std.mem.Allocator, value: redis_protocol.RespValue) !?FleetEvent {
    if (value != .array) return null;
    const top = value.array orelse return null;
...
fn decodeFleetEventTuple(alloc: std.mem.Allocator, item: redis_protocol.RespValue) !FleetEvent {
...
fn decodeForLeakCheck(alloc: std.mem.Allocator, item: *const redis_protocol.RespValue) !void {
    var ev = try decodeFleetEventTuple(alloc, item.*);
    ev.deinit(alloc);
}
...
test "decodeFleetEventTuple round-trips all present fields" {
    ...
    defer item.deinit(testing.allocator);
    var ev = try decodeFleetEventTuple(testing.allocator, item);
