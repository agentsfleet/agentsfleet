...
const PROBES = [_]Probe{
    ...
    .{ .method = .PATCH, .path = "/v1/tenants/me/models/" ++ ABSENT_MODEL_ID, .token = ADMIN, .owner = "tenant_model_entries.innerUpdateModelEntry", .body = "{\"model_id\":\"claude-sonnet-4-6\"}" },
    .{ .method = .POST, .path = "/v1/workspaces", .token = ADMIN, .owner = "workspaces/lifecycle.innerCreateWorkspace", .body = "{\"name\":\"pool-starved\"}" },
    .{ .method = .PATCH, .path = RUNNER, .token = PLATFORM, .owner = "fleet/runner_patch.innerPatchFleetRunner", .body = "{\"action\":\"drain\"}" },
    ...
    // A patch carrying none of `config_json` / `status` / `trigger_markdown` /
    // `source_markdown` short-circuits to a no-op 200 before any acquire, which
    // is why a name-only body reported success against a starved pool. `status`
    // is the cheapest of the four to clear validation: it takes exactly
    // "active", "stopped" or "killed" ("paused" is reserved for the anomaly
    // gate and rejected).
