...
//! refresh token vaulted so the broker can mint fresh access tokens later.
...
const TEST_EXPIRES_SECONDS: i64 = 86_399;
...
test "linear parseToken: extracts refresh-token pair" {
    var parsed = try std.json.parseFromSlice(std.json.Value, testing.allocator, "{\"access_token\":\"la\",\"refresh_token\":\"lr\",\"expires_in\":" ++ TEST_EXPIRES_SECONDS_TEXT ++ "}", .{});
    defer parsed.deinit();
    const tok = try parseToken(parsed.value.object);
    try testing.expectEqualStrings("la", tok.access_token);
    try testing.expectEqualStrings("lr", tok.refresh_token);
    try testing.expectEqual(TEST_EXPIRES_SECONDS, tok.expires_in_s);
}

test "linear parseToken: missing refresh token is rejected" {
    var parsed = try std.json.parseFromSlice(std.json.Value, testing.allocator, "{\"access_token\":\"la\",\"expires_in\":" ++ TEST_EXPIRES_SECONDS_TEXT ++ "}", .{});
    defer parsed.deinit();
