//! GET /v1/workspaces/{ws}/agents/{id}/events — operator-visible
//! event history for a single agent. Cursor-paginated newest-first
//! over `core.agent_events`. Optional filters: actor glob, since=
...
const log = logging.scoped(.http_agent_events);
...
pub fn innerListEvents(
    ...
    const rows = events_store.listForAgent(conn, hx.alloc, workspace_id, agent_id, filter) catch |err| {
