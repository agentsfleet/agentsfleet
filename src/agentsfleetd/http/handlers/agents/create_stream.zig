//! Install-time event-stream setup for POST /v1/workspaces/{ws}/agents.
//! Ensures the per-agent events stream + consumer group exist before the 201,
...
/// By the time this returns successfully, the per-agent events stream +
...
pub fn ensureEventStream(redis: *queue_redis.Client, agent_id: []const u8) !void {
    var attempt: usize = 0;
    while (true) : (attempt += 1) {
        redis_agent.ensureAgentConsumerGroup(redis, agent_id) catch |err| {
            const sleep_ms = installBackoffMs(attempt) orelse return err;
            log.warn(
                "create_stream_setup_retry",
                .{ .attempt = attempt + 1, .err = @errorName(err), .agent_id = agent_id, .sleep_ms = sleep_ms },
            );
