...
// seed/cleanup/publisher plumbing both suites repeat. Suite-specific agents
...
pub fn seedAgent(conn: *pg.Conn, agent_id: []const u8, name: []const u8) !void {
    _ = try conn.exec(
    ...
    , .{ agent_id, TEST_WORKSPACE_ID, name });
}

pub fn cleanupWorkspaceData(conn: *pg.Conn) void {
    _ = conn.exec("DELETE FROM core.agent_events WHERE workspace_id = $1::uuid", .{TEST_WORKSPACE_ID}) catch |err| std.log.warn(IGNORED_ERROR_FMT, .{@errorName(err)});
    _ = conn.exec("DELETE FROM core.agents WHERE workspace_id = $1::uuid", .{TEST_WORKSPACE_ID}) catch |err| std.log.warn(IGNORED_ERROR_FMT, .{@errorName(err)});
...
pub fn streamPath(alloc: std.mem.Allocator, agent_id: []const u8) ![]u8 {
    return std.fmt.allocPrint(alloc, "/v1/workspaces/{s}/agents/{s}/events/stream", .{ TEST_WORKSPACE_ID, agent_id });
}

pub fn activityChannel(alloc: std.mem.Allocator, agent_id: []const u8) ![]u8 {
    return std.fmt.allocPrint(alloc, "agent:{s}:activity", .{agent_id});
}
