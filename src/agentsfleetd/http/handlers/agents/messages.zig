// POST /v1/workspaces/{ws}/agents/{id}/messages — operator-driven chat ingress.
//
...
// RULE NSQ: schema-qualified SQL (core.agents).
...
const log = logging.scoped(.agent_messages);
...
pub fn innerAgentMessagesPost(hx: Hx, req: *httpz.Request, workspace_id: []const u8, agent_id: []const u8) void {
    ...
    if (!verifyAgentInWorkspace(hx, conn, workspace_id, agent_id)) return;
    ...
    const event_id = hx.ctx.queue.xaddAgentEvent(envelope) catch |err| {
        log.warn("xadd_failed", .{ .agent_id = agent_id, .actor = actor, .err = @errorName(err) });
        common.internalOperationError(hx.res, "failed to enqueue chat event", hx.req_id);
        return;
    };
    defer hx.ctx.alloc.free(event_id);

    log.info("posted", .{ .agent_id = agent_id, .event_id = event_id, .actor = actor });
    hx.ok(.accepted, .{
        .status = "accepted",
        .event_id = event_id,
    });
...
/// Verify agent exists, belongs to the path workspace, and accepts ingress.
/// Writes a 404 on mismatch (do not leak existence across workspaces), a 409
/// with a resume hint on a paused/non-active agent (a steer to
/// a paused agent must refuse loudly, never 202-and-strand), and returns
/// false on every refusal.
fn verifyAgentInWorkspace(
    hx: Hx,
    conn: *pg.Conn,
    ...
    const agent_workspace = row.get([]const u8, 0) catch {
        common.internalDbError(hx.res, hx.req_id);
        return false;
    };

    if (!std.mem.eql(u8, path_workspace_id, agent_workspace)) {
        hx.fail(ec.ERR_AGENTSFLEET_NOT_FOUND, ec.MSG_AGENTSFLEET_NOT_FOUND);
        return false;
    }
    ...
    const status = agent_config.AgentStatus.fromSlice(raw_status) orelse .stopped;
    if (!status.isRunnable()) {
        log.warn("steer_agent_paused", .{
        ...
        // paused vs stopped without re-fetching the agent.
