//! PATCH /v1/workspaces/{ws}/agents/{id} — partial update of a agent.
//!
...
//! txn locks exactly one `core.agents` row → deadlock structurally
...
// innerPatchAgent below; not external API surface.
...
/// Partial update of a agent. Validates inputs, persists in one SQL UPDATE
/// gated by the FSM, and publishes one control-stream signal per dirty
/// surface. Returns 404 if the agent is missing or already-killed (a
/// killed agent is a tombstone — no further state changes apply). Returns
/// 409 if the requested status transition is not allowed from the current
/// state (e.g. resume on an active agent, stop on a stopped agent).
pub fn innerPatchAgent(hx: Hx, req: *httpz.Request, workspace_id: []const u8, agent_id: []const u8) void {
    ...
    {
        hx.ok(.ok, .{ .agent_id = agent_id, .config_revision = @as(?i64, null) });
        return;
    }
    ...
    const outcome = patchAgentInTxn(hx.alloc, conn, workspace_id, agent_id, body) catch |err| {
        log.err("patch_db_failed", .{ .err = @errorName(err), .agent_id = agent_id, .req_id = hx.req_id });
    ...
    const revision = switch (outcome) {
        ...
        .lock_timeout => {
            log.warn("patch_lock_timeout", .{ .agent_id = agent_id, .req_id = hx.req_id });
            common.internalDbUnavailable(hx.res, hx.req_id);
            return;
        },
    };

    // No control-stream signal: `agentsfleetd` resolves a agent's status + config
    ...
    log.info("patched", .{ .id = agent_id, .workspace = workspace_id, .revision = revision, .status_set = body.status });
    if (body.status) |s| {
        hx.ok(.ok, .{ .agent_id = agent_id, .status = s, .config_revision = revision });
    } else {
        hx.ok(.ok, .{ .agent_id = agent_id, .config_revision = revision });
    }
}
...
fn validateBody(hx: Hx, body: PatchBody) bool {
    // config_json and trigger_markdown both drive core.agents.config_json;
    ...
    if (body.status) |s| {
        ...
        const allowed =
            std.mem.eql(u8, s, agent_config.AgentStatus.active.toSlice()) or
            std.mem.eql(u8, s, agent_config.AgentStatus.stopped.toSlice()) or
            std.mem.eql(u8, s, agent_config.AgentStatus.killed.toSlice());
        if (!allowed) {
...
fn patchAgentInTxn(
    ...
    const current = blk: {
        var sel_q = PgQuery.from(conn.query(
            \\SELECT name, status FROM core.agents
            \\WHERE id = $1::uuid AND workspace_id = $2::uuid
            \\FOR UPDATE
        , .{ agent_id, workspace_id }) catch |err| return mapPgErr(conn, err));
        defer sel_q.deinit();
    ...
    const active = agent_config.AgentStatus.active.toSlice();
    const stopped = agent_config.AgentStatus.stopped.toSlice();
    const killed = agent_config.AgentStatus.killed.toSlice();
    const paused = agent_config.AgentStatus.paused.toSlice();
