...
test "integration: api-class requests shed 429 at the ceiling; ops routes and 404s never shed" {
    ...
    try std.testing.expect(scrape.bodyContains("agent_api_in_flight_requests 0"));
}
...
test "integration: registry drain closes live streams and rejects new ones" {
    ...
    defer h.deinit();
    {
        ...
        try fixtures.seedAgent(conn, AGENTSFLEET_DRAIN, "bp-drain");
    }
...
test "integration: the SSE stream class is exempt from the api ceiling" {
    ...
    defer h.deinit();
    {
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        try fixtures.seedAgent(conn, AGENTSFLEET_STREAM_CLASS, "bp-class");
    }
...
test "integration: a stream rejected after registration releases its slot" {
    ...
    // a registry slot but fails authorization (unknown agent → 404) must
...
test "integration: finished streams return their socket fds to the OS" {
    ...
    defer h.deinit();
    {
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        try fixtures.seedAgent(conn, AGENTSFLEET_FD_CYCLE, "bp-fd");
    }
...
test "integration: SSE streams above the cap shed 503 while healthz stays alive" {
    ...
    defer h.deinit();
    {
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        try fixtures.seedAgent(conn, AGENTSFLEET_BACKPRESSURE, "bp-cap");
    }
    ...
    defer scrape.deinit();
    try scrape.expectStatus(.ok);
    try std.testing.expect(scrape.bodyContains("agent_sse_in_flight_streams 1"));
