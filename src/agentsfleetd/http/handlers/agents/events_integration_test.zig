...
//   GET /v1/workspaces/{ws}/agents/{id}/events
//   GET /v1/workspaces/{ws}/events
//   GET /v1/workspaces/{ws}/agents/{id}/events/stream  (auth surface only)
//
...
// Agent ids must NOT collide with cross_workspace_idor_test (…bbb01) or
...
fn seedTestData(conn: *pg.Conn) !void {
    ...
    // Seed a mix of actors across both agents. Distinct created_at for
...
fn insertEvent(conn: *pg.Conn, agent_id: []const u8, event_id: []const u8, actor: []const u8, event_type: []const u8, ts: i64) !void {
    ...
    , .{ uid, agent_id, event_id, TEST_WORKSPACE_ID, actor, event_type, ts });
}

fn cleanupTestData(conn: *pg.Conn) void {
    _ = conn.exec("DELETE FROM core.agent_events WHERE workspace_id = $1::uuid", .{TEST_WORKSPACE_ID}) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
...
test "integration: events GET — no bearer → 401" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
...
test "integration: events GET — since and cursor mutually exclusive → 400" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events?since=2h&cursor=abc", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: events GET — invalid since format → 400" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events?since=bogus", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: events stream — no bearer → 401" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events/stream", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: workspace events GET — sorted DESC + agent_id drill-down filter" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
    ...
    // ?agent_id=AGENTSFLEET_A drills down to that agent's three events.
    {
        const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/events?agent_id={s}", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
        defer ALLOC.free(url);
        const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
        defer r.deinit();
        try r.expectStatus(.ok);
...
test "integration: events GET — since=2h filters by relative duration" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events?since=2h", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
    const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
    defer r.deinit();
    try r.expectStatus(.ok);
...
fn insertEventWithParent(
    ...
    _ = try conn.exec(
        ...
        \\ON CONFLICT (agent_id, event_id) DO NOTHING
    , .{ uid, agent_id, event_id, TEST_WORKSPACE_ID, actor, status, resumes_event_id, ts });
}
...
test "integration: events GET — since=ISO8601 absolute timestamp" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}/events?since=2025-04-25T00:00:00Z", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
    const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
    defer r.deinit();
    try r.expectStatus(.ok);
