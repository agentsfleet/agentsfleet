...
//   - revision bump is visible to the §10a reloadAgentConfig path
//
...
// `src/http/handlers/agents/api_integration_test.zig` — same JWKS kid,
...
// Initial state — a webhook-only agent. Tests will PATCH it to add a
...
fn cleanup(conn: *pg.Conn) void {
    _ = conn.exec("DELETE FROM core.agent_events WHERE agent_id = $1::uuid", .{AGENTSFLEET_ID}) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
...
fn patchUrl() ![]const u8 {
    return std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}", .{ TEST_WORKSPACE_ID, AGENTSFLEET_ID });
}
...
fn readRow(conn: *pg.Conn) !struct {
    ...
    var q = PgQuery.from(try conn.query(
        ...
        \\FROM core.agents WHERE id = $1::uuid
    , .{AGENTSFLEET_ID}));
    defer q.deinit();
...
// `event_loop.reloadAgentConfig`, deleted at the M80 cutover. Config is now
