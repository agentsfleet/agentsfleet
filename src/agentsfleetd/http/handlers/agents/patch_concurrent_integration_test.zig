...
//   - Different agents in parallel: wall time stays sub-linear.
//
...
// uq_agents_workspace_name (workspace_id, name).
...
fn seedFixture(conn: *pg.Conn) !void {
    ...
    // uq_agents_workspace_name forbids two rows sharing (workspace_id, name);
...
fn patchUrl(agent_id: []const u8) ![]const u8 {
    return std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/agents/{s}", .{ TEST_WORKSPACE_ID, agent_id });
}
...
const Worker = struct {
    ...
    // Fires one raw INSERT into core.agent_events with FK ref to `zid`.
...
test "integration: concurrent PATCH different fields — both halves land, no deadlock" {
    ...
    var q = PgQuery.from(try conn.query(
        "SELECT config_json::text, source_markdown FROM core.agents WHERE id = $1::uuid",
        .{AGENTSFLEET_A},
    ));
    defer q.deinit();
...
test "integration: concurrent PATCH same field — last write wins, no deadlock" {
    ...
    defer h.releaseConn(conn);
    var q = PgQuery.from(try conn.query(
        "SELECT config_json::text FROM core.agents WHERE id = $1::uuid",
        .{AGENTSFLEET_A},
    ));
    defer q.deinit();
    const row = (try q.next()) orelse return error.RowNotFound;
...
test "integration: 10 concurrent PATCHes on same agent — all 200, no deadlock" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
...
// ── §4 — PATCH + DELETE on same agent → no deadlock, one final state ───

test "integration: concurrent PATCH + DELETE same agent — no deadlock" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
    ...
    //   (b) DELETE first → PATCH second: DELETE=204, PATCH=404 (agent gone)
...
// ── §5 — Different agents in parallel: near-linear wall time ────────────

test "integration: concurrent PATCH on different agents — parallel, sub-linear" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
    ...
    // Per-agent bodies — each PATCH carries the trigger variant whose name
    ...
    // would drive both rows onto the same value → uq_agents_workspace_name
...
test "integration: PATCH against held lock → 503 in <5.5s, no hang" {
    ...
    const Holder = struct {
        fn run(harness: *TestHarness, started: *std.atomic.Value(bool)) void {
            ...
            _ = c.exec(
                "SELECT id FROM core.agents WHERE id = $1::uuid FOR UPDATE",
                .{AGENTSFLEET_A},
            ) catch return;
...
// ── §7 — Concurrent PATCH + INSERT into agent_events serialize cleanly ──

// PATCH handler takes SELECT FOR UPDATE on the agent row inside its txn;
// an INSERT into core.agent_events with FK ref to core.agents(id) needs
...
test "integration: concurrent PATCH + INSERT into agent_events — both succeed, no deadlock" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
    ...
    defer h.releaseConn(conn);
    {
        var q_evt = PgQuery.from(try conn.query(
            "SELECT COUNT(*)::bigint FROM core.agent_events WHERE agent_id = $1::uuid AND event_id = $2",
