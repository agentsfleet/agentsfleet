...
//!
//!   GET  /v1/runners/me/memory/{agent_id}   → innerRunnerMemoryHydrate
...
//!   POST /v1/runners/me/memory/{agent_id}   → innerRunnerMemoryCapture
...
//! `WHERE agent_id = $1` at the database (never a fetch-all + in-memory filter).
...
const adapter = @import("../../../memory/agent_memory.zig");
...
const S_MALFORMED_AGENTSFLEET_ID = "agent_id must be a valid UUIDv7";
const S_MALFORMED_LEASE_ID = "lease_id must be a valid UUIDv7";
const S_NO_LIVE_LEASE = "runner holds no live lease for this agent";

// ── POST /v1/runners/me/memory/{agent_id} — capture ───────────────────────

/// Persist the run's memory under the path's agent. Fencing-verified; the
/// `agent_id` is validated against the runner's live lease. Each delta is
/// upserted (idempotent). Memory content is NEVER logged — only the count + scope.
pub fn innerRunnerMemoryCapture(hx: Hx, req: *httpz.Request, agent_id: []const u8) void {
    ...
    // AND it is for this path agent (IDOR cross-check), active + unexpired. The
    // agent's live fencing seq fences the write — a reclaimed holder is below it.
    ...
    const live_seq = (pushLeaseSeq(conn, runner_id, body.lease_id, agent_id, now_ms) catch {
    ...
    if (body.fencing_token < live_seq) {
        log.info("memory_push_fenced", .{ .agent_id = agent_id, .fencing_token = body.fencing_token, .live_seq = live_seq });
    ...
    const counts = storeDeltas(hx, conn, agent_id, body.memory, now_ms) orelse return;
    ...
    const swept = adapter.sweepExpiredDaily(conn, agent_id, now_ms - adapter.DAILY_RETENTION_MS) catch blk: {
        log.warn("memory_daily_sweep_failed", .{ .error_code = ec.ERR_MEM_UNAVAILABLE, .agent_id = agent_id });
        break :blk 0;
    };
    ...
    const evicted = adapter.enforceCap(conn, agent_id, protocol.MAX_MEMORY_ENTRIES_PER_AGENT) catch blk: {
        log.warn("memory_cap_evict_failed", .{ .error_code = ec.ERR_MEM_UNAVAILABLE, .agent_id = agent_id });
        break :blk 0;
    };
    metrics_memory.incCapEvictions(evicted);

    metrics_memory.incMemoryCaptured(counts.stored);
    log.info("memory_captured", .{ .agent_id = agent_id, .stored = counts.stored, .skipped = counts.skipped, .evicted = evicted, .swept = swept });
...
fn storeDeltas(hx: Hx, conn: *pg.Conn, agent_id: []const u8, deltas: []const protocol.MemoryDelta, ts_ms: i64) ?StoreCounts {
    ...
    for (deltas) |d| {
        ...
        if (bytes > protocol.MAX_MEMORY_PUSH_BYTES) {
            ...
            log.warn("memory_push_truncated", .{ .agent_id = agent_id, .stored = counts.stored, .cap = protocol.MAX_MEMORY_PUSH_BYTES });
            break;
        }
        const id = h.genId(hx.alloc);
        adapter.storeEntry(conn, id, agent_id, d.key, d.content, d.category, ts_ms) catch {
            metrics_memory.incMemoryPushFailure();
            log.warn("memory_store_failed", .{ .error_code = ec.ERR_MEM_UNAVAILABLE, .agent_id = agent_id });
...
// ── GET /v1/runners/me/memory/{agent_id} — hydrate ────────────────────────

/// Return a category-pinned byte-budget window of the path agent's memory —
/// every fitting `core` entry, then the newest non-core entries — scoped
/// `WHERE agent_id = $1` at the database and passed through the `.selective`
/// `Compactor` (the cold tail stays in Postgres). The runner must hold a live lease.
pub fn innerRunnerMemoryHydrate(hx: Hx, agent_id: []const u8) void {
    const runner_id = hx.principal.runner_id orelse {
        hx.fail(ec.ERR_RUN_INVALID_RUNNER_TOKEN, S_RUNNER_IDENTITY_REQUIRED);
        return;
    };
    ...
    _ = (liveLeaseSeq(conn, runner_id, agent_id, clock.nowMillis()) catch {
        common.internalDbError(hx.res, hx.req_id);
        return;
    }) orelse {
        hx.fail(ec.ERR_RUN_LEASE_NOT_FOUND, S_NO_LIVE_LEASE);
    ...
    const rows = adapter.listAll(hx.alloc, conn, agent_id) catch {
    ...
    log.info("memory_hydrated", .{ .agent_id = agent_id, .count = entries.len, .dropped = rows.len - entries.len, .dropped_bytes = dropped_bytes });
    hx.ok(.ok, protocol.MemoryHydrateResponse{ .memory = entries });
}

// ── lease authorization ────────────────────────────────────────────────────

/// The agent's live fencing seq IFF the presenting runner holds a live (active,
...
/// runner holds no live lease for the agent; error on DB failure.
pub fn liveLeaseSeq(conn: *pg.Conn, runner_id: []const u8, agent_id: []const u8, now_ms: i64) !?u64 {
    var q = PgQuery.from(try conn.query(
        \\SELECT COALESCE(a.fencing_seq, l.fencing_token) AS live_seq
        \\FROM fleet.runner_leases l
        \\LEFT JOIN fleet.runner_affinity a ON a.agent_id = l.agent_id
        \\WHERE l.runner_id = $1::uuid AND l.agent_id = $2::uuid
        \\  AND l.status = $3 AND l.lease_expires_at > $4
        \\ORDER BY l.created_at DESC
        \\LIMIT 1
    , .{ runner_id, agent_id, protocol.RUNNER_LEASE_STATUS_ACTIVE, now_ms }));
    defer q.deinit();
...
/// The agent's live fencing seq IFF the presenting runner holds the named LEASE for
/// the named agent, active + unexpired. Keyed by `lease_id` (like `/reports`) AND
/// `agent_id`, so a lease that exists but is for another agent yields null — the
...
pub fn pushLeaseSeq(conn: *pg.Conn, runner_id: []const u8, lease_id: []const u8, agent_id: []const u8, now_ms: i64) !?u64 {
    var q = PgQuery.from(try conn.query(
        \\SELECT COALESCE(a.fencing_seq, l.fencing_token) AS live_seq
        \\FROM fleet.runner_leases l
        \\LEFT JOIN fleet.runner_affinity a ON a.agent_id = l.agent_id
        \\WHERE l.id = $1::uuid AND l.runner_id = $2::uuid AND l.agent_id = $3::uuid
        \\  AND l.status = $4 AND l.lease_expires_at > $5
        \\LIMIT 1
    , .{ lease_id, runner_id, agent_id, protocol.RUNNER_LEASE_STATUS_ACTIVE, now_ms }));
    defer q.deinit();
    const row = try q.next() orelse return null;
    // fencing seqs are server-issued and monotonic (never negative); a negative
    // value is corrupt/tampered data — fail it cleanly instead of @intCast trapping.
