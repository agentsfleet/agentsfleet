...
//! as the per-fleet endpoint, scoped to workspace, with optional
//! `fleet_id` filter for drill-down. Reads `core.fleet_events`
...
pub fn innerListWorkspaceEvents(
    ...
    const rows = events_store.listForWorkspace(conn, hx.alloc, workspace_id, params.fleet_id, filter) catch |err| {
...
fn parseFilterParams(hx: hx_mod.Hx, qs: anytype) error{Failed}!FilterParams {
    ...
    const fleet_id = qs.get("fleet_id");
...
/// `<prefix>%` with `%` and `_` escaped — see fleets/events.zig for the
