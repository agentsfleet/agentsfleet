...
/// Verify the principal can access the path workspace and the fleet
/// belongs to that workspace, then return the validated fleet_id used to
/// scope the memory query (memory_entries.fleet_id, a $N::uuid parameter).
///
/// Runs under api_runtime (core.* access) before any SET ROLE.
pub fn resolveFleetInWorkspace(
    hx: Hx,
    conn: *pg.Conn,
    ...
    const fleet_ws = row.get([]const u8, 0) catch {
        common.internalDbError(hx.res, hx.req_id);
        return null;
    };
    if (!std.mem.eql(u8, workspace_id, fleet_ws)) {
        // 404 not 403 — don't leak existence across workspaces.
        hx.fail(ec.ERR_MEM_AGENTSFLEET_NOT_FOUND, S_AGENTSFLEET_NOT_FOUND);
        return null;
    }

    // The memory scope is the raw fleet_id (UUID) — the memory_entries.fleet_id
