...
//   GET /v1/workspaces/{ws}/fleets/{id}/events
//   GET /v1/workspaces/{ws}/events
//   GET /v1/workspaces/{ws}/fleets/{id}/events/stream  (auth surface only)
//
...
// Fleet ids must NOT collide with cross_workspace_idor_test (…bbb01) or
...
fn seedTestData(conn: *pg.Conn) !void {
    ...
    _ = try conn.exec(
        ...
        \\VALUES ($1, $2, (SELECT w.tenant_id FROM core.workspaces w WHERE w.id = $2), 'events-a', '---\nname: events-a\n---\ntest', '{"name":"events-a"}', 'active', 0, 0)
        \\ON CONFLICT DO NOTHING
    , .{ AGENTSFLEET_A, TEST_WORKSPACE_ID });
    _ = try conn.exec(
        \\INSERT INTO core.fleets (id, workspace_id, tenant_id, name, source_markdown, config_json, status, created_at, updated_at)
        \\VALUES ($1, $2, (SELECT w.tenant_id FROM core.workspaces w WHERE w.id = $2), 'events-b', '---\nname: events-b\n---\ntest', '{"name":"events-b"}', 'active', 0, 0)
        \\ON CONFLICT DO NOTHING
    , .{ AGENTSFLEET_B, TEST_WORKSPACE_ID });

    // Seed a mix of actors across both fleets. Distinct created_at for
...
fn insertEvent(conn: *pg.Conn, fleet_id: []const u8, event_id: []const u8, actor: []const u8, event_type: []const u8, ts: i64) !void {
    ...
    , .{ fleet_id, event_id, TEST_WORKSPACE_ID, actor, event_type, ts });
}

fn cleanupTestData(conn: *pg.Conn) void {
    _ = conn.exec("DELETE FROM core.fleet_events WHERE workspace_id = $1::uuid", .{TEST_WORKSPACE_ID}) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
    ...
    _ = conn.exec("DELETE FROM workspaces WHERE id = $1", .{TEST_WORKSPACE_ID}) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
}
...
test "integration: events GET — no bearer → 401" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
...
test "integration: events GET — since and cursor mutually exclusive → 400" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events?since=2h&cursor=abc", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: events GET — invalid since format → 400" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events?since=bogus", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: events stream — no bearer → 401" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events/stream", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
...
test "integration: workspace events GET — sorted DESC + fleet_id drill-down filter" {
    const h = seedAndHarness(ALLOC) catch |err| switch (err) {
        error.SkipZigTest => return error.SkipZigTest,
        else => return err,
    };
    ...
    // ?fleet_id=AGENTSFLEET_A drills down to that fleet's three events.
    {
        const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/events?fleet_id={s}", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
        defer ALLOC.free(url);
        const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
        defer r.deinit();
        try r.expectStatus(.ok);
...
test "integration: events GET — since=2h filters by relative duration" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events?since=2h", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
    const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
    defer r.deinit();
    try r.expectStatus(.ok);
...
fn insertEventWithParent(
    ...
    _ = try conn.exec(
        ...
        \\ON CONFLICT (fleet_id, event_id) DO NOTHING
    , .{ fleet_id, event_id, TEST_WORKSPACE_ID, actor, status, resumes_event_id, ts });
}
...
test "integration: resumes_event_id walks the chain via recursive CTE" {
    ...
    var q = PgQuery.from(try conn.query(
        ...
        \\    FROM core.fleet_events e
        \\    JOIN chain c ON e.event_id = c.resumes_event_id
        \\    WHERE e.fleet_id = $1::uuid
        \\)
...
test "integration: events GET — since=ISO8601 absolute timestamp" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events?since=2025-04-25T00:00:00Z", .{ TEST_WORKSPACE_ID, AGENTSFLEET_A });
    defer ALLOC.free(url);
    const r = try (try (h.get(url)).bearer(TOKEN_OPERATOR)).send();
    defer r.deinit();
    try r.expectStatus(.ok);
