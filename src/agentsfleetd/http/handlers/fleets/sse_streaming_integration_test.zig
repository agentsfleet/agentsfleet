...
// against the harness port, PUBLISHes frames to `fleet:{id}:activity`, and
...
fn setupHarness(alloc: std.mem.Allocator) !*TestHarness {
    ...
    inline for (.{ AGENTSFLEET_LATENCY, AGENTSFLEET_RECONNECT, AGENTSFLEET_SEQUENCE }, .{ "sse-latency", "sse-reconnect", "sse-sequence" }) |zid, name| {
        try fixtures.seedFleet(conn, zid, name);
    }
    return h;
}
...
/// connection setup.
fn openAndSettle(alloc: std.mem.Allocator, port: u16, fleet_id: []const u8, opts: SseClient.ConnectOptions) !SseClient {
    const path = try fixtures.streamPath(alloc, fleet_id);
...
fn insertFleetEvent(conn: *pg.Conn, fleet_id: []const u8, event_id: []const u8, ts: i64) !void {
    ...
    , .{ row_id, fleet_id, event_id, fixtures.TEST_WORKSPACE_ID, ts });
}
...
test "integration: SSE reconnect — durable backfill via /events covers the gap" {
    ...
    // Phase 1: live fleet sees event A while operator is connected.
    const ts_base: i64 = 1_900_000_001_000;
    {
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        try insertFleetEvent(conn, AGENTSFLEET_RECONNECT, "1900000001000-0", ts_base);
    }
    ...
    sc1.deinit();
    {
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        try insertFleetEvent(conn, AGENTSFLEET_RECONNECT, "1900000001001-0", ts_base + 1);
        try insertFleetEvent(conn, AGENTSFLEET_RECONNECT, "1900000001002-0", ts_base + 2);
    }

    // Phase 3: operator backfills via /events?since= and observes B + C.
    const events_path = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/fleets/{s}/events?since=1h", .{ fixtures.TEST_WORKSPACE_ID, AGENTSFLEET_RECONNECT });
