...
fn seedWorkspaces(conn: *pg.Conn, now_ms: i64) !void {
    _ = try conn.exec(
        ...
        \\VALUES ($1::uuid, 'GetDetailTest', $2, $2) ON CONFLICT (id) DO NOTHING
...
fn seedFleet(
    ...
    _ = try conn.exec(
        ...
        \\VALUES ($1::uuid, $2::uuid, (SELECT w.tenant_id FROM core.workspaces w WHERE w.id = $2::uuid), $3, $4, $5, '{}'::jsonb, 'active', $6, $7, $7)
