...
fn cleanupTestData(conn: *pg.Conn) void {
    _ = conn.exec("DELETE FROM core.zombie_sessions WHERE zombie_id IN ($1, $2, $3)", .{ AGENTSFLEET_IDLE, AGENTSFLEET_ACTIVE, AGENTSFLEET_OTHER_WS }) catch |err| std.log.warn("ignored: {s}", .{@errorName(err)});
...
test "integration: zombie messages — auth and body validation" {
    ...
    const url_idle = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/zombies/{s}/messages", .{ TEST_WORKSPACE_ID, AGENTSFLEET_IDLE });
    defer ALLOC.free(url_idle);
    // url_other: caller's workspace in URL path, but zombie lives in OTHER_WS — handler 404.
    const url_other = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/zombies/{s}/messages", .{ TEST_WORKSPACE_ID, AGENTSFLEET_OTHER_WS });
...
test "integration: zombie messages idle — 202 returns event_id from xadd" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/zombies/{s}/messages", .{ TEST_WORKSPACE_ID, AGENTSFLEET_IDLE });
...
test "integration: steer paused zombie — 409 UZ-ZMB-012; resumed zombie steers fine" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/zombies/{s}/messages", .{ TEST_WORKSPACE_ID, AGENTSFLEET_PAUSED });
    defer ALLOC.free(url);
    ...
    if (h.has_redis) { // resume → the same steer 202s (terminal refusals never block re-request)
        const conn = try h.acquireConn();
        defer h.releaseConn(conn);
        _ = try conn.exec("UPDATE core.zombies SET status = 'active' WHERE id = $1", .{AGENTSFLEET_PAUSED});
        const r = try (try (try h.post(url).bearer(TOKEN_OPERATOR)).json("{\"message\":\"wake up\"}")).send();
        defer r.deinit();
        try r.expectStatus(.accepted);
    ...
    const conn = try h.acquireConn();
    defer h.releaseConn(conn);
    _ = conn.exec("DELETE FROM core.zombies WHERE id = $1", .{AGENTSFLEET_PAUSED}) catch {};
    cleanupTestData(conn);
}
...
test "integration: zombie messages active — 202 returns event_id (same single ingress)" {
    ...
    const url = try std.fmt.allocPrint(ALLOC, "/v1/workspaces/{s}/zombies/{s}/messages", .{ TEST_WORKSPACE_ID, AGENTSFLEET_ACTIVE });
    defer ALLOC.free(url);
