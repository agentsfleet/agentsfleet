// Fleet configuration façade — re-exports types + parse/validate/markdown
// entry points so consumers keep a single import point (`config.X`).
//
// Directory-based fleet format (SKILL.md + TRIGGER.md). `agentsfleet install
...
//   - parseFleetConfig(alloc, config_json_bytes)  → FleetConfig struct
//   - extractFleetInstructions(source_markdown)    → system prompt slice (borrowed)
//
// Implementation lives in:
//   - config_types.zig     — value types + destructors
//   - config_parser.zig    — JSON → FleetConfig, per-field helpers
...
// Value types.
pub const FleetConfigError = config_types.FleetConfigError;
pub const FleetStatus = config_types.FleetStatus;
pub const FleetTrigger = config_types.FleetTrigger;
...
pub const FleetNetwork = config_types.FleetNetwork;
pub const FleetConfig = config_types.FleetConfig;
...
pub const extractFleetInstructions = config_markdown.extractFleetInstructions;
...
test {
    ...
    _ = @import("repository_binding_json.zig");
