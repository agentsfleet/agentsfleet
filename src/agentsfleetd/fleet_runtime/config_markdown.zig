// Fleet config markdown/frontmatter extraction.
//
// TRIGGER.md carries YAML frontmatter between `---` delimiters. Two entry
// points:
//   - extractFleetInstructions: borrow the body below the frontmatter.
//
...
pub fn extractFleetInstructions(source_markdown: []const u8) []const u8 {
    const fm = scanFrontmatter(source_markdown) orelse return "";
    return fm.body;
}

/// Aggregate returned by `parseTriggerMarkdownWithJson` — the parsed
/// FleetConfig plus the JSON that was used to derive it. The JSON is the
/// canonical `config_json` shape (what `parseFleetConfig` accepts) and is
/// what the install handler persists into core.fleets. Owning both lets
...
/// Same pipeline as `parseFleetFromTriggerMarkdown`, but also returns the
...
) (Allocator.Error || FleetConfigError)!ParsedTrigger {
    const fm = scanFrontmatter(trigger_markdown) orelse
        return FleetConfigError.MissingRequiredField;

    const json = yaml_frontmatter.yamlFrontmatterToJson(alloc, fm.yaml) catch |err| switch (err) {
        error.OutOfMemory => return error.OutOfMemory,
        error.ParseFailure => return FleetConfigError.MissingRequiredField,
    };
    errdefer alloc.free(json);

    const config = try config_parser.parseFleetConfig(alloc, json);
...
) (Allocator.Error || FleetConfigError)!SkillMetadata {
...
fn dupeRequiredString(
    ...
    const val = root.get(key) orelse return FleetConfigError.MissingRequiredField;
    const s = switch (val) {
        .string => |str| str,
        else => return FleetConfigError.MissingRequiredField,
    };
...
fn dupeOptionalStringArray(
    ...
    while (i < arr.items.len) : (i += 1) {
        ...
        const s = switch (arr.items[i]) {
            .string => |str| str,
            else => return FleetConfigError.InvalidTagFormat,
        };
...
test "parseTriggerMarkdownWithJson: returns both parsed config and owned JSON" {
    ...
    try std.testing.expectEqualStrings("test-fleet", parsed.config.name);
    var reparsed = try config_parser.parseFleetConfig(alloc, parsed.config_json);
    defer reparsed.deinit(alloc);
    try std.testing.expectEqualStrings("test-fleet", reparsed.name);
}

test "parseTriggerMarkdownWithJson: missing frontmatter → MissingRequiredField" {
    const alloc = std.testing.allocator;
    try std.testing.expectError(
        FleetConfigError.MissingRequiredField,
...
test "parseTriggerMarkdownWithJson: parse failure inside frontmatter → JSON freed (no leak)" {
    const alloc = std.testing.allocator;
    // Missing required `name:` — yaml→json succeeds, parseFleetConfig fails.
...
test "parseTriggerMarkdownWithJson: malformed gate condition parses leniently; flagged at write boundary" {
    ...
    // already-stored fleet keeps loading (no silent claim-time hard-fail). The
