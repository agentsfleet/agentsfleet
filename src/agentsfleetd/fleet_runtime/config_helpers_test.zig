...
// `config_parser.parseFleetConfig` — the array-iteration helper is not
...
fn parseTriggerForTest(src: []const u8) !FleetTrigger {
    const parsed = try std.json.parseFromSlice(std.json.Value, test_alloc, src, .{});
    defer parsed.deinit();
    return config_helpers.parseFleetTrigger(test_alloc, parsed.value.object);
}

fn freeTrigger(t: FleetTrigger) void {
    config_types.freeFleetTrigger(test_alloc, t);
}
...
// `parseFleetConfig` entry. On success the caller owns just the triggers
// slice — the rest of the parsed config is freed before return.
fn parseTriggersForTest(src: []const u8) ![]const FleetTrigger {
    ...
    var cfg = try config_parser.parseFleetConfig(test_alloc, wrapped);
...
fn freeTriggers(trs: []const FleetTrigger) void {
    for (trs) |t| config_types.freeFleetTrigger(test_alloc, t);
    test_alloc.free(trs);
}
...
test "parseWebhookSignature: missing secret_ref is rejected" {
    ...
    ;
    try std.testing.expectError(FleetConfigError.InvalidSignatureConfig, parseTriggerForTest(src));
}
...
test "parseFleetTrigger.webhook: events is admitted as null when absent" {
    const src =
...
test "parseFleetTrigger.webhook: events of length 1 is admitted" {
    const src =
...
test "parseFleetTrigger.webhook: events with mixed entries is admitted" {
    const src =
...
test "parseFleetTrigger.webhook: empty events array is rejected" {
    const src =
        \\{"type":"webhook","source":"github","events":[]}
    ;
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggerForTest(src));
}

test "parseFleetTrigger.webhook: events array over MAX_EVENTS_PER_TRIGGER is rejected" {
...
test "parseFleetTrigger.webhook: events entry containing whitespace is rejected" {
    const src =
...
test "parseFleetTrigger.webhook: empty events entry is rejected" {
    const src =
        \\{"type":"webhook","source":"github","events":[""]}
    ;
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggerForTest(src));
...
test "parseFleetTrigger.webhook: events entry over MAX_EVENT_NAME_LEN is rejected" {
...
test "parseFleetTrigger.webhook: non-string events entry is rejected" {
    const src =
...
test "parseFleetTrigger.webhook: non-array events value is rejected" {
    const src =
        \\{"type":"webhook","source":"github","events":"workflow_run"}
    ;
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggerForTest(src));
...
test "parseFleetTrigger.webhook: credential_name absent defaults to null" {
    const src =
        \\{"type":"webhook","source":"github"}
    ;
    const trig = try parseTriggerForTest(src);
...
test "parseFleetTrigger.webhook: credential_name override is dup'd into the trigger" {
    const src =
...
test "parseFleetTrigger: type=api is admitted" {
    const src =
        \\{"type":"api"}
    ;
    const trig = try parseTriggerForTest(src);
...
test "parseFleetTrigger: type=foobar is rejected with InvalidTriggerType" {
    const src =
        \\{"type":"foobar"}
    ;
    try std.testing.expectError(FleetConfigError.InvalidTriggerType, parseTriggerForTest(src));
}

// ── parseFleetTriggers: array length + cross-entry rules ─────────────────

test "parseFleetTriggers: single webhook entry is admitted" {
...
test "parseFleetTriggers: empty array is rejected" {
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggersForTest(
        \\[]
    ));
}

test "parseFleetTriggers: array over MAX_TRIGGERS_PER_AGENT is rejected" {
    // 9 entries — one over MAX_TRIGGERS_PER_AGENT (8).
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggersForTest(
        \\[
...
test "parseFleetTriggers: duplicate (type, source) tuple is rejected" {
    try std.testing.expectError(FleetConfigError.InvalidTriggerType, parseTriggersForTest(
        \\[
...
test "parseFleetTriggers: distinct webhook sources are admitted" {
    const trs = try parseTriggersForTest(
        \\[
        \\  {"type":"webhook","source":"github","events":["workflow_run"]},
...
test "parseFleetTriggers: duplicate api entries are rejected" {
    try std.testing.expectError(FleetConfigError.InvalidTriggerType, parseTriggersForTest(
        \\[
        \\  {"type":"api"},
        \\  {"type":"api"}
...
test "parseFleetTriggers: more than one cron entry is rejected" {
    try std.testing.expectError(FleetConfigError.InvalidTriggerType, parseTriggersForTest(
        \\[
...
test "parseFleetTriggers: webhook plus cron is admitted" {
    const trs = try parseTriggersForTest(
        \\[
        \\  {"type":"webhook","source":"github","events":["workflow_run"]},
    ...
    defer freeTriggers(trs);
    try std.testing.expectEqual(@as(usize, 2), trs.len);
    try std.testing.expectEqual(config_types.FleetTriggerType.cron, @as(config_types.FleetTriggerType, trs[1]));
    try std.testing.expectEqualStrings("0 3 * * *", trs[1].cron.schedule);
}

test "parseFleetTriggers: webhook plus api is admitted" {
    const trs = try parseTriggersForTest(
        \\[
        \\  {"type":"webhook","source":"github"},
        \\  {"type":"api"}
    ...
    defer freeTriggers(trs);
    try std.testing.expectEqual(@as(usize, 2), trs.len);
    try std.testing.expectEqual(config_types.FleetTriggerType.api, @as(config_types.FleetTriggerType, trs[1]));
}

test "parseFleetTriggers: non-object array element is rejected" {
    try std.testing.expectError(FleetConfigError.InvalidFieldType, parseTriggersForTest(
        \\[
        \\  {"type":"webhook","source":"github"},
        \\  "not an object"
