...
// success or the specific FleetConfigError variant on failure.
...
pub fn validateSkillName(name: []const u8) FleetConfigError!void {
    if (name.len == 0 or name.len > MAX_NAME_LEN) return FleetConfigError.InvalidNameFormat;
    for (name) |c| {
        const ok = (c >= 'a' and c <= 'z') or (c >= '0' and c <= '9') or c == '-';
        if (!ok) return FleetConfigError.InvalidNameFormat;
    }
}
...
pub fn validateSkillVersion(version: []const u8) FleetConfigError!void {
    if (version.len == 0) return FleetConfigError.InvalidVersionFormat;
    var part_count: usize = 0;
    var part_len: usize = 0;
    var part_first: u8 = 0;
    for (version) |c| {
        if (c == '.') {
            if (part_len == 0) return FleetConfigError.InvalidVersionFormat;
            if (part_len > 1 and part_first == '0') return FleetConfigError.InvalidVersionFormat;
            part_count += 1;
            part_len = 0;
            continue;
        }
        if (c < '0' or c > '9') return FleetConfigError.InvalidVersionFormat;
    ...
    if (part_len > 1 and part_first == '0') return FleetConfigError.InvalidVersionFormat;
    part_count += 1;
    if (part_count != 3) return FleetConfigError.InvalidVersionFormat;
}
