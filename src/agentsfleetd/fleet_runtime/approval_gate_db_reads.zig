...
pub fn listPending(
    ...
    const fleet_param: []const u8 = filter.fleet_id orelse "";
...
fn readPendingRow(alloc: Allocator, row: pg.Row) !PendingRow {
    ...
    const fleet_id = try dupTracked(alloc, &owned, try row.get([]const u8, 1));
    const fleet_name = try dupTracked(alloc, &owned, try row.get([]const u8, 2));
