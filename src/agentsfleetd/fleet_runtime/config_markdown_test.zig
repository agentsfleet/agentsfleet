...
const extractFleetInstructions = config_markdown.extractFleetInstructions;
...
test "extractFleetInstructions: returns body after frontmatter" {
    const md =
        \\---
        \\name: platform-ops
        \\---
    ...
    ;
    const instructions = extractFleetInstructions(md);
    try std.testing.expectEqualStrings("You are a lead collector.", instructions);
}

test "extractFleetInstructions: no frontmatter returns empty" {
    const instructions = extractFleetInstructions("Just plain markdown with no frontmatter.");
    try std.testing.expectEqualStrings("", instructions);
}

test "extractFleetInstructions: foo: ---bar inside YAML is not the closing delim" {
    const md =
        \\---
        \\name: foo: ---bar
        \\---
...
test "extractFleetInstructions: empty body after frontmatter" {
    const md =
        \\---
        \\name: x
        \\---
...
test "parseSkillMetadata: required fields populated, optional null" {
    ...
    const skill_md =
        \\---
        \\name: platform-ops-fleet
        \\description: Diagnoses platform health.
        \\version: 0.1.0
        \\---
        \\
        \\You are Platform Ops Fleet.
    ;
    var meta = try parseSkillMetadata(alloc, skill_md);
    defer meta.deinit(alloc);
    try std.testing.expectEqualStrings("platform-ops-fleet", meta.name);
...
test "parseSkillMetadata: non-string tag element → InvalidTagFormat" {
    ...
    ;
    try std.testing.expectError(
        FleetConfigError.InvalidTagFormat,
        parseSkillMetadata(alloc, skill_md),
    );
}
...
test "parseTriggerMarkdownWithJson: JSON shape has x-agentsfleet at top, runtime keys nested" {
    ...
    const x = root.get("x-agentsfleet") orelse return error.MissingUsefleetBlock;
