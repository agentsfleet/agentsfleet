// Anomaly detection: Redis-backed sliding-window counters per (fleet_id, tool, action).
...
/// Uses Redis INCR + EXPIRE for sliding window per (fleet_id, tool, action).
pub fn checkAnomaly(
    ...
    for (rules) |rule| {
        ...
        const count = incrAnomalyCounter(redis, fleet_id, tool, action, rule.threshold_window_s) catch {
            ...
            log.warn("redis_fail", .{ .fleet_id = fleet_id });
            return .normal;
        };
