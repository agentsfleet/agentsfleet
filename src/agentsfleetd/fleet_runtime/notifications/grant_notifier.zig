...
//!
//! Redis key: grant:notify:{fleet_id}:{grant_id}
...
fn buildGrantSlackMessage(
    ...
    const header = try std.fmt.allocPrint(alloc, "\u{1F510} *{s}* is requesting *{s}* access", .{ fleet_name, service });
...
fn storeToRedis(
    ...
    const key = std.fmt.bufPrint(&key_buf, "{s}{s}:{s}", .{
        GRANT_NOTIFY_KEY_PREFIX, fleet_id, grant_id,
    }) catch return;
...
test "buildGrantSlackMessage: produces valid JSON with grant_id and nonce in button values" {
    ...
    const msg = try buildGrantSlackMessage(alloc, "my-fleet", "slack", "Need to post to #hiring", "uzg_001", nonce);
...
test "buildGrantSlackMessage: escapes backslash in fleet_name" {
    const alloc = std.testing.allocator;
...
test "buildGrantSlackMessage: approve and deny buttons both carry the button value" {
    ...
    const msg = try buildGrantSlackMessage(alloc, "fleet", "discord", "post updates", grant_id, nonce);
    defer alloc.free(msg);
    const expected_value = grant_id ++ "|" ++ nonce;
