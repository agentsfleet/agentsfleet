...
const WS_MEM = "0195b4ba-8d3a-7f13-8abc-0000000c0002";
...
/// Seed memory for the probe fleet, plus the tenant → workspace → fleet chain it
/// now needs: schema/821 gave `memory_entries.fleet_id` a REFERENCES to
/// `core.fleets`, so every id in this table must be a real parent row. The rows
/// used to spread across fabricated fleet ids for selectivity — the fitness check
/// forces the index, so that spread bought nothing this suite asserts on, and one
/// real fleet replaces two hundred imaginary ones.
fn seedMemory(conn: *pg.Conn, rows: u32) !void {
    try base.seedTenant(conn);
    try base.seedWorkspace(conn, WS_MEM);
    try base.seedFleet(conn, FLEET_MEM, WS_MEM, "index-probe-memory", "{}", "# SKILL");
    _ = try conn.exec(
    ...
    , .{ MEM_KEY_PREFIX, FLEET_MEM, @as(i32, @intCast(rows)) });
