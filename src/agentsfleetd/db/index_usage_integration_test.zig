...
/// How many parent fleets the spread lands on. The probe filters ONE fleet with
/// `enable_seqscan = off`, so what the spread has to prove is that the index is
/// not degenerate — several distinct `fleet_id`s, not several hundred. It used to
/// create one parent per seeded row, which put ~200 rows through `core.fleets`
/// per run; deleting them left the table at 4 pages instead of 1, and because the
/// planner costs a scan from `relpages` a SIBLING suite's read then switched off
/// the index it asserts onto `fleets_pkey`. Only `VACUUM` reclaims those pages,
/// so the fixture was paying for a `VACUUM ANALYZE` to undo damage it had no
/// reason to do. Four keeps `core.fleets` inside one page.
const MEM_PARENT_FLEETS: i32 = 4;
...
fn seedMemory(conn: *pg.Conn, rows: u32) !void {
    ...
    // a hand-listed set would drift the moment the spread's modulus changed. The
    // modulus is passed as a PARAMETER to both statements rather than spelled as
    // a literal in each, so the two expressions cannot drift apart at all.
    //
    ...
    // not UUIDv7-shaped and the check refuses it.
    ...
    _ = try conn.exec(
        ...
        \\SELECT overlay(md5((g % $3::int)::text)::uuid::text placing '7' from 15 for 1)::uuid,
        \\       w.id, w.tenant_id,
        \\       $2 || (g % $3::int)::text, '', '{}'::jsonb, 'active', 0, 0
        \\FROM generate_series(1, $3::int) g
        \\CROSS JOIN core.workspaces w WHERE w.id = $1::uuid
        \\ON CONFLICT DO NOTHING
    , .{ WS_MEM, MEM_FLEET_NAME_PREFIX, MEM_PARENT_FLEETS });
    _ = try conn.exec(
        ...
        \\            ELSE overlay(md5((g % $5::int)::text)::uuid::text placing '7' from 15 for 1)::uuid END,
        \\       1750000000000 + g, 1750000000000 + g
        \\FROM generate_series(1, $4::int) g
        \\ON CONFLICT DO NOTHING
    , .{ MEM_KEY_PREFIX, FLEET_MEM, @as(i32, PROBE_FLEET_ROWS), @as(i32, @intCast(rows)), MEM_PARENT_FLEETS });
...
fn wipeMemory(conn: *pg.Conn) void {
    ...
    // Drop the parent fleets with their workspace. Sibling tests in this file
    // assert which index the planner picks on `core.fleets`, and that choice is
    // costed from the table's page count — so what matters is that this fixture
    // never grew the table enough to move it. `MEM_PARENT_FLEETS` keeps the seed
    // inside one page, which is why no VACUUM or ANALYZE follows this delete: a
    // table that was never bloated has nothing to reclaim.
