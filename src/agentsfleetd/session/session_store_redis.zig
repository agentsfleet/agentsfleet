...
pub const SessionStore = struct {
    ...
    pub fn approve(
        ...
        const ttl_str = std.fmt.bufPrint(&ttl_buf, DECIMAL_FMT, .{SESSION_TTL_SECONDS}) catch return Error.RedisError;
        const now_str = std.fmt.bufPrint(&now_buf, DECIMAL_FMT, .{clock.nowMillis()}) catch return Error.RedisError;
    ...
    pub fn verifyAndConsume(
        ...
        var ttl_buf: [12]u8 = undefined;
        const now_str = try std.fmt.bufPrint(&now_buf, DECIMAL_FMT, .{clock.nowMillis()});
        const win_str = try std.fmt.bufPrint(&win_buf, DECIMAL_FMT, .{CONSUME_REPLAY_WINDOW_MS});
        const att_str = try std.fmt.bufPrint(&att_buf, DECIMAL_FMT, .{MAX_VERIFY_ATTEMPTS});
        const ttl_str = try std.fmt.bufPrint(&ttl_buf, DECIMAL_FMT, .{SESSION_TTL_SECONDS});
