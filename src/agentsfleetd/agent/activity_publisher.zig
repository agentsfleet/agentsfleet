...
//! owns the agent's event loop), zero-or-N subscribers (SSE handlers).
...
//! and swallowed. The durable record (agent_events + telemetry +
...
fn buildChannel(buf: []u8, agent_id: []const u8) ![]const u8 {
    return std.fmt.bufPrint(buf, "{s}{s}{s}", .{ channel_prefix, agent_id, channel_suffix });
}

fn publishRaw(client: *Client, agent_id: []const u8, payload: []const u8) void {
    var channel_buf: [128]u8 = undefined;
    const channel = buildChannel(&channel_buf, agent_id) catch |err| {
        log.debug("channel_format_fail", .{ .agent_id = agent_id, .err = @errorName(err) });
        return;
    };
    client.publish(channel, payload) catch |err| {
        log.debug("publish_fail", .{ .agent_id = agent_id, .err = @errorName(err) });
    };
}
...
fn encodeAndPublish(
    ...
    publishRaw(client, agent_id, scratch.written());
}
...
pub fn publishEventReceived(
    ...
    encodeAndPublish(client, scratch, agent_id, KIND_EVENT_RECEIVED, .{
...
pub fn publishToolCallStarted(
    ...
    defer parsed.deinit();
    encodeAndPublish(client, scratch, agent_id, KIND_TOOL_CALL_STARTED, .{
...
pub fn publishToolCallProgress(
    ...
    encodeAndPublish(client, scratch, agent_id, KIND_TOOL_CALL_PROGRESS, .{
...
pub fn publishChunk(
    ...
    encodeAndPublish(client, scratch, agent_id, KIND_CHUNK, .{
        .kind = KIND_CHUNK,
        .event_id = event_id,
        .text = text,
    });
...
pub fn publishToolCallCompleted(
    ...
    encodeAndPublish(client, scratch, agent_id, KIND_TOOL_CALL_COMPLETED, .{
...
pub fn publishEventComplete(
    ...
    encodeAndPublish(client, scratch, agent_id, KIND_EVENT_COMPLETE, .{
