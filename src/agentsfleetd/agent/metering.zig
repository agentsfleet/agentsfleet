...
const agent_telemetry_store = @import("../state/agent_telemetry_store.zig");
...
const log = logging.scoped(.agent_metering);
...
/// Emit the `agent.delivery` OTel span for a finished run, from the same
/// (agent_id, workspace_id, tenant_id, event_id, token_count) tuple the
...
pub fn emitDeliverySpan(
    ...
    if (epoch_wall_time_ms <= 0) {
        log.warn("skip_delivery_span", .{ .reason = "non_positive_epoch", .agent_id = ctx.agent_id });
        return;
    }
    ...
    var span = otel_traces.buildSpan(tctx, "agent.delivery", start_ns, end_ns);
    _ = otel_traces.addAttr(&span, "agent_id", ctx.agent_id);
...
fn debitAndInsert(
    ...
    tenant_id: []const u8,
    ctx: PreflightContext,
    charge_type: agent_telemetry_store.ChargeType,
    nanos: i64,
    policy: balance_policy.Policy,
) DebitOutcome {
    const conn = pool.acquire() catch |err| {
        log.warn("acquire_fail", .{ .agent_id = ctx.agent_id, .err = @errorName(err) });
        return .{ .db_error = {} };
    };
    defer pool.release(conn);

    _ = conn.exec("BEGIN", .{}) catch |err| {
        log.warn("begin_fail", .{ .agent_id = ctx.agent_id, .err = @errorName(err) });
        return .{ .db_error = {} };
    };
    var tx_open = true;
    defer if (tx_open) {
        ...
        _ = tenant_billing.debit(conn, tenant_id, nanos) catch |err| switch (err) {
            error.CreditExhausted => {
                _ = tenant_billing.markExhausted(conn, tenant_id) catch |mark_err| {
                    log.warn("mark_exhausted_fail", .{ .agent_id = ctx.agent_id, .tenant_id = tenant_id, .err = @errorName(mark_err) });
                };
                _ = conn.exec(S_COMMIT, .{}) catch |commit_err| log.warn(COMMIT_FAIL_EVENT, .{ .err = @errorName(commit_err) });
                tx_open = false;
                onExhaustedDebit(ctx.agent_id, tenant_id, charge_type, nanos, policy);
                return .{ .exhausted = {} };
            },
            error.TenantBillingMissing => {
                ...
                conn.rollback() catch |rollback_err| log.warn(ROLLBACK_FAIL_EVENT, .{ .err = @errorName(rollback_err) });
                tx_open = false;
                log.warn("debit_fail", .{ .agent_id = ctx.agent_id, .tenant_id = tenant_id, .err = @errorName(err) });
                return .{ .db_error = {} };
            },
        };
    }

    agent_telemetry_store.insertTelemetry(conn, alloc, .{
        .tenant_id = tenant_id,
        .workspace_id = ctx.workspace_id,
        .agent_id = ctx.agent_id,
        ...
        tx_open = false;
        log.warn("telemetry_insert_fail", .{ .agent_id = ctx.agent_id, .event_id = ctx.event_id, .charge_type = charge_type.label(), .err = @errorName(err) });
        return .{ .db_error = {} };
    };

    _ = conn.exec(S_COMMIT, .{}) catch |err| {
        log.warn(COMMIT_FAIL_EVENT, .{ .agent_id = ctx.agent_id, .err = @errorName(err) });
        return .{ .db_error = {} };
    };
    tx_open = false;
