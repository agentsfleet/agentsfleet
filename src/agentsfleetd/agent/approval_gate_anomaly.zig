// Anomaly detection: Redis-backed sliding-window counters per (agent_id, tool, action).
...
/// Uses Redis INCR + EXPIRE for sliding window per (agent_id, tool, action).
pub fn checkAnomaly(
    ...
    for (rules) |rule| {
        ...
        const count = incrAnomalyCounter(redis, agent_id, tool, action, rule.threshold_window_s) catch {
            ...
            log.warn("redis_fail", .{ .agent_id = agent_id });
            return .normal;
        };
...
fn incrAnomalyCounter(
    ...
    const key = std.fmt.bufPrint(&key_buf, "{s}{s}:{s}:{s}", .{
        ec.GATE_ANOMALY_KEY_PREFIX, agent_id, tool, action,
