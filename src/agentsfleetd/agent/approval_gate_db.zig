...
//
// Writes to core.agent_approval_gates. The schema-level append-only trigger
...
/// ONLY the two hard-purge paths (account teardown, agent hard-delete) may
...
pub const SET_GATE_PURGE_BYPASS_SQL = "SET LOCAL agent.allow_gate_purge = 'on'";
...
pub fn recordGatePending(
    ...
    insertPendingRow(pool, alloc, agent_id, workspace_id, action_id, detail) catch |err| {
...
///
/// `agent_id_filter` binds the resolution to a specific agent when the
...
/// `action_id` and `agent_id` from the same untrusted payload (Slack
...
pub const ResolveArgs = struct {
    ...
    pub fn atomic(self: Self, pool: *pg.Pool, alloc: Allocator) !ResolveDbOutcome {
        ...
        var update_q = PgQuery.from(try conn.query(
            \\UPDATE core.agent_approval_gates
            \\SET status = $1, detail = $2, resolved_by = $3, updated_at = $4
            \\WHERE action_id = $5 AND status = $6
            \\  AND ($7::text = '' OR agent_id::text = $7)
            \\RETURNING id::text, action_id, workspace_id::text, agent_id::text,
            \\          status, COALESCE(updated_at, $4::bigint), resolved_by, detail
        , .{ self.outcome.toSlice(), self.reason, self.by, now_ms, self.action_id, PENDING_STATUS, self.agent_id_filter }));
        ...
        var select_q = PgQuery.from(try conn.query(
            \\SELECT id::text, action_id, workspace_id::text, agent_id::text,
            \\       status, COALESCE(updated_at, requested_at), resolved_by, detail
            \\FROM core.agent_approval_gates
            \\WHERE action_id = $1
            \\  AND ($2::text = '' OR agent_id::text = $2)
            \\ORDER BY requested_at DESC LIMIT 1
        , .{ self.action_id, self.agent_id_filter }));
...
fn readResolvedRow(alloc: Allocator, row: pg.Row) !ResolvedRow {
    ...
    const agent_id = try alloc.dupe(u8, try row.get([]const u8, 3));
