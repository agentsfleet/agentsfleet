// Approval gate — 3D Secure model for high-stakes Agent actions.
//
...
/// Status values stored in core.agent_approval_gates.status column.
...
pub fn requestApproval(
    ...
    const pending_key = try std.fmt.bufPrint(&pending_key_buf, "{s}{s}:{s}", .{
        ec.GATE_PENDING_KEY_PREFIX, agent_id, action_id,
    });
...
///   - Slack interactive callback (`/webhooks/{agent_id}/approval`)
...
///
/// See `ResolveArgs.agent_id_filter` for the cross-agent defense channels
