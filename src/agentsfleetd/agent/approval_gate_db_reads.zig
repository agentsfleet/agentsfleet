...
pub fn listPending(
    ...
    const agent_param: []const u8 = filter.agent_id orelse "";
    ...
    var q = PgQuery.from(try conn.query(
        \\SELECT g.id::text, g.agent_id::text, COALESCE(z.name, ''),
        ...
        \\FROM core.agent_approval_gates g
        \\JOIN core.agents z ON z.id = g.agent_id
        \\WHERE g.workspace_id = $1::uuid
        \\  AND g.status = $2
        \\  AND ($3 = '' OR g.agent_id = $3::uuid)
...
fn readPendingRow(alloc: Allocator, row: pg.Row) !PendingRow {
    ...
    const agent_id = try dupTracked(alloc, &owned, try row.get([]const u8, 1));
    const agent_name = try dupTracked(alloc, &owned, try row.get([]const u8, 2));
