...
//!   byevent ref: agent:gate:byevent:{agent_id}:{event_id} → "action_id|deadline_ms"
...
pub fn recordEventGateRef(
    ...
    const key = try std.fmt.bufPrint(&key_buf, REF_KEY_FMT, .{ ec.GATE_EVENT_REF_KEY_PREFIX, agent_id, event_id });
