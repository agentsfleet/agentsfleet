...
pub fn buildSlackApprovalMessage(
    ...
    const desc = try std.fmt.allocPrint(alloc, "Agent `{s}` wants to execute:\n- Tool: `{s}`\n- Action: `{s}`\n- Details: {s}", .{
        agent_name, detail.tool, detail.action, detail.params_summary,
    });
    defer alloc.free(desc);
    const fallback = try std.fmt.allocPrint(alloc, "Approval required for {s}: {s}.{s}", .{
        agent_name, detail.tool, detail.action,
    });
...
test "buildSlackApprovalMessage: JSON-escapes quotes in user input" {
    const alloc = std.testing.allocator;
    // Agent name with a quote — must not break JSON
    const msg = try buildSlackApprovalMessage(
        alloc,
        "test\"agent",
        "act-1",
