// Agent config markdown/frontmatter extraction.
//
// TRIGGER.md carries YAML frontmatter between `---` delimiters. Two entry
// points:
//   - extractAgentInstructions: borrow the body below the frontmatter.
//
...
pub fn extractAgentInstructions(source_markdown: []const u8) []const u8 {
    const fm = scanFrontmatter(source_markdown) orelse return "";
    return fm.body;
}

/// Aggregate returned by `parseTriggerMarkdownWithJson` — the parsed
/// AgentConfig plus the JSON that was used to derive it. The JSON is the
/// canonical `config_json` shape (what `parseAgentConfig` accepts) and is
...
/// Same pipeline as `parseAgentFromTriggerMarkdown`, but also returns the
...
) (Allocator.Error || AgentConfigError)!ParsedTrigger {
    const fm = scanFrontmatter(trigger_markdown) orelse
        return AgentConfigError.MissingRequiredField;

    const json = yaml_frontmatter.yamlFrontmatterToJson(alloc, fm.yaml) catch |err| switch (err) {
        error.OutOfMemory => return error.OutOfMemory,
        error.ParseFailure => return AgentConfigError.MissingRequiredField,
    };
    errdefer alloc.free(json);

    const config = try config_parser.parseAgentConfig(alloc, json);
...
) (Allocator.Error || AgentConfigError)!SkillMetadata {
...
fn dupeRequiredString(
    ...
    const val = root.get(key) orelse return AgentConfigError.MissingRequiredField;
    const s = switch (val) {
        .string => |str| str,
        else => return AgentConfigError.MissingRequiredField,
    };
...
fn dupeOptionalStringArray(
    ...
    while (i < arr.items.len) : (i += 1) {
        ...
        const s = switch (arr.items[i]) {
            .string => |str| str,
            else => return AgentConfigError.InvalidTagFormat,
        };
...
test "parseTriggerMarkdownWithJson: returns both parsed config and owned JSON" {
    ...
    try std.testing.expectEqualStrings("test-agent", parsed.config.name);
    var reparsed = try config_parser.parseAgentConfig(alloc, parsed.config_json);
    defer reparsed.deinit(alloc);
    try std.testing.expectEqualStrings("test-agent", reparsed.name);
}
...
test "parseTriggerMarkdownWithJson: parse failure inside frontmatter → JSON freed (no leak)" {
    const alloc = std.testing.allocator;
    // Missing required `name:` — yaml→json succeeds, parseAgentConfig fails.
