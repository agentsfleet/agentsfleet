...
test "parseAgentConfig: credential names validated (no op:// paths)" {
    ...
    try std.testing.expectError(
        AgentConfigError.InvalidCredentialRef,
        config_parser.parseAgentConfig(alloc, json),
    );
}
...
test "parseAgentConfig: unknown tool name accepted (gate moved to runner sandbox)" {
    const alloc = std.testing.allocator;
    ...
    ;
    var cfg = try config_parser.parseAgentConfig(alloc, json);
...
test "validateSkillName: kebab slug accepted" {
    try config_validate.validateSkillName("platform-ops-agent");
...
test "validateSkillName: empty rejected" {
    try std.testing.expectError(
        AgentConfigError.InvalidNameFormat,
...
test "validateSkillVersion: empty rejected" {
    try std.testing.expectError(
        AgentConfigError.InvalidVersionFormat,
