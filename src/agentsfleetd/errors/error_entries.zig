...
pub const ENTRIES = [_]Entry{
    ...
    eu("UZ-VAULT-005", .conflict, "Secret name already taken", "A secret already exists at this (workspace_id, key_name). Creation claims a free name and never overwrites: PUT the existing secret to replace its whole body, or DELETE it first. Postgres decides this, so two concurrent creates on one name resolve to one winner and one 409.", "A secret with that name already exists. Rename this one, or open the existing secret and replace its value."),
    ...
    eu("UZ-PREFS-001", .bad_request, "Unknown preference key", "The {key} path segment is not in the preference registry. Only named keys the dashboard declares can be written; check the spelling against the API reference.", "That setting doesn't exist. Reload the page and try again."),
    eu("UZ-PREFS-002", .bad_request, "Preference value too large", "A preference value is a small client-owned toggle, capped at 1 KiB. Store larger state in a resource of its own, not in a preference.", "That setting is too large to save. Reload the page and try again."),
    ...
    eu("UZ-LIBRARY-001", .bad_request, "Pagination cursor is malformed", "The `starting_after` value is not a cursor this endpoint issued: it failed base64url decoding, carried an unexpected version, or was not canonical JSON in the documented key order. Send back the `next_cursor` from the previous page verbatim rather than composing one. A rejected cursor NEVER falls back to an unpaged read.", "That page link is no longer valid. Go back to the first page and try again."),
    eu("UZ-LIBRARY-002", .bad_request, "Pagination cursor does not match this request", "The cursor decoded cleanly but describes a different query: its tenant, workspace, filters, or limit disagree with the ones on this request. Cursors are bound to the query that produced them, so changing a filter mid-pagination requires starting over from the first page.", "The filters changed since that page was loaded. Start again from the first page."),
    eu("UZ-LIBRARY-003", .bad_request, "Pagination or filter input out of bounds", "`limit` must be between 1 and 100, and a filter value such as `provider` must be at most 128 bytes once normalized. Both are hard input bounds, not throttles — pick a smaller page or a shorter value.", "That request asked for too much at once. Try a smaller page size or a shorter filter."),
    eu("UZ-LIBRARY-004", .service_unavailable, "Catalogue version unavailable", "The catalogue revision could not be read, so no cache generation can be selected. The endpoint returns no cached data rather than risk serving a page and a price from different generations; billing fails closed for the same reason.", "The model catalogue is temporarily unavailable. Try again shortly."),
    eu("UZ-LIBRARY-005", .internal_server_error, "Response exceeded its size ceiling", "A compliant response would have exceeded this endpoint's encoded-body ceiling. The endpoint refuses rather than truncating, because a silently short page is worse than a failed one. Unreachable while `limit` stays within bounds and per-item projection stays bounded — a firing is a defect, not a user-facing outcome.", "We couldn't build that page. Try again shortly, and contact support with the request id if it continues."),
    eu("UZ-LIBRARY-006", .service_unavailable, "Data service temporarily unavailable", "Acquiring a pooled connection or running the query failed transiently. The connection is released before this response is built, so a retry is safe.", "We couldn't reach the database. Try again shortly."),
    eu("UZ-LIBRARY-008", .conflict, "Credential was deleted while it was being referenced", "A writer creating a reference to a credential lost the lock race to a concurrent delete of that same credential. The write rolled back cleanly and nothing was persisted, so retrying is safe — the credential will simply be gone.", "That key was deleted while you were saving. Refresh and pick another key."),
    // ── REPAIR (an approved repair proposal applied to a repository) ──────────
    eu("UZ-REPAIR-001", .conflict, "Repair base moved", "The proposal was written against a base commit the branch has since moved past, so the approved diff no longer describes the code a reviewer saw. Nothing was pushed. A later investigation can propose again against the new head.", "The branch changed after this fix was approved, so we didn't apply it. A fresh proposal can be raised against the current code."),
    eu("UZ-REPAIR-002", .unprocessable_entity, "Repair exceeded its approved bounds", "The approved diff touches a path outside the file allowlist it declared, or breaches the file-count or diff-size cap. Nothing was pushed.", "This fix tried to change more than it was approved to change, so we didn't apply it."),
    eu("UZ-REPAIR-003", .conflict, "Repair already applied", "This proposal already produced its branch and draft pull request; the approval was replayed or delivered twice. The first outcome stands and no second pull request was opened.", "This fix was already applied — the existing draft pull request is the one to review."),
    eu("UZ-REPAIR-004", .bad_gateway, "Repair could not be pushed", "The upstream repository rejected the branch or the draft pull-request creation. No partial state is left behind. Check the app installation's repository permissions before retrying.", "We couldn't open the pull request for this fix. Check the repository connection, then try again."),
    e("UZ-REPAIR-005", .unprocessable_entity, "Repair proposal malformed", "A run's proposal block failed shape or size validation, so no proposal was stored and no approval was requested. The run's diagnosis is preserved."), // reachable: no — the run degrades to diagnosis-only and logs this code; no dashboard surface fetches it
    // ── GATE ─────────────────────────────────────────────────────────────────
    // ── STARTUP ──────────────────────────────────────────────────────────────
