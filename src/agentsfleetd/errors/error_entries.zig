...
pub const ENTRIES = [_]Entry{
    ...
    eu("UZ-VAULT-005", .conflict, "Secret name already taken", "A secret already exists at this (workspace_id, key_name). Creation claims a free name and never overwrites: PATCH the existing secret to rotate its body, or DELETE it first. Postgres decides this, so two concurrent creates on one name resolve to one winner and one 409.", "A secret with that name already exists. Rename this one, or open the existing secret and replace its value."),
