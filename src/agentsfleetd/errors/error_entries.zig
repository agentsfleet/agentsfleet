...
pub const ENTRIES = [_]Entry{
    ...
    eu("UZ-LIBRARY-003", .bad_request, "Pagination or search input out of bounds", "`limit` must be between 1 and 100, and a search term must be at most 128 bytes once normalized. Both are hard input bounds, not throttles — pick a smaller page or a shorter term.", "That request asked for too much at once. Try a smaller page size or a shorter search."),
    ...
    eu("UZ-LIBRARY-007", .not_found, "Fleet library entry not available here", "No entry matches this tier and id in this workspace. One code covers both absent and present-in-another-workspace on purpose: distinguishing them would let a caller enumerate entries belonging to workspaces it cannot read.", "We couldn't find that fleet. It may have been removed — refresh the library."),
