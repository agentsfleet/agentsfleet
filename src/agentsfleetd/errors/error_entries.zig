...
pub const ENTRIES = [_]Entry{
    ...
    e("UZ-WH-020", .unauthorized, "Webhook credential not configured", "The trigger's source is not a recognized webhook provider (github, slack, linear), or the workspace holds no secret for it. Set a recognized source. Then store a random value of at least 32 bytes in that secret's webhook_secret field, set the same upstream, and resend."), // reachable: no — external webhook sender sees this, not the dashboard
    ...
    eu("UZ-PROVIDER-003", .bad_request, "Secret JSON missing required field", "The stored secret must include `provider`. `api_key` is required for a hosted provider, optional for `openai-compatible` and for a local runtime (ollama, vllm, and the rest of that set). `model` belongs on the registry entry.", "That secret needs a provider set. It needs an API key too, unless it points at a custom endpoint or a model server you run yourself. Edit it under Secrets & ENVs."),
