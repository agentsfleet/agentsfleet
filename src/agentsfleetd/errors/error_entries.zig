...
pub const ENTRIES = [_]Entry{
    // ── UUIDV7 ──────────────────────────────────────────────────────────────
    e("UZ-UUIDV7-009", .bad_request, "Invalid identifier shape", "The identifier is not a valid version 7 universally unique identifier (UUID)."), // reachable: no — CLI/API path-param validator, never a dashboard-fetch path
    // ── INTERNAL ─────────────────────────────────────────────────────────────
    ...
    e(S_UZ_INTERNAL_003, .internal_server_error, S_TITLE_REQUEST_FAILED, "An internal operation failed. Check the err= field. If it continues, run 'agentsfleetd doctor'."), // reachable: no — deliberately generic catch-all; a specific dashboard-reachable failure gets promoted out of this bucket instead
    // ── REQUEST ──────────────────────────────────────────────────────────────
    ...
    e("UZ-AUTH-011", .bad_request, "Verification code did not match", "The 6-digit code does not match the one shown in your browser. Re-enter it and try again."), // reachable: no — `agentsfleet login` CLI device-flow, rendered in the terminal not the dashboard
    ...
    e("UZ-AUTH-013", .gone, "Login session aborted", "This login session was aborted: too many wrong codes, a cancel, or a newer session. Start over with `agentsfleet login`."), // reachable: no — CLI login flow
    e("UZ-AUTH-014", .conflict, "Login session not approved", "This login session is not approved yet. Approve it in your browser, then submit the code."), // reachable: no — CLI login flow (the dashboard side is an approve action, not a fetch of this code)
    ...
    eu("UZ-AUTH-022", .forbidden, "Insufficient scope", "Your token lacks a required scope. The error detail names it; see the [Scopes](/api-reference/scopes) reference.", "You need an additional scope for that. Ask an agentsfleet admin to grant the scope this action requires."),
    e("UZ-AUTH-023", .unauthorized, "Command-line credential revoked", "This credential was revoked by a logout or by a newer login from this machine. Run `agentsfleet login` to get a new one."), // reachable: no — CLI authenticate path
    ...
    // indistinguishable without spelling out the probe it defeats, and points
    // at no enumeration — there is deliberately no way to list credentials.
    e("UZ-AUTH-024", .not_found, "Command-line credential not found", "You have no live credential with that identifier. It may be revoked, or it may not be yours."), // reachable: no — CLI credential management
    e("UZ-AUTH-025", .unauthorized, "Credential exchange failed", "Login could not exchange the browser session for a durable credential, so nothing was saved. The session likely expired. Run `agentsfleet login` again."), // reachable: no — produced by the command-line client during login's exchange, never emitted by the daemon
    // ── API (serving-plane backpressure) ─────────────────────────────────────
    e("UZ-API-001", .too_many_requests, "Too many requests", "The API is at its request limit. Wait for the Retry-After delay, then retry."), // reachable: no — instance-wide backpressure shed, hit before routing; not a rendered dashboard error
    e("UZ-API-002", .service_unavailable, "Activity stream capacity reached", "The API is at its activity-stream limit. Close unused dashboard tabs or retry shortly."), // reachable: no — SSE connect rejection surfaces as a stream-level reconnect, not a rendered problem+json page
    ...
    e("UZ-WH-010", .unauthorized, "Invalid webhook signature", "The webhook signature did not verify. The stored signing secret must match the one configured upstream."), // reachable: no — external webhook sender sees this, not the dashboard
    e("UZ-WH-011", .unauthorized, "Stale webhook timestamp", "The webhook timestamp is more than 5 minutes off. That means a replay or clock skew."), // reachable: no — external webhook sender sees this, not the dashboard
    e("UZ-WH-020", .unauthorized, "Webhook credential not configured", "This source is not a recognized webhook provider, or the workspace holds no webhook secret for it. Create a random secret of at least 32 bytes, store it as the source's webhook_secret, set the same value upstream, then resend."), // reachable: no — external webhook sender sees this, not the dashboard
    e("UZ-WH-021", .not_found, "Connector installation is not mapped", "Reconnect the provider App to the intended workspace, then redeliver the event."), // reachable: no — log-only ingress outcome
    ...
    e("UZ-WH-030", .payload_too_large, "Webhook payload too large", "The webhook body exceeds the 1 MiB limit. Reduce the payload size."), // reachable: no — external webhook sender sees this, not the dashboard
    // ── SLACK CONNECTOR ──────────────────────────────────────────────────────
    // Events ingress (M106 §2). 010/011 are the only rejections Slack ever sees
    // as a 4xx; 020 is a benign no-op (an uninstalled team), so its entry
    // carries `.ok` — the code is a log/telemetry reason, never a wire error
    // status; the handler answers 200.
    e("UZ-SLK-010", .unauthorized, "Invalid Slack signature", "The Slack signature did not verify. The Slack app's signing secret must match the one vaulted at slack-app/signing_secret."), // reachable: no — Slack-to-server events ingress, not a dashboard fetch
    e("UZ-SLK-011", .unauthorized, "Stale Slack timestamp", "The Slack timestamp is more than 5 minutes off. That means a replay or clock skew."), // reachable: no — Slack-to-server events ingress, not a dashboard fetch
    e("UZ-SLK-020", .ok, "Slack team not installed", "This Slack team has no connector install; the event is acknowledged and ignored. Re-run Connect Slack from the dashboard."), // reachable: no — a 200-ack log/telemetry reason, never a wire error status
    e("UZ-SLK-022", .bad_gateway, "Slack token exchange failed", "The Slack OAuth code could not be exchanged for a bot token. Retry the connect flow; if it continues, check the Slack app credentials."), // reachable: no — server-side OAuth exchange step, not a page the dashboard fetches and renders
    ...
    e("UZ-SLK-030", .bad_gateway, "Slack answer post failed", "The answer could not be posted to Slack: missing chat:write, a 429, or an outage. It retries with backoff; the run does not fail."), // reachable: no — async worker log-only, never returned to an HTTP caller
    // ── TOOL ─────────────────────────────────────────────────────────────────
    ...
    eu("UZ-AGT-008", .bad_request, "Invalid fleet config", "Config JSON is malformed. Check the trigger, tools, credentials, and budget fields in TRIGGER.md frontmatter. See the [Authoring a fleet](/fleets/authoring) guide.", "That fleet's config isn't valid. Check the trigger, tools, credentials, and budget fields, then try again."),
    ...
    eu("UZ-AGT-010", .conflict, "Fleet state transition not allowed", "That action is not valid from the fleet's current state. The error detail names the refused transition.", "That action isn't available for this Fleet right now — check its current status and try again."),
    ...
    eu("UZ-AGT-013", .internal_server_error, "Fleet install rolled back", "Event-stream setup failed during create. Nothing was kept, so retry. If it continues, check queue connectivity.", "We couldn't finish setting up your fleet. Nothing was created — try again."),
    eu("UZ-AGT-014", .precondition_failed, "Fleet source is stale", "Someone else saved first: `If-Match` names an old version. Re-read the fleet, re-apply your edit, and retry with the new `etag`.", "Someone else edited this Fleet's source since you opened it. Reload to see their change, then re-apply your edit."),
    eu("UZ-AGT-015", .not_found, "Event not found", "This fleet has no event with that identifier in this workspace. An event in another workspace answers the same.", "We couldn't find that event. It may have aged out, or the identifier doesn't match one on this Fleet."),
    // ── SCHEDULE ─────────────────────────────────────────────────────────────
    e("UZ-SCHED-001", .unprocessable_entity, "Invalid schedule", "cron must be a valid five-field expression and timezone must name a supported region."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    e("UZ-SCHED-002", .not_found, "Schedule not found", "No schedule matches this identifier for the selected Fleet."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    e("UZ-SCHED-003", .conflict, "Schedule limit reached", "A Fleet can hold at most 32 schedules. Remove one before creating another."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    e("UZ-SCHED-004", .bad_gateway, "Schedule provider unavailable", "QStash did not confirm the change. Run the schedule sync command shown by the client."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    e("UZ-SCHED-005", .unauthorized, "Invalid schedule signature", "QStash delivery signature verification failed. Confirm the vaulted signing keys and ingress URL."), // reachable: no — machine-to-machine ingress, not fetched by ui/packages/app
    e("UZ-SCHED-006", .conflict, "Schedule update busy", "Another schedule mutation holds the synchronization lease. Retry after the reported delay."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    e("UZ-SCHED-007", .service_unavailable, "Schedule service unavailable", "QStash credentials are not configured. Follow the registration playbook and roll agentsfleetd."), // reachable: no — CLI/API and machine ingress, not fetched by ui/packages/app
    e("UZ-SCHED-008", .conflict, "Schedule source already exists", "A schedule already uses this source key for the selected Fleet. Pick a different source or update the existing schedule."), // reachable: no — CLI/API schedule surface, not fetched by ui/packages/app
    // ── Fleet Bundle ───────────────────────────────────────────────────────
    ...
    // ── VAULT ────────────────────────────────────────────────────────────────
    eu("UZ-VAULT-001", .bad_request, "Secret data must be a non-empty JSON object", "The body's 'data' field must be a JSON object with at least one key. Strings, arrays, scalars, and `{}` are rejected.", "That secret needs at least one field. Enter it as a JSON object with one or more keys — not a bare string or list."),
    ...
    eu("UZ-VAULT-004", .conflict, "Secret still referenced by model entries", "Model registry entries still reference this secret. Remove them first, then delete it. The error detail names the count.", "This key is used by one or more models in your registry. Remove those entries first, then delete the key."),
    eu("UZ-VAULT-005", .conflict, "Secret name already taken", "A secret with that name already exists in this workspace. Create never overwrites: update the existing secret, or delete it first.", "A secret with that name already exists. Rename this one, or open the existing secret and replace its value."),
    ...
    eu("UZ-PROVIDER-002", .bad_request, S_TITLE_SECRET_NOT_FOUND, "The named secret_ref does not exist in the tenant's primary workspace. Create it with `agentsfleet secret create <NAME> --data=@-`.", "We couldn't find that secret. Store it under Secrets & ENVs, then try again."),
    eu("UZ-PROVIDER-003", .bad_request, "Secret JSON missing required field", "The stored secret must include `provider`. `api_key` is required for a named provider, optional for `openai-compatible`. `model` belongs on the registry entry.", "That secret is missing required fields. It needs a provider set (and an API key for a named provider) — edit it under Secrets & ENVs and add them."),
    eu("UZ-PROVIDER-004", .bad_request, "Model not in library", "That model is not in the model library. Pick one from GET /v1/models, or ask for it to be added.", "That model isn't in our library yet. Pick a listed model, or ask us to add support for it."),
    eu("UZ-PROVIDER-005", .bad_request, "Custom endpoint base_url invalid or unsafe", "`base_url` must be https and must not target a loopback, private, link-local, or cloud-metadata host. Only an `openai-compatible` credential may carry one.", "That endpoint URL isn't allowed. Use a public https URL for your custom endpoint."),
    eu("UZ-PROVIDER-006", .not_found, "Library model not found", "No library model matches this id. List the library to find one, or add the model first.", "We couldn't find that model in the library. Refresh the list and try again."),
    ...
    eu("UZ-PROVIDER-009", .internal_server_error, "Platform model key not configured", "No platform default model is configured. An operator must set one via PUT /admin/platform-keys first.", "Platform defaults aren't set up on this deployment yet. Keep your current provider for now, or contact support."),
    eu("UZ-PROVIDER-010", .internal_server_error, "Tenant has no primary workspace", "This tenant has no primary workspace, which should never happen. Contact support with the request id.", "Something's off with your account setup. Contact support with the request id below."),
    ...
    eu("UZ-MODELS-003", .conflict, "Model entry already exists", "This model and key pair is already registered for this tenant. Edit the existing entry instead.", "You already have this model registered with that key. Edit the existing entry instead."),
    eu("UZ-MODELS-004", .not_found, "Model entry not found", "No model entry matches this id for this tenant. It may already be deleted — refresh the list.", "We couldn't find that model entry. It may have already been removed — refresh the list."),
    // ── PREFS (per-user dashboard UI prefs, /v1/workspaces/{workspace_id}/preferences) ──
    eu("UZ-PREFS-001", .bad_request, "Unknown preference key", "The {key} path segment is not a registered preference key. Check the spelling against the API reference.", "That setting doesn't exist. Reload the page and try again."),
    eu("UZ-PREFS-002", .bad_request, "Preference value too large", "A preference value is capped at 1 KiB. Store larger state in a resource of its own.", "That setting is too large to save. Reload the page and try again."),
    // ── CATALOG (platform fleet-library lifecycle, /v1/admin/fleet-libraries — M128) ──
    eu("UZ-CATALOG-001", .not_found, "Fleet library entry not found", "No catalog entry matches this id. It may already be deleted — refresh the catalog.", "We couldn't find that fleet. It may have already been removed — refresh the page."),
    eu("UZ-CATALOG-002", .conflict, "Cannot publish a fleet with no bundle", "No bundle has been fetched for this entry, so there is nothing to publish. Fetch it from its repository first.", "There's no bundle for this fleet yet. Fetch it from its repository first, then publish."),
    eu("UZ-CATALOG-003", .conflict, "Cannot delete a published fleet", "This fleet is published and installable. Unpublish it first, then delete it.", "This fleet is published. Unpublish it first, then delete it."),
    eu("UZ-CATALOG-004", .conflict, "Catalog id already taken by another repository", "This catalog id already belongs to a different source repository. Rename the bundle, or retry with replace to overwrite deliberately.", "A different repository already owns this fleet's name. Rename the bundle, or confirm you want to replace it."),
    eu("UZ-CATALOG-005", .precondition_failed, "Catalog entry changed since you loaded it", "Another operator saved first: `If-Match` names an old version. Refetch the row, re-apply your edit, and retry with the new `etag`.", "Someone else edited this catalog entry since you opened it. Refresh to see their change, then re-apply your edit."),
    // ── LIBRARY (bounded library reads — tenant models, catalogue, Fleet gallery) ──
    eu("UZ-LIBRARY-001", .bad_request, "Pagination cursor is malformed", "`starting_after` is not a cursor this endpoint issued. Send back the previous page's `next_cursor` verbatim; never compose one.", "That page link is no longer valid. Go back to the first page and try again."),
    eu("UZ-LIBRARY-002", .bad_request, "Pagination cursor does not match this request", "The cursor belongs to a different query: its tenant, workspace, filters, or limit differ. After changing filters, start from the first page.", "The filters changed since that page was loaded. Start again from the first page."),
    eu("UZ-LIBRARY-003", .bad_request, "Pagination or filter input out of bounds", "`limit` must be 1 to 100, and a filter value at most 128 bytes. Use a smaller page or a shorter value.", "That request asked for too much at once. Try a smaller page size or a shorter filter."),
    eu("UZ-LIBRARY-004", .service_unavailable, "Catalogue version unavailable", "The catalogue revision could not be read, so no data is served. Retry shortly.", "The model catalogue is temporarily unavailable. Try again shortly."),
    eu("UZ-LIBRARY-005", .internal_server_error, "Response exceeded its size ceiling", "The response would exceed this endpoint's size ceiling, so it is refused rather than truncated. Report the request id — this is a server defect.", "We couldn't build that page. Try again shortly, and contact support with the request id if it continues."),
    eu("UZ-LIBRARY-006", .service_unavailable, "Data service temporarily unavailable", "The database query failed transiently. Retrying is safe.", "We couldn't reach the database. Try again shortly."),
    eu("UZ-LIBRARY-008", .conflict, "Credential was deleted while it was being referenced", "That credential was deleted while this write referenced it. Nothing was saved. Retrying is safe; the credential will be gone.", "That key was deleted while you were saving. Refresh and pick another key."),
    // ── GATE ─────────────────────────────────────────────────────────────────
    // ── STARTUP ──────────────────────────────────────────────────────────────
    ...
    e("UZ-STARTUP-006", .internal_server_error, "Service could not start", "The service ran out of memory reading environment variables at startup. Check host memory pressure."), // reachable: no — pre-listen boot check, never an HTTP response
