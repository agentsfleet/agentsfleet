...
pub const ENTRIES_RUNTIME = [_]Entry{
    ...
    e("UZ-EXEC-016", .unauthorized, "Runner token rejected", "The control plane rejected this host's agt_r runner token (401/403) on the heartbeat. Retrying can't fix it — mint a fresh agt_r and issue the host's runner token again, then restart the runner."), // reachable: no — runner control-loop code, logged (server_stopped reason=token_rejected) not dashboard-fetched
    e("UZ-EXEC-017", .conflict, "Assignment exceeds host capability", "The policy assigned to this runner needs an enforcement mechanism its host cannot deliver. The runner row names the missing mechanism; fix the host or relax the assignment from the dashboard."), // reachable: no — runner control-loop code, logged when the heartbeat verdict reads degraded
    // ── RELAY ────────────────────────────────────────────────────────────────
    // ── APPROVAL GATE ────────────────────────────────────────────────────────
    ...
    eu("UZ-APPROVAL-004", .service_unavailable, "Approval service unavailable", "Gate service unavailable \u{2014} default-deny applied. Check Redis connectivity.", "Approvals are temporarily unavailable. We deny requests while this service is down. Try again shortly."),
    ...
    e("UZ-GH-002", .bad_gateway, "GitHub token mint failed", "GitHub did not return an installation token (upstream 5xx, network, or a malformed exchange response). " ++
        "This is temporary. Retry shortly; if the error continues, check GitHub status and the App configuration."), // reachable: no — response goes to the runner's credential-mint call, not to a dashboard fetch
    ...
    eu("UZ-CONN-003", .bad_gateway, "Connector vendor call exceeded its deadline", "An outbound provider call timed out, could not start its timeout guard, or could not reach the provider. " ++
        "The call never runs without a deadline. Retry once, then check provider status and network access if the error continues.", "We couldn't reach that service right now. Try again shortly."),
    ...
    eu("UZ-CONN-007", .internal_server_error, "Connector catalog lookup failed", "The vault existence check for connector app/fleet keys failed (a Postgres error). " ++
        "Retry. If the error continues, check database connectivity and the vault schema state.", "We couldn't load your connectors right now. Try refreshing — if it keeps failing, contact support."),
