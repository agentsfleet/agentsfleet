...
pub fn snapshotFleetFields() FleetFields {
    return .{
        .fleet_triggered_total = g_triggered.load(.acquire), // safe because: pairs with no publisher; scrape-time read of an independent counter
    };
}
...
test "single trigger increments the counter by exactly one" {
    resetForTest();
    defer resetForTest();
    const before = snapshotFleetFields().fleet_triggered_total;
    incFleetsTriggered();
    const after = snapshotFleetFields().fleet_triggered_total;
...
test "concurrent triggers from N threads lose no update" {
    ...
    const Runner = struct {
        fn run(iters: usize) void {
            var i: usize = 0;
            while (i < iters) : (i += 1) incFleetsTriggered();
        }
    };
    ...
    try std.testing.expectEqual(
        ...
        snapshotFleetFields().fleet_triggered_total,
    );
}
