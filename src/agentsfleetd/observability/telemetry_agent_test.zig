...
// AgentTriggered / AgentCompleted comptime struct shape.
test "AgentTriggered properties return expected keys" {
    const props = (telemetry.AgentTriggered{
        .distinct_id = "ws_1",
        .workspace_id = "ws_1",
        .agent_id = "z_1",
        .event_id = "e_1",
    ...
    try std.testing.expectEqualStrings("agent_id", props[1].key);
    try std.testing.expectEqualStrings("event_id", props[2].key);
    try std.testing.expectEqualStrings("source", props[3].key);
}

test "AgentCompleted properties return expected keys and numeric types" {
    const props = (telemetry.AgentCompleted{
        .distinct_id = "ws_1",
        .workspace_id = "ws_1",
        .agent_id = "z_1",
        .event_id = "e_1",
...
// null PostHog client must not panic on Agent captures.
// ProdBackend is instantiated directly because the comptime-selected Backend
// in test builds is TestBackend.
test "ProdBackend with null client does not panic on Agent capture" {
    var prod = telemetry.ProdBackend{ .client = null };
    prod.capture(telemetry.AgentTriggered, .{
        .distinct_id = "ws",
        .workspace_id = "ws",
        .agent_id = "z",
        .event_id = "e",
    ...
    prod.capture(telemetry.AgentCompleted, .{
        .distinct_id = "ws",
        .workspace_id = "ws",
        .agent_id = "z",
        .event_id = "e",
