...
const Ring = struct {
    ...
    pub fn push(self: *Self, entry: SpanEntry) bool {
        while (true) {
    ...
    pub fn pop(self: *Self) ?SpanEntry {
...
fn flushBatch() void {
    ...
    const envelope_fmt = "{{\"resourceSpans\":[{{\"resource\":{{\"attributes\":[{{\"key\":\"service.name\",\"value\":{{\"stringValue\":\"{s}\"}}}}]}},\"scopeSpans\":[{{\"scope\":{{\"name\":\"agentsfleetd\"}},\"spans\":[{s}]}}]}}]}}";
