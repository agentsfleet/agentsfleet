...
test "failures bucket by runner and reason" {
    ...
    try std.testing.expect(contains(out, "agent_runner_failures_total{runner_id=\"r1\",reason=\"oom_kill\"} 2"));
    try std.testing.expect(contains(out, "agent_runner_failures_total{runner_id=\"r1\",reason=\"timeout_kill\"} 1"));
    try std.testing.expect(contains(out, "agent_runner_failures_total{runner_id=\"r2\",reason=\"renewal_terminate\"} 1"));
}
...
test "executions split by outcome" {
    ...
    var buf: [8192]u8 = undefined;
    const out = try render(&buf);
    try std.testing.expect(contains(out, "agent_runner_executions_total{runner_id=\"r1\",outcome=\"processed\"} 3"));
    try std.testing.expect(contains(out, "agent_runner_executions_total{runner_id=\"r1\",outcome=\"agent_error\"} 1"));
}
...
test "a seen runner renders a last_seen_seconds series" {
    ...
    var buf: [4096]u8 = undefined;
    const out = try render(&buf);
    try std.testing.expect(contains(out, "agent_runner_last_seen_seconds{runner_id=\"r1\"}"));
}
...
test "active_leases tracks grant then release" {
    ...
    mr.incRunnerActiveLeases("r1");
    var buf: [4096]u8 = undefined;
    try std.testing.expect(contains(try render(&buf), "agent_runner_active_leases{runner_id=\"r1\"} 2"));

    mr.decRunnerActiveLeases("r1");
    try std.testing.expect(contains(try render(&buf), "agent_runner_active_leases{runner_id=\"r1\"} 1"));
}
...
test "active_leases clamps below zero and emits no negative series" {
    ...
    try std.testing.expect(!contains(out, "agent_runner_active_leases{runner_id=\"r1\"}"));
}
...
test "cardinality overflow routes to _other with the reason preserved" {
    ...
    try std.testing.expect(contains(text, "agent_runner_failures_total{runner_id=\"_other\",reason=\"oom_kill\"} 1"));
    try std.testing.expect(contains(text, "agent_runner_failures_overflow_total 1"));
}
