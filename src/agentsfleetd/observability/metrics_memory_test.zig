...
test "memory capture counter accumulates and renders" {
    ...
    try std.testing.expect(contains(try render(&buf), "fleet_memory_entries_captured_total 5"));
}
...
test "memory push-failure counter renders" {
    ...
    mm.incMemoryPushFailure();
    var buf: [8192]u8 = undefined;
    try std.testing.expect(contains(try render(&buf), "fleet_memory_push_failures_total 2"));
}
...
test "hydration window gauge reflects the last set size" {
    ...
    var buf: [8192]u8 = undefined;
    try std.testing.expect(contains(try render(&buf), "fleet_memory_hydration_window_entries 7"));
}
...
test "test_existing_memory_families_unchanged" {
    ...
    try std.testing.expect(contains(out, "# HELP fleet_memory_entries_captured_total "));
    try std.testing.expect(contains(out, "# TYPE fleet_memory_entries_captured_total counter"));
    try std.testing.expect(contains(out, "fleet_memory_entries_captured_total 5"));
    try std.testing.expect(contains(out, "# TYPE fleet_memory_push_failures_total counter"));
    try std.testing.expect(contains(out, "fleet_memory_push_failures_total 1"));
    try std.testing.expect(contains(out, "# TYPE fleet_memory_hydration_window_entries gauge"));
    try std.testing.expect(contains(out, "fleet_memory_hydration_window_entries 7"));
}
...
test "test_metrics_render_memory_loss_families" {
    ...
    try std.testing.expect(contains(out, "# HELP fleet_memory_hydration_dropped_entries_total "));
    try std.testing.expect(contains(out, "fleet_memory_hydration_dropped_entries_total 2"));
    try std.testing.expect(contains(out, "# HELP fleet_memory_hydration_dropped_bytes_total "));
    try std.testing.expect(contains(out, "fleet_memory_hydration_dropped_bytes_total 120"));
    try std.testing.expect(contains(out, "# HELP fleet_memory_cap_evictions_total "));
    try std.testing.expect(contains(out, "fleet_memory_cap_evictions_total 3"));
    try std.testing.expect(contains(out, "# HELP fleet_memory_capture_truncated_total "));
    try std.testing.expect(contains(out, "fleet_memory_capture_truncated_total 1"));
    try std.testing.expect(contains(out, "# HELP fleet_memory_capture_skipped_total "));
    try std.testing.expect(contains(out, "fleet_memory_capture_skipped_total 1"));
    try std.testing.expect(contains(out, "# HELP fleet_memory_search_zero_hits_total "));
    try std.testing.expect(contains(out, "fleet_memory_search_zero_hits_total 1"));
}
...
test "a loss counter alone activates the exposition (loss is never invisible)" {
    ...
    mm.incSearchZeroHit();
    var buf: [8192]u8 = undefined;
    try std.testing.expect(contains(try render(&buf), "fleet_memory_search_zero_hits_total 1"));
}
