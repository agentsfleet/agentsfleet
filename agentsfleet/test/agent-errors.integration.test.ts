// Error-path integration tests for agent.ts stop / resume / kill / delete.
// Covers the requireAgentId validation branch (invalid uuidv7 format)
...
const authedScope = <T>(fn: (stateDir: string) => Promise<T>): Promise<T> =>
  withAuthedStateDir({ workspaceId: WS_ID, sessionId: "sess_agent_err" }, fn);

// ---------------------------------------------------------------------------
// requireAgentId — invalid format branch (lines 66-69 in agent.ts)
// ---------------------------------------------------------------------------

describe("agent id validation — invalid format", () => {
  test("`stop` with a non-uuidv7 id exits non-zero and prints the format hint", async () => {
    await authedScope(async () => {
      await withMockApi({}, async (apiUrl, calls) => {
        ...
        expect(err.read()).toContain("agent_id");
