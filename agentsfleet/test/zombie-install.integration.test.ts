      ...
      try {
        const routes: MockRoutes = {
          [`POST /v1/workspaces/${WS_ID}/zombies`]: () =>
            jsonResponse(201, { zombie_id: AGENTSFLEET_ID, name: "text-mode-zombie" }),
        };
  ...
  test("install --json emits structured JSON on stdout", async () => {
    await authedScope(async () => {
      ...
      try {
        const routes: MockRoutes = {
          [`POST /v1/workspaces/${WS_ID}/zombies`]: () =>
            jsonResponse(201, { zombie_id: AGENTSFLEET_ID, name: "json-mode-zombie", webhook_urls: {} }),
        };
        await withMockApi(routes, async (apiUrl) => {
          const out = bufferStream();
          const err = bufferStream();
...
describe("install — webhook URL output", () => {
    ...
    await authedScope(async () => {
      ...
      try {
        const routes: MockRoutes = {
          [`POST /v1/workspaces/${WS_ID}/zombies`]: () =>
            jsonResponse(201, {
              zombie_id: AGENTSFLEET_ID, name: "webhook-zombie",
  ...
  test("install uses directory basename when server response omits name", async () => {
    await authedScope(async () => {
      ...
      try {
        await withMockApi(
          { [`POST /v1/workspaces/${WS_ID}/zombies`]: () => jsonResponse(201, { zombie_id: AGENTSFLEET_ID }) },
          async (apiUrl) => {
            const out = bufferStream();
            const code = await runCli(
              ["install", "--from", dir],
    ...
    await authedScope(async () => {
      ...
      try {
        ...
        await withMockApi(routes, async (apiUrl, calls) => {
          const out = bufferStream();
          const code = await runCli(
            ["zombie", "update", AGENTSFLEET_ID, "--from", dir],
            { stdout: out.stream, stderr: bufferStream().stream, env: { AGENTSFLEET_API_URL: apiUrl } },
          );
          expect(code).toBe(0);
          expect(out.read()).toContain(AGENTSFLEET_ID);
    ...
    await authedScope(async () => {
      ...
      try {
        ...
        await withMockApi(routes, async (apiUrl) => {
          const out = bufferStream();
          const code = await runCli(
            ["--json", "zombie", "update", AGENTSFLEET_ID, "--from", dir],
            { stdout: out.stream, stderr: bufferStream().stream, env: { AGENTSFLEET_API_URL: apiUrl } },
          );
          expect(code).toBe(0);
          const parsed = JSON.parse(out.read()) as {
  ...
  test("bad bundle path exits ConfigError (exit 5)", async () => {
    await authedScope(async () => {
      await withMockApi({}, async (apiUrl, calls) => {
        ...
        const err = bufferStream();
        const code = await runCli(
          ["zombie", "update", AGENTSFLEET_ID, "--from", "/no/such/bundle/dir"],
          { stdout: out.stream, stderr: err.stream, env: { AGENTSFLEET_API_URL: apiUrl } },
        );
        expect(code).toBe(5);
