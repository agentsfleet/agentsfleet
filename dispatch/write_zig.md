...
- **Surface a Pub Surface & Struct-Shape Gate block** (defined in `AGENTS.orly.md`) before saving a new `*.zig` file or any edit that adds a new `pub` symbol. The gate forces choosing the layout and justifying every new `pub` against an external consumer.
