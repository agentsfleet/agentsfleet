...
find_follow_on() {
  local candidate
  local found=''
  for candidate in "$repo_root"/docs/v2/{pending,active,done}/*_P1_OBS_BOUNDED_SIGNAL_EXPORT.md; do
    [[ -f "$candidate" ]] || continue
    [[ -z "$found" ]] || fail "bounded signal export follow-on exists in multiple lifecycle directories"
    found="$candidate"
  done
  [[ -n "$found" ]] || fail "bounded signal export follow-on is absent"
  printf '%s\n' "$found"
}
...
check_inventory() {
  ...
  if rg --glob '*.zig' --glob '!**/*_test.zig' --quiet -- 'capture\(telemetry(_mod)?\.FleetCompleted' "$repo_root/src/agentsfleetd"; then
...
check_limits() {
  ...
  require_literal "$observability" '| logs | 2047 records; body truncated at 512 bytes | up to 50 records every 5 seconds |'
  require_literal "$observability" '| traces | 1023 spans; 12 attributes per span | up to 50 spans every 5 seconds |'
  require_literal "$observability" '| OTLP metrics | 1023 samples; at most 256 coalesced series | drains up to 1023 every 5 seconds, then coalesces label sets |'
...
check_metric_cardinality() {
  ...
  require_literal "$observability" $'| `workspace` | prohibited; follow-on removes it | remove the label and its process-local guard; do not accumulate tenant series across restarts |'
  ...
  require_literal "$observability" $'| `model` | prohibited; follow-on removes it | remove the label and its dashboard selector; do not bucket an evolving catalog |'
}
...
check_follow_on() {
  local follow_on
  follow_on="$(find_follow_on)"
  require_literal "$follow_on" 'std.process.Init.io'
  require_literal "$follow_on" 'std.Io.Event.set'
  require_literal "$follow_on" 'metrics wakes at 768'
  require_literal "$follow_on" $'Remove the unbounded `model` and `workspace` metric labels'
  require_literal "$follow_on" $'`aggregate_cap`, `serialize_failed`, `partial_rejected`, `export_rejected`, and `export_uncertain`'
  require_literal "$follow_on" 'four runner 4xx rejections, four 5xx server errors, two sampled successes'
  require_literal "$follow_on" "64-character lowercase SHA-256 digest of \`fleet_id || 0x00 || event_id\`"
  require_literal "$follow_on" 'one absolute monotonic shutdown deadline'
...
run_case() {
  case "$1" in
    ...
    follow-on) check_follow_on ;;
    *) fail "unknown case '$1'" ;;
  esac
}

if [[ "${1:-all}" == 'all' ]]; then
  for audit_case in inventory volume limits log-route collector metric-source metric-cardinality trace-map trace-budget architecture follow-on; do
    run_case "$audit_case"
  done
else
  run_case "$1"
