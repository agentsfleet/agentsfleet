...
//
//   derived (never hand-edit)  id, aliases, dial, base_url, display
//   curated (hand-owned)       source, source_url, endpoint, field_map,
//                              rate_unit, models, notes
//
...
const CURATED = ["source", "source_url", "endpoint", "rate_unit", "field_map", "models", "note", "notes"];
...
for (const id of [...priorOrder, ...newcomers]) {
  ...
  if (derived.base_url) row.base_url = derived.base_url;
  else if (prev?.base_url) row.base_url = prev.base_url;
  ...
  if (!prev) {
    ...
    row.source = "manual";
    row.models = [];
    row.note = "Dialable, not yet priced. Self-managed only until rates are curated (zero rates never enter the cost path — schema/400_model_library.sql).";
  }
  providers[id] = row;
  stats.native++;
...
const revised = {
  ...
  _generator: [
    "Provider identity, aliases, dial, base_url and display are GENERATED by",
    ...
    "Curated by hand: source, source_url, endpoint, rate_unit, field_map, models.",
    "",
