-- Approval gate audit log for Agent actions.
...
CREATE TABLE IF NOT EXISTS core.agent_approval_gates (
    uid             UUID GENERATED ALWAYS AS (id) STORED PRIMARY KEY,
    CONSTRAINT ck_agent_approval_gates_uid_uuidv7 CHECK (substring(uid::text from 15 for 1) = '7'),
    ...
    -- status: enum maintained in application code (src/agent/approval_gate.zig
...
CREATE INDEX IF NOT EXISTS idx_agent_approval_gates_agent_status
    ON core.agent_approval_gates (agent_id, status);

CREATE INDEX IF NOT EXISTS idx_agent_approval_gates_action_id
    ON core.agent_approval_gates (action_id);

-- Inbox list query: oldest pending rows per workspace.
CREATE INDEX IF NOT EXISTS idx_agent_approval_gates_workspace_status_requested
    ON core.agent_approval_gates (workspace_id, status, requested_at);

-- Sweeper query: pending rows past their timeout_at, scanned every cycle.
CREATE INDEX IF NOT EXISTS idx_agent_approval_gates_pending_timeout
    ON core.agent_approval_gates (timeout_at)
    WHERE status = 'pending';

CREATE OR REPLACE FUNCTION core.agent_approval_gates_append_only() RETURNS trigger AS $$
BEGIN
    IF TG_OP = 'DELETE' THEN
        RAISE EXCEPTION 'agent_approval_gates is append-only -- DELETE is not permitted';
    END IF;
    IF TG_OP = 'UPDATE' AND OLD.status != 'pending' THEN
        RAISE EXCEPTION 'agent_approval_gates -- only pending rows can be updated';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_agent_approval_gates_append_only
    BEFORE UPDATE OR DELETE ON core.agent_approval_gates
    FOR EACH ROW EXECUTE FUNCTION core.agent_approval_gates_append_only();

GRANT SELECT, INSERT, UPDATE ON core.agent_approval_gates TO api_runtime;
