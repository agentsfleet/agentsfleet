-- Fleet memory -- schema, role, table Data Definition Language (DDL), and grants.
...
-- deliberately carries no foreign key (FK) to core.fleets -- the role isolation is the
-- boundary, and a cross-schema FK would couple memory to core.
--
-- Scope: every row belongs to one Fleet, keyed by the canonical fleet_id field.
-- agentsfleetd's runner-memory adapter derives fleet_id from the lease it issued and scopes every query
-- WHERE fleet_id = $1 (never a fetch-all + in-memory filter). agentsfleetd fully owns
...
-- instance_id -> fleet_id column change here is a fresh CREATE, not an in-place
...
-- memory_entries: primary store for fleet persistent memory.
...
-- fleet_id: the owning fleet (UUID) — our identifier end to end, no legacy instance_id prefix.
...
-- Required for ON CONFLICT (key, fleet_id) upserts in the runner-memory adapter.
CREATE UNIQUE INDEX IF NOT EXISTS idx_memory_entries_key_fleet_id
    ON memory.memory_entries(key, fleet_id);

CREATE INDEX IF NOT EXISTS idx_memory_entries_category
    ON memory.memory_entries(category);

CREATE INDEX IF NOT EXISTS idx_memory_entries_fleet_id
    ON memory.memory_entries(fleet_id);
