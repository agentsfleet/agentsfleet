-- Integration grants for fleet-to-service authorization. A fleet must
-- have an approved grant for a service before agentsfleet will inject
-- credentials for it. Fleet-initiated, human-approved.
...
CREATE TABLE IF NOT EXISTS core.integration_grants (
    ...
    CONSTRAINT uq_integration_grants_fleet_id_service
        UNIQUE (fleet_id, service)
);

CREATE INDEX IF NOT EXISTS idx_integration_grants_fleet_id
    ON core.integration_grants (fleet_id);
