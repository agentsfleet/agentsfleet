...
-- cache key, so a candidate built from revision N lands under a key containing
-- N and a request that has read N+1 simply looks somewhere else. That is what
...
--
-- Singleton by construction. `id` is constrained to a single numeric value
-- rather than left to convention, so a second generation row cannot be inserted
-- by a future writer that did not know the table was meant to hold one. The
-- CHECK is numeric — RULE STS bans string-literal value sets in schema, and a
-- boolean/integer identity guard is not one.
--
...
-- Seed the singleton so every reader finds a row. A revision read that returns
-- no row is indistinguishable from a failed read, and the endpoint answers
...
INSERT INTO core.model_catalogue_revision (id, revision, updated_at_ms)
VALUES (1, 0, 0)
