-- core.fleet_approval_gates is append-only by trigger (schema/009): gates are
...
-- account erasure (the Clerk user.deleted webhook) and a fleet hard-delete
-- must remove the gate rows, or their workspace/fleet foreign keys abort the
-- purge transaction and the erasure can never complete. Those transactions opt
-- in via a transaction-scoped setting (SET LOCAL fleet.allow_gate_purge),
