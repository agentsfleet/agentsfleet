-- Platform default Language Model (LLM) key reference table.
...
--   2. platform_provider_defaults active row → admin workspace vault.secrets  → platform default
...
-- M100, by the fk_platform_provider_defaults_model foreign key below — the Database (DB) makes the model-delete
...
--                      host). Validated https + Server-Side Request Forgery-safe in the app.
...
CREATE TABLE IF NOT EXISTS core.platform_provider_defaults (
    ...
    CONSTRAINT ck_platform_provider_defaults_uid_uuidv7 CHECK (substring(uid::text from 15 for 1) = '7'),
    ...
    CONSTRAINT uq_platform_provider_defaults_provider UNIQUE (provider),
...
GRANT SELECT, INSERT, UPDATE ON core.platform_provider_defaults TO api_runtime;
