-- fleet.runner_leases — one row per issued lease (fleet-runner split). The
...
-- checkpointFleetSession (fleet_id). The lease persists exactly that context
...
--   but does not verify it yet — the single-fleet loopback skeleton has no
