...
-- Webhook HMAC secrets live in vault.secrets keyed by `agent:<source>` (or
-- `agent:<credential_name>` when the trigger frontmatter overrides) — this
-- table holds no secret pointers.
-- Status transitions: active → paused → active | active → stopped (terminal)
-- Status values enforced in application code (error_codes.AGENT_STATUS_*)
-- required_tags: capability tags this agent needs to be placed (GitLab-tags /
...
CREATE TABLE IF NOT EXISTS core.agents (
    ...
    CONSTRAINT ck_agents_uid_uuidv7 CHECK (substring(uid::text from 15 for 1) = '7'),
...
-- api_runtime creates, reads, updates agents for CLI install/up/kill operations
-- and reads config + status at lease-issue time.
GRANT SELECT, INSERT, UPDATE ON core.agents TO api_runtime;

-- Partial index for Slack event routing: find the active agent with a
-- slack_event trigger for a given workspace (lookupSlackAgent in slack_events.zig).
...
CREATE INDEX IF NOT EXISTS idx_agents_slack_event_trigger
    ON core.agents(workspace_id, created_at)
    WHERE status = 'active';
...
CREATE INDEX IF NOT EXISTS idx_agents_required_tags_gin
    ON core.agents USING gin (required_tags);
