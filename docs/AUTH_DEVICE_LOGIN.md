...
| **One pepper in Vault** | `AUTH_SESSION_CODE_PEPPER` (defeats offline brute-force of the verification code), at `op://ops/ZMB_CD_{PROD,DEV,LOCAL_DEV}/auth-session-code-pepper/credential`. Boot fails fast if it is missing — `preflight.rs` reads it as a required knob. A second item, `audit-log-pepper`, was provisioned for an audit log that was never built and no code ever read; it was retired in M197 and comes back with the audit log, not before. Provisioning is documented in `playbooks/founding/01_bootstrap/001_playbook.md §1.3b`. |
