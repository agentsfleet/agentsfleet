...
Preserve existing state transitions, owner binding, code HMACs, encrypted payloads, attempt limits, expiry and the permitted 60-second same-fingerprint response retry. Guard transitions in PostgreSQL transactions and commit before returning protected ciphertext or authorizing a connector token exchange. A concurrent verify loser re-reads committed state and applies the same-fingerprint retry rule; zero updated rows alone is not a consumed error.
...
PostgreSQL auth state must use primary reads and durably acknowledged commits, with the service's actual failover durability verified before rollout; synchronous_commit=on alone does not establish synchronous replication. PlanetScale documents durable replica confirmation and most-caught-up promotion. The remaining unavailable-confirming-replica question is a shared platform risk for Indy to disposition, as recorded in the canonical design; no exception is currently approved.

The cutover fence covers every device/connect writer. Count legacy Redis sessions/nonces but exclude their values from exports and imports. In-flight logins and connect flows restart after cutover, regardless of its duration; new PostgreSQL auth tables start empty. Existing issued CLI credentials and installed connector grants remain valid.
...
| **Two peppers in Vault** | `AUTH_SESSION_CODE_PEPPER` (defeats offline brute-force of the verification code) and `AUDIT_LOG_PEPPER` (pseudonymizes `session_id` in the auth audit log target) — both at `op://ops/ZMB_CD_{PROD,DEV,LOCAL_DEV}/{auth-session-code,audit-log}-pepper/credential`. Boot fails fast if either is missing. Provisioning is documented in `playbooks/founding/01_bootstrap/001_playbook.md §1.3b`. |
