...
|---|---|---|
| GitHub run | the fleet reads the linked run, its jobs and their annotations over `http_request` under the read prefix, as its SKILL.md directs (M206_003 §2); the fleet reads commits and compare itself under the read egress prefix (`afd_gate/src/policy/egress/read.rs:14-27`) | A read mint asks for `contents`, `actions` and `checks` read (`afd_credential/src/credential/github.rs:149`), but the platform App grants no Checks permission (`playbooks/operations/github_app_registration/001_playbook.md:33-38`); until it does and each installation accepts, a mint asking for it is expected to fail (unverified). Job logs answer with a redirect to storage the exact-host allowlist cannot name (`src/runner/network/AllowList.zig:154`), and the tool never follows redirects; that hop is owner-held. |
...
| Continuous Integration (CI) evidence reach | 🟡 the read mint asks for `actions` and `checks` read and the read rule covers the CI paths (M206_003 §1–§2); the App registration lacks Checks: read | the App grants it and installations accept; the log's storage hop is owner-held | M206_003 |
