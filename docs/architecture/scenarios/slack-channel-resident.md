...
>
> **Shipped in the retired Zig daemon (M106), and in the Rust daemon (M206_002 §5).** Specced in [`../../v2/done/M106_001_P1_API_DOCS_INFRA_UI_SLACK_RESIDENT_CHANNEL_BOT.md`](../../v2/done/M106_001_P1_API_DOCS_INFRA_UI_SLACK_RESIDENT_CHANNEL_BOT.md). This page narrates what the Zig daemon did. The Rust daemon routes a mention to a fleet whose `TRIGGER.md` names the channel, and a mention no such fleet takes to the channel's resident: `slack-channel-<team>-<channel>`, installed through `Fleets::install` on the first mention with a policy built in code (`rustd/crates/afd_ingress/src/slack/resident.rs:74`), and bound once in `core.connector_channels` (`rustd/crates/afd_api_ingress/src/handler/mention/resident.rs:42`). Concurrent first mentions converge on the fleet name, as the Zig daemon's did. [`slack-incident-responder.md`](./slack-incident-responder.md) §4 carries the routing. The function names below (`innerCreateFleet`, `insertFleetOnConn`) are the Zig daemon's.
