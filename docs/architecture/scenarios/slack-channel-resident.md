...
>
> **Shipped in the retired Zig daemon (M106); not in the Rust daemon.** Specced in [`../../v2/done/M106_001_P1_API_DOCS_INFRA_UI_SLACK_RESIDENT_CHANNEL_BOT.md`](../../v2/done/M106_001_P1_API_DOCS_INFRA_UI_SLACK_RESIDENT_CHANNEL_BOT.md). This page narrates what the Zig daemon did. The Rust daemon routes a mention to a fleet whose `TRIGGER.md` names the channel (`rustd/crates/afd_api_ingress/src/handler/mention.rs:191`); a mention no such fleet takes is acknowledged and dropped as `unsupported_event` (`mention.rs:228`), and nothing in Rust reads or writes `core.connector_channels` yet. `docs/v2/active/M206_002_*` §5 restores the resident beside channel subscriptions, and [`slack-incident-responder.md`](./slack-incident-responder.md) §4 carries the routing. The function names below (`innerCreateFleet`, `insertFleetOnConn`) are the Zig daemon's.
