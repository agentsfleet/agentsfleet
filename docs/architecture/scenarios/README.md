...
| [`slack-incident-responder.md`](./slack-incident-responder.md) | A mention in a failed Continuous Integration (CI) run's Slack thread reaches exactly one fleet, which answers in that thread with GitHub and Grafana evidence; a draft fix Pull Request (PR) waits for a workspace member's approval. Zero, one, and several fleets per channel. Specified in M206, not built. |
