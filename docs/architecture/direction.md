...
- **Context is bounded — no unbounded growth across long-running incidents.** Three layers: memory_checkpoint (L1), tool-result rolling window (L2), and agent-owned run chunking (L3) — the runtime observes context fill and logs it but cannot interrupt the in-run model loop, so the agent snapshots and wraps up; a runaway run is bounded by the `budget` caps and the lease runtime deadline, not a continuation counter. See [`capabilities.md`](./capabilities.md) §4.
