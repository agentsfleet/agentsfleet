...
| Table | `core.fleet_bundle_templates` (platform) · `core.tenant_fleet_bundle_templates` (tenant) | `core.fleets` |
...
GitHub repo (author's source of truth)
   SKILL.md · TRIGGER.md · review.sh · lib/…
        │  POST /v1/admin/fleet-templates  ·  POST /v1/workspaces/{ws}/fleet-templates
        ▼
agentsfleetd
...
## Two-tier template catalog (M103)

Templates onboard into one of two catalog tiers, each its own table:

- **Platform tier — `core.fleet_bundle_templates`** (slug id, e.g. `github-pr-reviewer`). The global shop-window; a platform operator holding the `platform-template:write` scope onboards via `POST /v1/admin/fleet-templates`. Migration-seeded rows bootstrap the catalog and become installable once onboarded (which populates their content hash + snapshot).
- **Tenant tier — `core.tenant_fleet_bundle_templates`** (UUIDv7 id + `workspace_id` FK CASCADE). A workspace's own templates; a tenant admin holding `template:write` onboards via `POST /v1/workspaces/{ws}/fleet-templates`, deduped on `(workspace_id, content_hash)`.

The workspace gallery `GET /v1/workspaces/{ws}/fleet-templates` returns the union of all platform rows and that workspace's tenant rows, and nothing from another workspace.
...
| **Postgres template tables** | `skill_markdown`, `trigger_markdown` | the SKILL.md / TRIGGER.md text |
