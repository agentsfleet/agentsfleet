...
| Webhook ingest (GitHub App + manual) | The App receiver verifies the platform webhook signature, resolves installation → workspace, filters explicit repository/event/grant fleet subscriptions, and applies per-delivery/per-fleet replay protection before queueing. The manual fleet-addressed receiver remains available with its workspace webhook secret. Both normalise into `actor=webhook:github`. | App ingress + webhook receiver |
...
x-agentsfleet:
  model: accounts/fireworks/models/kimi-k2.6   # opaque pass-through; the control
                                                ...
                                                # the real model from
                                                # core.tenant_model_selection at
                                                # lease time.
  context:
