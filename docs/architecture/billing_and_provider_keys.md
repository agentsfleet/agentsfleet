...
**A local runtime is exempt from both credential gates, and from neither by accident.** Activating a model on hardware the operator owns used to require passing two checks it could not pass: catalogue membership, which a model id written by `ollama pull` can never satisfy, and a non-empty `api_key`, which a server that authenticates nobody has none of. Both now read one list — `LOCAL_RUNTIME_PROVIDERS` in `src/agentsfleetd/secrets/metadata.zig`, the leaf both the activation handler and the credential probe can import — and the allowlist parity check requires that list to equal the `activation_floor` set, so a provider cannot gain one exemption without the other or without a floor row behind it. Every hosted provider still fails closed on both. The exemptions do not compose further: a local runtime is still forbidden a `base_url`, so waiving its key never widens where it dials — NullClaw's fixed localhost entry remains the only destination.

Both client surfaces re-check these rules locally, which is why the server change alone changed nothing an operator could see: the CLI refused `--provider ollama` without `--api-key` and checked `--model` against the catalogue, and the dashboard gated Save on a key and offered a model picker holding only the floor row. Each restates the list once (`cli/src/constants/custom-endpoint.ts`, `ui/packages/app/lib/types.ts`) because the three runtimes share no module graph, and the parity check requires all four lists to be equal so no surface can lag the others.
