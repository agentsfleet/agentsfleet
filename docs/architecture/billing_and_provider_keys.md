...
Posture changes only whether the per-token component is added (platform) or not (self-managed); the run fee is the same. That difference is the friction-reducing signal: on-ramp on platform without a key, graduate to self-managed once the cost-vs-convenience tradeoff tilts. `RUN_NANOS_PER_SEC` is spelled in three repository files (`nanos.rs` + `app/lib/types.ts` + `cli/src/constants/billing.ts`), and `audits/cross-tier-rates.sh` requires a bump to land in all three.
