...
3. [`scenarios/`](./scenarios/) — three end-to-end walkthroughs following one persona (John Doe) across his journey: default cold install, switching to self-managed with Fireworks + Kimi K2.6, and the credit pool draining and tripping the gate.
...
| **Coding agent** | The workstation tool the human types into (Claude Code / Amp / Codex CLI / OpenCode) — drives `agentsfleet`; distinct from "the agent" (host) it operates. [(more)](./user_flow.md#80-the-wedge-surface-agentsfleet-install-platform-ops-skill) |
| **Steer** | A human-initiated message via `agentsfleet steer {id} "…"` or the dashboard chat composer; lands as `actor=steer:<user>`. [(more)](./user_flow.md#83-triggering-the-agent) |
| **Webhook trigger** | An external system POSTing to `/v1/webhooks/{agent_id}/{source}`; lands as `actor=webhook:<source>`. [(more)](./user_flow.md#83-triggering-the-agent) |
| **Trigger panel** | The dashboard card on `/agents/{id}` that renders the local `gh`/`curl` command to register the webhook on the provider — the platform never holds the user's provider PAT. [(more)](./user_flow.md#84-working-from-claude-or-the-dashboard) |
...
| **Cron trigger** | A NullClaw-managed schedule firing on time; lands as `actor=cron:<schedule>`. [(more)](./user_flow.md#83-triggering-the-agent) |
