...
The operator plane is addressable. `GET /v1/fleets/runners/{runner_id}` returns the runner record with derived liveness, a live-work summary, and lifetime counters computed from `fleet.runner_leases` joined to `core.fleet_events` — never from the per-runner Prometheus families, which are process-global, restart-zeroed and capped (§The four per-runner families). `GET /v1/fleets/runners/{runner_id}/leases` pages by keyset over `(created_at, id)`, each lease joined to its Fleet event so outcome and failure cause arrive in one read. Outcome settles server-side into one closed tag — `running` / `succeeded` / `failed` / `expired` / `unknown` — computed from the lease's own status first, so an expired lease is never credited with its reclaimer's later success. Both routes require `runner:read`; neither item struct carries `token_hash` or `request_json`, so emitting either is a compile error rather than a review catch.
...
**The lease read's index support is load-bearing, not incidental.** `runner_leases` gains a row per claim and another per reclaim, and nothing prunes it — no retention sweep touches the table; only tenant offboarding and the runner/fleet cascades delete rows. A runner's count is therefore the integral of its working life: one worker turning a short event every `LEASE_TTL_MS` accrues roughly 2.9k rows/day, and `MAX_WORKER_COUNT` workers make that ~184k. Schema slot 041 indexes the two access paths the read uses — `(runner_id, created_at DESC, id DESC)` for the page and `(fleet_id, event_id, fencing_token)` for the per-row reclaim derivation — which turns a full-history scan plus sort into a bounded index scan and the reclaim probe into an index-only scan. Read cost stays flat as history grows; `db/index_usage_integration_test.zig` pins both shapes and both plans. The detail read's lifetime counters still aggregate the whole history at read time, unlike `core.fleet_activity_counters`, which is maintained incrementally at write time — the windowed-counter follow-up is what bounds that.
