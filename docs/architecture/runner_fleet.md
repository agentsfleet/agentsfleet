...
- **Engine spawns need the process Io.** The engine's compat layer (`nullclaw.compat`) spawns its curl provider transport through the process `Io`; the runner hands it over via `nullclaw.compat.initProcess(init)` as `main`'s first statement — the same call NullClaw's own binary makes. Skipping it does not fail loud at startup: the compat layer silently falls back to Zig's `Io.Threaded.init_single_threaded`, whose allocator is `.failing` (Zig 0.16), so every provider spawn dies **pre-fork** with a synthetic `OutOfMemory` that `mapError` then mislabels `oom_kill` on the event. Both daemon and `__execute` child enter the same `main`, so the one call covers every mode.
