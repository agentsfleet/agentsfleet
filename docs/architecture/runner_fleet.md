...
> **Launch slice vs the deferred L7 proxy.** The above is the **launch** egress model (own-netns + host-side `nftables` IP-allowlist, resolve-at-setup). The **L7 DNS-pinning forward proxy** — SNI/`CONNECT` name allowlisting for rotating-CDN host sets, DNS-rebinding pin, encrypted-SNI default-deny — is **deferred to the untrusted-runner GA tier**, where the operator's host list is untrusted and CDN IPs churn faster than a lease. The launch IP-allowlist (small, stable: inference + 8 registries) is the foundation it will sit on. Standing residual: an allow-listed write-capable host (e.g. `github.com`) is still an exfil channel by design — closed only by short-lived/scoped tokens, a credential-model change, not this layer.
