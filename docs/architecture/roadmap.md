...
M80_006 shipped per-lease renewal (§3 — a *live* runner keeps its lease). The operator plane (§1: `GET`/`PATCH /v1/fleet/runners`, cordon/revoke) and heartbeat-lapse reassignment (§2: expire a *dead* runner's affinity so its work re-leases to a healthy host) were carved out after a design study. That study resolved (Jun 2026): the **read** — `GET /v1/fleet/runners` (paginated, platform-admin-gated, **derived** liveness, no `token_hash`) — lands in **M84_001**; the **mutation + reassignment** — `PATCH /v1/fleet/runners/{id}` cordon/drain/revoke, the `status`→`admin_state` typed enum, `UZ-RUN-009`, the append-only `fleet.runner_events` log, and the liveness sweeper that closes §2 — land in **M84_002**. The model: typed `admin_state` (intent) + **derived** liveness (runtime, never stored) + `runner_events` (history); **no JSONB status** (cross-validated). The deeper points the study surfaced still hold:
...
- **Eligibility — which runner can take it?** A cordoned/lapsed runner's work can't route anywhere: the target must satisfy trust class, scope, sandbox tier, and capacity — the same filter the **M85_001** scheduler owns (label placement; "M80_007" was the stale reservation — it shipped as the observability spec). M84_002 reassignment re-leases to any eligible runner and composes with M85_001's filter when it lands; reassigning without it risks routing prod / other-tenant work to an unfit host (see *Local-runner affinity trust scope*, runner_fleet.md).
...
The study is done: the `GET /v1/fleet/runners` read + honest derived liveness are specced in **M84_001** (active), and the `PATCH` mutation surface, `admin_state` (`cordoned`/`revoked`/`draining`/`drained`), and the `UZ-RUN-009` "Runner revoked" code are specced in **M84_002** (pending) — they stay unwired until M84_002 lands. Until then, heartbeat-lapse recovery is bounded by the lease TTL backstop + the pull-triggered reclaim that M80_002 already ships.
