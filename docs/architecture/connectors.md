...
- **Residual window: the connect phase.** Name resolution + the TCP dial happen before a pooled handle exists to arm — they are OS-bounded (connect timeouts), not watchdog-bounded. The armed surface is the read phase, where the M100/M106 incidents actually lived (vendor accepts, then stalls). A future dial-phase bound is a follow-up, not a regression: pre-M108 the *entire* call was unbounded.
