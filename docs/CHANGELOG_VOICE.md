...
- **Rate constants stay pinned across runtime surfaces:** `audits/cross-tier-rates.sh` checks `rustd/crates/afd_billing/src/nanos.rs`, `ui/packages/app/lib/types.ts`, and `cli/src/constants/billing.ts`. When public docs quote a rate, update `~/Projects/docs/snippets/rates.mdx` in a paired Pull Request. In `*.mdx`, import its named values instead of hand-typing money amounts.
