...
Successful `agt_t` authentication updates `core.api_keys.last_used_at` on the matched active key. The backend stores and compares only the SHA-256 hash; the raw key is returned once at creation time and is never persisted.
