...
**Known gap — every tenant key carries identical power, and it is not narrowable.** `core.api_keys` has no scopes column and no workspace column; `tenant_api_key.zig` applies `scopes.defaultScopes(.tenant_api_key)` — the `.tenant_owner` set minus `approval:resolve` — at principal construction on every request. A read-only key, a fleet-only key, or a workspace-restricted key cannot be minted today, so revocation is the only lever an operator has. Clerk cannot supply a narrower set either, and this is the one place where "Clerk is the source of truth for scopes" does not reach: Clerk holds scopes on a **user**, and a tenant key deliberately has none — `240` makes `created_by` a plain provider subject rather than a foreign key so the key outlives the admin who minted it, and resolving that admin's live scopes would undo exactly that. Closing the gap means per-key scopes chosen at creation, stored on the row, and intersected with the default grant at principal construction: a schema column and one intersection, not a redesign. Its sibling for `afc_` is the *Known gap* under §CLI credential — resolved, not granted.
