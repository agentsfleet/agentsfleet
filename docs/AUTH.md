...
**The verdict is memoized per process** (`auth/runner_token_cache.zig`, consulted by the `LookupFn` host in `cmd/serve_runner_lookup.zig` — the middleware itself is unchanged). An `agt_r` is opaque, so unlike the JWT plane it cannot be verified without going to look; unmemoized, that is one `fleet.runners` read per heartbeat and per lease poll per runner, forever. The memo stores `sha256(token) → {runner_id, active}` — never the token — and an entry lives at most `HEARTBEAT_INTERVAL_MS`. That expiry is the trust bound: the machine that serves a cordon/drain/revoke/delete drops the entry itself the moment the write commits, and any *other* machine's memo expires within one heartbeat — so a runner taken out of service authenticates nowhere later than one liveness cycle after the operator's write. Misses are never memoized: a token that fails the lookup keeps costing a lookup, so unauthenticated garbage cannot evict live runners from the table.
