...
**Categories:** API, UI
**Batch:** B1
**Branch:** fix/m133-live-stream-closure
...
| `ui/packages/app/tests/e2e/acceptance/fixtures/seed.ts` | EDIT | Keep API-side fleet seeding aligned with the current library-backed install body and walk every workspace-list cursor so acceptance resolves newly-created workspaces beyond page one. |
...
### §1 — The multiplexed handler (fan-in, not pattern-subscribe) — **DONE**
...
- **Dimension 1.1 — DONE** — an authorized subscriber receives multiplexed frames from every readable fleet in the workspace, each tagged with the correct `fleet_id` → Test `test_workspace_stream_fans_in_all_readable_fleets`
- **Dimension 1.2 — DONE** — the fan-in subscribes per-fleet channel via the hub, never `PSUBSCRIBE` (no cross-tenant firehose) → Test `test_workspace_stream_uses_scoped_fanin_not_pattern`
- **Dimension 1.3 — DONE** — the whole connection claims exactly one stream-registry slot regardless of fleet count → Test `test_workspace_stream_claims_single_registry_slot`

### §2 — The authorization boundary (a subscriber sees only its workspace) — **DONE**
...
- **Dimension 2.1 — DONE** — a caller authorized for workspace A never receives any frame published on a workspace-B fleet's channel, even under concurrent publishes → Test `test_workspace_stream_never_leaks_other_workspace_frames`
- **Dimension 2.2 — DONE** — a caller with no read access to the path workspace gets `ERR_FORBIDDEN` before any subscribe → Test `test_workspace_stream_forbidden_for_unauthorized_workspace`
- **Dimension 2.3 — DONE** — membership revoked mid-stream: each re-enumeration tick re-runs the RLS-scoped query under fresh authorization, so a caller who loses access is unsubscribed from the lost fleets by the next tick → Test `test_workspace_stream_membership_revocation_unsubscribes`

### §3 — Cap, backpressure, reconnect, and a changing fleet set — **DONE**
...
- **Dimension 3.1 — DONE** — at cap (or draining) the connection is refused with `ERR_SSE_STREAM_CAP` + Retry-After, before any subscribe → Test `test_workspace_stream_refused_at_cap`
- **Dimension 3.2 — DONE** — a slow consumer drops oldest frames per-fleet (bounded ring) and the connection survives; a fully vanished client is torn down at the next heartbeat → Test `test_workspace_stream_slow_consumer_drops_oldest`
- **Dimension 3.3 — DONE** — a fleet created mid-stream is picked up on the next re-enumeration and its frames begin arriving; a fleet deleted mid-stream is unsubscribed with no error to the client → Test `test_workspace_stream_picks_up_and_drops_fleets_midstream`
- **Dimension 3.4 — DONE** — reconnect resumes from sequence 0; `Last-Event-ID` request headers are ignored; the client recovers the gap with a `since` floor (last confirmed `created_at` minus overlap) then follows `next_cursor` pages, merging by event id — the discipline `fleet-stream-backfill.ts` already implements (a bare event id is NOT a cursor: `parseCursor` decodes the composite `(created_at, event_id)` keyset and rejects anything else as 400 `InvalidCursor`) → Test `test_workspace_stream_reconnect_resets_seq_client_backfills`
- **Dimension 3.5 — DONE** — hub or Redis unavailable at connect time is refused with a transient 503 (`internalDbUnavailable`, the per-fleet `startStreamThread` shape), never a hang or a half-open stream → Test `test_workspace_stream_503_when_hub_unavailable`

### §4 — The wall consumes one stream; the N-connection mode dies — **DONE**
...
- **Dimension 4.1 — DONE** — the wall opens exactly one `EventSource` for the workspace; tiles update from `fleet_id`-tagged frames → Test `test_wall_opens_single_workspace_stream`
- **Dimension 4.2 — DONE** — the workspace stream unavailable (503 / Redis down) leaves every tile on its last-event snapshot, no dead tiles, one reconnect loop → Test `test_wall_degrades_to_snapshot_when_stream_unavailable`
- **Dimension 4.3 — DONE** — a malformed or untagged frame is dropped, never routed to a wrong tile or crashing the wall → Test `test_workspace_stream_drops_malformed_frame`
- **Dimension 4.4 — DONE** *(added at PLAN — Indy, Jul 15, 2026: "make the user experience better")* — a burst of frames across many tiles coalesces into ONE render per animation frame, not one render per frame; a 60-fleet burst must not cost 60 React renders → Test `test_wall_coalesces_burst_into_one_render`
- **Dimension 4.5 — DONE** *(added at PLAN)* — a real server-emitted `hello` frame received by the production client carries the fanned-in fleet set; a tile absent from that set shows its `last known` eyebrow → Test `test_wall_marks_tiles_live_from_hello_frame`
- **Dimension 4.6 — DONE** *(added at PLAN)* — a real server-emitted `catching_up` frame received by the production client surfaces a "catching up" state rather than silently showing stale tiles → Test `test_wall_surfaces_catching_up_on_drops`

### §6 — The fan-in must not reintroduce per-viewer cost — **DONE** *(added at PLAN — Indy, Jul 15, 2026: "code optimized performance, concurrency … robust stability")*
...
- **Dimension 6.1 — DONE** — V concurrent streams on one workspace cost ONE fleet enumeration per cadence, not V → Test `test_fleet_set_cache_enumerates_once_for_many_viewers`
- **Dimension 6.2 — DONE** — a connection whose cached version is unchanged does no query and no re-subscribe work on its tick → Test `test_fleet_set_cache_version_unchanged_is_a_no_op_tick`
- **Dimension 6.3 — DONE** — authorization is NOT shared: a caller revoked mid-stream is torn down on its next tick even though the workspace's fleet set never changed → Test `test_workspace_stream_revocation_is_per_caller_not_cached`
...
| R5 | Per-fleet console stream not regressed | `rg -c 'http/handlers/fleets/sse_streaming_integration_test.zig' src/agentsfleetd/tests.zig && make test-integration` | `1` import and exit 0 | P0 | ✅ import 1; integration suite green |
...
- **Closure gate — historical Jul 22 state.** The isolated live scale/Redis-kill/churn/RSS/shutdown dimensions and QStash registration/smoke were green; full browser acceptance was the only open item at that point. The Jul 26 closure decision below supersedes this earlier gate state.
- **Next-agent handoff — superseded Jul 26.** PR `#542` merged and its backend revision deployed successfully. Current-main artifacts plus direct operator observation established that Fleets appear live on the wall; M136_001 now owns the deliberately unrun exhaustive deployed wall checks.
- **Closure continuation, Jul 26** — PR `#542` merged as `86d3379de3ddfd0d7a32a2df6aea0c0ac63a9019`; deploy workflow `29921940802` successfully pushed and verified that exact backend revision, but its Command-Line Interface acceptance failure cancelled browser acceptance. Development has since advanced to `86fe979`. Current-main run `30201104322` reached the wall with the seeded Fleet rendered `data-state="live"`; its downloaded screenshot/trace show four live tiles and the expected workspace route. The first failure was stale harness drift from commit `9293c9c08`, which changed the overlay's accessible name to `Manage fleet: <name> — Agent <callsign> — <status>`, so the dedicated `live-counter` and `pulse-wall` projects were never scheduled. The remaining artifacts likewise identify harness races rather than a workspace-stream failure: login lifecycle shared another parallel journey's teardown prefix; logs navigated away before the optimistic steer reached the durable event list; operator journey only read page one after visibly creating and switching to its workspace; Skill source could already be expanded. This continuation repairs those assumptions on current `main`. Focused validation is green: wall Vitest `12/12`, package typecheck, package lint (`0 warnings and 0 errors`), Playwright discovery (`57 tests in 33 files`), and `git diff --check`.
- **Non-interactive acceptance constraint, Jul 26** — no browser cookies were imported and no macOS Keychain/password prompt was invoked, per operator constraint. This worktree had no `.fixture-jwts.json`; `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` and `ACCEPTANCE_E2E_PROVISION_TOKEN` were absent, so it could not run deployed browser automation locally. That limitation is preserved honestly; M136_001 inherits the exhaustive same-environment proof rather than this spec claiming those projects ran.
...
- **Closure state — DONE, Jul 26.** §4 and §7 are DONE, including production receipt of real server-emitted control frames and isolated live scale/chaos/RSS/shutdown evidence. §5 is NOT TRIGGERED because M131 client aggregation has no measured paging breach. Indy directly eyeballed Fleets being added to the deployed wall and explicitly accepted that evidence for M133 closure. The unrun exhaustive `live-counter` and `pulse-wall` checks are not relabelled as automated passes: M136_001 now owns exact-count lifecycle, one-workspace-stream, Fleet-specific routing, reconnect, and replay proof using the real `github-pr-reviewer` Fleet.
