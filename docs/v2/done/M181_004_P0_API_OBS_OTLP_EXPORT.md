...
- **Coverage ratchet raised to 98 ahead of the code (2026-09-02)** — CI's `test-coverage-rustd` failed the then-97% floor at 96.7445%. Measured locally on `f8f9eab48`: **96.7582%, 35787 of 36986 lines covered, 1199 missed**. The split matters and was put to Indy: **311** of those missed lines are in the 37 files this branch touches, **888** are in 205 files it does not. Covering every line this milestone owns reaches **97.60%** — 98% needs a further ~149 lines from `afd_fleet/lease/pull.rs` (28), `lease/mint.rs` (27), `afd_connector/{complete,app,connect}.rs` (19/19/18), `afd_credential/error.rs`, `afd_db/test_util.rs` and others M181_004 never touches.
  > Indy (2026-09-02): selected **"Floor 98 now, tests follow"** — over covering this branch's lines and setting the floor to the ~97.6% earned. The floor moves to 98 in `71ea97bf3`, so `test-coverage-rustd` reports red until a later effort reaches it — red by decision, not by omission. The ratchet LEADS the code here, which is the reverse of how 96→97 was set, and the makefile's provenance note says so. Indy owns when and how it goes green.
  - **For the next agent:** M181_004 is finished on every other axis — all nine rubric rows graded, both remaining review findings deferred with Indy's quote, greptile's P1 fixed in `1b38fce1e`. Coverage is the one red check and it is red on purpose. Do not lower `RUSTD_COVERAGE_FLOOR` to clear it; the verdict text in `make/test-integration-rustd.mk` names that as the thing the ratchet exists to prevent.
