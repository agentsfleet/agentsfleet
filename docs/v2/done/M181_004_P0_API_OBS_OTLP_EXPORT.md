...
| `docs/metrics.census.tsv` | EDIT | the six understated series ceilings Indy approved correcting (Discovery), the two families this build moves to the unproduced ledger, and the eleven Indy struck from the contract at REVIEW |
...
Every `memory_*` family and the remaining `repair_*` families DO have producers — the ledger closed at TWELVE rows: fourteen anticipated here, twenty-three after the coverage audit, then eleven struck from the census outright at REVIEW because their mechanism is a never rather than a not-yet (Discovery).
...
| 4.3 | integration | `otlp_outage_logs::*` | a refused collector counts LOG RECORDS lost and reaches `entries_discarded_total` with `signal="logs"`; a live one counts nothing; shutdown is delegated; the emit stays off the caller's path |
...
- **Oversize files the branch inherited (CHORE(close), 2026-09-02)** — S7 grades every file the diff TOUCHES, so it fired on four `.rs` files that were already over the 350-line cap on `origin/main` and that this branch grew further: `afd_runner/src/sweep/repair.rs` 465→503, `agentsfleetd/src/preflight.rs` 453→464, `agentsfleetd/src/serve.rs` 392→410, `afd_tenant/src/signup.rs` 351→354. Forty-six `.rs` files on `main` are over the cap; the harness's own LENGTH GATE grades STAGED files and passed on every commit, and `models.rs` (367→253) was the only file this branch pushed ACROSS the cap. Offered as amend-the-rubric / split-all-four / grade-red.
  > Indy (2026-09-02 11:0x): selected **"Split all four under 350"** — context: S7 grading, over the recommendation to narrow the rubric instead. All four split by concern in `a34da00c2`; `Cargo.lock` and `Cargo.toml` cannot be split and left the row's scope instead.
- **Three crates missing from Files Changed (CHORE(close), 2026-09-02)** — R2 graded ❌: `afd_gate/**`, `afd_events/**` and `afd_fleet_lifecycle/**` were in the diff and not in the table. Each carries exactly one `producers::fleet::ready_write_failed()` call and the dependency it needs — the shape the `afd_fleet/**` row already describes as "the one call site that owns the operation". Table row added; not scope creep, an under-specified table.
- **Unproduced ledger — eleven struck, twelve kept (REVIEW, 2026-09-02)** — the ledger was presented at 23 rows, grouped by reason. Eleven of them were a NEVER rather than a not-yet: the eight `agentsfleet_redis_pool_*` families (the Zig hand-rolled a pool because a blocking client can only have one command in flight per socket; the multiplexed async client has no such limit, so what the pool existed to solve stopped being a problem rather than being ported) and the three `agentsfleet_sensitive_*` erasure families (no request- or response-buffer erasure path exists in this daemon). A ledger holding both kinds stops working as a to-do list.
  > Indy (2026-09-02): selected **"Strike the 11 now"** — over the recommendation to accept 23 and note the split for a later milestone. Struck from `docs/metrics.census.tsv`, `metrics/declared/redis.rs` deleted whole, the three `SENSITIVE_*` constants removed from `metrics/declared/memory.rs`, and the eleven rows plus their two now-unused reason constants removed from `metrics::produced`. Census 71 → 60 families; ledger 23 → 12 rows; claimed unchanged at 48. `produced.rs`'s module doc previously used the connection pool as its worked example and now states the never/not-yet rule instead.
- **The four review findings (REVIEW, 2026-09-02)** — the adversarial review left four for Indy. Offered with a recommendation to fix two and defer two.
  > Indy (2026-09-02): selected **"Take my call: fix 1 and 2"** — context: disposition of the four findings left at REVIEW.
  - **FIXED — the SDK's `internal-logs` feature is dropped.** `opentelemetry-otlp` with it on logs the endpoint URL and the collector's response body at debug level, and the endpoint is read from the same place as the credential beside it. It defeated the hand-written `Debug` on `OtlpConfig` and the preflight fault-by-knob-name landed earlier in this branch: a value that must not reach stderr must not reach it through a dependency's own diagnostics either.
  - **FIXED — the log signal has a drop counter.** `CountingLogExporter` wraps the OTLP log exporter, counts refused RECORDS, and feeds `agentsfleet_otlp_entries_discarded_total` with `signal="logs"` — the family and labels the other two signals already use, and the producer `Signal::Logs` had been declared without. `Exports::records_lost()` exposes the handle beside `spans_lost` and `cycles_lost`. It deliberately raises NO `tracing` warning: a warning there becomes a log record, which the subscriber hands to the batch processor, which hands it back to the exporter that is failing. Four tests in `tests/otlp_outage_logs.rs`. The strict fix also required declaring `opentelemetry_sdk`'s `logs` feature at the workspace, which the binary had been getting only through `opentelemetry-otlp`'s feature unification — `cargo test -p afd_observability` is where that showed up.
  - **DEFERRED — lease decrements on reclaim, retention, liveness, stale-fence and renew.** `lease_taken` fires at `afd_fleet/src/lease/deliver.rs:116` and `lease_released` only at `report.rs:146`, so `agentsfleet_runner_active_leases` drifts upward on every other path. The census already declares the gauge best-effort and self-healing on restart. Five paths in `afd_fleet`, each with its own fencing semantics, is fleet-correctness work with its own test surface rather than export-transport work.
  - **DEFERRED — `gen_ai.request.model` cardinality.** Tenant-influenced and bounded only by the SDK's 2000-series default. Capping it would REVERSE the recorded decision to drop the Zig's 256 cost-series budget in Rust, and a reversal made in passing at CHORE(close) is the worst moment for that call. An overrun announces itself as `otel.metric.overflow`.
- **Coverage ratchet raised to 98 ahead of the code (2026-09-02)** — CI's `test-coverage-rustd` failed the then-97% floor at 96.7445%. Measured locally on `f8f9eab48`: **96.7582%, 35787 of 36986 lines covered, 1199 missed**. The split matters and was put to Indy: **311** of those missed lines are in the 37 files this branch touches, **888** are in 205 files it does not. Covering every line this milestone owns reaches **97.60%** — 98% needs a further ~149 lines from `afd_fleet/lease/pull.rs` (28), `lease/mint.rs` (27), `afd_connector/{complete,app,connect}.rs` (19/19/18), `afd_credential/error.rs`, `afd_db/test_util.rs` and others M181_004 never touches.
  > Indy (2026-09-02): selected **"Floor 98 now, tests follow"** — over covering this branch's lines and setting the floor to the ~97.6% earned. The floor moves to 98 in `71ea97bf3` and **PR #649 stays blocked on `test-coverage-rustd` until a separate effort reaches it.** This is a deliberate hold, not an unfinished lane: the ratchet LEADS the code here, which is the reverse of how 96→97 was set, and the makefile's provenance note says so.
  - **Consequence for the next agent:** #649 is complete on every other axis — all nine rubric rows graded, both remaining review findings deferred with Indy's quote, greptile's P1 fixed in `1b38fce1e`. The one red check is coverage, and it is red on purpose. Do not lower `RUSTD_COVERAGE_FLOOR` to clear it; the verdict text in `make/test-integration-rustd.mk` names that as the thing the ratchet exists to prevent.
