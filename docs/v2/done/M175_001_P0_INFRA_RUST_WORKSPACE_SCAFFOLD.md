...
- **Skill-chain outcomes**
  ...
  - `orly-babysit-prs` — Pull Request #629. Continuous Integration (CI) settled at 30 pass / 2 skip / 0 fail after one fix: `safety-gates` was red because `check-architecture-doc` found eleven citations of targets and scripts §6 deleted, in `docs/` — a directory the first orphan sweep never walked. Greptile posted two findings, both dispositioned:
    - **P1, `.github/workflows/test.yml` — "Rust lanes bypass required checks".** Mechanism correct, outcome intended. Invariant 5 keeps the Rust job out of the `test` aggregate's `needs` on purpose while the port is built, and the §4 Failure Modes row names the alternative — a coverage context blocking merges — as the failure being avoided, with Pull Request #627 as precedent. Nothing Rust serves is reachable by a client until M181, and promoting the context is that milestone's decision. Replied with the evidence; no code change.
    - **P2, `src/lib/contract/fixture_sample.zig` — "Union fixtures cover one variant".** Valid and fixed. The emitter carried every TAG of an enum but only the FIRST variant of a union, which is inconsistent and left `ActivityFrame`'s other three variants and `ExecutionResult.Outcome`'s `failed` arm never byte-compared. A union fixture now carries every variant, and the reflection walks one level of nested type declarations so a union declared inside a struct gets its own fixture. Fixtures 57 → 64, tests 166 → 180. Proven by seeding drift into the THIRD variant and into the `failed` arm: both go red now, neither would have before.
