...
- **Dimension 1.4** — **DONE** — the deploy step refuses rather than falling through when the collector never reaches its desired count of **health-passing** machines, so a daemon is never pointed at an app that is still starting. Fly reports `started` when the VM is up, which precedes the collector binding `:4318`; readiness is `fly.toml`'s `[checks.health]` verdict against the collector's own `health_check` extension on `:13133`, and a machine declaring no check counts as unproven rather than ready → Tests `test_fails_when_machines_never_start` · `test_fails_when_running_count_is_below_desired` · `test_fails_when_machines_run_but_never_pass_health_checks` · `test_fails_when_a_machine_declares_no_health_check` · `test_the_two_refusals_name_different_causes` · `test_deploys_from_context_when_no_machines_exist`
