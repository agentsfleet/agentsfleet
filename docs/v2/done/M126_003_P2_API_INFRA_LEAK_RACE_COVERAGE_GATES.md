...
| `src/agentsfleetd/cmd/serve_lifecycle_integration_test.zig` | CREATE | boot → SIGTERM → drain full-daemon test; drives the real `serve.run` on a thread, `/healthz`-200 boot witness, holds one live agt_t-authed SSE stream, asserts EOF on drain; runs in the valgrind lane |
| `src/agentsfleetd/observability/telemetry.zig` | EDIT | `Telemetry.initProd` guards `builtin.is_test` (comptime `Backend` = `TestBackend` has no `.client` field) — a latent compile bug §5.1 is the first test to expose by referencing `serve.run`; production path unchanged |
...
- **Dimension 5.1** — full-daemon boot → SIGTERM → drain: server up, streams attached, sweepers running; SIGTERM; asserts joined threads, freed state, exit path — and the test executes inside the §1 valgrind lane → Test `test_daemon_boot_sigterm_drain_clean` — DONE (`serve_lifecycle_integration_test.zig`, DB+Redis-gated integration). Drives the **real `serve.run`** on a thread under `std.testing.allocator` (the freed-state oracle) — NOT `TestHarness.deinit` (a hand-maintained mirror of serve.zig's teardown that can't catch a regression in the real defer order). **Design deviation from the seam approach** (see Discovery): no `serve_shutdown.zig` `publishedEvent()` seam — a Fable-5 re-review found `publishServer` fires *before* httpz binds inside `listen()`, so a publish witness can fire for a server that lost the port race and died; the witness is instead a `/healthz`-200 poll (also proves the signal handler is installed, since `serve.run` installs it before server init), which subsumes the seam → **zero shutdown-choreography production edits**. Env-gated + boot-probed (real pg connect + `enforceServeMigrationSafety` + TLS-redis connect → SkipZigTest) so `serve.run`'s boot-path `process.exit(1)` can never kill the suite; env-map built from scratch (never a live snapshot — a dev POSTHOG/OTLP key would spawn real exporters). Boot-retry ≤3 fresh ports (allocFreePort TOCTOU). Holds one live agt_t-API-key-authed SSE stream (DB-backed `api_key_lookup`, no JWKS fetch), raises `SIGTERM` only post-healthz, asserts the client hits **EOF** (the freed-under-a-live-stream defect class `streams.drain()` closes), then a bounded `run_done` wait + `thread.join()` prove every join completed, a post-shutdown `/healthz` connection-refused proves the listener tore down, and `testing.allocator` proves zero leaks. Restores `SIG_DFL` (INT+TERM) + `serve_shutdown.reset()` in defers. **Isolation (VERIFY finding):** a real full-daemon boot is too invasive to interleave with the ~2000-test shared `make test-integration` binary — it perturbs process-global state (the telemetry ring records a `.server_started`, `model_rate_cache` is deinit'd, redis connections churn → `ConnectionBusy`), which surfaced as 14 unrelated failures (tenant_provider / patch_concurrent) that all pass in isolation. Fixed by gating the test on `AGENTSFLEET_LIFECYCLE_ISOLATED`: it runs ONLY in its own filtered process — the `make memleak` **boot-drain lane** (which sets the env) — and `SkipZigTest`s in the general suite. This is the correct home for a full-daemon lifecycle test (isolated process, under the leak gate). §5.2–§5.5 (not full-daemon) still run in `make test-integration`. **Validated: boot-drain lane runs it (≈3.4 s, marker + leak-clean); the general suite skips it (1 skipped, marker absent); the shared integration suite is green again.**
...
- **Dimension 7.2** — two seed probes establish the pattern: `model_rate_cache` rebuild/swap soak (complements the §3 injectability with the native-side view) and hub subscribe/unsubscribe churn soak → Tests `test_rate_cache_rebuild_rss_bounded`, `test_hub_churn_rss_bounded` — DONE (by EXTENDING the existing files, not new ones). Rate cache: `model_rate_cache_integration_test.zig` gains "RSS growth over the rebuild/swap soak stays bounded (production page_allocator)" — it deliberately does NOT call `setBackingAllocatorForTest`, so the soak runs on the production `page_allocator` layer `testing.allocator` cannot see (§3.1 owns that in-process oracle); 8-cycle warm-up to the allocator plateau precedes the baseline, 256-cycle soak, saturating growth (`after -| baseline`) bounded < 2 MiB. Hub: `subscription_hub_test.zig` gains "RSS growth over subscribe/unsubscribe churn stays bounded (cold hub, production allocator)" — a **cold hub on `std.heap.smp_allocator`** (the intended production general-purpose allocator; conn == null makes `wireSend` a warn-free no-op, no Redis), 32-cycle warm-up, 16_384-iteration soak of balanced subscribe/unsubscribe, `channelCount() == 0` residue check, saturating growth < 4 MiB. Both skip on `rss.currentBytes() == null` (unsupported platform), never false-fail. **Validated pass vs live pg** (rate cache) and native (hub).
...
- **Consults** — Architecture / Legacy-Design / gate-flag triage: the question asked + Indy's decision.
  ...
  - §5.1 design (Fable-5 advisor re-review, xhigh, Jul 11): the planned `serve_shutdown.zig` `publishedEvent()` seam was **dropped**. Rationale: `publishServer` fires before httpz binds inside `listen()`, so a publish witness can fire for a server that lost the `allocFreePort` bind race and died; a `/healthz`-200 poll is the correct witness (and proves the handler is installed). Re-arming the `common.Event` by `= .{}` in `reset()` is additionally a plain-store-vs-atomic-load data race (UB). Net: no shutdown-choreography production edit (the seam row is removed from Files-Changed). A separate latent bug surfaced — `Telemetry.initProd` did not compile in test builds (comptime `Backend` = `TestBackend` has no `.client` field); §5.1 is the first test to reference `serve.run`, so it exposed it. Fixed in the same diff (guarded by `builtin.is_test`; production unchanged) and added to Files-Changed.
