...
**Test Baseline:** `unit=2640 integration=561` at the comparison revision — derived, not measured: the branch's measured `unit=2662 integration=570` less the 22 unit and 9 integration tests this diff adds and the 0 it removes. A baseline worktree needs a full Rust build and the machine had 16 GiB free against a 34 GiB `target/` per worktree. **Final, measured at close:** unit=2662 passed / 0 failed / 591 ignored across 155 binaries (+22) · integration=571 passed / 0 failed across 134 binaries (+10: the nine above plus `test_removing_another_workspaces_entry_answers_204_and_leaves_it`, added by the skill chain) · TypeScript app 2916 · cli 1749. Both lanes exit 0.
...
| `rustd/crates/afd_api_tenant/src/handler/library_entry.rs` · `library_entry/{list,remove,tests}.rs` (CREATE) · `src/{lib.rs,handler/mod.rs,openapi.rs}` · `rustd/crates/afd_observability/src/metrics/label/library.rs` · `rustd/crates/afd_observability/tests/producers_drive.rs` · `docs/metrics.census.tsv` · `docs/architecture/observability.md` (EDIT) | CREATE / EDIT | The owned-entry collection and its single-entry removal, in their own module rather than inside `workspace_library.rs` — that module is at 287 lines against a 350 cap and serves a different collection. The three edits are the wiring the routes do not exist without: the dispatch arm at `lib.rs:89`, the module declaration at `handler/mod.rs:16`, and the utoipa `paths(...)` registration at `openapi.rs:91`. The new `workspace_entries` surface label is a census change: the label set is closed, so its ceilings in `docs/metrics.census.tsv` and its allowed values in `observability.md` move with it. `producers_drive.rs` drives the producers from `Surface::ALL` rather than a list repeated in the test, because a hand-kept copy stops driving the surface that was just added — which is exactly what it did. |
...
|-----------|------|------|---------------------------------------------|
| 1.1 | unit | `test_delete_grant_slot_is_registered_and_single_privilege` | The slot text holds exactly one `GRANT`, naming `DELETE` and `core.tenant_fleet_library`, and no `ALTER` or `DROP`; the migration array contains the filename and `version_of` derives 917. `test_every_library_producer_runs_its_body` drives every member of `Surface::ALL`, so the `workspace_entries` label cannot raise a census ceiling with nothing exercising it. |
...
| 2.3 | integration | `test_delete_is_idempotent_under_replay_and_concurrency` | Onboard, `DELETE` → 204, empty body, and the follow-up owned read omits the entry; the same `DELETE` twice → 204 both times, row count unchanged after the second; two simultaneous `DELETE`s → both 204, one row removed, no error in either. And `test_removing_another_workspaces_entry_answers_204_and_leaves_it`: a `DELETE` naming another workspace's entry answers 204 with their row count unchanged and ours intact — the reply cannot carry this, since an unowned identifier and one already gone answer alike on purpose, so the row is the only observation. |
...
| 6.1 | unit | `test_architecture_and_changelog_record_the_removal` | `docs/architecture/fleet_bundles.md` contains `delete_workspace_library_entry` and a sentence superseding slot 460's position; the docs branch adds exactly one new `<Update>` block. The block lives in `~/Projects/docs/changelog.mdx`, never in this repository — `dispatch/write_changelog.md` names that file as the only place an `<Update>` is written, so "the diff" here is the docs branch's, not this one's. |
...
| R10 | The docs branch carries this workstream's revised pages (§6) | `git -C ~/Projects/docs diff --name-only main...chore/m204-workspace-library-removal-changelog` | the four §6 paths present: `fleets/library.mdx`, `cli/agentsfleet.mdx`, `api-reference/error-codes.mdx`, `changelog.mdx` | P1 | ✅ all four present, at `2a394e1`. The command answers SIX — `fleets/install.mdx` and `workspaces/managing.mdx` are `M204_002`'s pages, on this branch because one milestone means one docs branch. The row read "four, exactly" until the close re-ran it. |
...
| S2 | Unit tests pass | `make test-unit-all` | exit 0 | P0 | ✅ `exit=0` — 2662 Rust passed / 0 failed; TypeScript coverage floor 100% on all four metrics. Re-run at close on tonight's tree: `All unit lanes passed`, Rust 2662 passed / 0 failed / 591 ignored across 155 binaries, cli 1749, app 2916. |
| S3a | Lint green | `make lint-all` | exit 0 | P0 | ✅ `exit=0`. Re-run at close on tonight's tree: `exit=0`, after a rustfmt on the test added by the skill chain. |
| S3b | Integration lane green (live Postgres and Dragonfly) | `make test-integration-rustd` | exit 0 | P0 | ✅ `exit=0` — 8 new cases, the grant probe and both ledger-key cases all `ok`. Re-run at close on tonight's tree: 571 passed / 0 failed across 134 binaries. |
| S3c | Version sync | `make check-version` | exit 0 | P0 | ✅ `exit=0`. Re-run at close on tonight's tree: `all versions match 0.49.0`. |
