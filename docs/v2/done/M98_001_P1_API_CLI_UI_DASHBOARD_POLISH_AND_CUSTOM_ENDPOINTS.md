...
- **Review Finding A — custom-endpoint activation was broken (fixed Jun 24, 2026):** `/review` found that `PUT /tenants/me/provider` (`applySelfManaged`, `http/handlers/tenant_provider.zig`) gated *every* self-managed activation on `model_rate_cache.lookup_model_rate` — a pre-existing gate from the platform-key model work, never in this spec's Files-Changed. A custom `openai-compatible` model is absent from `core.model_caps` by design (provider-direct billing: self_managed posture charges run-fee only and never reads the rate cache — confirmed in `tenant_billing.resolveRenewSliceRates`), so the credential POSTed fine but never activated (400 `UZ-PROVIDER-004`). The §8 activation tests (8.2/8.4) are e2e-tier and skip without a live API twin, so the happy path was never asserted — that gap let it reach `done/`. **Fix:** `openai-compatible` bypasses the catalogue gate (`resolveSelfManagedCap`) and stores the unknown/auto cap sentinel (`context_cap_tokens = 0`, resolved downstream by `execution_policy.autoToolWindow` + per-fleet frontmatter); named providers still require a catalogued model. The diagnostic keystone integration test (`credentials_json_integration_test.zig`) was flipped from asserting the bug to a real regression — POST custom credential → PUT activate → **200** + resolved view, api_key never echoed — green via full `make test-integration`.
- **Review Finding B — DNS-rebinding TOCTOU on the non-streaming LLM dial (fixed Jun 24, 2026):** `/review` found that nullclaw's non-streaming POST helpers (`curlPostTimed`/`curlPostFormTimed`) built the Server-Side Request Forgery (SSRF)-safe resolve pin then discarded it to `std.http`, which re-resolves the host at connect time — a custom `base_url` makes the host attacker-chosen, so a hostile DNS answer could rebind to `169.254.169.254`/RFC1918 on the second lookup (a Time-Of-Check-To-Time-Of-Use, TOCTOU). The runner sandbox (no in-sandbox resolver; nft pins destination IPs) mitigates this in nft-enforced Linux deployments, but the dial was genuinely incorrect and not every mode has that backstop, so it's fixed at the source as defense-in-depth. **Fix (Indy-authorized):** released **agentsfleet/nullclaw v2026.5.29-zmb.3** routing both helpers through the curl `--resolve` path — pins the validated address while keeping credentials off the child argv (headers in a 0600 temp file, body on stdin, the posture the SSE path already uses) + a regression test; `build.zig.zon` re-pinned to zmb.3, agentsfleet builds + cross-compiles green.
