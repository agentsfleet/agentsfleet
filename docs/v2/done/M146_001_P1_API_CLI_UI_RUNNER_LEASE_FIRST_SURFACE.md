...
| `src/agentsfleetd/db/index_usage_integration_test.zig` | EDIT | Planner-fitness helper disables bitmap scans (deterministic among sibling fleet-prefixed indexes after the memory keyset index); gains slot 041's shape-and-plan proof with the row-accumulation derivation |
...
| `schema/040_memory_entries_keyset_index.sql` | CREATE | Index supporting `(fleet_id, created_at, key)` ordering |
| `schema/041_runner_leases_operator_read_indexes.sql` | CREATE | The §2 lease read's two access paths — `(runner_id, created_at, id)` for the page and `(fleet_id, event_id, fencing_token)` for the per-row reclaim probe (REVIEW, measured) |
...
- **Clean-state integration run (Tier 3) exposed two latent defects the state-riding runs masked.** (1) `runner_read_integration_test`'s shared seed token hash: `fleet.runners` carries `UNIQUE(token_hash)`, so the walk test's multi-runner seed collides on a clean database — every earlier green rode pre-existing rows through `ON CONFLICT (id) DO NOTHING`. Seed hashes are now per-runner unique (`prefix || id`), and Dimension 1.1's non-emission assertion checks the prefix, which is stronger. (2) The memory keyset index gave the planner a second fleet-prefixed index on `memory_entries`, making the pre-existing planner-fitness tests' bitmap-scan pick arbitrary — they sometimes chose the keyset composite for the `updated_at`-ordered probe. Their helper now also disables bitmap scans, so the one index supplying the ORDER BY wins deterministically (verified live via EXPLAIN). The `updated_at` index stays load-bearing: the runtime lease-time loader (`memory/sql.zig`) still orders by `updated_at DESC, id DESC`.
...
### REVIEW round (adversarial, five fresh-context reviewers)

Three reviewers reported (security, performance, testing); the API-surface and chaos passes died on a model-quota limit mid-sweep and are noted as partial coverage. Every finding below was re-verified against source before acting — two of the reported claims did not survive that check unchanged, and one of my own fixes was corrected by measurement.

- **Slot 041 lands: the §2 lease read shipped without index support.** `fleet.runner_leases` carried only `(runner_id, status)` (slot 018) and `(fleet_id, status, fencing_token)` (slot 033). Neither serves the page's `WHERE runner_id = $1 ORDER BY created_at DESC, id DESC`, and neither carries `event_id` for the per-row `is_reclaim` probe. **Indy asked for the data, not the structural argument**, so it was derived and measured rather than asserted. Accumulation: one row per claim plus one per reclaim, and *nothing prunes the table* — no retention sweep touches it, only tenant offboarding and the runner/fleet cascades — so a runner's count is the integral of its working life. Rate: each of `worker_count` workers (default 1, `MAX_WORKER_COUNT` 64) cycles lease→execute→report, and a lease outliving one `LEASE_TTL_MS` (30 s) renews rather than re-claims, so one worker turning a short event every ~30 s accrues ~2.9k rows/day — the spec's own motivating runner (4,021 leases) inside two days, ~184k rows/day at 64 workers. Measured at 5,000 leases across 5 fleets: page read **15.94 ms → 0.405 ms**, full-history Seq Scan plus top-N heapsort → 25-row Index Scan with no sort node, and the reclaim probe → Index Only Scan instead of ~997 index entries plus heap visits *per returned row*. The shape is the point: before, page cost grew with history; after, it is flat. Proof is committed as an EXPLAIN test in `db/index_usage_integration_test.zig` (catalog-read index shape + a plan assertion per access path), not a one-off measurement.
- **One of my own claims corrected by the measurement.** The first draft of 041's header said the composite also serves the list's `COUNT(*) WHERE runner_id`. It does not — that still plans as a Seq Scan (1.1 ms, unchanged), which is correct while the runner's history is most of the table. The header now says so.
- **The new slot moved the memory index's registration assertion.** `test_memory_keyset_index_migration_registered` asserted the memory keyset slot was the migration array's *last* entry — true only until the next slot. Rewritten to the durable invariant: the keyset slot is registered and is the slot creating the index (still fails on a renumber or a drop).
- **Client-triggerable 500s on three keyset parsers (security, confirmed).** `runners_list`, `runner_events` and `api_keys/list` bound the cursor's id half straight into a `::uuid` seek, so `starting_after=1744000000000:not-a-uuid` reached Postgres and surfaced as a cast error's 500 with an error-level log, while the sibling `runner_leases` pre-validates and answers 400. All three now validate with `id_format.isUuidV7` before touching the pool. Covered by four refusal cases in the api-keys suite (both cursor-form mismatches, the non-UUID id, and a non-cursor) plus a non-UUID arm in the runners-list and events envelope tests.
- **Unencoded path interpolation in the runner client (security, confirmed).** All five `runnerId` fetch builders in `lib/api/runners.ts` interpolated raw while the same file's `grafanaHrefFor` encoded; route params percent-decode, so `x%2Fy` became a path separator. All five now `encodeURIComponent`. Contained rather than an elevation (the request rides the caller's own token and the daemon enforces scope per resolved route), but the inconsistency was real.
- **The CLI's next-page hint was not runnable (testing, confirmed — a real user-facing defect).** `memory list` requires `--fleet <id>`, but the hint printed `agentsfleet memory list --starting-after <cursor>`; copy-pasting it hit `--fleet <id> is required`. The stem is now the verb, and the emit site appends the resolved `--fleet` and cursor. The sibling `fleet list` hint was correct because that verb has no required flag — which is exactly why copying its shape broke. Two tests added (hint printed with `--fleet`; search prints none).
- **An assert-free test of mine, deleted and replaced (testing, confirmed).** The RunnerTile late-answer test unmounted, resolved, and asserted nothing — React 19 silently no-ops a post-unmount state set, so removing the `cancelled` guard left it green. Replaced with an observable proof of the same guard: switch runners mid-flight, then let the stale answer land last and assert it does not overwrite the current work line.
- **Two comments that lied, corrected.** `api_keys/sql.zig` claimed each ordering "is served by an index from schema slot 033" — slot 033 explicitly names `core.api_keys` list sorts as *deliberately* unindexed at ~100-key scale; the comment now says that. `SELECT_RUNNER_DETAIL` claimed its read-time aggregate "mirrors how `core.fleet_activity_counters` counts for Fleets" — that table is maintained incrementally at write time and read in constant time, so the shapes differ; the comment now states the read grows with history and names the windowed follow-up that bounds it.
- **Memory panel walk: pages at the server ceiling now.** §10's Dimension 10.8 mandates walking to exhaustion, so the walk stays, but it asked for the server's default page (50) and therefore paid double the round trips; it now requests 100. The deeper shape question (an accumulate-forever collection walked on every fleet-detail render) is flagged below rather than redesigned here.
- **`parseEventTypeSet` duplicate tokens (testing, partially upheld).** The claim was a comment/behavior mismatch, and it was: the comment implied each tag appears at most once, while repeats within the cardinality bound are accepted. The behavior is right — the filter matches with `= ANY(...)`, where a duplicate selects the same rows — so the comment was corrected rather than the parser.

### Integrating `main` — a migration slot collision

M145 merged to `main` while this branch was open and claimed schema slot **39** for `039_vault_kek_default_retire.sql`; this branch had claimed the same number for the memory keyset index. Two migrations at one version is not a merge conflict git can see — both files exist, and only the array in `schema/embed.zig` overlaps — so the collision had to be caught by reading `main` before integrating rather than by a failing merge.

This branch's slots renumber to keep `main`'s landed number: the memory keyset index becomes **040** and the runner-lease operator-read indexes become **041**. Both are `CREATE INDEX IF NOT EXISTS`, so a database that already applied them under the old numbers re-applies as a no-op; the migration recorder simply sees two new versions. Every reference moved with them — the array, the memory registration test's pinned slot, the statement comments in `memory/sql.zig` and `fleet/sql.zig`, the liveness plan test's rationale, the architecture doc, and this spec. The renumber is also why the memory registration test's assertion had to stop pinning array position: a slot number that another branch can take is not a stable anchor, while "this version is registered and creates this index" survives both a renumber and a reorder.

**Surfaced to Indy, not decided here** (each is a wire-shape or interaction change beyond this spec's Files-Changed scope):
- 🎯 **Runner wall work-line fan-out** — `RunnerTile` fires one `listRunnerLeasesAction` per *busy* tile in a mount effect; Next.js serializes server actions per client, so B busy tiles are B sequential round trips before the last work line resolves (wall page size 50). Slot 041 makes each call cheap, which is why this is no longer urgent, but the shape stays sequential. 🔧 The real fix is to return live-lease count and fleet names on the list endpoint's page query (the page SQL already runs a per-row lease EXISTS) and drop the per-tile action — a response-shape change plus an OpenAPI and dimension update. 🏆 One read instead of B. ⚠️ Left as is, a 50-runner wall stays visibly progressive.
- 🎯 **API-keys list re-walks the whole collection on every sort click** — the component's own comment notes the list is always complete client-side, yet each header click re-runs the server-action walk. 🔧 Sort the in-memory array. 🏆 Zero requests per sort. ⚠️ Harmless at single-digit key counts, which is why §8 depaginated in the first place.
- 🎯 **Runner detail page fetches serially** — `getRunner` is awaited before the view read, though the view needs only `runnerId`, already in scope. 🔧 `Promise.all` the pair. 🏆 Halves detail-page time to first byte. ⚠️ Two sequential daemon reads per view switch.

- **Metrics review** — one event added, `runner_viewed`, with the properties the Metrics table allows and no host identifier; the four `agentsfleet_runner_*` Prometheus families are unchanged and both new routes label by route template only. No funnel changes, so no analytics playbook update — the explicit no-change reason.
- **Skill-chain outcomes** — `/write-unit-test`: resolved ledger (surface table above), 77/79 spec-named tests present with the two absent resolved by amendment rather than by writing them; app coverage floor closed with behaviour tests. `/write-integration-test`: the 44 new integration tests exercise real Postgres and Redis through the full router and middleware chain, with failure injection by pool drain for the 503 arm and per-test unique prefixes for parallel tolerance. gstack `/review`: five fresh-context reviewers dispatched, three reported (security, performance, testing) and two died on a model-quota limit mid-sweep — partial coverage, recorded rather than glossed; every reported finding re-verified against source before acting, two claims did not survive that check unchanged, and one of my own fixes was corrected by measurement. `kishore-babysit-prs`: runs after the push.
- **Runner UI completeness (asked directly at REVIEW)** — all 21 UI dimensions implemented and each of the 15 components verified to have a production importer, so nothing is built only for its own test; 38/38 component tests plus the app's 100% branch floor cover the failure arms. Never rendered in a browser against a live daemon: the acceptance walk targets the dev deployment, which cannot serve the two new endpoints until this merges, which is the S4 skip.
