...
**Solution summary:** These are gates and safety checks that fail to fire or fire too loosely; per `AGENTS.md` the sanctioned direction is to repair the check, not silence it. Parse the exact version token from `--version` and compare with `==`; wrap `deploy.sh main()` in `flock -n` on a fixed lock path and die when held. Source `common.sh` and call both vault gates at the top of the two `credential_rotation` scripts, mirroring `ip_allowlisting`. Drop the frozen milestone alternation for a validate-every-identifier scan and wire the script into a new `make check-architecture-doc` target inside `lint-all`. Widen both regex char classes to include underscore, move the `_?` inside the capture group, and cover the fix with a self-test the make target runs (the originally-planned doc citation is dropped — Discovery consult 6 shows it could not have detected the regression).
...
| `playbooks/operations/teardown/{database,redis}/{01_credential_check,02_teardown,03_verify}.sh` | EDIT | six more ungated vault readers, surfaced by `/review` when the parity detector was fixed to see helper-based reads. Same preamble (§3). Added at VERIFY per Indy's Jul 10 call — see Discovery consult 7. |
| `src/runner/cmd/version.zig` | EDIT (widened) | now also strengthens the test to pin the version to whitespace field 2, not merely "present somewhere" — `/review` showed the weak assertion would pass a reformat that breaks every deploy. Still no output-shape change (Out of Scope holds). |
...
| `scripts/check_route_registration_doc_test.py` | CREATE | asserts the widened regexes capture underscore targets and flag a phantom one (§5). The real-corpus case `skipTest`s when the REST guide (a dotfiles symlink) is unresolved, so the file runs everywhere. |
...
| `.github/workflows/lint.yml` | EDIT | new `safety-gates` CI job running `check-playbooks` (with the vault-parity gate) + `check-architecture-doc` + `check-deploy-safety`, wired into the `lint` aggregation's `needs:` so those gates block merge. Added at babysit per Indy's Jul 10 approval (greptile P1 on PR #505 — see Discovery): `make lint-all` was local-only, so none ran in CI. `check-route-registration-doc` is deliberately excluded — its subject doc is a dotfiles symlink that dangles in a CI checkout (see Discovery). |
...
- **Dimension 4.1** — DONE — a fixture architecture file citing an unshipped identifier (e.g. `M999`) makes the gate exit non-zero; the real corpus (which cites M52+ up to M121) resolves clean, proving M52+ is no longer skipped → Test `test_arch_doc_validates_all_m_ids`
- **Dimension 4.3** — DONE — a fixture `roadmap.md` citing a `pending/`-only identifier resolves clean, while the same citation in any other architecture file fails; `M999` fails in both → Test `test_arch_doc_roadmap_resolves_pending`
- **Dimension 4.2** — DONE — `check-architecture-doc` exists in `make/quality.mk` and is a prerequisite of `lint-all` → Test `test_arch_doc_wired_into_lint_all` (grep of the make file)
...
**The planned doc citation is dropped** (Discovery consult 6). It could not have worked: the pre-fix `DOC_MAKE_TARGET_RE` does not capture `` `make _fmt_check` `` at all, so narrowing the char class again would leave the gate green with the citation sitting right there. Only a self-test detects that revert, and `check_route_registration_doc_test.py` does — verified by mutating each half of the fix and watching tests fail.

- **Dimension 5.1** — DONE — the widened `DOC_MAKE_TARGET_RE` captures `` `make _fmt_check` `` and `MAKE_TARGET_DEF_RE` matches a `_lint_zig_test_depth:` definition line → Test `test_underscore_targets_captured`
- **Dimension 5.2** — DONE — a doc citing `` `make _no_such_target` `` (a phantom underscore target) is reported as a `PHANTOM TARGET`, proving underscore citations are now checked → Test `test_phantom_underscore_target_flagged`
...
3. Every `playbooks/operations/**` script that reads the vault passes both gates — enforced by the `check-playbooks` parity grep (Dimension 3.3), not review. The grep detects a reader by the `op://` reference scheme, so helper-based reads (`playbooks_read_ref_or_empty`) and non-`op read` spellings are all covered; all 13 readers across `credential_rotation`, `observability`, `ip_allowlisting`, and `teardown` resolve.
4. `check_architecture_doc.sh` carries no frozen milestone range and runs in CI, not merely `lint-all` — the `safety-gates` job in `.github/workflows/lint.yml` is a merge-blocking prerequisite of the `lint` aggregation, so a broken reference fails the PR rather than only a developer's local `make lint-all` (Dimensions 4.1/4.2). `pending/` resolves in `roadmap.md` and nowhere else (Dimension 4.3). The same CI job runs `check-deploy-safety` and `check-playbooks` (with the vault-parity gate), closing the "wired into lint-all but never run" gap for the gates whose inputs are real repo files. `check-route-registration-doc` stays local-only because its subject doc is a dotfiles symlink absent in CI (see Discovery).
...
| `test_arch_section_14_present` — asserted `ship_reflection.md`, deleted in `6aacaedd0` (Discovery consult 5) | `grep -rn 'ship_reflection\|section_14' scripts/` | 0 matches |
| milestone identifiers in shell source (MSID) | `grep -rnE 'M[0-9]+_[0-9]+' scripts/check_architecture_doc.sh playbooks/operations/credential_rotation/*.sh playbooks/operations/observability/01_credentials.sh` | 0 matches |
...
- **Consults** — three judgment calls surfaced at PLAN, before any edit; Indy decided all three on Jul 10, 2026.
  ...
  5. **§4 — the gate also guarded a deliberately deleted file.** With the milestone scan repaired, the real corpus still failed: `test_arch_section_14_present` asserts `docs/architecture/ship_reflection.md`, which commit `6aacaedd0` removed — *"docs(m72_002): drop ship_reflection.md + `_unshipped/` outright per Captain"*. A third reason this script never ran, and dead code by RULE NDC. RULE NLR (touch-it-fix-it) directs removal in the same diff rather than a consult, since the cleanup is a feasible ~15-line deletion; the stale `PENDING SHIP` comment in the orphan-marker check goes with it. Not a weakened guarantee: the check asserted a file that must not exist.

  6. **§5 — the planned doc fixture is both cross-repo and ineffective.** `docs/REST_API_DESIGN_GUIDELINES.md` is a symlink to `~/Projects/dotfiles/docs/`, so editing it is a dotfiles commit+push to `master`, shared by every repo. More decisively, the fixture cannot work: the pre-fix `DOC_MAKE_TARGET_RE` (`` `make ([a-z][a-z0-9-]*)` ``) does not capture `` `make _fmt_check` ``, so narrowing the char class again would leave the citation uncaptured, unchecked, and the gate green. The citation would only have caught deletion of the `_fmt_check` target. The doc edit is dropped and the coverage moves to `check_route_registration_doc_test.py`, whose assertions were verified against three separate mutations of the fix.
...
- **Architecture consult** — `docs/architecture/direction.md` (platform determinism + gate discipline) read before §4. The direction doc's stance — a gate that cannot fail is not a gate — is what forces both halves of §4 (unfreeze *and* wire into `lint-all`) to land together, and is why option (b) was chosen in consult 2 over a permanently-skipped local test.
  ...
  7. **`/review` P0 — the vault parity gate was blind to helper-based reads.** The adversarial diff review (three fresh-context reviewers) found the `check-vault-gate-parity` detector matched only a literal `op read`, so it missed every script that reads through `common.sh`'s `playbooks_read_ref_or_empty` helper — including `ip_allowlisting`, the reference implementation the whole §3 pattern is copied from. Six teardown scripts (`teardown/{database,redis}/{01,02,03}.sh`) read production PlanetScale/Upstash credentials via that helper with no vault gate, and the parity check reported green. **Indy's call: gate all six** (rather than narrow the gate's scope or defer), matching the observability decision. The detector was redesigned to match the `op://` reference scheme every reader shares — immune to helper indirection, `op --account X read`, and whitespace tricks in one move — and verified by mutating a gate out of both a teardown script and `ip_allowlisting`. Teardown now requires `ALLOW_VAULT_READS=1` on top of its existing `ALLOW_*_TEARDOWN` approval, and gains the `op`-auth pre-check it lacked.

- **Skill-chain outcomes** — `/write-unit-test`: red-green proof run against the pre-fix code exposed two vacuous passes (a lock test staged a non-existent binary; a route-reg test asserted "no phantom" over an empty citation set) — both fixed, both now red pre-fix and green on HEAD. `/review`: one P0 (above, fixed) plus five verified findings fixed — roadmap carve-out keyed on basename not path (a nested `scenarios/roadmap.md` could launder pending ids); `check_architecture_doc.sh` passed vacuously on a missing `ARCH_DIR`; the deploy happy-skip path reported "ok" to Discord even when `systemctl start` failed; `vault_gate_test.sh` had no positive-path test (an always-blocking gate passed) and its reachability check could be evaded by `op --account X read`; `version.zig`'s test pinned the version too loosely. Pre-existing findings left out of scope and surfaced here, not fixed: secrets passed to `curl -H`/`-d` land in process argv (`02_service_health.sh`, `02_prometheus.sh`, `03_dashboard.sh` — unchanged by this diff), and `01_credentials.sh` prints a secret's character length. `/review` recommendation after fixes: two reviewers approve, the P0 reviewer's blocker resolved.
- **Post-open review (greptile, PR #505)** — three findings. **Fixed:** (P2) `check_architecture_doc.sh` line 90 expanded `$ROADMAP_BASENAME`, a dangling reference left by the `ROADMAP_REL_PATH` rename (RULE ORP miss) — under `set -u` it would crash the unresolved-ref path with an unbound-variable error instead of naming the milestone; fixed both the code and the comment twin, added a regression test that forces the failure path (previously untested — the real corpus always resolves). (P1) `deploy.sh is_already_installed` — even after the earlier `systemctl start` fix, a service that starts then immediately dies would take the version-skip path and notify "ok"; it now runs `verify_healthy` on the had-to-start path and reports not-installed on failure, with a fast injectable-timing regression test. Both fixes mutation-verified. **Surfaced to Indy, not fixed here (needs a CI decision):** (P1) the new gates are wired into `make lint-all`, but CI's `lint.yml` runs individual `make lint-*` jobs and never `lint-all`, so `check-architecture-doc`, `check-deploy-safety`, and `check-vault-gate-parity` do not run on merge — the same "gate that never fires" class this spec targets. Two affected gates (`check-playbooks`, `check-route-registration-doc`) were already `lint-all`-only before this PR. **Indy approved fixing it in this PR** (Jul 10): a new `safety-gates` job in `lint.yml`, wired into the `lint` aggregation's `needs:`, so the gates block the merge — the milestone's own theme, applied to itself. **A CI run then exposed a further wrinkle:** `check-route-registration-doc` fails in CI because its subject, `docs/REST_API_DESIGN_GUIDELINES.md`, is committed as an absolute-path symlink into a developer's dotfiles and dangles in a fresh checkout (that gate had never run in CI, so the dependency was never exercised). Nine top-level `docs/*.md` are such symlinks. Resolution: the CI job runs the three gates whose inputs are real repo files — `check-playbooks` (carrying vault-parity), `check-architecture-doc`, `check-deploy-safety` — and `check-route-registration-doc` stays in `make lint-all` + the pre-commit hook, which run where the dotfiles resolve. Making it a CI gate would need the doc de-symlinked first, a dotfiles-owned change out of this scope. The route-reg self-test now `skipTest`s when the guide is unresolved, so it stays runnable everywhere. Verified in a dotfiles-less `ubuntu:24.04` checkout: the three CI gates pass, the route-reg test skips cleanly. `check-deploy-safety`'s flock lock tests run (not skip) because GitHub sets `CI=true`.
