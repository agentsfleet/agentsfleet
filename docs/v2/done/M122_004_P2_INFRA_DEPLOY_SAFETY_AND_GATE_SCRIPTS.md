...
| `.github/workflows/lint.yml` | EDIT | new `safety-gates` CI job running `check-playbooks` + `check-route-registration-doc` + `check-architecture-doc` + `check-deploy-safety`, wired into the `lint` aggregation's `needs:` so the gates block merge. Added at babysit per Indy's Jul 10 approval (greptile P1 on PR #505 — see Discovery): `make lint-all` was local-only, so none of these gates ran in CI. |
