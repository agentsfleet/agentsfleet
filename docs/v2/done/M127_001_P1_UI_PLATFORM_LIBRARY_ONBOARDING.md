...
- **Skill-chain outcomes** —
  ...
  - **Two pre-existing CI flakes seen while babysitting, neither from this diff.** (1) `metrics_runner_no_duplicate_slot_under_contention` (Zig) failed once, passed on rerun; this diff touches no metrics code. (2) `test-unit-app` failed once with `ReferenceError: window is not defined` **after all 143 files reported passing** — the stack is `design-system/src/design-system/Toast.tsx:102`, the toast fade `setTimeout` firing after the test environment is torn down. It landed on a **docs-only** commit whose test code was byte-identical to the run that had just passed, and the only `Toast` consumer is `ApprovalsList`, which this spec never touches. Passed on rerun. The underlying race — a timer outliving its environment — is a real latent flake in a shared component and will keep surfacing; it wants a fix of its own, not a retry.
- **Deferrals** —
