...
| `schema/028_fleet_library_catalog_reconcile.sql` | DELETE | Its only job was reconciling the seed; the seed is gone, and nothing in `schema/` may insert a catalog row (Invariant 5) |
| `schema/embed.zig` | EDIT | Drop the 28 registration. Versions return to a contiguous 1..27 — no gap, no renumbering, and the contiguity tests pass untouched |
...
- **Dimension 1.2** — DONE — a row that is `public` but holds NO bundle (the shape a deployed database still carries) is invisible to every tenant-facing read, and the operator can clean it up from the dashboard — unpublish, then delete. Enforced in the QUERIES, not by migrating rows → Test `test_public_row_with_no_bundle_is_hidden`
...
| 1.2 | integration | `test_public_row_with_no_bundle_is_hidden` | a `public` row with a null `content_hash` → absent from the workspace gallery AND from `GET /v1/fleets/bundles`; present in the admin catalog; unpublish + delete removes it |
...
|---|--------------------------------|---------------------|----------|----------|-----------------|
| R1 | No fleet reaches a tenant unpublished; the catalog is runtime data with a working lifecycle (§1, §2, §3) | `make test-integration` | exit 0 | P0 | ✅ **green in CI** (`test-integration` + `test-integration-kernel`) — unrunnable locally for want of Postgres, so CI is the grading run. The 13 new integration tests executed against a real database |
...
| S5 | No leaks on the new query paths | `make memleak` | exit 0 | P0 | ✅ **green in CI** (`memleak`). `_lint_zig_pg_drain` ✅ (660 files) covers the drain discipline locally |
...
- **Consults** —
  ...
  - *Both migrations deleted (Indy, Jul 13, 2026).* First pass kept `028` (seed stripped) and added `029` to normalize bundle-less rows on a deployed database to `draft`. Indy: "this is not needed delete the file schema/028… why do we need schema/029". Following that through showed **neither is needed, and keeping them was papering over a code hole.** `029` only existed because a row that is `public` with no bundle gets *advertised* by `GET /v1/fleets/bundles` and then dead-ends at install — and that is because `SELECT_BUNDLES_LIST` filtered on `visibility` alone, never on `content_hash`. A pre-existing bug, not a data problem. Fixing the query makes a bundle-less row invisible to **every** tenant-facing read regardless of its state, so there is nothing left to migrate: both files deleted, versions back to a contiguous **1..27** (no gap, so the contiguity tests pass untouched and nothing had to be renumbered into the version-already-applied trap), and the four legacy rows in production are now harmless *and* removable from the new dashboard itself — unpublish, then delete. Correct-by-construction beat migrating state, and it is what "less dependency on the static stuff in sqls" actually asks for.
...
- **Skill-chain outcomes** —
  ...
  - `kishore-babysit-prs` (PR #517, 3 polls): **CI green — 36 pass, 2 skipped, 0 fail.** Greptile quiet after two consecutive empty polls; its own review check passes. **Two findings, both actioned.**
    - **Greptile P1 (`onboard.zig`) — VALID, and the sharpest finding of the milestone.** The id-collision guard was a `SELECT` before the write. Two operators adding two DIFFERENT repositories whose bundles declare the same UNUSED name would both see "no such row", both proceed, and the second would silently swap out the first — every tenant installing that fleet would get the wrong bundle, with no `replace` ever given. This is the **same check-then-act class already fixed twice on this branch** by the local `/review` (the delete and publish paths); it was missed on the add path. The guard now lives in the statement: `ON CONFLICT ... WHERE $15::boolean OR core.fleet_library.source_repo = EXCLUDED.source_repo`. Zero rows means the id belongs elsewhere → `UZ-CATALOG-004`. Nothing to race. Fixed + replied in `f440151eb`.
    - **Greptile P2 (`PlatformCatalogTable.tsx`) — real, but the stated mechanism was wrong.** `ConfirmDialog` owns its own pending state and disables both buttons while `onConfirm` resolves, so the described double-DELETE cannot occur (replied with the code, and asked for a re-rank). The observation underneath it was right: `confirmDelete` ran outside the table's pending gate, so an operator could publish or refetch a row whose DELETE was still in flight. Every row action now gates on one `busy` signal. Fixed in `f440151eb`.
    - **The suites that could not be graded locally all ran and passed in CI:** `test-integration` ✅ (the 13 new integration tests against a real Postgres — the publish gate, delete guard, collision race, and draft-not-installable all hold), `test-integration-kernel` ✅, `memleak` ✅, `test-unit-app` ✅ (the 100% coverage gate).
- **Deferrals** — none. Every `/review` finding was fixed in-diff.

**Graded in CI, not locally — and CI ran them.** The integration suite (R1) and the leak run (S5) need Postgres, which this worktree has not got; both are **green in CI**, so they are graded from CI's output, not this agent's word.

**R5 remains the one genuine gap.** `acceptance-e2e-prod` is SKIPPED in CI (same as it was for M127), and the suite cannot run locally either: no Clerk keys, no `agentsfleetd`. The five end-to-end scenarios are written, compiled, and typechecked, and **nothing has executed them.** They must be run against a real environment before this is trusted end to end. It also carries an environment dependency worth restating: the operator fixture works only if the Clerk session-token template projects `public_metadata.scopes` to a top-level `scopes` claim (`docs/AUTH.md` §Session token) — the same requirement that already governs `/admin/runners` and `/admin/models`.
