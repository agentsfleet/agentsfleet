...
- **Consults** — Architecture / Legacy-Design / gate-flag triage: the question asked + Indy's decision.
  ...
  - **RESOLVED — §2, an assignment that declares no registry gets one real check, and that is accepted.** Raised at REVIEW: `PolicyFields.tsx` defaults `registry_allowlist` to `""`, so the DEFAULT policy declares no registry, and `targetsFor` then supplies no resolve or dial target. `grade` marks both the DNS and egress checks `ok` with "nothing assigned to test", so a runner whose `/etc/resolv.conf` opens but whose DNS is dead reports `all_ok`. The M167 incident itself is still caught — a dangling resolver fails the resolver check under every posture — but the adjacent failure is not. Options put: **A** — resolve the `control_plane_url` host when the posture permits network (resolve, never dial; operator-configured rather than the `DEFAULT_REGISTRY` guess) · **B** — make `all_ok` require that something was actually tested · **C** — leave it.
    > Indy (2026-08-17): chose "Leave it as-is" — context: §2 coverage on an undeclared registry, decided **C**.

    Read as: the self-test proves what the assignment declared and nothing more. The probe never reports a fault it did not observe, and each untested check states plainly why it was skipped, so an operator reading the panel sees the difference between "passed" and "not tested". The aggregate `all_ok` overstates on a default policy; that is a known and accepted limitation, recorded here rather than discovered later. An operator who wants the DNS and egress checks exercised declares a registry.
