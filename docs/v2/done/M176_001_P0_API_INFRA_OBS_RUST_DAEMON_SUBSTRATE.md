...
- **Handshake** (filled at PLAN, Aug 23, 2026):
  ...
  - `ASSUMPTIONS I'M MAKING:`
    1. **The Zig daemon stays the only production binary.** Nothing here is deployed; `deploy-dev.yml` keeps its manual-dispatch-only trigger and `test_daemon_deploy_retired` keeps passing. Parity is measured against Zig, never enforced onto it.
       *(Superseded by M181_001, 2026-08-31: the Rust daemon is what `deploy-dev.yml` now ships, so its merge trigger is restored and `test_daemon_deploy_retired` — later `test_daemon_deploy_ships_the_rust_daemon` — was deleted with `retirement.rs`. The assumption was correct for M176 and is recorded, not rewritten.)*
