...
| `scripts/ensure_fly_app.sh` · `scripts/ensure_fly_app_test.py` · `make/check-safety-gates.mk` | EDIT | §0 — create-if-absent, so the script's contract covers a first deploy instead of assuming someone ran `fly apps create` by hand; the self-test covers the new branch |
...
- **Dimension 0.3** — **DONE** — the collector stand-up survives a first run: no step addresses the app before it can exist. Both deploy workflows call the `--create-only` path 0.2 shipped ahead of `flyctl secrets set`, and each names the organisation its environment creates into, since the script refuses rather than guessing. Applied on Indy's in-session approval of the CI/CD edit → Test `test_collector_standup_is_order_safe`
- **Dimension 0.4** — **IN_PROGRESS — parked, needs the development deploy, which Indy grades on merge** — the development deploy reaches green, and M181_005's R1 — closed ungraded — is graded from that run → Test `test_collector_carries_every_signal`
...
- **Dimension 1.1** — **IN_PROGRESS — parked, needs live staging** — the black-box parity suite, the runner parity lane and the dry lane are green against the Rust daemon on staging → Test `test_soak_suites_green`
- **Dimension 1.2** — **IN_PROGRESS — parked, needs live staging** — per-route-class latency is within the budget the lane embeds → Test `test_latency_budget`
- **Dimension 1.3** — **IN_PROGRESS — parked, needs live staging** — resident memory stays within the named ceiling across the soak window under sustained load, with a flat growth trend → Test `test_memory_ceiling_soak`
- **Dimension 1.4** — **IN_PROGRESS — parked, needs live staging** — chaos probes hold mid-soak: replay suppressed, leases fenced, streams reconnect → Test `test_soak_chaos_invariants`
- **Dimension 1.5** — **IN_PROGRESS — parked, needs live staging** — cross-DIGEST state handoff in both directions: the current image serves and writes production-shaped state, the previous digest then boots on the same stores and resumes serving correctly, and the reverse. Rollback safety is demonstrated, not inferred from "same schema". Graded by the parity lane in COMPARE mode across each direction — `make test-parity BASE_URL=<previous-digest> COMPARE_URL=<current>` — rather than by a bespoke lane whose only caller would be one rubric row. The cross-IMPLEMENTATION version of this dimension is deleted, not deferred: it named a Zig daemon that is not built → Test `state_handoff_is_bidirectional`
...
- **Dimension 3.3** — **IN_PROGRESS — parked, needs real runs to fill the runbook's evidence tables** — the runbook's evidence tables are filled from real runs, not left as empty rows → Test `test_runbook_probes`
...
- **Dimension 4.2** — **DONE** — `src/agentsfleetd/**` is removed and no reference survives outside the written record (`docs/v2/`, `docs/v1/`) → Test `no path under src/agentsfleetd is referenced after the deletion`
...
- **Dimension 4.4** — **DONE** — every gate, make target and playbook whose scope was the daemon either narrows or is removed, and none is left scanning nothing and reporting green. A gate covering daemon AND runner narrows; only an empty scope is deleted → Test `no gate reports a vacuous pass over a deleted tree`
...
|---|--------------------------------|---------------------|----------|----------|-----------------|
| R0a | The collector apps exist and the dev deploy is green (§0) | `flyctl apps list \| grep otelcol` then the `deploy-dev` run | both apps resolve; the deploy's collector step passes | P0 | graded on merge — §0.4 |
...
| R5 | The daemon tree is gone (§4.2) | `test ! -d src/agentsfleetd` | exit 0 | P0 | exit 0, Sep 04 2026 |
| R6 | Nothing references it (§4.2) | `git grep -l "src/agentsfleetd" -- ':!docs/v2' ':!docs/v1'` | no output | P0 | no output, Sep 04 2026 |
...
- **Deleting the tree was the easy half; what it was holding up was the finding (Sep 04, 2026).** 857 files under `src/agentsfleetd/` went with `git rm`, and `build.zig` fell from 325 lines to a graph that builds only what the runner links by source — `test-lib`, `bench-incident`. Everything else in §4 is what that deletion exposed, and the rule Indy set — *reason from what the runner depends on* — decided each one. `src/lib/s3/r2.zig` (R2 snapshots), `src/lib/tripwire`, and `common.CacheTable` had no runner consumer: each was reachable only through the daemon's graph or its own self-test, and each went. `src/build/{auth_tests,daemon_tests,pg}.zig` lost their only callers; `fixtures.zig` lost its daemon half and kept `addRunner`, which `build_runner.zig:151` still calls — the orphan check that missed that hit was truncated by `head -3`, and §4.5's runner build is what caught it.
  ...
  **`orly update` to 0.9.3 retired one of the two managed files, and revealed two more (Sep 04, 2026).** The parity oracle this branch carried as a local patch on `audits/ufs.sh` is upstream now (orly#30, released 0.9.3), so it is the pack's own text rather than drift, and `dispatch/write_any.sh` stopped citing the deleted registry. Nine pack files in total now name the Rust file that took each job. `audits/cross-tier-rates.sh` arrived for the first time and passes, pinning `RUN_NANOS_PER_SEC = 100000` across four surfaces — which is what proves the `afd_billing` path it now names is the real definition site. R6 falls from 49 files to 41. **What the update did NOT retire, and what it broke.** `audits/error-codes.sh` still hard-codes the Zig registry as the file it greps: re-aiming it at `afd_core` changes what the gate DOES, not what it cites, so it was reported upstream rather than decided. `docs/REST_API_DESIGN_GUIDELINES.md` went backwards — the pack still carries pre-Rust text, and accepting it would have undone `573cd2e49` and added two citations, so the file is restored to what `main` ships and its §7 rewrite is the upstream item. `audits/ufs_strings.sh` is deleted: this repository split the gate at its length cap in `01ac4f154`, 0.9.3 ships the unsplit 439-line file, and an unsourced half is an orphan. That split belongs upstream too. Between them, `error-codes.sh` (9) and the REST guide (25) hold 34 of the 41 remaining citations, so R6 and Dimension 4.2 turn on a second orly release, not on work in this tree.

  **The sweep, and the two things it turned up that were not path swaps (Sep 04, 2026).** Every repo-owned citation outside the CLI and dashboard now names the file that took the job: the runner handlers in `data_flow.md`, the metric declarations the census is graded against, the frontmatter parser and reader, and two playbook rows that describe what the retired daemon DID and now say so in the past tense without naming a file. `audits/zig-discipline-roster.txt` loses four prefixes whose directories are gone; `lint-zig.py --discipline` still passes, over 3 roster prefixes rather than 7. **`scripts/mint-scope-personas.mjs` is deleted on Indy's call** — it rewrote scope-token constants inside 21 Zig test files, reported 21 missing files on its last dry run, and had no caller in any make target, workflow or package script. **Its allowlist entries are NOT deleted, and that is a correction rather than a caveat.** Removing the two `.gitleaks.toml` entries for `test_fixtures.zig` and `test_scope_tokens.zig` looked like dead configuration and was not: a full-history scan reads deleted blobs, and dropping them turned `gitleaks detect` from `no leaks found` into 21 findings. They are restored with a comment saying why they outlive their files. **R6 is amended, on Indy's call, from excluding `docs/v2/done` to excluding `docs/v2`.** The row could not pass as written: the spec that deletes the tree names it twelve times to describe the deletion, the pending successor names it three, and two archived reviews name it ten between them. Specs and archived reviews are the record of the deletion, and rewriting a review would falsify what was found at the time. The row now grades source, configuration and live documentation, which is what it was for.

  **0.9.4 closed 4.2 and 4.4, and the gate it rebuilt found work of its own (Sep 04, 2026).** orly#31 released 0.9.4 with the two files 0.9.3 left. `docs/REST_API_DESIGN_GUIDELINES.md` §7 now describes the router that exists — built FROM the route table, so a path cannot be served under a spelling the table does not carry — and §8 an axum handler returning `Result<Response, Refusal>`; the pack's base is this repository's own copy, which was already correct and would have been overwritten again. `audits/error-codes.sh` stopped hard-coding one repository's registry: it autodetects Rust or Zig, takes four `ORLY_ERROR_*` overrides, and SKIPS with a reason where a repository has neither, rather than hard-exiting or passing vacuously. Two Rust facts it had to learn: `#[cfg(test)]` blocks live inside the file they cover, and a doc comment naming a retired code is prose rather than a call site. Forty-two client comments were rewritten in the same sweep, four of them in the past tense because the thing they mirrored has no successor — the `agt_a` key class, the cosmetic `install:*` frames, the `event_received`/`event_complete` kinds, and a machine-name grammar retired for refusing names like `Kishore's MacBook Pro`. **R6 returns no output and R5 exits 0**, so 4.2 is DONE; no gate is left scanning a deleted tree and reporting green, so 4.4 is DONE. **Two findings this leaves, neither blocking.** Re-aimed at Rust the registry gate reports 132 declared and flags the roughly forty Zig-era codes `afd_core/tests/error_code.rs` lists as expectation data — under the gate's own rule those lines take its intentional-fake annotation, and the gate is unwired from `make/harness.mk`, so nothing is red today; re-wiring it is a decision, not a repair. And the same rot survives one pattern over: `cli/` and `ui/` carry 89 bare `.zig` references across 55 files, of which 52 filenames no longer resolve. They fall outside the `src/agentsfleetd` string R6 greps, so they are a follow-up rather than a gap in this row.

  **0.3 landed, and the ordering test it named did not exist for 0.1 or 0.2 either (Sep 04, 2026).** Indy approved the CI/CD edit in session and it is applied: both deploy workflows create the collector app before addressing it, each naming its own organisation. Before this, nothing set those secrets on a fresh app — `flyctl secrets set --app` was the first command to name it, and it fails on an app that does not exist. `otelcol-dev` exists today only because it was created by hand: one release, made about an hour before the daemon deploy that followed it. `otelcol-prod` has no releases and no secrets at all, so production is the first real exercise of this path. **Why the test is not ceremony.** `ensure_fly_app.sh`'s own header records that this block was inline in both workflows, that the copies drifted within a day, and that one of them ran after the deploy it was supposed to precede — passing actionlint throughout. Ordering is the failure linting cannot see, so `scripts/ensure_fly_app_test.py` asserts the sequence and the organisation in both files, and it fails when either is removed. It rides `check-deploy-safety`, already inside `lint-all`, so CI grades it rather than a human remembering to. **A gap this turned up:** 0.1 and 0.2 name four tests — `test_every_fly_app_is_created_by_the_priming_playbook`, `test_an_absent_app_is_created_before_the_deploy`, `test_a_failed_creation_refuses_rather_than_falling_through`, `test_create_only_creates_without_deploying` — and none of them exists anywhere in the repository. Both dimensions are marked DONE. `ensure_fly_app.sh` is the script both deploys now depend on and has no behavioural test at all, though it was written to be testable (`$FLYCTL` and the poll bounds are injectable for exactly that). Reported rather than repaired: writing them is scope, not a fix. **Production secrets are deliberately not staged, on Indy's call ("skip prod secrets in fly").** `otelcol-prod` has no secrets and no releases, and nothing is set by hand ahead of the window: the release workflow now creates the app and sets the five values on the first production run, which is the path this dimension exists to make safe. Staging them early would prove nothing that run does not, and it would put the upstream credential pair in an app that is not yet serving. §0.4 and R0a grade the development half on merge; the production half is graded at the change window with §2.

  **CHORE(close): what is parked, and the words that parked it (Sep 04, 2026).** The milestone closes with §0.4, all of §1 and §3.3 outstanding, each needing a live environment this branch cannot reach, plus two pieces of scope Indy declined. Verbatim, so none of it reads as an agent's own deferral:

  > Indy (2026-09-04): "0.4 dev deploy we do it on merge" — context: §0.4 and rubric R0a, graded from the deploy the merge triggers rather than from this branch.

  > Indy (2026-09-04): "i dont need 3, 4, 5 and the tests. park them and chore close" — context: no further work on §3, §4 or §5, and the missing `ensure_fly_app.sh` behavioural tests that 0.1 and 0.2 name are not written. §4 and §5 are already fully DONE; §3.3 is parked, since its evidence tables need runs nobody has made.

  > Indy (2026-09-04): "skip changelog" — context: no changelog entry ships with this milestone. The user-visible half is a `UZ-PROVIDER-011` refusal on `PUT /v1/admin/platform-keys`, its newly documented `400`, and a message byte bound stated against decoded bytes; the error-code reference row for it merged as docs#184. An entry was drafted and is held unpushed on `chore/m181-006-platform-key-refusals` in the docs repository if it is wanted later.

  > Indy (2026-09-04): "skip prod secrets in fly" — context: `otelcol-prod` is left with no secrets and no releases; the release workflow sets them on its first production run.

  The spec therefore stays in `docs/v2/active/` at `Status: IN_PROGRESS`. Everything gated on a live environment is marked parked in place rather than closed, so the next session reads what is outstanding from the dimension list rather than from a handoff.
