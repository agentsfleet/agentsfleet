...
| `docs/AUTH.md` · `docs/architecture/{README,capabilities,connectors,data_flow,high_level,roadmap,runner_fleet,user_flow}.md` · `docs/architecture/scenarios/github-pr-reviewer.md` | EDIT | full admin-key → workspace install → repository subscription → fleet wake → token-mint walkthrough; preserve manual and Slack paths |
...
| 4.2 | deterministic documentation check | flow-term grep across `docs/architecture/connectors.md`, the playbook, and `github-pr-reviewer.md` | internal docs + playbook cover the complete flow and reviewer test state |
