...
| D49 | **DONE** | `playbooks/003_priming_infra/001_playbook.md` §3.3 + `docs/AUTH.md` Roadmap | NEW §3.3 "Clerk — Session Token Customization" with DEV+PROD UI walkthrough, per-env audience guidance, V9.1–V9.5 verification table, rollback procedure (I9.5). AUTH.md Roadmap header flipped to "Stage 1 SHIPPED in M74_002 §9"; Stage 1 subsection retitled "shipped, M74_002 §9" with the *Delta vs today* table converted to *What shipped* (D40–D49 outcomes); stale "Verification owed before Stage 1 spec opens" section folded out (rolled into playbook V9.x table). |
