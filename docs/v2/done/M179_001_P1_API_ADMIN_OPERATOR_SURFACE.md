...
**Goal (testable):** the admin plane (`/v1/admin/fleet-libraries*`, `/v1/admin/platform-keys*`, `/v1/admin/models*`) and the operator plane (`/v1/fleets/bundles`, `/v1/fleets/runners*`; `/v1/fleets/streams` not ported — Dimension 4.4) serve from `agentsfleetd-rs` with scope-gate, response-shape, and bundle-import behaviour equal to the Zig daemon, graded by the route-inventory test and the integration subset (full-route OpenAPI coverage is M181's oracle).
...
`/v1/fleets/runners[/{runner_id}[/events|/leases]]` (list, detail, `PATCH` admin-state transitions). The `/v1/fleets/streams` overview is **not ported** — see Dimension 4.4. Cordon/drain/revoke/rotate write `fleet.runners` + append `fleet.runner_events`; delivery to the runner is the M177 auth read — this milestone only writes state. Three-category status (admin_state + derived liveness + events) rendered as the Zig daemon does.
...
- **Dimension 4.4** — streams overview shape parity on seeded fleets → Test `test_streams_overview_parity` — **NOT PORTED (declared divergence)**

**Declared divergence — `/v1/fleets/streams` is not served by the Rust daemon.**
The Zig retains it (`src/agentsfleetd/http/routes.zig`'s `fleet_streams_list`,
a per-instance operator diagnostic over its SSE registry). Indy's call while
merging this milestone into M178: nothing consumes the route — no UI, no CLI,
and it is absent from the public OpenAPI document by its own carve-out — so
porting it would mean carrying a live-stream census whose only reader is the
endpoint itself. The route variant, its handler and
`afd_fleet::streams::LiveStreams` are all absent from the Rust tree rather than
present-and-unmounted, and `afd_api`'s route-inventory test omits the path so
the absence is graded rather than exempted. `test_streams_overview_parity` does
not exist on the Rust side; it graded a registry that no production path ever
wrote to, which is why the deletion costs no coverage.
...
Routes (per src/agentsfleetd/http/route_template.zig):
  ...
  /v1/fleets/streams  — Zig only; NOT ported (see Dimension 4.4)
