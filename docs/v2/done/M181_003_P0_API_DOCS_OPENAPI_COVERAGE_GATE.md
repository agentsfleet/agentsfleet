...
| `rustd/crates/afd_approval/src/gate_status.rs` · `afd_approval/src/{lib,inbox,inbox/row}.rs` | ADD/EDIT | §4.7 — `GateStatus`, the reader's five-arm status vocabulary the inbox filter now reads, so `auto_killed` is reachable while `Decision` still cannot write it |
| `rustd/crates/afd_fleet_lifecycle/src/{edit,edit/rewrite}.rs` | EDIT | §4.7 — the rewrite carries what the replacement `TRIGGER.md` declares, and `patch` runs the same credential pre-flight the install does |
| `rustd/crates/afd_api/tests/integration_fleet_install_credentials.rs` | ADD | §4.6 — the 424 proved at the wire: the status, the registry code, and the `missing_secrets` key the lifecycle-crate test cannot reach |
...
- **Dimension 4.3** — DONE — the approvals list reads the five parameters the document already declares and the dashboard already sends (`ui/packages/app/lib/api/approvals.ts:69-73`: `status`, `fleet_id`, `gate_kind`, `cursor`, `limit`) and returns a real `next_cursor`. The parser lands in `handler/approval/query.rs`; `approval.rs` became `handler/approval/` in the move, and the decision verb split into `resolve.rs` so all three files clear the file cap. Limit 1..200 default 50 as Zig (`approvals/list.zig:20-21`), and the refusal sentences are the Zig's, kept verbatim.
  Two shapes worth recording. The cursor is `afd_core::paging::Cursor`, whose timestamp form is `{millis}:{id}` in the CLEAR — `keyset_cursor.zig`'s spelling, which this endpoint issues; the event listings' base64 form comes from a different Zig module and is not this one. A text-boundary cursor is refused, since this listing orders by instant alone. And the status vocabulary is what a FILTER can express: `pending` resolves to an absent filter, because that is how the statement binds it, and the three decisions map to `Decision`. `auto_killed` is a column spelling no decision writes, so it is refused by name rather than silently answered with the pending page.
  → Tests: nine parser cases inline in `query.rs` (the filters, the limit band, `pending` equal to absent, an unserved status, a Zig-minted cursor, and five cursors this endpoint never issued); and `integration_workspace_approvals_listing.rs` over live Postgres — each filter narrows, a full page hands back a cursor that resumes strictly after it, a short page ends the walk, and four unreadable parameters refuse 400. The fixture gained a second gate and its own signed-in person, since `core.users` holds one row per subject and the two files run concurrently.
...
- **Dimension 4.6** — DONE — the adversarial review's own findings, fixed in the same PR. The cursor was unreachable from the dashboard: its wire form carries a colon, `URLSearchParams` percent-escapes it, and `parameter()` reads raw, so every page after the first answered 400 — the headline feature broken for its only client. The 409 was the same mistake's other half: `ui/packages/app/lib/api/approvals.ts:47` types the conflict as the full resolve response, matching `approvals/resolve.zig:169-180`, and the minimal body carried neither outcome nor resolver, so the operator's alert would have rendered two undefined values. Both fixed; `envelope::Resolution` carries the five members Zig sends, and the resolver still stays out of `detail`. `fleet_id` reached `SELECT_GATE_PAGE`'s `$3::uuid` unvalidated, so a typo was a 500 with an error log line rather than a 400. The refusal pass walks all eight operations a `PathItem` carries, not five, and reads `4XX`/`5XX`/`default` beside a status; the contract test calls that same `is_a_refusal` rather than a second spelling of it. Declared credentials collapse to a set. The install stopped holding a pool connection while the pre-flight took another.
  → Tests: the walk pages one row at a time over two gates sharing a millisecond and asserts the union, so a cursor ignoring the `(created_at, id)` tie-break fails; the fleet filter asserts EXCLUSION against a second fleet rather than a count; both new refusal constructors joined the status table; the document gate refuses a problem body on a success as well as demanding one on a refusal; and `integration_fleet_install_credentials.rs` proves the 424 at the wire, which the lifecycle-crate test could never reach.
- **Dimension 4.7** — DONE — the two review leftovers Indy selected (2026-09-03, "Pictures 1 and 2"). The status filter routed through `Decision`, whose three arms are what an operator WRITES, so `auto_killed` — a state the column really holds and `ui/packages/app/lib/api/approvals.ts:8` declares — was refused 400 and those gates were unreachable through the only listing that shows them. `afd_approval::GateStatus` is the reader's five-arm vocabulary; `Filter::status` reads it, `Decision` still cannot express the killer's verdict, and no operator can write one by hand. And the install was not the only door to an unrunnable fleet: `PATCH` takes a replacement `TRIGGER.md`, which declares credentials too, so `require_the_declared_credentials` became `require_the_stored_credentials`, `pub(crate)` over a name slice, and `edit::patch` calls it on the rewrite's own declaration.
  → Tests: the parser resolves all five spellings and refuses a sixth; the listing serves `?status=auto_killed`; and a live-datastore case installs with one credential stored, edits to a configuration naming a second, and asserts the refusal names only the newly declared one.

### Deferred from review, on Indy's call

Two findings were surfaced with evidence and left, both quoted below.

- **The 424 names credential names to a `fleet:write` holder.** `HIERARCHY` (`afd_auth/src/scope.rs:247`) has twelve rows and every one stays inside a family, so `fleet:write` expands to `{fleet:write, fleet:read}` and never to `secret:read` — which is what guards `GET /secrets`. By complement the refusal therefore also says which names the workspace HOLDS. Kept because `create_fleet_bundle.zig:117-130` sends the same list, and parity is this milestone's rule. Indy chose "Pictures 1 and 2", declining the scope gate.
- **The keyset seek and sort are two operators.** `SELECT_GATE_PAGE` seeks on `(g.created_at, g.id::text)` and orders by `g.created_at ASC, g.id ASC`. They agree for canonical UUIDs under C and glibc collations; the deployment's actual collation was not checked from this machine, and a disagreement would skip rows at a page boundary in silence. Reviewer confidence 5/10. Fix if taken: order by `g.id::text` so both are one expression.
