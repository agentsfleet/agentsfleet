...
- **The two cross-runtime pins stay, by owner decision.** `cross_runtime_rates.rs` holds four money constants equal across `afd_billing`, `cli/src/constants/billing.ts` and `ui/packages/app/lib/types.ts`; `cross_runtime_timing.rs` holds the five lease-clock constants equal between `afd_core::timing` and `src/lib/common/constants.zig`, which the runner renews against and which `deploy-dev-build.yml:36` still builds. Asked whether to delete them with the rest of the family, given that deleting removes the detector while the duplication remains: > Indy (Sep 21, 2026): "I am okay to have diagreement via the unit test failing" — context: the ask named the drift each one catches (a dashboard quoting a price the server does not charge; a runner renewing against a deadline the daemon will not honour) and offered removing the duplication instead. **Follow-on, deferred with that acknowledgement:** remove the second copy rather than police it. The timing pin turns out to enforce an agreement that was never needed — `src/runner/daemon/renew_driver.zig:120` reads `self.deadline_ms`, so the runner is already TOLD its deadline by the server and needs no `LEASE_TTL_MS` of its own; `RENEWAL_WINDOW_MS` and `RENEWAL_TICK_MS` are runner-local policy that only has to satisfy `window < TTL`, not equal a number the daemon also spells. Dropping the runner's TTL copy and relaxing the other two to an inequality retires four of the five pinned constants. The rates side is the same move in the other direction: the clients display what the API hands them, or the TypeScript is generated from the Rust. Both tests then delete themselves as a consequence.
- **Found while auditing the text scans, not fixed here: the cross-language citations are almost all rot.** Asked whether every cross-runtime coupling fails when one side changes. It does not. Two are held by a test — `cross_runtime_rates.rs` and `cross_runtime_timing.rs`, nine constants between them. The rest are held by a doc comment, which fails never: **860 citation lines across 358 Rust files naming 315 distinct `.zig` paths, of which 296 name a file deleted with the Zig daemon.** Ninety-seven of those lines assert an active constraint in words — "pinned byte-for-byte", "mirrors" — against a referent that no longer exists, so they cannot fail and cannot be checked by a reader either (`afd_auth/src/error.rs:125` against `auth/scopes.zig`; `afd_crypto/src/aad.rs:3` against `crypto_store_write.zig`; four sites in `afd_tenant/src/workspace/name.rs` against `lifecycle.zig`). Nineteen paths still resolve, four of them the `src/lib/contract/` files genuinely shared with the runner, and those citations are real. The sweep — strip the dead reference, keep the sentence it justified — is its own milestone: the diff would be mechanical but per-line, and at 358 files it would bury this spec's actual change. RULE NLG arguably already forbids the shape, since a comment naming the retired daemon as authority is legacy framing.
