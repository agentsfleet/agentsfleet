...
- **A second macOS-vs-Linux gap, this time in the numbers rather than the
  collector.** PR #608's merge conflict with `main` blocked every CI run until
  Aug 16 — the whole spec's `runner`/`lib`/`agentsfleetd` floors, including the
  95.18% `runner` measurement Dimension 4.2 above calls DONE, were taken on a
  dev Mac and never checked against the platform the gate actually runs on
  until this branch's first real cycle. `src/runner/engine/{seccomp,landlock,
  cgroup}.zig` are real Linux sandboxing enforcement; their Linux branches
  comptime-eliminate to stubs on macOS, so those lines never entered the
  denominator locally. On Linux CI they compile in for real, and only
  `sec_enforcement_integration_test.zig`'s privileged lane — which
  `test-coverage-zig` is not — exercises them. First Linux measurement: 87.48%
  (2767/3163), not 95.18%. `agentsfleetd` (90.12% vs 90.20% local) and `lib`
