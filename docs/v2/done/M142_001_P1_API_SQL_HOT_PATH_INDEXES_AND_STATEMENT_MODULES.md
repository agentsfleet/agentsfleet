...
**Solution summary:** Four indexes land in one additive migration, each justified by a named query that today scans or sorts without one, and each guarding a table that grows without bound. (Authored as nine, trimmed to four at Indy's ~100-runner scale decision — see Discovery.) Three list reads are restructured so the expensive per-row work happens after pagination rather than before it, and the credential list collapses to a single query. The filter defect is fixed by escaping the backslash the translator currently passes through. Alongside, inline statement text moves into per-domain `sql.zig` modules — the pattern already used by eleven domains — across the data-access layer. (An adoption-ratio checker was built and then removed at Indy's direction — the modules are the value; a percentage gate on them was over-engineering for this team size.) No wire protocol, response shape, or stored data changes.
...
| File | Action | Why |
|------|--------|-----|
| `schema/033_hot_path_indexes.sql` | CREATE | Four indexes, each with the query that justifies it named in a comment, and each on a table that grows with usage rather than with headcount. |
| `schema/embed.zig` | EDIT | Register the new slot. This is the ONLY registration edit — see the correction below. |
| `make/quality.mk` | EDIT | Consolidates the script-driven convention gates behind one `lint-governance` target, at Indy's request — `lint-zig` had grown to 14 prerequisites. (The adoption checker this originally wired in was removed; `lint-governance` remains.) |
...
- **Dimension 3.2** — the credential list issues a fixed number of queries regardless of credential count → Test `test_secret_list_query_count_is_constant` — **DONE**, proven by counting real table access (`pg_stat_all_tables` scan delta on `vault.secrets`, flushed with `pg_stat_force_next_flush`) rather than by asserting on source text: listing 1 credential and listing 20 record the same scan delta.
...
- **Dimension 5.1–5.4** — **WITHDRAWN at Indy's direction.** The adoption checker, its tests, and its conformance wiring were built (reaching 80.8%) and then removed: a percentage gate on SQL-module adoption is process weight this team does not need. The `sql.zig` extraction itself stays — ten domains now keep their statement text in one grepable place, which was the actual goal. What remains verifiable is the shape, not a ratio: every statement module is a constant surface, and each moved statement is byte-identical to the text it replaced.
...
- **§3.1 correction — the runner list was never "one lease lookup per runner".** The spec's premise does not survive measurement. PostgreSQL inlines the `filtered` CTE and turns the correlated `EXISTS` into a **hashed SubPlan evaluated once** (`loops=1`), so the per-row cost the spec describes never existed. The real cost is worse in a different way: building that hash reads the WHOLE `runner_leases` table on every request. Against a 200 000-row lease table, the original plan cost **6 472 shared buffer hits** (6 468 of them the lease seq scan); the restructured plan costs **79**, doing `loops=25` index lookups on `idx_runner_leases_runner_id_status` — exactly page size. An 82x reduction, so the restructure ships, but the Dimension's stated reason is corrected here rather than quietly inherited.
- **§3.1 payload identity, proven by set difference.** `EXCEPT` in both directions between the original and restructured queries over the same fixture returns 0 rows, across 25 compared rows of which 3 carry `has_live_lease = true` — so the column under test is genuinely mixed, not trivially equal. `total` is unchanged by construction: `COUNT(*) OVER()` is a window function, computed over the full row set before `LIMIT`.
- **§3.1 regression attribution.** A first test run showed 52 failures, which a controlled A/B (stash the change, re-provision, re-run; restore, re-provision, re-run) attributed entirely to the Postgres container restarting mid-run under a sibling worktree. Both conditions return 114 pass / 59 skip / 0 fail.
- **§5 started early, out of necessity.** The restructure pushed `runners_list.zig` to 357 lines, over the 350 cap. Rather than trim prose, the statement moved to a new `http/handlers/fleet/sql.zig`, which is what §5 prescribes for this domain anyway. The file lands at 320 lines and the domain gains its statement module.
- **§3.2 shape — the fix had to land in `crypto_store`, not the handler.** The list's two-pass structure was not incidental: a per-key decrypt-load cannot run while another result is open on the same connection, so pass 1 had to close before pass 2 could start. Collapsing it meant giving the store a workspace-wide read (`loadAllForWorkspace`) and splitting the row-decrypt out of `load` (`decryptRowAt`) so one query and one unwrapped Key Encryption Key (KEK) serve every row. Decryption needs no statement of its own, which is what makes the single pass legal. Column order in `SELECT_SECRETS_FOR_WORKSPACE` deliberately matches `SELECT_SECRET`'s ciphertext block so one decrypt routine serves both.
- **§3.2 — two bugs the tests caught, worth naming.** `freeEntries` initially released each entry's secret material but not the backing slice, leaking two allocations per call; fixing that naively then double-freed, because the error path passed `out.items`, which aliases the `ArrayList` buffer that `deinit` releases. The two levels are now separate functions, with the alias documented at the call site.
- **§3.2 — a zeroization invariant test was strengthened, not bumped.** `crypto_store_test` pinned `secureZero(u8, &kek)` at exactly 2; the new bulk read makes it 3. Rather than edit the constant — which would have proved nothing about the third KEK — the assertion now ties the zeroing count to the `cp.loadKek()` count, so any future KEK loaded without a matching zero fails regardless of how many exist.
- **Post-merge audit and scale decision — the slot was cut from ten indexes to four.** Indy set the operating scale at **~100 runners** and asked for no over-engineering. That number is decisive, because it splits the ten indexes by what makes their table grow. Four guard tables that grow with USAGE OVER TIME — `runner_leases` (a row per claim, never pruned), `fleet_events` (a row per event, the highest insert rate in the system), `memory_entries` (per fleet, capped at 1 000 each), `runner_affinity` (per fleet, and the sweep's only multiplicative read). Those climb at 100 runners exactly as they would at 10 000, so they stay. The other six indexed list sorts over `fleet.runners`, `core.fleets` and `core.api_keys` — tables that gain rows when a person adds a runner, a fleet or a key. At ~100 rows those fit in a page or two, the sweep's own `LIMIT 100` is the whole table, and sorting is already free. They were dropped. The queries are unchanged, so re-adding one later is a one-line slot if counts reach the low thousands.
- **Audit correction — the heartbeat write-cost claim was wrong.** A first pass at this audit asserted that indexing `updated_at` cost `fleet.runners` its heap-only (HOT) updates on the 10-second heartbeat. Measured at 100 runners over 2 000 single-row updates, HOT was **0% both with and without the index**. The cause is neither: `uid UUID GENERATED ALWAYS AS (id) STORED` is re-evaluated on every UPDATE, which disqualifies HOT on this table unconditionally. An identical table without the generated column measured **17% HOT** under the same load. So slot 033 added no incremental write cost here, and the six indexes were dropped for simplicity at this scale, not to recover HOT. Recorded because the wrong claim was nearly shipped in the PR description.
- **`idx_fleet_events_workspace_id_created_at` retired too (slot 034, third drop).** Found by the audit: slot 033's three-column index leads with the same two columns, so the old one is a strict prefix and answers nothing the new one cannot. Keeping both maintained two near-identical indexes on the highest-insert table in the system. Dropped on the same structural argument as the memory index, which Indy had already approved for that shape.
- **Test fixtures were seeding a fleet that could not exist.** The memory fixtures used 4 000 entries per fleet against a production cap (`MAX_MEMORY_ENTRIES_PER_AGENT`) of 1 000. At 4 000 of 40 000 rows the probe fleet is a tenth of the table, and for an UNBOUNDED fetch of that share PostgreSQL correctly prefers a sequential scan — so the relocation test failed on the fixture, not the index. Both fixtures now derive the probe size from the production constant, and the plan assertion passes at the real shape (1 000 of 40 000).
