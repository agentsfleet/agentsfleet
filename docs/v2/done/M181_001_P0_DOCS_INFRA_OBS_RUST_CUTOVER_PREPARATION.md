...
- **Dimension 2.3** — the daemon serves from the distroless image, proven by the parity lane's single-target mode against a container-hosted daemon; the release job's runtime check covers the CI side → Test `test_runtime_on_production_base` (graded with §4's lane) — **DEFERRED to M181_002 §1** (see Discovery 2026-08-31; the grading method needs the full route surface §1 lands)
...
- **Re-measured after M180 merged, and the attribution was partly wrong (2026-08-31):** the entry below reads the 31 unmounted routes as M180's webhook/connector/ingress surface. M180 merged into this branch the same day, and the lane was re-run against the merged tree: **31 fell to 13**. So M180 owned 18 of them and not the other 13. Of what remains, ten were never M180's — seven `tenants` routes (`/v1/tenants/me/models` and `/v1/tenants/me/provider`, full CRUD on both), two `workspaces` `fleet-libraries` routes, and `GET /v1/auth/sessions/{session_id}` — and three are stragglers in families M180 otherwise landed (`POST /v1/ingress/{provider}`, `GET /v1/connectors/{provider}/callback`, `POST /v1/webhooks/{fleet_id}/grant-approval`). The lane itself worked in both runs: the daemon booted from the distroless image, answered `/healthz`, and honoured the declared `GET /metrics` divergence. The deferral stands with a better reason — the route surface is incomplete for reasons wider than one milestone — and `make test-parity LOCAL=1` is now MANDATORY in M181_002 §1, which carries the enumerated remainder. Recorded because a deferral resting on a wrong cause is a deferral nobody can close.

- **2.3 is PARKED, and the parity lane is why — working, not broken (2026-08-31):** `make test-parity LOCAL=1` now runs end to end against a container-hosted daemon: the image builds for `linux/arm64`, the container boots, and `/healthz` answers. What it then reports is 31 declared routes returning 404 and 1 declared divergence honoured. That is the lane doing its job, not a defect on this branch — the missing families are 12 `workspaces`, 7 `tenants`, 5 `webhooks`, 3 `connectors`, 2 `ingress`, 2 `auth`, and the webhook/connector/ingress surface belongs to M180, whose branch has not merged. Out of Scope above already says so: the route parity gate is "all of M181_002, which needs the full route surface this milestone deliberately does not wait for", and `main`'s own `e41b9af4c` records the sequencing as "preparation now, swap after the ingress merge". So the dimension's DISTROLESS claim is proven by this run and its GRADING METHOD is not available until the route surface lands. Rather than grade it on evidence the dimension does not name, it is DEFERRED to M181_002 §1, which lands the full route surface the lane needs. Recorded in that spec as well, so the dimension has an owner rather than a note. **User-acked deferral, verbatim:** > Indy (2026-08-31): "2.3 fine defer" — asked again at CHORE(close) with the parity evidence in hand, and confirmed: park it rather than amend how it is proven.

- **The Test Specification table and the Dimension list disagree about what 2.3 is (2026-08-31):** the Dimension at §2 names `test_runtime_on_production_base` (the distroless image); the Test Specification row for 2.3 names `test_deploy_binary_selection` ("the knob flips the served binary on a staging machine"). Two different claims under one number, and only the second needs a staging machine. Recorded rather than renumbered: renumbering a shipped table mid-close would churn every reference to it, and M181_002 owns both halves. Whichever milestone picks 2.3 up should split the number first.
