...
**Workstream:** 001
**Date:** Jun 24, 2026
**Status:** DONE — §1 (resolver), §2 (fallback + 11-route rewire), §3 (Suspense streaming on `/fleets`, `/events`, `/approvals`), §4 (billing dedup), §5 (code-split: 5 `next/dynamic` island shims) implemented + unit/RTL-tested (full suite green, 100% coverage), build clean. The two live-environment checks (e2e workspace-fetch audit, §5.3 assistant-ui QA) are **deferred to post-deploy** (Indy-acked Jun 25 2026; see Discovery): `api-dev`'s database was torn down, so signup-bootstrap returns `UZ-INTERNAL-003` (`/readyz` only pings DB connectivity → it falsely reported healthy), and there is no turnkey local backend. The 0-RTT mechanism is unit-proven; the live runs execute against the restored deployment post-merge + via M101_002.
...
- [ ] Soft navigation issues zero list fetches with a valid cookie — `AGENTSFLEET_E2E_AUDIT=1 bun run test:e2e:acceptance` — **⏳ deferred post-deploy** (Jun 25 2026: harness staged + verified, but `api-dev`'s DB was torn down → signup-bootstrap 500s; runs against the restored deployment post-merge + via M101_002). Mechanism unit-tested (`test_resolver_prefers_cookie_no_fetch` fetch-spy-not-called + `workspace-fetch-audit.test.ts`).
...
- e2e + §5.3 live-verification deferral (Indy, Jun 25 2026): a pickup session staged the full acceptance harness against the dev backend — Clerk DEV creds (`op://ZMB_CD_DEV/clerk-dev/*`) resolved into gitignored `.env`/`.env.local`, fixtures provisioned, the `AGENTSFLEET_E2E_AUDIT=1` dev server booting on :3101, Clerk user provisioning + Svix-signed `user.created` webhook all verified working. The run blocks only at tenant bootstrap because **`api-dev`'s database was torn down** (confirmed by a sibling agent): `POST /v1/auth/identity-events/clerk` returns 500 `UZ-INTERNAL-003` for every new user, while `/readyz` only pings DB connectivity so it falsely reports healthy. Local-backend fallback was rejected — `make up`'s Postgres/Redis collide with the shared :5432/:6379 another agent's `test-integration` uses, and there is no turnkey local-OIDC recipe (`make acceptance-e2e` just mirrors CI against a provisioned backend). Decision: **close M101 as merge-ready and defer the two live checks to post-deploy** (merge → deploy restores the backend) + M101_002, since the 0-RTT mechanism is unit-proven.
  - > Indy (2026-06-25): "The api-dev db ... reported that the db was torn down though the health and readyz reports as true. That is the reason the signup bootstrap is returning 500." — context: root-cause of the e2e block is an api-dev infra outage, not M101 code.
  - > Indy (2026-06-25): "Push the PR lets it run the tests and we will go from there. When PR is merged the deployment happens. Important aspect is to have the test-integration succeed" — context: authorizes closing M101 + deferring the e2e audit and §5.3 design-review to post-deploy.
...
| e2e (audit) | `AGENTSFLEET_E2E_AUDIT=1 bun run test:e2e:acceptance` | **⏳ deferred post-deploy.** Harness fully staged + verified this session (Jun 25 2026): Clerk DEV creds (`op://ZMB_CD_DEV/clerk-dev/*`) resolved into gitignored `.env`/`.env.local`, fixtures set, the audit-instrumented dev server boots on :3101, Clerk user provisioning + Svix-signed `user.created` all succeed. The run blocks only at tenant bootstrap: `api-dev`'s database was torn down, so `POST /v1/auth/identity-events/clerk` returns 500 `UZ-INTERNAL-003` for every new user (`/readyz` only pings DB connectivity, so it falsely reports `database:true`). No turnkey local backend exists (`make up`'s Postgres/Redis collide with the shared :5432/:6379). The 0-RTT mechanism is unit-proven (`test_resolver_prefers_cookie_no_fetch` + `workspace-fetch-audit.test.ts`); the live audit runs against the restored deployment post-merge + via M101_002. Indy-acked (Discovery, Jun 25 2026). | ⏳ deferred |
| §5.3 live QA | `/design-review` + browse on `/fleets/[id]` | **⏳ deferred post-deploy** — needs the running authenticated app + a live streaming fleet (backend + `agentsfleet-runner`); the same `api-dev` teardown blocks it. Static guard (`test_fleetthread_uses_design_tokens`) passes. Runs against the restored deployment post-merge. Indy-acked (Discovery, Jun 25 2026). | ⏳ deferred |
