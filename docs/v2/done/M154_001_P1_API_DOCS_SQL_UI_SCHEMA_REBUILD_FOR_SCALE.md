...
| `playbooks/operations/teardown/database/03_verify.sh` | EDIT | **Added during the close-out audit** — the table did not list it, but the diff carries it and the cause is this milestone's own role changes. Its app-role check was a hand-maintained list that had rotted: it named `ops_readonly_agent`, retired with the old product noun, while missing `ops_readonly_fleet` and every role a later migration adds — verifying a subset while telling the operator to expect a count that no longer matched. Now catalogue-derived, the same shape as the schema loop beside it |
...
- **Dimension 4.1** — the table, its store, its handler and its endpoint are gone, with no orphaned reference → Test `test_accrual_surface_fully_removed` + `test_accrual_reader_and_route_are_gone_from_the_sources` (`state/accrual_surface_integration_test.zig`) — **DONE**, and both halves are mutation-proven. Pointing `RETIRED_TABLE` at a table that does exist turned the catalogue assertion red (`accrual_surface_integration_test.zig:47`, `expected 0, found 1`); re-introducing `metering_periods` into a scanned source turned the source scanner red (`:83`, `retired accrual symbol is back: metering_periods` → `error.RetiredAccrualSurfaceReturned`). Full-suite run, `EXIT_CODE=2`, `810/820 tests passed (7 skipped, 3 failed)`
...
- **Dimension 5.1** — no index appears or disappears without a deliberate edit to the declared roster, so a reader deleted by a later milestone cannot leave its index behind → Test `every index in the schema is declared, and every declared index still exists` (`db/index_usage_integration_test.zig`) — **DONE**, mutation-proven in **both** directions from a single roster edit: renaming one entry produced `UNDECLARED INDEX: billing.idx_usage_ledger_workspace_id` (a real index absent from the roster) *and* `MISSING INDEX: billing.idx_usage_ledger_this_index_does_not_exist` (a roster entry absent from the catalogue), failing at `index_usage_integration_test.zig:815`. The two-way claim is therefore tested, not asserted
...
- **Dimension 5.2** — the tenant charges keyset pages without a sort node, because its index carries the tiebreak column (RULE KYS) → Test `the tenant charges keyset pages without sorting, because its index carries the tiebreak` (`db/index_usage_integration_test.zig`) — **DONE**. Its proof is that writing it turned red against production code and stayed red until the defect below was fixed; it is green on the from-empty clean-state run
...
- **Dimension 8.5** — `core.fleet_keys` is absent from the catalogue, and the retired origination path is unreferenced across the tree → Test `test_fleet_keys_surface_fully_removed` (the catalogue half) + rubric R6's grep (the reference half) — **DONE**
  - **Split during the close-out audit.** The test asserts only that `core.fleet_keys` is absent from `pg_tables` and that no index is named for it; it scans no sources. The Test Specification row claimed it proved zero references to four symbols across five trees, which it never did. The reference half is real but lives in R6's grep, and covers `core.fleet_keys`, `/integration-requests` and `/fleet-keys`. `agt_a` and `grant-approval` were never in scope to retire: the first names the workspace-scoped key prefix, the second is a live reserved webhook action.
...
- **Dimension 9.1** — a run that fails at the model call reports the error that actually stopped it, not the outcome label → Test `a run failure propagates its own error instead of collapsing to FleetRunFailed` (`runner/engine/run_context_test.zig`). Proven by mutation: restoring the collapse turns it red with the stub provider's rejection reaching `fleet.runSingle` first. — **DONE**
- **Dimension 9.2** — the fleet filter scopes both the rows and the pager's total, by id and by exact name, case-insensitively; a value no fleet matches is an empty page, not an error → Test `test_runner_leases_fleet_filter_scopes_rows_and_total` (`http/runner_read_integration_test.zig`) — **DONE** on the from-empty clean-state integration run. Stated plainly: this rests on the suite executing it, not on an individual mutation proof of its own — the standard the other §9 server-side Dimensions are held to
- **Dimension 9.3** — the workspace and fleet filters intersect rather than one overriding the other: pairing a fleet with a workspace it does not belong to matches nothing → Test `test_runner_leases_workspace_and_fleet_filters_intersect` (`http/runner_read_integration_test.zig`) — **DONE** on the clean-state run, same evidence standard as 9.2
- **Dimension 9.4** — an empty or over-long fleet filter is refused, while an unmatched name is an ordinary empty page — the boundary between "refused" and "matched nothing" is explicit → Test `test_runner_leases_unbounded_fleet_filter_is_refused` (`http/runner_read_integration_test.zig`) — **DONE** on the clean-state run, same evidence standard as 9.2
- **Dimension 9.5** — the filter query parses as GitHub's does: both tokens in any order, quoted values kept whole, unrecognised tokens dropped rather than guessed at, last occurrence winning on a repeat → Test `lease-filter-query.test.ts` — **DONE** via `make test-unit-all` (rubric S1)
- **Dimension 9.6** — applying a filter drops the cursor trail walked through the old result set, and clearing one filter leaves the other in place → Test `LeaseTable.test.tsx` (`applies both filter tokens and drops the cursor trail with the old result set`, `clears one filter without disturbing the other`) — **DONE** via `make test-unit-app` (rubric S4, `Tests 2160 passed`)
...
| 8.5 | integration | `test_fleet_keys_surface_fully_removed` (`http/handlers/fleets/grant_surface_integration_test.zig`) + rubric R6's grep | **Assertion corrected — the row described a scan its test does not perform.** The test asserts the *catalogue* half only: zero `core.fleet_keys` in `pg_tables`, and zero indexes named for it. It reads no sources. The cross-tree half is enforced by R6's grep over `core.fleet_keys`, `/integration-requests` and `/fleet-keys`, which is green. The original row also claimed zero `agt_a` and `grant-approval`; both are still present and legitimately so — `agt_a` names the workspace-scoped key prefix (`errors/error_entries_runtime.zig`), and `grant-approval` is a live reserved webhook action with 17 sites. §8 retired the fleet-key table and the `/integration-requests` origination path, not those two |
...
| R3 | The accrual surface is gone (§4) | `grep -rnE 'metering_periods\|get_tenant_metering_periods\|slice_seq' src/ ui/ public/openapi.json \| grep -vE ':[0-9]+:[[:space:]]*(//\|--)' \| grep -v accrual_surface_integration_test` | no output | P0 | ✅ no output. **Re-run and regraded during the close-out audit — the earlier ✅ had gone stale.** It was graded at `f63877fa5`, before `80f868b2f` added `accrual_surface_integration_test.zig`; the same command then returned 3 matches, all of them in that file. The exclusion is the second carve-out on this row and rests on the same reason as the first: a codebase that *documents* a retirement must not fail the criterion asserting it, and a codebase that *enforces* one must not either — the test holds those symbols precisely because it scans production sources for them. Verified narrow: the exclusion drops exactly one path, and this criterion still covers the whole tree, which Dimension 4.1's test does not (it embeds three named files) |
...
| S5 | No leaks (allocator wiring touched) | `make memleak` | exit 0 | P0 | ✅ exit 0, `memleak gate passed (agentsfleetd + runner + lib lanes + boot→drain lifecycle)` — all four lanes green (`agentsfleetd`, `runner`, `lib`, boot→SIGTERM→drain). **Regraded from a real run during the close-out audit.** The quoted string is the success `echo` at `make/bench.mk:72`, so its presence in this cell never evidenced a run having happened — and the prior handoff recorded that the target had never been run. The claim was right; its provenance was not. It is now run-backed |
...
|-----------------------|------|----------|
| `metering_periods`, `fleet_metering_store` | `grep -rnE -w "metering_periods\|fleet_metering_store" src/ ui/ public/ \| grep -vE ':[0-9]+:[[:space:]]*(//\|--\|\*)' \| grep -v accrual_surface_integration_test` | 0 matches |
...
- **Privilege split for the wallet and secret store** — M154_002, its own Pull Request (PR), both halves together.
...
- **Deferrals** — every "deferred to follow-up" needs an **Indy-acked verbatim quote** here, format `> Indy (YYYY-MM-DD HH:MM): "<quote>" — context: <which item, why>`.
  ...
  > Indy (2026-08-04): "Don't mark Dimension 6.1 DONE — Indy's call, it stays untested. Don't 'strengthen' 3.3; it's a documented regression guard that cannot fail today." — context: Dimension 6.1 ships with no test and stays `NOT DONE`. The Dimension as written asserts a *comment convention* the catalogue cannot see, so any test would parse `schema/*.sql` text and pass on a comment naming the wrong constant. The invariant worth pinning is literal-vs-constant drift, which is a different assertion needing Indy's call rather than a quiet substitution. Dimension 3.3 is likewise left exactly as written: it is a forward-looking regression guard that cannot fail against today's schema, and the spec says so rather than implying it proves erasure today.

- **Close-out audit (third session)** — `make memleak` was run for the first time
  and passed (exit 0, all four lanes); rubric row S5 had previously quoted the
  target's success `echo` from `make/bench.mk:72`, which never evidenced a run. The
  full suite was re-run from an empty volume (`make down` → `make up` →
  `make test-integration`, exit 0) rather than off the warm stack every prior green
  came from. Dimensions 5.1 and 4.1 were mutation-proven (`EXIT_CODE=2`,
  `810/820 tests passed (7 skipped, 3 failed)`). Test Specification row 8.5 was
  corrected: it described a cross-tree scan its test does not perform. Four defects
  were found and fixed that no Dimension covered — a `public/openapi.json` bundle
  desynced from its own split source, an undeclared 404 on the new event-detail
  endpoint, two banned-vocabulary comment hits, and two UFS magic numbers.

- **Test delta (VERIFY)** — baseline `unit=3344 integration=510` at CHORE(open);
  close-out `unit=3426 integration=583`. Delta **+82 unit, +73 integration** on a
  code-adding diff.
