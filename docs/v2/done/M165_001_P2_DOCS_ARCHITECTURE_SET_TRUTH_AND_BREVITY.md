...
| `scripts/check_architecture_doc_anchors.sh` | ADD | Scope addition (see Discovery) — the four cross-page pointer spellings, split out of the gate so both files stay inside the 350-line cap |
| `docs/DESIGN_SYSTEM.md` | EDIT | Scope addition (see Discovery) — carried an implementation roadmap for workstreams that have shipped, and a preview path on one developer's machine |
...
| `src/agentsfleetd/errors/error_entries.zig` | EDIT | Scope addition (see Discovery) — the UZ-WH-020 hint stated a fix with no fact and named one cause of two; this string is the source the user-facing error registry generates from |
| `src/agentsfleetd/errors/gen_error_codes.zig` | EDIT | Scope addition (see Discovery) — the generated page's `verified:` stamp is a literal here; moved forward with the regeneration |
...
- **The anchor assertion read one pointer spelling out of four, and the first reply to the reviewer was wrong.** A cross-page section can sit after the link — ``[`page.md`](./page.md) §Section`` — or inside its text — ``[`page.md` §Section](./page.md)`` — and either can quote the anchor. Both extractors required `§` after the destination, so the inside-link form was never read: its target heading could be renamed or deleted with the gate still green. Six live pointers used it, in `capabilities.md`, `fleet_bundles.md`, `scaling.md`, `runner_fleet.md`, and `data_flow.md` twice. The review comment naming this was answered "the corpus has none", which was false; the correction is recorded on the pull request. All four spellings are now read, and reading them exposed one anchor — `fleet_bundles.md` pointing at `data_flow.md §C` — that prefix-matched three unrelated headings before reaching the intended `C. EXECUTE`; it is quoted now.
- **The gate outgrew the file cap, so the pointer spellings became their own file.** `check_architecture_doc.sh` stood at 351 lines against a 350 cap before this change and the fix would have added ten more. The extraction moved to `check_architecture_doc_anchors.sh`, which reads doc paths and emits `src::target::anchor` triples — a cohesive concern with one home, and both files inside the cap.
- **Pointer extraction had to stop being a set of greps.** Fixing the inside-link spelling exposed two more holes of the same shape, both found by the reviewer re-reading the corrected file. One link can carry several sections — `§"B. TRIGGER" and §"C. EXECUTE"` — and a grep keyed on the link read the first and skipped the rest. A `../` target had its prefix dropped, so a sibling-directory pointer resolved to a path that does not exist and the gate skipped it rather than checking it. Extraction is now a left-to-right scan per line: a link sets the current target, each section that follows resolves against it, and the target keeps its prefix. Coverage went 48 anchors to 53 with none lost, and the four pointers that had been hiding are corrected — `direction.md §C (EXECUTE)`, `README.md` naming `runner_fleet.md` in prose while linking `memory.md`, and a jammed-in descriptor on both `scaling.md` and `scenarios/slack-channel-resident.md`.
- **Ambiguity is now a failure, not a pass.** Prefix-matching alone accepted `§C` because *some* heading started with C; on `data_flow.md` three do. An anchor matching more than one heading names none of them, so the assertion requires exactly one match and says to quote the full heading text. This retires the whole short-anchor class rather than the one instance that was found.
- **Most section references carry no link, and none of them were being read.** The assertion only ever saw references attached to a link — 53 of them. The other ~126 name a heading on their own page (`§Anti-patterns`, `§Scope catalogue`) and were invisible. The gate resolves those against the source page now, and coverage went 53 to 179. Two references had been dangling in plain sight: `§Webhook auth`, cited from `data_flow.md` and `AUTH.md`, and `§Out of Scope`, cited from `connectors.md`.
- **Why they drifted, which is not why the linked ones did.** Three distinct causes, and only the first is drift. **(1) A disambiguating rename during a scope split.** `2d250eff2` (M102) renamed `## Webhook auth (separate surface)` to `## Manual fleet-webhook auth (separate surface)` because M102 added a *second* webhook surface, so the unqualified name stopped being unambiguous; three sub-headings took a `Manual-route` prefix in the same commit. The two citations were written 2026-05-01 and correct then. Note this renamed the **lead**, not the trailing subtitle, so prefix-matching gives no protection — the earlier assumption that renames only touch the tail is wrong, and `agent`→`fleet` and `zrn_`→`agt_r` were simply subtitle-only by luck. **(2) A spec-to-architecture citation leak.** `§Out of Scope` names a section of the `M108_001` spec, where the template mandates one; architecture pages have no such section, so it pointed at nothing from the day it was copied across. It now names the deferred work directly and cites the spec by identifier, which the milestone assertion validates. **(3) No boundary convention for an inline `§`.** `§C. EXECUTE step 3`, `§B's fifth producer`, `§Scope catalogue (meaning)` — all read correctly to a person and parse as nothing. This was the bulk of it, ~15 sites.
- **The fix is the convention, not the corrections.** A section reference is quoted, and any descriptor sits outside the quotes; a same-page reference stays unlinked, so no page name is duplicated where a local shorthand already reads well. Only a reference to another page gains a link, because nothing else says which page it means. Twenty-eight sites were corrected under that rule. Had the references been checked, `2d250eff2` would have failed in July and the author would have updated both citations in the rename commit — the one moment anyone knew which of the two webhook surfaces was meant.
- **Two scanner limits surfaced while fixing the corpus, both real.** Hard-wrapped prose splits a quoted anchor across the line break, so `scaling.md` and `web_app.md` looked broken while being correctly written; the scanner holds a line whose quote is still open. And a link binds only to the end of its sentence — `(see other.md §"X"). The figure in §"Y" was wrong` names two different pages, and binding to the nearest link got the second one wrong.
- **Fourteen diagrams had drifted out of alignment.** The `zombied`-to-`agentsfleetd` renames changed name lengths, the box borders were never re-padded, and two diagrams still carried `fleet:Z1`-style zombie-era stream names. All fourteen are redrawn or machine-re-padded; `runner_fleet.md`'s topology also dropped its "BEFORE (deleted)" half, which described the architecture the cutover removed. Per Indy's call, no gate guards diagram alignment — a one-time rename artifact, not a recurring class.
- **`data_flow.md` was missing the sixth producer.** M157_003 shipped a repair-verification dispatcher: after a human merges a repair and GitHub reports production status, it correlates workspace + repository + commit and lands one `actor=system:repair-verifier` event on the verifier fleet's stream (`repair_verification_dispatcher.zig` → `xaddOnce`, exactly-once via a Redis once-key, same envelope, `event_type=webhook`). The page said five producers and enumerated four actor spellings; it now says six and lists all of them, with the crew walkthrough left to `scenarios/production-deploy-repair.md`, which owns it. `connectors.md`'s Provider impact table was also missing the registered Zoho Desk connector (`registry.zig` pins five providers; the table listed four).
- **The verified stamp was pinned twice, and the second pin was a trap.** `gen_error_codes_test.zig` asserted the literal date the generator also carried, so any honest bump failed the suite. The generator now exports `VERIFIED_DATE`, the prelude is built from it, and the test asserts through the export — one source, byte-identical output, and the next bump cannot fail this way.
- **Brand marks replaced ad-hoc emoji where a company is named.** Provider and vendor tables use the real logos (Simple Icons CDN; Slack via Iconify after its Simple Icons takedown), sized inline at 14 px. Concept icons in the index table stay emoji. Logos and icons never enter headings: specs cite heading text verbatim, and a prefix would break every citation.
- **The design-system page described its own implementation as future work.** `docs/DESIGN_SYSTEM.md` carried a W1–W6 roadmap for workstreams that have since shipped — `@fontsource/commit-mono` resolves in `tokens.css`, and the page's own EXECUTE doc-reads row is live — plus a preview reference pointing into one developer's `~/.gstack` directory. Both are cut under RULE NDC.
