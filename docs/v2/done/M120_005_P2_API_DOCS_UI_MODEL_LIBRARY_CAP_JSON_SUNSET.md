...
- **Dimension 3.2** — a failed non-auth fetch (network error, `5xx`) degrades pickers to free-text entry exactly as today → Test `test_catalogue_fetch_failure_degrades_free_text` — **DONE**
- **Dimension 3.4** — a `401` from the catalogue action routes the user to the sign-in page, never the free-text degrade (Indy-directed post-review) → Test `test_catalogue_401_routes_to_sign_in` — **DONE**
...
|------|-------|----------------------------------------------------------|
| Unauthenticated read | missing/invalid Bearer on `/v1/models` | `401` problem+json; the dashboard provider routes the user to the sign-in page (Indy-directed — an expired session must never become a silent free-text degrade) |
| Client fetch fails (non-auth) | network error, `5xx` | existing `ModelCatalogueProvider` degrade path — pickers fall back to free-text entry, page fully functional |
...
|---|--------------------------------|---------------------|----------|----------|-----------------|
| R1 | Zero old names or retired-path refs (§1, §3, §4) | `grep -rn "model_caps\|ModelCaps" src/; grep -rn "model_caps\|ModelCap\b\|CAP_JSON" ui/; grep -rn "_um/\|cap\.json" src/ ui/ public/ scripts/ \| grep -v -e "handlers/model_library_integration_test.zig" -e "openapi/paths/models.yaml" -e "public/openapi.json"` | no output from any grep (exact-file allowlist: the 404 pin test + the three retirement-prose sites — an exact list, not a word filter, so a reintroduced reference cannot hide behind the word "retired") | P0 | ✅ all three greps empty |
...
| `_um`/`cap.json` path literals | `grep -rn "_um/\|cap\.json" src/ ui/ public/ scripts/ \| grep -v -e "handlers/model_library_integration_test.zig" -e "openapi/paths/models.yaml" -e "public/openapi.json"` | 0 matches (exact-file allowlist, mirroring R1) |
...
- **Skill-chain outcomes** —
  ...
  - `/review` (CHORE(close) step 2, Jul 11, 2026): eight-angle adversarial review (3 correctness + reuse/simplification/efficiency + altitude + conventions). **Applied:** fixture seeding/cleanup goes through the store's own `create`/`remove` (was raw SQL — the 5th independent copy of the insert); `sql.zig` internal consistency (`IS_REFERENCED…` builds from `TABLE`; `LIST_RATES_FOR_CACHE` reuses `RATE_COLUMNS` + `FROM_TABLE`, cache indices realigned); stale `Public*` store naming → `LibraryRow`/`LibraryList`/`listForLibrary`; single-call-site `appendPublic` inlined; six catch-switches that re-implemented `try` collapsed; hand-copied helper mocks → `importOriginal` spread; TS pin-test marker added; empty-catalogue test made FK-safe (skips when `platform_provider_defaults` holds a reference); new `POST-without-token → 401` test pins auth-before-method ordering; four change-narration comments removed (comment-voice rule), which shrank the R1 allowlist; `sql.zig` header scoped to "production" queries; changelog notes the unported `?model=` filter. Cross-file tracer round: the suite's seed rows renamed to suite-unique model ids with loud affected-count assertions (four sibling suites seed the same real `(provider, model_id)` pairs under their own uids — the fixture's `ON CONFLICT DO NOTHING` was silently no-oping in full runs and passing on coincidental rates); the FK-skip now warns before skipping; a dead mock seam (`getModelLibraryMock` in the dashboard helper mocks — the component seam moved to the action) removed. **Refuted:** rate-constants drift-pin loss (the CROSS-TIER RATES pre-commit gate pins the three constants across four files; no served surface remains to drift); missing 410/Sunset window (hard cutover Indy-acked).
- **Deferrals** —
  - Greptile P2 override — `> Indy (2026-07-11 14:1x): "I know this is not a bug - What does free-text mean? But this is a blunder you need to fix. On an expired seesion of 401, user must go to the signin page?" — context: overrides the intentional-degrade posture; a 401 from the catalogue action now routes to /sign-in (Dimension 3.4), landed in-PR.`
  - Caching posture of the authed read (efficiency + removed-behavior finders): per-request DB read replaces the ISR/CDN-cached public document — accepted in this spec's Out of Scope ("uncached by design; small payload, once per session"). Improvement candidate for a future slice: serve `GET /v1/models` from the existing in-process rate cache (it already holds every column except `updated_at_ms`), zeroing the per-request DB read. Recorded as a candidate, not a commitment.
  - Backpressure probe couples to a product route's auth posture (altitude finder): the deeper fix is a purpose-built harness probe route; the coupling is documented at the probe's declaration for now.
  - `route_scopes.zig`'s `&NONE` bucket now means both "no auth at all" and "authed, no capability scope" (altitude finder): a single route descriptor deriving both the scope table and the middleware table would make the distinction structural; comment-only today.
  - Catalogue-seeding consolidation across the four self-seeding suites into `test_fixtures_provider.zig` (reuse finder), and a shared UTC-date formatter for `formatVersion` + the webhook normalizer (needs the approval-gated `src/lib/` promotion).
  - `check_openapi_url_shape.py` keeps an empty `STATIC_CATALOGUE_CARVE_OUTS` set as the named home for future owner-directed exceptions (simplification finder argued delete; kept deliberately, mirroring the empty `VENDOR_PATH_CARVE_OUTS`).
