...
- **Second-wave perf scope expansion (Jul 02, 2026, Indy-acked).** After CHORE(close), Indy approved folding a perf wave into the open PR #468: review-finding fixes (`8625561b`), the Slack signing-secret once-per-process cache (steady-state ingress = zero vault reads, `fae0a39c`), the `connector:outbound` consumer de-block (non-blocking `XREADGROUP` + idle poll; `scaling.md` back to 9·R, `cc4056c1`), and the per-suite-test-tenant refactor killing the shared-tenant contention class behind both of this milestone's flake families. `> Indy (2026-07-02): "tenant refactor" … "Fold into #468" — context: approved perf refactor 4 (per-suite tenants for the state/metering + slack integration suites) and folding the wave into PR #468 rather than a follow-up PR.` The tenant refactor also surfaced and fixed two latent issues outside its scope-list but inside its mission: the 8625561b `resetBilling` sweep mis-aimed two calls in `tenant_billing_integration_test.zig` at the shared tenant (re-aimed via the new `resetBillingFor`), and `integration_roundtrip_test`'s `cleanupAll` leaked its fixed-id `fleet.metering_periods` slice (`evt-seed-1`), making that suite collide with itself on any same-database rerun (cleanup added, mirroring the seven sibling suites).
- **Watchdog: built → reverted → returns as platform scope (Jul 01–02, 2026).** The perf target "move the thread re-fetch off ingress" was architecturally blocked (the runner cannot call Slack — locked decision; lease-time fetch is worse; no deferred-task substrate), so a deadline-watchdog bounding the timeout-less ingress fetch was built instead. Indy challenged its provenance (no spec dimension asked for it) and it was reverted from #468 — the call-site wiring survives as `watchdog-bounded-fetch.patch` in the session scratchpad, but the `watchdog.zig` module itself was never preserved (discovered Jul 02). The next session found the prior art the patch had reinvented: the runner's `call_deadline.zig` `CallWatchdog` (M100) — socket-shutdown-at-deadline, fail-closed, production-tested. Disposition: `> Indy (2026-07-02): "Yes i agree on building watchdog, i am sure we would have fixed this before?" — context: greenlit as connector-platform scope; M108_001 promotes the existing CallWatchdog to src/lib as the shared bounded-outbound primitive instead of re-authoring the reverted patch.`
