...
- **Dimension 2.1** — mint of an **on-demand** integration with no approved grant → 403 `UZ-GRANT-001`, no token, no upstream call, `credential_mint_denied` warn logged; a `static` handle or unknown id is NOT gated (falls through to the existing vault/not-connected path — matches the lease classifier + Out of Scope) → Test `test_mint_requires_approved_grant` + unit `isOnDemand gates the connector integrations`
...
- **Skill-chain outcomes** — `/review` (Jul 06, 2026, medium, correctness + security-bypass panels): security-bypass panel returned clean (no alternate mint path, fleet-scoped grant read binds the lease's `fleet_id`, no id-normalization bypass). Correctness panel found one real defect (2 findings, same root cause): the mint handler gated **every** integration uniformly, contradicting the lease classifier (`mintableId`, on-demand only) and this spec's own Out of Scope. Impact: (1) the `static` inline-token integration became an unrecoverable dead-end — `static` isn't in `SUPPORTED_SERVICES` so its grant can't be requested, yet the blanket gate demanded one; (2) unknown/typo'd ids were remapped from `UZ-CRED-*` not-connected to a misleading `UZ-GRANT-001`. Fixed (commit `ff7b66cd`): `isOnDemand()` guards the grant check — github/zoho/jira/linear gated, static + unknown fall through. Mint grant tests rewritten onto github + the `FakeGitHub` broker (the static tests now double as proof that static mints with no grant); `isOnDemand` unit test added; red-green re-proven; full integration + lint + memleak re-green.
...
- **VERIFY notes** — Test Delta: `unit=2335 integration=255` vs baseline `unit=2327 integration=249` — +8 unit (grant-lookup pure tests, supported-services, `isOnDemand`, `customEndpoint` split), +6 integration (4 grant + 2 pre-existing mint suites reclassified under the `integration:` name prefix the tier counter keys on, commit `061200b9`). R5's authored grep was refined at VERIFY (broad `integration_grants` pattern matched comments/route-enum names/CASCADE docs — verified one-by-one as non-readers before narrowing to `FROM core.integration_grants`). S1/S2 graded via the repo's real target names (`test-unit-agentsfleetd`/`lint-zig`) — the authored `make test`/`make lint` targets don't exist in this repo's Makefile.
- **Deferrals** — none.
