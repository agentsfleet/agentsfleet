...
- **Consults** — Architecture / Legacy-Design / gate-flag triage: the question asked + Indy's decision.
  ...
  - > Indy (2026-07-31): "1. Yes 2. Yes move it to /done in your PR 3. Explain this hearbeat issue" — context: (1) approves deleting the stale `RUNNER_HOST_ID` workflow comment (`.github/workflows/deploy-dev.yml`) — the one CI/CD edit in this branch; (2) directs moving the stale `M147_001` spec to `done/` inside this PR; (3) the heartbeat-vs-PATCH verdict race is explained below and recorded as a follow-up candidate.
  - > Indy (2026-07-31): "i can live with the race you said" — context: the heartbeat-vs-PATCH verdict overwrite (bounded ≤1 beat, self-healing) is ACCEPTED as-is; the `policy_version` no-op-the-stale-writer guard stays a follow-up candidate, not M148 scope.
  - > Indy (2026-07-31): "Is the backfill onetime? if so remove it? and we can run a manual patch? … yes done by hand" — context: migration 042's degraded backfill `UPDATE` is REMOVED on Indy's reaffirmed direction, over Orly's recorded pushback (the backfill closed the review's fail-open window for dormant rows and not-yet-upgraded daemons, which read healthy while carrying no assignment until repaired). The manual statement is documented in `042_runner_assigned_policy.sql` and proven against the real schema in `assigned_policy_integration_test.zig`; a live upgraded host still converges by itself on its first beat. The REVIEW note above ("042 backfills pre-policy rows degraded") stands as history of what the review added — this entry supersedes it.
