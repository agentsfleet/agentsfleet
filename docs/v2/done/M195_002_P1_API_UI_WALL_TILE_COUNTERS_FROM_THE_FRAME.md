...
**Test Baseline:** `unit=5920 integration=445` — `make test-unit-all` (Rust 2501 + app 2713 + 146 + 560) and `make test-integration-rustd` (445 passed), both exit 0 at `a4e0ef2bd`, measured in the base checkout sitting at that exact revision. Final: `unit=5966 integration=453` (Rust 2510 + app 2746 + 146 + 564; 453 passed), both exit 0.
...
- **REVIEW (Sep 11, 2026) — four findings taken, three declined with reasons.** Taken: the counters statement binds `workspace_id` beside `ANY(fleet ids)` like every sibling statement, and `WorkspaceFleets::counters` takes the workspace (security, defence in depth — the set was server-derived, never client-supplied); the store guards the hello map and each pair at the trust boundary (`Number.isSafeInteger`, non-negative, subscribed keys only) so a `null` entry cannot throw halfway through a greeting; the store keeps the GREATER of the standing and carried figures, because both counters only grow and a frame that crossed a `hello` in flight would otherwise walk the tile backwards; a `catching_up` re-announces the set with fresh counters, since the dropped frames are the ones that moved them. Also taken from the performance pass: the receive, continuation, park and resolve read on the connection their own write held (no second acquire on the hot path), and the sweep reads once per distinct fleet. Declined: the per-`hello` read is uncached and per viewer by design (hello fires on connect and on a set change, never on a steady tick; a reconnect storm is bounded by the pool's acquire budget and degrades the greeting, never the stream); the hello carries the whole workspace set, accepted at the production fleet ceiling and bounded on the client by the subscribed set; the two correlated subselects against `LEFT JOIN LATERAL` stay as the `sql.rs` module note measured them. The wall's refused-counters arm has no test: the router has no `WorkspaceFleets` fake by design, and the unit-lane fixture's unreachable pool refuses at the enumeration before a greeting is built (`needs-infra`).
