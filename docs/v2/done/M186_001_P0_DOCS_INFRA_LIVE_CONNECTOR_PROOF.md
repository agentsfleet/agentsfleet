...
**Workstream:** 001
**Date:** Jul 20, 2026
**Status:** SUPERSEDED by M187_001 — closed to `done/` Sep 03, 2026 as the record, not as work. §1–§5 never ran, and their Files Changed names `src/agentsfleetd/**` CREATE rows that M187_001 §5 deletes. The dimensions moved (§0 → M187_001 §3.1–§3.4 · §2–§4 → §3.5–§3.6 · §5 → §1.4); this file stays as the record. The IN_PROGRESS history below is preserved as history, not as current state.
...
> **Closed to `done/` (Sep 03, 2026).** The reactivation clause above never fired: by the time the Rust daemon was ready to replace the Zig one, M187_001 had taken this scope and re-planned it onto `rustd/crates/afd_connector/**`. Reactivating this file would land Zig into a tree M187_001 §5 deletes, so there is no condition left that returns it to `active/`. It moves out of `pending/` because `pending/` means scheduled work, and every dimension here is now M187_001's. The file is retained whole — the dated quotes and the incident history are why.
