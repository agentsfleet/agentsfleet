...
- **Dimension 1.7** — an eager ship rides a deadline capped at the staleness window, so it can never block the read loop (and its renewal ticks) longer than batching would have; cap-triggered flushes keep the full deadline → Test `test_eager_deadline_capped_at_window` — **DONE**
...
| 1.7 | unit | `test_eager_deadline_capped_at_window` | eager ship → deadline capped at `EAGER_DEADLINE_CAP_MS` (= staleness window), caller deadline restored after the ship |
