...
**Test Baseline:** unit=6665 integration=522 on the branch, both lanes exit 0, 0 failed. At the comparison revision: unit 6654 passed / 0 failed / 532 ignored, integration 511 + 1 exclusive / 0 failed. Delta +11 unit passed, +10 unit ignored, +10 integration passed — the ten new integration tests are `#[ignore]`d, so they are compiled and counted by the unit lane and executed by the integration lane.
...
- **Review finding: `fleet_name` changes what a purge leaves behind (Sep 20, 2026)** — the adversarial pass found that slot 915's prose argued erasure-neutrality for `fleet_id` ("an opaque identifier ... not customer content") and never made the argument for `fleet_name`, which is operator-authored. Before this slot a purge erased the name outright: the `core.fleets` row went and the foreign key nulled the only other column naming it. It now outlives the fleet and, since schema/710 grants no DELETE, leaves only with the tenant. Behaviour kept and the argument written into the slot: what survives is bounded and authored rather than observed — a 64-byte lower-case kebab slug `FleetName::parse` refused anything else from (`afd_fleet_runtime/src/name.rs:24`) — it is the same string `/fleets` already returns to the same authenticated reader, and a charge the wallet was debited for has to say what it paid for. A redaction path that nulls `fleet_name` on purge while keeping `fleet_id` was the alternative; it partly undoes the "Both" decision and was not taken.
