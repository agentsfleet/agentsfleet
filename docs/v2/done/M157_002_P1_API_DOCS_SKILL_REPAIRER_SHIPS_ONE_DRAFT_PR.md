...
**Test Delta (VERIFY):** unit=3512 integration=589 — +19 unit, +2 integration against the CHORE(open) baseline
...
| `src/agentsfleetd/state/sql.zig` | EDIT | The store's SQL constants (insert, branch lookup, deploy-status stamp) |
| `src/agentsfleetd/http/handlers/webhooks/github_repair_link.zig` | CREATE | The two webhook arms' bodies (repair-branch PR opened → linkage insert; linked-branch `workflow_run` → deploy stamp) — `github.zig` stays thin routing |
| `src/agentsfleetd/db/index_usage_integration_test.zig` | EDIT | The branch's two discretionary indexes join `DECLARED_INDEXES` with their named readers |
| `src/agentsfleetd/fleet_runtime/webhook/normalizer/github_app.zig` | EDIT | REVIEW fix: repair-branch traffic is dropped on the default ingress route too, so a fleet cannot ingest its own failed repair as a fresh incident |
| `src/agentsfleetd/fleet_runtime/webhook/normalizer/github_app_test.zig` | EDIT | The guard's three cases: failed repair run, the crew's own draft Pull Request, and an ordinary branch still accepted |
...
|---|--------------------------------|---------------------|----------|----------|-----------------|
| R1 | Write kind parks past both holes (§1) | `zig build test-integration-bin -Dtest-filter=write_kind && zig-out/bin/agentsfleetd-integration-tests` | both park tests' OK lines | P0 | ✅ `131/840 test_write_kind_parks_without_gates_config...OK` + `132/840 test_write_kind_ignores_rule_fallthrough...OK` + `133/840 test_write_kind_park_records_the_row_the_mint_reads...OK` (post-REVIEW run; full suite with per-test lines — the filter recipe is the vacuous-pass trap) |
| R2 | Diff stays inside Files Changed | `git diff --name-only origin/main...HEAD` | 0 paths missing from the table | P0 | ✅ 0 missing after the Aug 10 table repair (phantom `repair_pr_links_test.zig` row removed; `state/sql.zig`, `github_repair_link.zig`, `index_usage_integration_test.zig` rows added); the spec's own lifecycle file and the M157_001 `active/`→`done/` move are the two conventional exemptions |
| R3 | Unapproved mint cannot write (§2) | `zig build test-integration-bin -Dtest-filter=write_mint && zig-out/bin/agentsfleetd-integration-tests` | refusal tests' OK lines | P0 | ✅ `579/839 test_write_mint_refuses_unapproved...OK` + `580/839 test_write_mint_refuses_binding_drift...OK` (full-suite run) |
| R4 | No token names workflows | `rg -n '"workflows"' src/agentsfleetd/credentials/` | 0 matches in emitting code (test fixtures exempt) | P0 | ✅ 0 matches |
| R5 | Linkage round-trips (§4) | `zig build test-integration-bin -Dtest-filter=repair_link && zig-out/bin/agentsfleetd-integration-tests` | insert + stamp OK lines | P0 | ✅ `794/840 test_pr_opened_arm_inserts_once...OK` + `795/840 test_deploy_stamp_and_unknown_branch_noop...OK` + `796/840 test_repair_link_store_immutability...OK` (post-REVIEW run; the stamp test now also proves a same-named branch in another repository does NOT stamp) |
| R6 | Scenario doc carries the shipped design (§5) | `rg -in 'daemon applies\|git revert\|rolls? back' docs/architecture/scenarios/production-deploy-repair.md` | 0 matches | P0 | ✅ 0 matches |
| R7 | Crew uploads through the shipped endpoint (§3) | e2e `test_crew_folder_two_member_onboard` | OK line | P1 | ✅ `417/839 test_crew_folder_two_member_onboard...OK` |
| R8 | Workflow-secrets audit recorded (§6) | PR Session Notes section present | per-workflow inventory listed | P1 | ✅ drafted in the Session Notes (§6 block: `dry.yml`/`dry-smoke.yml` `OP_SERVICE_ACCOUNT_TOKEN` finding); lands with the PR |
| S1 | Unit tests pass | `make test` | exit 0 | P0 | ✅ `✓ All unit lanes passed` (`make test-unit-all`, the superset lane, solo Aug 10 post-REVIEW; zig lanes clean, Bun 1430/0, vitest 501/501) |
| S2 | Lint clean | `make lint-all` | exit 0 | P0 | ✅ `✓ All lint checks passed`, exit 0 |
| S3 | Integration passes | `make test-integration` | exit 0 | P0 | ✅ post-REVIEW, solo from a clean reset: `833 passed; 7 skipped; 0 failed` of 840 per-test OK lines, exit 0 (`make test-integration` itself also green; the direct binary run is cited because the make lane prints no per-test lines and a summary you cannot see is not evidence) |
| S5 | No leaks | `make memleak` | exit 0 | P0 | ✅ `✓ memleak gate passed (agentsfleetd + runner + lib lanes + boot→drain lifecycle)`, exit 0 |
| S6 | Cross-compile | `zig build -Dtarget=x86_64-linux && zig build -Dtarget=aarch64-linux` | exit 0 | P0 | ✅ both targets built, exit 0 |
| S7 | No secrets | `gitleaks detect` | exit 0 | P0 | ✅ `no leaks found` (4226 commits scanned) |
| S8 | No oversize NEW source file (RULE FLL scope) | `git diff --diff-filter=A --name-only origin/main...HEAD \| grep -v '\.md$' \| xargs wc -l 2>/dev/null \| awk '$1>350 && $2!="total"'` | no output | P0 | ✅ no output — all 7 branch-added files ≤350; command amended Aug 10 to RULE FLL's enforced new-file scope (the old spelling flagged five pre-existing test files this branch only grew; spec amends to the rule per "rules are the constant") |
| S9 | Orphan sweep | Dead Code Sweep greps | 0 matches | P0 | ✅ N/A per Dead Code Sweep section — no files deleted; `repository_binding` gained its consumer |
...
- **VERIFY findings (Aug 10)** — (1) The handed-off 16-failure lane decomposed into 1 real (the two new indexes absent from `DECLARED_INDEXES`; fixed `278753ffb`) + 15 phantoms from a concurrent orphaned test process clobbering the shared test Postgres during the Aug 08 capture — every phantom found its own outcome already committed (append-only history preserved the twin's rows as 409s). Solo from clean resets the suite is green twice over: direct `832 passed; 7 skipped; 0 failed` and canonical `34/34 steps; 832/839 (7 skipped)`. (2) Harness observation, report-only: all Zig test binaries exit abnormally under `zig build --listen` AFTER reporting full results — zig prints `failed command:` on green runs while counting the step succeeded; cosmetic, harness-wide, predates this branch. (3) Files-Changed table repaired: phantom `state/repair_pr_links_test.zig` row removed (file never existed — the store's tests live in `webhook_http_integration_test.zig`); `state/sql.zig`, `webhooks/github_repair_link.zig`, `db/index_usage_integration_test.zig` rows added. (4) S8 amended to RULE FLL's enforced new-file scope (rules are the constant).
- **REVIEW round (Aug 10)** — six independent passes over the diff (testing, maintainability, security, data-migration, an adversarial reader, and a cross-model reader). No boundary bypass was found in the three fences; the park cannot be reached around, the mint reads only durable rows, and no token path can name `workflows`. Nine defects were real and are FIXED in this branch: (1) the park staged the Slack card BEFORE the durable row and swallowed a failed insert, so a Postgres blip shipped an answerable card whose answer could never authorise a token — `recordGatePending` now propagates, the row is written first, and the park fails closed like its Redis legs; (2) the write gate keyed on the binding alone, so a write fleet minting Slack or Zoho was refused on a repository decision its token could not act on — now scoped to the GitHub integration; (3) the gate lookup took the newest gate of ANY kind, so a later grant card for the same event permanently shadowed an approved write answer — the kind moved into the `WHERE` clause; (4) same-millisecond `created_at` ties resolved arbitrarily — `id DESC` settles them; (5) an approval stamped after its own card expired could still mint (the sweeper runs on an interval, so a stale click wins that window) — the mint now refuses an answer later than its deadline; (6) the deploy stamp matched on `(fleet, branch)` only, so a run finishing on an identically-named branch in ANY other repository stamped this incident's outcome — `repository` is now in the predicate, with a regression test; (7) schema 830's content-frozen trigger omitted `id` from its frozen set; (8) RULE NDC — `lookupByEvent`/`Link` had no production reader and are removed, the integration test reads the row itself; (9) the durable park→row→mint seam had NO test (every release test resolves through Redis, so a malformed insert would keep all 832 green while every production mint refused forever) — `test_write_kind_park_records_the_row_the_mint_reads` now asserts kind, status, event id and stated binding on the row the daemon itself wrote.
- **REVIEW fix 10 — the ingress recursion (Aug 10)** — verified rather than inherited: `/v1/ingress/github` never calls the repair interceptor, and the `github_app` normalizer filtered only on `conclusion == failure`, so a FAILED workflow run on a repair branch normalized into an ordinary incident and woke the fleet to investigate its own repair — a fresh approval card per cycle, each one a human's time. The normalizer now drops repair-branch traffic (both the workflow run and the crew's own Pull Request) on that route as well, which is §4's stated invariant — *repair-branch traffic is linkage traffic, never incident traffic* — finally holding on the path most installations actually use. The loop was human-gated even before this (every cycle needs an approved gate to mint), so it could not run away silently; it was card spam and operator confusion, not autonomy.
- **REVIEW deferrals (Aug 10, agent-identified, no Indy ack sought — none is claimed as "deferred scope")** — these are recorded as follow-up work, NOT as delivered Dimensions: the default `/v1/ingress/github` path still records no `repair_pr_links` rows for standard GitHub App installations (the linkage arm lives in the per-fleet manual webhook handler, and giving the ingress route one needs fleet/workspace resolution it does not currently do) — the deploy stamp is therefore absent rather than wrong on that path; the deploy stamp is last-write-wins across every workflow on the branch with no workflow identity or monotonic run id, and a run completing before the Pull Request opens drops its result entirely — both want append-only run rows rather than a mutable pair; the branch-derived incident id is not checked against a real event and Pull Request provenance (author, installation, fork-vs-base) is unverified, though unguessable UUIDv7 event ids plus first-writer-wins make the race unwinnable in practice; an approved gate has no spend ceiling, so retries of the same event mint repeatedly against one answer; the kind-park deliberately skips the anomaly counter, which the spec's §1 accepts by naming the human the brake, but at card-storm scale alert fatigue is the failure mode. Diff-scoped mutation testing did not run — the repository carries no Zig mutation harness.
- **§6 correction (Aug 10)** — the Aug 08 audit line overstated the finding and is corrected here. `OP_SERVICE_ACCOUNT_TOKEN` is supplied in the 1Password step's own `env:` block, so it is step-scoped; `export-env: true` exports only the resolved `op://` values. The service-account token therefore never reaches the branch-controlled `make dry-app` step — what does reach it is the dev-tier `CLERK_SECRET_KEY`. Dropping the secrets outright would break the lane: `@clerk/nextjs` ≥7.4 hard-throws without both keys (`ui/packages/app/playwright.config.ts:55`), `proxy.ts` runs `clerkMiddleware`, and the dev-server health check is the Clerk-rendered `/sign-in`. `export-env: false` buys nothing because the step needing the key IS the branch-controlled one. Making the lane secretless (placeholder Clerk keys) is the real fix and needs an experiment, not a blind edit; no workflow file is modified by this branch.
- **Skill-chain outcomes** — `/write-unit-test` + `/write-integration-test` audits complete (Aug 10): diff ledger 23/23 resolved — 22 tested, 1 explicit won't-test (`parkWriteKind` `.unreadable` wait branch mirrors the pre-existing rules-path discipline; forcing it needs Redis-corruption injection infra); negative-path >50% on changed surface; UZ-REPAIR-010/011/012 each asserted by code; diff-scoped mutation testing flagged as not-run (no Zig mutation harness in repo — follow-up-scale). Full ledger in PR Session Notes. gstack `/review` + `kishore-babysit-prs`: populated below at CHORE(close).
