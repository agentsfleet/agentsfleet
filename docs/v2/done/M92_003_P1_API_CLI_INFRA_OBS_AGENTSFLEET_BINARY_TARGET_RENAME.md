...
**Goal (testable):** after merge and the verified image push, `zig build` emits `agentsfleetd` (and the runner graph `agentsfleet-runner`) with no zombie-named binary in any build output, the npm package installs an `agentsfleet` bin, every renamed make target resolves and the full suite is green in CI, `docker manifest inspect ghcr.io/agentsfleet/agentsfleetd:dev-latest` succeeds, and a repo-wide word-boundary grep for `zombiectl|zombied|zombie-runner` matches only the eval-pinned keep ledger.
...
`Dockerfile`, compose names, fly `image` lines. Indy builds from this branch and pushes `ghcr.io/agentsfleet/agentsfleetd` (the runner ships as a baremetal binary via `deploy.sh` — it has no container image; the earlier two-image phrasing was over-specified); merge waits on manifest verification so deploys never pull a missing image.

- **Dimension 7.1** — DONE — compose/fly/Dockerfile flipped; `app =` lines byte-stable (E3 baseline + spec-prose delta only) → Eval `E3` + `E1`
- **Dimension 7.2** — new-name image pushed and verified (e2e, manual-verified: Indy push, agent `docker manifest inspect`) → Eval `E6` output in PR body
...
Locked surface — changes here require amending this spec: fly `app` values; API endpoints and `x-usezombie*` headers; `config.ts` constant names. `INSTALL_COMMAND` flips exactly once, only after Eval `E10` verifies, with its `config.test.ts` pin updated in the same edit. The new names — `agentsfleet`, `agentsfleetd`, `agentsfleet-runner`, `ghcr.io/agentsfleet/agentsfleetd`, compose `agentsfleetd-api`/`agentsfleet-postgres`/`agentsfleet-redis`, `agentsfleet-runner.service`, `@agentsfleet/design-system`, `agentsfleet-app`, `agentsfleet-website`, `@agentsfleet/cli`, install host `agentsfleet.dev` — each change exactly once, in this spec.
...
# E6: Image pushed (Indy step; run after push — agentsfleetd only; the runner is a baremetal binary, no image)
docker manifest inspect "ghcr.io/agentsfleet/agentsfleetd:dev-latest" >/dev/null && echo "agentsfleetd OK"
...
- GitHub org/repo URL refs (`github.com/usezombie/...`) — the org+repo rename happened upstream (Indy, Jun 12, 2026; now `github.com/agentsfleet/agentsfleet`); GitHub serves redirects, so existing URL refs keep resolving and the residue sweep rides M92_004. Only the ghcr namespace (no redirects — proven `manifest unknown`) was forced into this spec.
