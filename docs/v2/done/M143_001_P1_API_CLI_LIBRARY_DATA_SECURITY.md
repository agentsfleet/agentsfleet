...
### Files Changed — amendments (review decisions D1–D4 and the detail route strip)

Owner-decided (§Discovery): the four decisions the post-#564 review left open, the
three tests it mandated, and the removal of the workspace fleet-library detail
route no product consumer was ever built for.

| File | Action | Why it was required |
|------|--------|---------------------|
| `src/agentsfleetd/state/model_library_cache.zig`; `state/model_library_cache_test.zig` | EDIT | `Cache.clear()` added and the module doc corrected: a bump clears the table only on the MUTATING replica (siblings age superseded pages out under bucket pressure), and the honest worst case is ~45 KB/page × 256 slots ≈ 11.5 MiB — the removed byte ceiling's arithmetic had presented the fixture-median as the bound. Clear test added. |
| `src/agentsfleetd/http/handlers/admin/model_library_admin.zig`; `handlers/admin/model_library_admin_integration_test.zig` | EDIT | `invalidateRates()` becomes `invalidateCaches(hx)`: the three committed-mutation sites clear the page cache in the same step as the rate cache. Integration test proves clear-on-commit and no-clear-on-409. |
| `src/agentsfleetd/queue/redis_fleet.zig` | EDIT | The `NOGROUP` repair recreates the group at the stream's END (`$`), not `0`: `0` re-delivered up to ~10k already-XACKed entries — historical agent runs re-executing with real provider spend and connector writes. The write path keeps the beginning via the unchanged `ensureFleetConsumerGroup` (fresh stream — the spellings coincide). Doc rewritten without "transparent"/"lossless". |
| `src/agentsfleetd/fleet/assign_ready_integration_test.zig`; `fleet/assign_ready_faults_integration_test.zig`; `fleet/event_lifecycle_integration_test.zig` | EDIT | Repair tests reshaped to `$` semantics: the repairing poll answers no-work, the repair fires exactly ONCE (`INFO commandstats` witness, hoisted to the shared base), does not re-fire on later polls, and a post-repair publish leases. The stranded pre-repair entry is skipped BY DESIGN. New non-`NOGROUP` classifier test: a `WRONGTYPE` at the stream key propagates unrepaired — no create loop. |
| `src/agentsfleetd/auth/middleware/runner_bearer.zig` | EDIT | The DB-failure→`UZ-AUTH-004` classification gains its test — `MockLookup.return_err` was dead scaffolding. A misclassification walks a healthy runner into the consecutive-auth-reject shutdown, which is the property the token-cache deletion rests on. |
| `src/agentsfleetd/state/tenant_billing_rates.zig`; `state/tenant_billing_edge_test.zig`; `fleet_runtime/metering.zig` | EDIT | The reachable `std.debug.panic` on an uncatalogued model becomes `error.ModelNotPriced`: an admin DELETE of a non-default row plus a tenant still naming the model is an operational state, not a programmer bug, and the free-trial short-circuit masks it only until the trial ends. `computeStageChargeAt` (injected clock) makes the refusal testable before that date; the estimate gate fails open, renew/settle closed. |
| `src/agentsfleetd/http/handlers/tenant_model_entries_view.zig` | EDIT | Double-free disarmed: `source_default` is nulled once its strings transfer to `platform_default`, so exactly one errdefer owns them on a late failure. |
| `src/lib/common/cache_table_test.zig` | EDIT | The same-key re-put comment stated the opposite of its code (that bucket was full, so the put rotates and frees); corrected, and the duplicate-resident-at-clear hazard now has its own leak-checked test. |
| `src/agentsfleetd/http/handlers/library/gallery_detail.zig` | DELETE | The workspace detail route had no product caller: the `fleet-library.ts` server-brokered consumer this table promised was never built, the CLI reads only the gallery, and the runner materializes support files from object storage. Removed rather than shipped unconsumed. |
| `src/agentsfleetd/http/routes.zig`; `route_table.zig`; `route_table_invoke_library.zig`; `route_scopes.zig`; `route_admission.zig`; `route_template.zig`; `route_trace.zig`; `handlers/library/api.zig` | EDIT | The `workspace_fleet_library_detail` variant unwired — the same exhaustive switches that forced its registration force its removal. |
| `src/agentsfleetd/http/route_matchers_library.zig`; `route_matchers_library_test.zig` | EDIT | Detail matcher and `FleetLibraryDetailRoute` removed (the matcher was dead on `main` — its only caller was this branch's wiring); the tests pin the former URL to "no route". |
| `src/agentsfleetd/http/handlers/library/gallery_keyset_integration_test.zig` | EDIT | The detail status-ladder test becomes the no-route pin: the former URL answers 404 even for a resident entry. |
| `src/agentsfleetd/errors/error_registry.zig`; `errors/error_entries.zig`; `errors/internal_op_error_sweep_test.zig` | EDIT | `UZ-LIBRARY-007` retired with its only producer (RULE ORP); the internal-op ratchet re-tightened 88 → 86 per its own doctrine. |
| `src/agentsfleetd/observability/library_read_counters.zig`; `http/library_page_bounds_integration_test.zig` | EDIT | `FLEET_DETAIL_MAX_*` budgets retired with the route; the bounds suite measures the gallery read only. |
| `public/openapi/paths/fleet-library.yaml`; `public/openapi/root.yaml`; `public/openapi/components/schemas.yaml`; `public/llms.txt`; `public/skill.md`; `public/agentsfleet-manifest.json` | EDIT | The detail operation removed from the OpenAPI document and all three public inventories (`policyClasses` kept in sync); `make check-openapi` green at 77 served routes. |
| `ui/packages/app/lib/types.ts`; `src/agentsfleetd/http/handlers/library/gallery.zig` | EDIT | The `support_files` notes now record the workspace plane carries the manifest nowhere; the admin catalog remains its only reader. |
| `src/lib/common/sync_test.zig` (new); `common/sync.zig`; `src/lib/tests.zig` | CREATE/EDIT | The review found `sync.zig` had zero tests and no test-root registration — the RwLock non-serializing-readers claim (the page cache's central property) is now proved with a second thread and the deterministic `Event` handshake, plus writer exclusion, `WaitGroup` reuse, and `Event` timeout coverage. |
| `docs/architecture/scaling.md` | EDIT | Named the deleted `auth/runner_token_cache.zig` as live and claimed zero Postgres round-trips per poll; rewritten by the review fixes, idle-cost table included. |
| `src/agentsfleetd/http/handlers/model_library.zig`; `state/model_library_store.zig` | EDIT | The two function-length findings that were this branch's own: `innerGetModelLibrary` (52) sheds the revision-read into `readRevisionOrFail`; `listLibraryPage` (51) sheds the row projection into `projectLibraryRow`. |
| `src/agentsfleetd/types/model_identity.zig`; `docs/architecture/billing_and_provider_keys.md`; `state/tenant_billing_edge_test.zig` | EDIT | Three stale comments the review flagged: a deleted function name, a boot-warm claim the rework removed, and a header describing a fn that did not exist (it does now — `computeStageChargeAt`). |
| `src/agentsfleetd/http/handlers/model_library.zig`; `handlers/model_library_page_integration_test.zig` | EDIT | Pre-push review finding: a hand-minted cursor whose id is not a UUID reached the page SQL's `::uuid` cast and surfaced as a 503 + error log any tenant could mint at will; now rejected as `UZ-LIBRARY-001` before the SQL, with a forged-cursor integration case. |
| `src/agentsfleetd/state/model_library/sql.zig`; `fleet_library/gallery_sql.zig`; `state/model_library_store.zig`; `http/handlers/model_library_page.zig`; `handlers/library/gallery.zig`; `handlers/library/query.zig`; `handlers/library/library_query_normalization_test.zig` | EDIT | Pre-push review finding, owner-escalated to the full restructure: LIKE-escaping ran in Zig BEFORE the SQL-side NFKC fold, so compatibility characters that fold INTO `%`/`_`/`\` (fullwidth ％ et al.) arrived as live wildcards — `q=％` matched everything. The pattern is now built in SQL after the fold in BOTH search paths; `likeContains` and its unit tests retired with it (RULE NDC), and the literal-wildcard contract is pinned at the integration tier where the real fold runs, fullwidth case included. |
...
**Amended (§Discovery — detail route strip): the Fleet detail row is retired with the stripped route; `FLEET_DETAIL_MAX_*` left `library_read_counters.zig` with it.**
...
**Amended (§Discovery — detail route strip): only the collection matcher remains; the detail matcher and its tier validation left with the route, and the tests pin the five-segment shape to no-route.**
...
**Amended again (§Discovery — detail route strip): `FleetDetail` is retired with the route. On the workspace plane `support_files` is served nowhere; the admin catalog remains its only reader.**
...
| `UZ-LIBRARY-007` | — | *retired* (§Discovery — detail route strip): left the registry with its only producer | was: non-enumerating 404 for absent-or-foreign detail |
...
| Foreign detail | *retired with the detail route (§Discovery)* — the former URL resolves to no route at all | route probe against a resident entry | 404, unrouted | — | `test_fleet_keyset_and_detail_status` |
...
|---|---|---|---|---|---|
| R1 | Data/security tests pass | `make test-integration` | exit 0 | P0 | ✅ exit 0 on the post-#564 tree carrying the `$`-repair, the D1–D4 fixes, the detail route strip, and the pre-push review fixes (both lanes re-run after the SQL fold restructure). The repair suites now prove no-work-on-repair, exactly-once (server-side `commandstats` witness), no re-fire, and post-repair delivery; the earlier four-round history behind the repair design is in §Discovery. |
| R2 | OpenAPI and CLI agree | `make check-openapi && make test-unit-cli` | exit 0 | P0 | ✅ exit 0 on both, post-strip: route coverage reports 77 served `/v1` routes all documented (the detail operation is gone from both sides together). CLI 1367 pass, 0 fail. |
| R3 | Diff is scoped | `git diff --name-only origin/main` | 0 unlisted paths | P0 | ✅ 0 unlisted across 121 changed paths (spec self-reference excluded) after the D1–D4 + strip amendment table. This re-grade caught `docs/architecture/scaling.md` and the new `sync_test.zig` missing from the tables — added rather than quietly passed. |
| S1 | Unit/lint/conform | `make test-unit-all && make lint-all && make harness-verify` | exit 0 | P0 | ✅ exit 0 on all three on the final tree. `test-unit-all`: 34/34 steps, 1967/2261 passed (294 skipped), runner 379/386, lib 143/143 (the new `sync` suite included), Zig merged-line coverage 61.40% ≥ 60%, app 1926 + website 173 green. `lint-all` needed one fix first — the strip left a dead `matchers_library` import that `zlint unused-decls` caught. `harness-verify`: all gates green. |
| S2 | Memory/build/secrets | `make memleak && zig build -Dtarget=x86_64-linux && zig build -Dtarget=aarch64-linux && gitleaks detect` | exit 0 | P0 | ✅ exit 0 on all four, on the final tree. `memleak` clean across every lane — `agentsfleetd`, `runner`, `lib`, and the boot→SIGTERM→drain lifecycle under the gate against live Postgres + TLS Redis. Both Linux targets cross-compile. `gitleaks`: no leaks (~161.9 MB scanned). |
...
```
Test Delta: unit 3051→3172 (+121) · integration 407→446 (+39) vs CHORE(open)
...
inflated by the `origin/main` merges this branch absorbed (#561, #562, #564),
...
(expiry, removal, sweep) those tests covered; the review round then added back
the `sync` suite, the clear/duplicate-resident tests, the auth-503 and
classifier tests, and the reshaped repair proofs, while the detail strip
removed the status-ladder suite in favour of the no-route pin.
...
### Review decisions D1–D4, and the detail route strip (post-#564)

The post-rework gstack review (4 specialists + a Codex cross-model pass) left four decisions open; Indy answered all four on 2026-07-27, and a fifth finding arrived from a sibling worktree's audit the same day.

- **D1+D2 — `Cache.clear()`, wired to the admin mutation path.** Chosen over a 32 KiB per-entry cap or halving `MAX_ENTRIES`. The 8 MiB ceiling had been removed on wrong arithmetic — the fixture-median 9.2 KB/page was presented as the bound, while the true worst case (`MAX_LIMIT` 100 rows × `MODEL_ID_MAX`+`PROVIDER_MAX` identities) is ~45 KB/page × 256 slots ≈ 11.5 MiB. `invalidateCaches` now clears both caches after a committed mutation, which also makes the module's clear-on-bump documentation true; it previously described a clear nothing called.
- **D3 — the `NOGROUP` repair recreates the group at `$`.** "Keep `0` and fix only the doc" and "split into its own PR" declined. At `0` the repair re-delivered up to ~10k already-XACKed entries — historical agent runs re-executing with real provider spend and Slack/GitHub writes. At `$` nothing historical re-runs; entries appended during the groupless window are skipped — a bounded, re-submittable loss in an already-abnormal state, where re-execution cannot be un-spent. The write path keeps the beginning (fresh stream; the spellings coincide).
- **D4 — both pre-existing bugs fixed in this branch.** The reachable panic (an admin DELETE of a non-default catalogue row plus a tenant still naming that model; masked by the free-trial short-circuit only until the trial ends) became `error.ModelNotPriced`, with a clock-injected negative test that runs today. The `tenant_model_entries_view.zig` double-free is disarmed by nulling the source once its strings transfer. No deterministic test exists for the double-free's error path — that would need failure-injection (tripwire) wiring in the handler, recorded as a known gap rather than silently skipped.
- **The workspace fleet-library detail route is stripped.**

> Indy (2026-07-27): "Strip the route, but why did you build that what is the use case here?" — context: a sibling-worktree audit found `GET /v1/workspaces/{ws}/fleet-libraries/{tier}/{id}` had no product caller.
> Indy (2026-07-27): "I want to stip the workspace fleet-library detail route, why was it added in teh first place? what was the plan for /{tier}/id url?" — the decision reaffirmed after the plain-terms explanation.

  The route existed as the shed-target for `support_files` off the gallery card and for a dashboard click-through (`fleet-library.ts` server-brokered detail) this spec's Files Changed promised but no session ever built. The audit's caller sweep held up: no UI path, no CLI path (the install preview reads `requirements` from the card, which the card kept), no runner path (support-file bytes come from object storage via the lease's bundle hash), and the admin dashboard reads the manifest from the admin route. Two of the audit's claims did not hold and are corrected here: the detail route WAS integration-tested end-to-end (the status ladder + a 200 carrying `support_files`), and the card never shed `requirements` — only `support_files` moved. Consequences recorded through this spec: Dimension 3.1's detail half and Dimension 4.2's detail surfaces superseded, the former URL pinned to no-route by the keyset suite, `UZ-LIBRARY-007` retired with its only producer, the Fleet-detail budget row retired, the OpenAPI document and all three public inventories dropped the operation, and `matchWorkspaceFleetLibraryDetail` — dead on `main` today, its only caller being this branch's wiring — went with it.
- **The pre-push gstack `/review` round (post-rubric).** The specialist army was
  partially dispatched: the security pass completed with three informational
  findings (two actioned — the forged-cursor 503 and the NFKC/LIKE fold gap —
  and one left in the appendix at confidence 4: the documented HMAC zero-key
  fallback in `catalogue_key.zig`, near-unreachable and recorded in-code); the
  testing pass died on an API transport error; the owner interrupted the
  remaining four passes — accepted as sufficient given the full 4-specialist +
  Codex round the rework had already received. On the fold gap the owner
  escalated past the recommended codepoint blocklist:

> Indy (2026-07-27): "Restructure: escape after fold in SQL" — the full-completeness option; both search paths now build their LIKE pattern in SQL after NFKC.
