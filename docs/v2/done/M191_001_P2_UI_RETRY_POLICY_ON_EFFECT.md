...
| `lib/api/errors.ts` | EDIT | `HTTP_STATUS_REQUEST_TIMEOUT`, `RETRY_CODE_TIMEOUT` and `isDefiniteRefusal` move beside `ApiError`: status facts a client component reads without the policy's dependency; `retry.ts` re-exports them |
...
| `lib/api/client.ts` | EDIT | the send and the read each classify their own failure with what only that side knows (whether headers arrived); every request runs under the policy, a write with one attempt, so a cancel lands the same way; no API change |
...
| `lib/api/retry.schedule.test.ts` | CREATE | the schedule's own guarantees: deadline, cap, full jitter, provenance, interruption, telemetry, configuration |
...
| `lib/api/client.retry.test.ts`, `lib/api/client.defaults.test.ts`, `lib/api/approvals.resolve.test.ts`, `lib/errors.test.ts` | EDIT | read the status facts from `lib/api/errors`; `client.retry.test.ts` also proves the terminal status through the transport and the bound option signal |
...
- **Dimension 1.2** — DONE — attempts stop when the total deadline is reached even though the attempt ceiling is not → Test `a read gives up at the deadline, not at the attempt count`
...
lib/api/errors.ts declares HTTP_STATUS_REQUEST_TIMEOUT, RETRY_CODE_TIMEOUT, isDefiniteRefusal; retry.ts re-exports them
```
