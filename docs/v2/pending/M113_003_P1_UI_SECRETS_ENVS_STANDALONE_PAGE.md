...
### §1 — Restore `/credentials` as a real page + its own nav entry
...
### §2 — Revive `CredentialsList.tsx` as the standard list
...
### §3 — Add Secret via dialog, not an always-inline form
...
- [ ] `/credentials` renders standalone content — verify: `make test-unit-app`
- [ ] Sidebar shows "Secrets & ENVs" — verify: `make test-unit-app` (Dimension 1.2)
- [ ] `CustomSecretsList.tsx` fully removed — verify: `test ! -f ui/packages/app/app/(dashboard)/credentials/components/CustomSecretsList.tsx`
- [ ] Zero remaining references to the deleted component — verify: Eval Command E8
