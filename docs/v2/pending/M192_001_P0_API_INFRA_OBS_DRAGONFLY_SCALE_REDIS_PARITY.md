...
Use canonical receipt/ledger atomicity, zero charges, renewal policy, gates and counters; no second billing marker. Prove cross-fleet budget enforcement and wallet/ledger rollback together. Missing/mismatched import receipt closes all admission paths. Deliver operator-only empty-source receipt initialization here for owned local/Cloud fixtures; refuse nonempty/wrong-target state. §7 extends the same tool with populated import.
...
Dependencies: §6; extend §2's empty-source initializer with populated import. The offline tool reads old Redis state; the new daemon accepts only Dragonfly clusters. Rehearse the one coordinated durability/Swarm cutover.
...
Every prefix has counts/types and disposition; unknown keys block. Count auth sessions/nonces only, exclude them from export/import and restart in-flight flows. Per Indy's pre-production override, cancel before schema changes; after the first new migration commits, recover forward with the new build. No reverse migration or old-build boot proof.
...
- **Dimension 7.1**: rehearsal preserves source state through pre-migration cancellation, import, and forward recovery → Test `test_cutover_recovery_preserves_source_state`.
...
| 2.3 | integration | `test_durable_store_failure_refuses_acceptance` | Absent/incomplete/wrong-target/unreadable import receipt and failed commit yield retryable refusal, readiness false, no ingress success, lease, background dispatch or device/connect auth; daemon cannot self-complete receipt; the §2 operator initializer authorizes an owned empty fixture and refuses nonempty/wrong-target state before §5/§6 tests. |
...
| 7.1 | integration | `test_cutover_recovery_preserves_source_state` | Fresh/populated upgrades and interrupted forward recovery converge; cancellation before schema changes preserves the old deployment, while committed new migrations retain their ledger and require a compatible new build. Auth counts only: no import, old flows reject/new flows pass, issued credentials/grants survive; work/claims/mirrors/billing reconcile; unknown keys, detected billing damage without disposition and overlapping writers fail; fence-window provider redelivery preserves deduplication. |
