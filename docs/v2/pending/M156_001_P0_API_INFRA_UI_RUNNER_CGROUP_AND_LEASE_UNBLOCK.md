...
### §3 — A failed config load names its cause

The failure that took the dev fleet down logged an error code and nothing else, which is why diagnosis needed a host login rather than a journal read. An error that is caught must be named. **Implementation default:** log the error name alongside the existing code and keep the returned error unchanged, because callers already branch on it.
...
- **Consults** — Architecture: `docs/architecture/runner_fleet.md` §Capability flows up states the daemon probes `subtree_control` at startup; today's lazy enablement contradicts the doc, and §1 closes the divergence in the code rather than amending the doc. Gate-flag triage: none yet.
  ...
  - **Open — needs Indy's ack before EXECUTE:** `.github/workflows/deploy-dev.yml` is a CI/CD path and edits there are forbidden without explicit approval. The pre-deploy probe in §5 requires one.
