...
- **Handshake** — restated: *`FleetBudget` is dead config on a cost path. Make it live at the two points where the platform already asks "may this run proceed?", give the refusal its own durable label so triage is unambiguous, and change nothing else about how runs are metered, billed, or reported.*
  ...
  4. **The gate is fail-open on a database (DB) fault**, mirroring `balanceCoversEstimate` (`metering.zig:77`: *"Any DB failure returns true (fail-open) so the gate never turns into an availability incident"*). A stricter budget gate than the platform's own credit gate would be an inconsistent guarantee.
...
| Stored budget is unparseable | hand-edited `config_json` in the database | renew gate refuses (fail-closed on *invalid data*, distinct from fail-open on *unavailable data*): a ceiling we cannot read is not a ceiling we may ignore (§2.3) |
