...
- **Dimension 6.1** — The architecture doc records the one table shape and the picker's tier label, and the diff carries one new changelog entry → Test `test_architecture_and_changelog_record_the_table_shape`
- **Dimension 6.2** — The `~/Projects/docs` pages showing CLI table output are revised on their own branch → Test `test_docs_branch_carries_the_revised_pages`
...
| 6.1 | unit | `test_architecture_and_changelog_record_the_table_shape` | `docs/architecture/web_app.md` contains the one-table-shape rule and the picker's tier label; the diff against the comparison revision adds exactly one new `<Update>` block. |
| 6.2 | manual | `test_docs_branch_carries_the_revised_pages` | Procedure: in `~/Projects/docs` on `chore/m204-one-table-shape-changelog`, `git diff --name-only main` lists the CLI reference page and `changelog.mdx`. Required person: Indy, who owns that repository's branch. Evidence: the branch name and command output in Pull Request Session Notes. |
...
- **Renaming `schedule rm` to `schedule remove`, or the reverse.** The two verbs for one idea are a real inconsistency, surfaced in Discovery, and renaming either changes a shipped command for every caller and every script — a decision this workstream records rather than takes. Out of scope until Indy picks the spelling.
