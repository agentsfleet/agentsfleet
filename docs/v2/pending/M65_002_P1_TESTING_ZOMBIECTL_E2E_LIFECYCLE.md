...
- `zombiectl/src/**` — no CLI behavior changes EXCEPT the one carve-out for the `help` subcommand route (see Files Changed: `zombiectl/src/program/routes.js`). That route is the smallest possible change to make the help-triplet assertion in §3 true and lands in the same PR. Any other CLI bug surfaced while writing the suite lands in a separate PR.
...
**§4c — Invalid-arg-value matrix (item 10).** With the same `ZOMBIE_TOKEN`, hit every auth-required command that takes a positional identifier with a syntactically-valid but nonexistent value. Implementing agent enumerates from `command-matrix.js`'s `requiresIdentifier` table. Expected coverage at minimum: `zombiectl status zmb_nonexistent`, `zombiectl kill zmb_nonexistent`, `zombiectl stop zmb_nonexistent`, `zombiectl resume zmb_nonexistent`, `zombiectl logs zmb_nonexistent --since 1m`, `zombiectl workspace use ws_nonexistent`, `zombiectl workspace delete ws_nonexistent`, `zombiectl agent delete key_nonexistent`, `zombiectl grant delete grant_nonexistent`. Each exits ≠ 0; the JSON-mode form (`--json`) returns `{error: {code, message}}` with `code` registered in `src/errors/error_registry.zig` (UZ-* surface — RULE ERROR REGISTRY GATE).
...
| `lifecycle-with-token.spec.js §4c → invalid-arg-value matrix with ZOMBIE_TOKEN` | For every row in `REQUIRES_IDENTIFIER`, `runZombiectl([...row.args, "<nonexistent-id>", "--json"], { env: { ZOMBIE_TOKEN, ZOMBIE_API_URL } })` exits ≠ 0; JSON envelope on stdout has `error.code` matching the row's `expectedErrorCode` AND the code is registered in `src/errors/error_registry.zig`. Matrix-driven. (Item 10.) |
