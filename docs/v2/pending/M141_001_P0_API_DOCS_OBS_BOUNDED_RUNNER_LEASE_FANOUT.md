...
| File | Action | Why |
|------|--------|-----|
| `src/agentsfleetd/queue/fleet_ready.zig` | CREATE | Own the readiness index (Redis hash, field=fleet_id, value=monotonic counter): mark by atomic increment, bounded randomized peek returning tokens, and token-guarded compare-and-clear. |
...
- **Dimension 1.3** — concurrent marks for one fleet only ever advance its token: N racing marks leave it strictly greater than before, never regressed to an earlier value → Test `test_concurrent_marks_never_regress_token`
...
fleet_ready (new internal module)
  Backing store: ONE Redis hash — field = fleet_id, value = a monotonic per-fleet
  counter. NOT a set: a set has no per-member value to hold the token, and packing it
  into the member breaks dedup and direct removal.

  Ready = struct { fleet_id: []const u8, token: u64 }

  mark(client, fleet_id)          best-effort, never propagates. Called only AFTER a
                                  successful append. Takes NO caller token: it performs
                                  an atomic per-field increment, so concurrent marks
                                  cannot land out of order and regress the value.
...
| Readiness cleared with work pending | Ingress appends and marks between the two null reads and the clear; ingress holds no claim, so nothing serializes it against the poll | The token-guarded compare-and-clear sees an advanced token and becomes a no-op, leaving the new mark intact so the next poll leases it. Falling back to the sweeper here would be a multi-second pickup delay on a healthy system, not an acceptable degradation. A sweeper re-mark landing mid-drain is the same shape and equally safe. Concurrent marks are covered by the same property from the other side: because the store increments rather than accepting a caller value, no interleaving can regress the token back to one a poll already observed. |
...
| 1.3 | integration | `test_concurrent_marks_never_regress_token` | N concurrent marks for one fleet leave a token strictly greater than the starting value, and no interleaving yields an earlier one. Fails against any caller-supplied token. |
