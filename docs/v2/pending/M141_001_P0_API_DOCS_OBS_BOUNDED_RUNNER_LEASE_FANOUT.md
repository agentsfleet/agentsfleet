...
- **Dimension 1.3** — a token is never reused and never regresses: N racing marks for one fleet leave it strictly greater, and a fleet cleared and then re-marked receives a token strictly greater than the one the clearing poll held, never an equal one → Test `test_tokens_never_repeat_or_regress`
...
fleet_ready (new internal module)
  ...
  mark(client, fleet_id)          best-effort, never propagates. Called only AFTER a
                                  ...
                                  tokens neither regress nor repeat.
...
| Readiness cleared with work pending | Ingress appends and marks between the two null reads and the clear; ingress holds no claim, so nothing serializes it against the poll | The token-guarded compare-and-clear sees an advanced token and becomes a no-op, leaving the new mark intact so the next poll leases it. Falling back to the sweeper here would be a multi-second pickup delay on a healthy system, not an acceptable degradation. A sweeper re-mark landing mid-drain is the same shape and equally safe. Concurrent marks and delete-then-recreate are covered by the same property: tokens come from one shared never-reset sequence, so no interleaving regresses to a value a poll observed and no cleared field can re-mint one. |
...
| 1.3 | integration | `test_tokens_never_repeat_or_regress` | N concurrent marks for one fleet leave a strictly greater token with no interleaving yielding an earlier one; and mark → clear (field deleted) → mark yields a token strictly greater than the cleared one, so a stale compare-and-clear cannot match. Fails against a caller-supplied token AND against a per-field counter. |
