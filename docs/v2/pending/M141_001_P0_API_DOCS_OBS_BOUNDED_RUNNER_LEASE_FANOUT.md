...
- **Dimension 1.3** — a token is never reused and never regresses: N racing marks for one fleet leave it strictly greater; a fleet cleared and then re-marked receives a strictly greater token, never an equal one; and deleting the sequence key out-of-band still yields strictly greater tokens afterwards — including when many marks were drawn inside a single clock tick before the delete, which a bare-timestamp seed would re-issue → Test `test_tokens_never_repeat_or_regress`
...
fleet_ready (new internal module)
  ...
  mark(client, fleet_id)          best-effort, never propagates. Called only AFTER a
                                  ...
                                  sequence seeds from a clock floor SCALED by a per-tick
                                  headroom multiplier — never zero, never a bare
                                  timestamp (many marks fit in one tick and would
                                  overrun the next tick's floor). See §1 for the
                                  residual that scaling does not remove.
...
| 1.3 | integration | `test_tokens_never_repeat_or_regress` | N concurrent marks leave a strictly greater token, no interleaving yielding an earlier one; mark → clear (field deleted) → mark yields strictly greater; and drawing N tokens inside one clock tick, deleting the sequence key, then marking again still yields strictly greater. Fails against a caller-supplied token, a per-field counter, a zero-seeded sequence, AND a bare-timestamp seed. |
