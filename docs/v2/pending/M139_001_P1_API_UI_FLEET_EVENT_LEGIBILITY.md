...
- **Dimension 1.5** — Completion frame carries `failure_label` + `failure_detail`; the dashboard merge renders the real sentence live, no reload → Test `test_stream_merge_renders_failure_detail`
- **Dimension 1.6** — Over-cap and control-character detail is truncated and escaped at the report write; the stored value is what Inspect shows → Test `test_detail_truncated_and_escaped_at_write`
...
- **Dimension 2.1** — Outcome presentation includes the detail when present and exactly the canned sentence when absent → Test `test_outcome_includes_detail_when_present`
...
- **Consults** — Architecture / Legacy-Design / gate-flag triage: the question asked + Indy's decision. Jul 22, 2026: Indy confirmed Full scope (chat + table + backend cause) and the design bracket — `/design-shotgun` before implementation, `/design-review` after.
