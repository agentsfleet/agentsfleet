...
- **Dimension 2.5** — the verify arm fails when the catalogue is empty and passes when it matches the allowlist → Test `test_verify_fails_on_empty_catalogue` — **DONE.** Initially marked DONE on the implementation alone, with no test; `/write-unit-test`'s diff ledger caught that the named test did not exist, and the marking was wrong. Now tested both directions: a stubbed catalogue reporting 0 rows fails AND names the count, and one reporting 7 passes. A non-numeric result also fails, so an unreadable count cannot pass as a populated catalogue.
...
- **Skill-chain outcomes** — `/write-unit-test`, `/review`, `kishore-babysit-prs` results (order per `AGENTS.md` CHORE(close); iteration counts, findings dispositioned).
  ...
  **`/write-unit-test`** — the diff ledger found four spec-named tests that did not exist, including one behind a **falsely marked DONE** (Dimension 2.5, marked on the implementation with no test). Three written, one marked `needs-infra`, the false marking corrected.

  **`/review`** — adversarial pass over the five highest-risk surfaces. Specialist subagents were not dispatched (standing instruction not to spawn agents unprompted); the pass was run directly. Three findings, all self-inflicted, all fixed:

  1. **P1 — the outage preceded the gate that authorises it.** `stop_writers.sh` scales the application to zero as gate step 1, while the typed confirmation lives in `02_teardown.sh` at step 3. Invoking the teardown gate with `ENV=prod` merely to read that prompt — or fluffing the answer — would have taken production down first, and the gate then exits leaving zero machines with no step that restores them. Scaling to zero is its own destructive act, so it now carries its own typed confirmation before scaling, matching `02_teardown`'s shape. Proven by `stop_writers_refuses_without_confirmation`, which asserts a wrong answer scales *nothing*.
  2. **P2 — the new lane could pass vacuously on a stale binary.** `check-migrate-unprivileged.sh` preferred `zig-out/bin/agentsfleetd` when present, so a leftover artifact from an earlier checkout would migrate a different schema than the one under review — vacuous in the one place that must not be. It now prefers building, falling back to the artifact only where no toolchain exists (Continuous Integration's host runner, where the container built it from this same checkout moments earlier).
  3. **Documented, not changed — §1's blast radius.** Sweeping by ownership equals sweeping test fixtures only while fixture users own nothing else; adding one to a shared workspace would empty it, where prefix scoping would have spared it. Bounded by the destructive-target guard (localhost / `*-dev` / e2e only) and by fixture users being provisioned solely from `global-setup`. Recorded in the code so a future change to fixture membership is forced to re-scope the sweep.
