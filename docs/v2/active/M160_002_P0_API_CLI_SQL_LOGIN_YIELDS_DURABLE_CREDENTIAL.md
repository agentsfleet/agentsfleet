...
- **Consult — why `agt_t` is granted in code while `afc_` resolves from Clerk (Aug 13, 2026, §3).** Raised as an apparent inconsistency: if Clerk is the single source of truth for capabilities, a tenant key should read from it too. It cannot. Clerk holds scopes on `public_metadata` of a **user**, and an `agt_t` key has no user — `240_api_keys.sql:23` makes `created_by` a plain provider subject string rather than a foreign key precisely so the key outlives the admin who minted it. Resolving that admin's live scopes would re-couple the two and undo the decision, so a departing admin would silently disarm working automation. Clerk is therefore the source of truth for *people*; a tenant credential is not one, and `scopes.defaultScopes(.tenant_api_key)` grants it a fixed set at principal construction. **The instinct does find a real gap, elsewhere:** `core.api_keys` carries no scopes column at all, so every tenant key holds identical power and a read-only or fleet-only key cannot be minted. The fix is per-key scopes chosen at creation and stored on the row, not Clerk resolution. Left for its own milestone; explicitly not folded in here.
