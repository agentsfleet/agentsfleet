...
| `cli/src/commands/login-helpers.ts` | EDIT | `resolveDirectToken` and `saveDirectToken` deleted with the `--token` flag (§3); the device flow becomes the only writer of `credentials.json` |
| `cli/src/program/cli-tree.ts`, `cli/src/program/handlers-bind.ts` | EDIT | The `--token` option and its binding are removed, so the flag is refused as unknown rather than parsed and ignored |
...
| `cli/package.json`, `cli/bun.lock` | EDIT | Out of scope, folded in on Indy's Aug 13 call (see Discovery): `effect` moves from `4.0.0-beta.103` to `4.0.0-rc.108` |
| `cli/test/login-exchange.unit.test.ts` | CREATE | The load-shape refusals, the mint-response refusals, the machine-name grammar, and the orphan sweep |
...
| `src/agentsfleetd/auth/claims.zig` | EDIT | Out of scope, folded in on Indy's Aug 13 call (see Discovery): the `custom` claim ladder, the `workspaceId` alias, and the `scope`/`scp` fallbacks are removed, so each claim is read from one key (§5) |
| `src/agentsfleetd/auth/oidc.zig` | EDIT | `Provider` collapses to its one dialled variant; the enum survives so `OIDC_PROVIDER` is still validated at boot and no deployment workflow needs editing |
| `src/agentsfleetd/auth/claims_test.zig` | EDIT | The custom-ladder and camel-alias tests go; three negatives arrive, including the one that pins `scopes` over `scope`. The canonical Clerk fixture is corrected — it spelled the claim `scope` and passed only on the removed fallback |
| `src/agentsfleetd/config/runtime_loader_test.zig` | EDIT | The retired provider name must now be refused at boot, and the supported one must still load |
| `docs/AUTH.md` | EDIT | Records the second credential class and how it differs from the tenant key; the tenant-key scope gap is rewritten to lead with Indy's standing decision rather than this agent's rebuttal of it |
...
Out of scope as drafted, folded in on Indy's Aug 13 call (see Discovery). `clerk_metadata_payload.zig` is the only thing that writes our `public_metadata`, and it writes exactly `tenant_id` and `scopes`. The readers accepted far more: a whole second ladder behind `OIDC_PROVIDER=custom` reading `custom_claims` / `app_metadata` / namespaced keys, a `workspaceId` camelCase alias, and three spellings of the capability claim. The capability one was live and mis-ordered — it tried OAuth2's `scope` **before** our `scopes`, so a token carrying both would have supplied a capability set we never provisioned, on the authorisation path. The suite's own canonical Clerk fixture spelled it `scope` and passed on that fallback, which is how it went unseen. **Implementation default:** each claim is read from exactly one key, because a reader that accepts a fact from wherever it might appear cannot say which value it trusted.

- **Dimension 5.1** — a token carrying both `scope` and `scopes` resolves to `scopes` → Test `test_oauth2_scope_cannot_displace_provisioned_scopes` — DONE
- **Dimension 5.2** — `scope` and `scp`, alone or as arrays, yield no capability rather than a fallback → Test `test_retired_scope_spellings_are_unread` — DONE
...
- **Consult — the local CLI lane cannot see a coverage regression (Aug 13, 2026, EXECUTE).** `fc46221a4` pushed green locally and failed `test-unit-cli` in Continuous Integration (CI) with zero failing tests. The job was refused by the coverage floor: 99.96% line against a 100% floor, naming `login-exchange.ts:105-107` — the `Effect.fail` that `decodeMinted` feeds when a mint response is not a credential. That branch is the parse boundary Dimension 2.5 hardened, and nothing exercised it. The blind spot is structural, not an oversight in the run: `make test-unit-cli` invokes `bun test`, while the CI job invokes `bun run test`, which is `bun run build && bun ./scripts/enforce-coverage.mjs`. Only the second grades coverage, so a fully green repository lane is not evidence about that job. Closed here by testing the eight shapes that decode to nothing usable; the lane divergence itself is left open and named for Indy, since aligning the make target changes what a gate enforces.

- **Scope decision — `--token` is removed rather than repaired, Aug 13, 2026 (§3).** The finding below proposed gating the direct-seeding write with the loader's own shape check. Indy asked the prior question instead: if `AGENTSFLEET_API_KEY` already carries a tenant key on every request and takes precedence over the stored credential, what is the flag for? Nothing that the environment variable does not already do — it is a second path to the same outcome, it writes a credential to disk that the caller must then manage, and it is the only path that can write a value the loader will later refuse. Deleting it removes the defect instead of patching it, and leaves one rule: interactive is the device flow, unattended is `AGENTSFLEET_API_KEY`. Dimensions 3.8 through 3.10.
  > Indy (2026-08-13): "If we support the AGENTFLEET_API_KEY, then why do we need --token (let us remove this)." — context: followed by "go" and "add the above to your §3". The removal takes the piped-stdin seeding with it, since `resolveDirectToken` is one path serving both and the same reasoning applies to each.

- **Consult — `agt_t` scopes come from Clerk; this entry previously said otherwise and was wrong (Aug 13, 2026, §3, corrected same day).** The original entry argued that Clerk could not supply a tenant key's scopes because Clerk holds scopes on a **user** and an `agt_t` key has no user. That is false: Clerk carries `public_metadata` on **organisations** as well, and `auth/claims.zig` already reads `org_id`/`organization_id`, so the tenant-shaped home was there the whole time. The entry also contradicted this workstream's own `afc_` design, which refuses to snapshot scopes onto `core.cli_credentials` because that "would make it a second store of a fact Clerk owns" — and the proposed fix, a scopes column on `core.api_keys`, is exactly that second store. One scope authority, or none. The `created_by` objection survives only against a mechanism nobody proposed: resolving the *minting admin's* live user scopes would undo `240_api_keys.sql:23`'s deliberate non-foreign-key, and is not the shape. Which Clerk object carries a key's scopes — the tenant's organisation, or a machine identity the key mirrors — is the open design question, and belongs to its own spec.
  > Indy (2026-08-13): "I think for the agt_t the scopes must be from the clerk - which i said before and i dont know why you keep forgetting" — context: a standing decision that this agent had twice overwritten with its own analysis, in `docs/AUTH.md` and in this log. Both corrected; the decision now leads and the objection sits beneath it. The mechanism itself is not folded in here — only §5's removal of the competing claim readers is.

- **Scope decision — the legacy claim readers are removed here, Aug 13, 2026 (§5).** Raised by Indy while reviewing the `agt_t` correction: a suspicion that some flag or metadata path in the tenant/scope reading was legacy. It was, in three places, all in `auth/claims.zig`. Orly recommended a separate spec on the fold-test grounds that this *adds* rather than *completes* — M160_002 does not otherwise touch that file. Indy overrode.
  > Indy (2026-08-13): "I wanted to fold this change in this PR" — context: taken after Orly's recommendation to route it elsewhere. Same call shape as the Effect bump above, and the second time this session that a completes-vs-adds recommendation lost to Indy's timing.

- **Finding — `--token` writes a value the loader will refuse (Aug 13, 2026, DOCUMENT).** Dimension 1.4 shape-checks the credential file on read: `isPersistable` (`cli/src/services/credentials.ts:69`) accepts a well-formed `afc_` credential or an `agt_t` tenant key, nothing else. The direct-seeding path validates the supplied value against `/v1/me` and writes on success, and that write is *not* gated by the same check. A Clerk session token therefore still passes the ping, lands in `credentials.json`, and then reads back as logged-out on the next command — the failure surfaces one command after the mistake instead of at it. Recorded in `AUTH_DEVICE_LOGIN.md` as current behaviour. Whether the write path should refuse a non-persistable value up front is a user-facing call for Indy; no §3 or §4 dimension requires it, and the seeding path's documented inputs (a tenant key for a Continuous Integration runner) are unaffected.

- **Scope decision — Effect to the release candidate, Aug 13, 2026 (EXECUTE).** Raised on reviewing dependency currency: every other CLI dependency is at or within a caret of latest, but `effect` was pinned exact at `4.0.0-beta.103` while the 4.x line had moved to `beta.107` and opened an `rc` tag at `4.0.0-rc.108`. Orly recommended deferring to its own milestone after this Pull Request (PR) lands, on the grounds that beta-to-release-candidate is where Effect lands its final breaks and the branch was red at the time. Indy overrode the timing.
  > Indy (2026-08-13): "effect needs to go to rc" — context: taken with the follow-up selection to fold the bump into PR #600 rather than open a second stream. The bump proved a drop-in: `tsc --noEmit` clean, 1452 pass / 0 fail, coverage floor still 100%. It also removed five transitive dependencies from the published CLI — `find-my-way-ts`, `ini`, `multipasta`, `toml`, and `yaml` — which the release candidate no longer pulls into core.
