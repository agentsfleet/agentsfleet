...
- **Dimension 1.1** — the route table declares the method set of every route, and it equals what the router mounts, with the difference empty in both directions → Test `test_declared_verbs_match_the_mounted_router` — **DONE** (2 passed, all 81 identities × 5 verbs probed). `Route::verbs` is landed: all ten families declare their methods, total at both levels, and `Verb` moved to `route/verb.rs` because `route/mod.rs` was at its length cap. `TenantRoute::fleet_bundle_verbs` is subsumed by the general accessor.
...
**Deferral — `backfill` parity (Dimension 1.4).** `doctor` was kept at first, then dropped 2026-09-01 by the same standard — see M181_003's Discovery.
