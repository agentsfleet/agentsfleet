...
| `ui/packages/app/app/(dashboard)/w/[workspaceId]/library/` (`page.tsx`, `loading.tsx`, `actions.ts`, `copy.ts`, `lib/reads.ts`, `components/WorkspaceLibraryList.tsx`, `components/RemoveLibraryDialog.tsx`) | CREATE | The workspace Fleet library page with its loading state, server action, copy as named constants (RULE UFS), owned-entry list and removal confirmation. It mirrors `secrets/`, whose `lib/reads.ts` split keeps the page component off the fetch. |
...
| R2 | Both new operations exist, and the platform one is untouched (§2) | `grep -c 'list_workspace_library_entries\|delete_workspace_library_entry' public/openapi.json; grep -c delete_platform_fleet_library public/openapi.json` | `2` then `1` | P0 | |
