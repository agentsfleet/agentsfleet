...
- **Dimension 4.2** — the page records a p50 and a p95 for the Secrets navigation and for each of its two reads over a fixed sample of visits, and names which read is the slower → Test `test_secrets_stage_timings_are_measured_and_attached`. **Scope note from EXECUTE:** the navigation percentiles are implemented; the PER-READ pair is not. The fetch audit counts reads, it does not time them, and timing them means instrumenting `ui/packages/app/lib/api/client.ts` — a production transport file this spec's Files Changed does not carry. Recorded rather than smuggled in: naming which read is slower needs that instrumentation, and the decision to add it is the operator's.
...
- **Dimension 5.1** — measured: the Runners navigation returns, and its wait is attributed across the scope check, the credential resolve, each list attempt, and each retry sleep, with the observed attempt count recorded per navigation → Test `test_runners_navigation_returns_and_its_wait_is_attributed`. **NOT IMPLEMENTED at this commit,** for the same reason as 4.2's per-read half: per-stage attribution and the observed attempt count are transport-level facts, and `lib/api/client.ts` is outside Files Changed. One stage of it needs no instrumentation and is already settled by reading — `hasScope` resolves from session claims (`lib/auth/platform.ts:32`), so the scope check costs no upstream call and cannot be the wait.
