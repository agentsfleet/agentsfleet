...
- **Skill-chain outcomes**
  ...
  - `/review` adversarial pass found five real defects, four of them introduced by this workstream, all verified before fixing. The worst: the two new gate-test files matched no `unittest discover` pattern in the repository, so 43 tests — including the only coverage of a script that deletes Continuous Integration caches nightly — never ran. That is the same "enforcement in appearance only" defect this branch deletes a checker for. Discovery is now `check_*_test.py`, which raised the self-test count from 8 to 114. Also: `lint-governance` gained duplicate prerequisites from an over-broad `.PHONY` edit; the coverage lane's per-component exit code was never actually captured (errexit killed the subshell before the write) and an empty status file made the numeric test fail OPEN; the reachability checker shelled out to `zig build list-tests` without the cache environment, so it wrote Zig's default `.zig-cache` (693 MB in this worktree) and the lint job's new cache was a no-op.
