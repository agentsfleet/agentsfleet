...
| `src/runner/child_supervisor.zig` | EDIT | `establishEgress()` — resolve allowlist + `EgressScope.create` before fork, `.attachChild(pid)` after, `defer .destroy()`, fail-closed to `UZ-RUN-007`. |
...
| Stale nft objects | a crashed/half-torn-down lease leaves a permissive table/set/rule behind | teardown is idempotent and the per-lease table/chain/set are exec-id-named; setup deletes any pre-existing same-named objects before install (no inherited permissive state). Caught by `test_egress_teardown_idempotent`. |
