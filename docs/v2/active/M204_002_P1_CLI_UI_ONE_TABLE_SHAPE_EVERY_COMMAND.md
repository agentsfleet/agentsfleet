...
| `cli/src/program/entry/help-formatter.ts` (EDIT) · `cli/test/golden/help-no-color.txt` (EDIT) · `cli/test/entry-help-formatter.unit.test.ts` (EDIT) | EDIT | The help stops listing `--wizard`, which moves the golden fixture that pins the help byte-for-byte. The term match is the whole flag name rather than a prefix, so a flag called `--wizardly` keeps its help; `entry-help-formatter.unit.test.ts` carries that case. |
...
- **Dimension 1.1** — DONE — Every entity table renders name, then its identifier, then its domain columns, then `AGO`, and the order comes from one helper rather than from each call site → Test `test_every_entity_table_shares_one_column_order`
- **Dimension 1.2** — DONE — An age renders for each magnitude from seconds to years, a missing or unparseable timestamp renders the empty cell rather than a wrong number, and a timestamp in the future does not render a negative age → Test `test_ago_renders_every_magnitude_and_refuses_to_invent_one`
- **Dimension 1.3** — DONE — A table whose payload carries no timestamp still renders `AGO`, holding the empty cell, so absence reads as absence rather than as a column nobody added → Test `test_a_table_without_timestamps_still_renders_the_column`
- **Dimension 1.4** — DONE — No entity list expresses a column order any more: every one renders through the helper, and the only tables still naming their own columns are the label/value detail tables, which are a different shape and not an entity list → Test `test_every_entity_list_renders_through_the_helper`
- **Dimension 1.5** — DONE — Machine-readable output does not move: the reshape is a text-renderer change, so every `--json` answer is byte-identical to the pre-diff one → Test `test_json_output_is_unchanged_by_the_table_reshape`
...
- **Dimension 2.1** — DONE — `--log-level debug` makes the request line, the attempt ordinal and the retry verdict observable on the error stream, and `--log-level none` emits none of them, so the flag governs records that exist → Test `test_log_level_governs_records_that_exist`
- **Dimension 2.2** — DONE — The rendered help does not advertise `--wizard`, and the flag still builds a command for a caller who names it → Test `test_help_does_not_advertise_the_undesigned_flag`
- **Dimension 2.3** — DONE — `--completions` emits a script the target shell parses for each of the four shells it offers, and an unoffered shell is refused naming the four → Test `test_completions_emit_a_script_each_shell_parses`

### §3 — One fleet has a one-fleet command

`status` answers for the workspace and there is no way to ask about one fleet. **Implementation default (owner decision):** `status` keeps exactly its current shape and takes NO identifier, and the one-fleet read is a new `fleet show <fleet_id>` — the spelling every other single-resource read on this surface uses, and the one `schedule show` uses for its own pair. An optional argument on `status` was built first and withdrawn: it made one command mean two things, where the surface already had a word for the second. **Implementation default:** the read fetches that one fleet rather than filtering the list, because the list is paged and a filter would answer "no such fleet" for anything past the first page. **Implementation default:** the identifier runs `validateRequiredId` like every other identifier-taking verb, so a malformed one is refused before a request is issued.

- **Dimension 3.1** — DONE (unit; the acceptance walk is written and grades post-deploy) — `fleet show <fleet_id>` reports that one fleet, `status` keeps reporting every fleet in the active workspace and refuses a positional, and a malformed identifier is refused client-side without a request → Test `test_status_reports_one_fleet_or_the_whole_workspace`
...
The public output changes, so the docs branch is part of the work. **Implementation default:** `~/Projects/docs` work happens on `chore/m204-workspace-library-removal-changelog` — the branch M204_001 already cut there — and is never edited through this worktree. A separate branch was planned and dropped on contact with the repository: one milestone, one docs branch, matching the one Pull Request the fold produces.

- **Dimension 7.1** — DONE — Each convention is recorded in the doc that owns it: the list output shape in `user_flow.md` §Facts, and the tier every gallery surface names in `fleet_bundles.md` beside the union that makes two same-named entries possible. The changelog entry is deferred by the owner, quoted in Discovery → Test `test_architecture_and_changelog_record_the_table_shape`
- **Dimension 7.2** — DONE — The pages are revised on `chore/m204-workspace-library-removal-changelog`, M204_001's own docs branch rather than a second one: this workstream is folded into that spec and rides one Pull Request, so a second docs branch for one milestone would fragment what the fold joined → Test `test_docs_branch_carries_the_revised_pages`
...
| 7.1 | unit | `test_architecture_and_changelog_record_the_table_shape` | `docs/architecture/user_flow.md` §Facts carries a `List output shape` row naming name, identifier, domain columns, `AGO`; `docs/architecture/fleet_bundles.md` states that every gallery surface names an entry's tier and why (a union, two tiers, same names, different identifier shapes). No `<Update>` block is asserted — the changelog is deferred by the owner, whose words are in Discovery. |
| 7.2 | manual | `test_docs_branch_carries_the_revised_pages` | Procedure: in `~/Projects/docs` on `chore/m204-workspace-library-removal-changelog`, `git diff --name-only main...HEAD` lists `cli/agentsfleet.mdx`, `fleets/install.mdx`, `fleets/library.mdx` and `workspaces/managing.mdx` alongside M204_001's own three. Required person: Indy, who owns that repository's branch. Evidence: commit `2a394e1`, pushed, with the command output in Pull Request Session Notes. |
| 1.5 | unit | `test_json_output_is_unchanged_by_the_table_reshape` | Through the real binary against a mock API: `secret list`, `list`, `library` and `api-key list` with `--json` each parse as JSON, carry the server's raw instant, and contain neither the `AGO` header nor an age-shaped token. `workspace list --json` answers from the LOCAL store rather than the read it just made, so it asserts the same absence without the server instant — stated here rather than left to look like an omission. |
...
- **Deferrals** — one. > Indy (2026-09-21): "skip changelog update" — context: §7's changelog half, asked and answered while the architecture docs were being written. The entry is not written and no `<Update>` block is in the diff, so the pre-PR changelog criterion is red unless it is overridden or the branch is read as internal-only; that call is the owner's at CHORE(close), not this spec's to make. Everything else stands: Out of Scope names what was never in scope, with a reason for each, and the `schedule rm` versus `library remove` spelling is recorded there as an open owner decision rather than as deferred work.
