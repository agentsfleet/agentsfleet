...
- **Dimension 2.1** — valid installation handle → installation token with a near-future `expires_at` → `test_github_mints_installation_token`
- **Dimension 2.2** — uninstalled/revoked → typed `reconnect_required` (`UZ-GH-*`), no token → `test_github_revoked_reconnect`
- **Dimension 2.3** — the App key never appears in any `ExecutionPolicy`/`secrets_map`/log/frame → `test_github_key_never_leaves_broker`
