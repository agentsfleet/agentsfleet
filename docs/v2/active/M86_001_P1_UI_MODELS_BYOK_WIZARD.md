...
**Depends on:** M48_001 (self-managed provider — the action/data model this rides), M45_001 (structured vault credentials — the credential shape)
...
**Solution summary:** Reshape the Models page into an inline-progressive, gated wizard. Platform-managed stays a one-click escape hatch. The BYOK path becomes two numbered steps on one screen: **(1)** select an existing credential or create one inline via a structured provider-key form; **(2)** choose the model and Set active, locked until step 1 is satisfied. A paste-to-fill helper auto-detects the provider from the key prefix and prefills the catalogue default model. A one-line switch-safety note states new runs use the new provider while in-flight agents finish on their current one. No backend, no new endpoint — reuses `setProviderSelfManagedAction`, `resetProviderAction`, and `createCredentialAction`.
...
| File | Action | Why |
|------|--------|-----|
| `…/settings/models/page.tsx` | EDIT | Dual-state render: active-config card + "Change provider" when configured; wizard front-and-center when not. |
...
| `…/settings/models/components/ProviderKeyFields.tsx` | EDIT | Remove the `/credentials` dead-end Alert+Link; host the credential combobox + inline-create trigger + paste-to-fill. |
| `…/settings/models/components/InlineProviderKeyCreate.tsx` | CREATE | Structured provider-key create form (provider/api_key/model) → `createCredentialAction`; selects on success. |
| `…/settings/models/lib/detect-provider.ts` | CREATE | Pure: API-key-prefix → `{provider, defaultModel}` lookup from named-constant table + the public model-caps catalogue. |
...
| `…/app/tests/detect-provider.test.ts` | CREATE | Prefix detection + default-model prefill + unknown-prefix no-op. |
| `…/app/tests/e2e/acceptance/settings-models.spec.ts` | EDIT | e2e: inline BYOK setup with zero starting credentials. |

> No backend, schema, or `lib/api/*` change — `createCredential`, `setTenantProviderSelfManaged`, `resetTenantProvider` are reused as-is.
...
Replace the empty-vault dead-end with an inline structured provider-key create. The `/credentials` Alert+Link is removed entirely (NDC).
...
- **Dimension 2.2** — Zero-credential vault renders the inline create as the primary path; no `<a href="/credentials">` is present anywhere in the BYOK path. → Test `test_empty_vault_no_credentials_link`
...
Two delights accepted at CEO review. Both client-side, no backend.

- **Dimension 3.1** — Pasting an API key whose prefix maps to a known provider sets the provider and prefills the catalogue default model; an unknown prefix leaves both untouched (no error). → Test `test_paste_to_fill_provider_and_model`
...
```
detectProvider(apiKey: string): { provider: string; defaultModel: string } | null
  - pure; prefix table is a named constant; defaultModel resolved from the public model-caps catalogue
  - null when no prefix matches (caller leaves fields untouched)

Reused as-is (NO new endpoints/actions):
...
| Unknown key prefix | user pastes a non-matching key | `detectProvider` returns null; provider/model fields untouched; no error toast. |
| Empty vault | tenant has zero credentials | inline create is the primary path; NOT a dead-end link. |
...
1. The BYOK path renders **no** link or navigation to `/credentials` — enforced by `test_empty_vault_no_credentials_link` + `test_inline_create_no_navigation` (grep-style assertions on rendered output and router mock).
...
3. `detectProvider` is pure and table-driven (named constants, UFS) — enforced by `detect-provider.test.ts` and the lint/UFS gate.
...
| 2.2 | unit | `test_empty_vault_no_credentials_link` | zero credentials → rendered markup contains no `href="/credentials"`; inline form present. |
| 2.3 | unit | `test_inline_create_no_navigation` | success path → router push mock never called. |
| 3.1 | unit | `test_paste_to_fill_provider_and_model` | known prefix → provider+model filled; unknown prefix → both empty, no throw. |
...
- [ ] No `/credentials` link in the BYOK path — verify: `grep -rn 'href="/credentials"' ui/packages/app/app/\(dashboard\)/settings/models/ | head` → 0
...
```bash
# E1: no cross-page bounce in BYOK path
grep -rn 'href="/credentials"' "ui/packages/app/app/(dashboard)/settings/models/" | head && echo "CHECK(empty=pass)"
# E2: typecheck + lint
...
|---------|------|----------|
| `provider-key-no-credentials` test id + `/credentials` Alert/Link in BYOK path | `grep -rn 'provider-key-no-credentials' ui/packages/app \| head` | 0 (or moved intentionally) |
...
| No-bounce grep | `grep -rn 'href="/credentials"' …/models/` | {paste} | |
