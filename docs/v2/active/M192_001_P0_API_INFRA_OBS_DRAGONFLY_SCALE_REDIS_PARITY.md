...
| `rustd/crates/afd_datastore/**` (renamed from `afd_redis`), new `src/transport.rs`, `src/{lib,client,dedicated,kv,ready}.rs`, `src/hub/pump.rs`, `src/streams/{tail,once}.rs`, new `src/ready/partition.rs` | RENAME / EDIT / CREATE | One cluster transport behind `Redis`, hub on sharded pub/sub, dedicated reader on its own cluster connection, all-primaries scan, stream-tagged once key, partitioned readiness. |
| Every `afd_redis` reference: `rustd/crates/*/Cargo.toml`, `rustd/crates/**/*.rs`, `make/*.mk`, `docs/**`, `.github/**` | EDIT | `afd_redis` → `afd_datastore` (RULE ORP sweep); `REDIS_*` environment names unchanged. |
