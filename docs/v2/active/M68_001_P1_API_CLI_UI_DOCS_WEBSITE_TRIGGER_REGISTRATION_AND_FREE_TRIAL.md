...
| N6 | `public/openapi/paths/auth-identity-events.yaml` | NEW | Per REST_API_DESIGN_GUIDELINES §6 (tag 1:1 with resource). New tag `auth-identity-events`. Registered in `public/openapi/root.yaml`. Documents the `POST /v1/auth/identity-events/clerk` operation: request body shape (Clerk's payload, pass-through), Svix headers, 202 / 401 / 422 responses, idempotency on `svix-id`. File-cap-aware (≤400 lines per §6). |
| N7 | `public/openapi/root.yaml` | EDIT | Add the `auth-identity-events` tag declaration and the `$ref` to `paths/auth-identity-events.yaml`. |
...
(No new subcommands. Trigger registration runs in the install-skill via gh, not via a new
 zombiectl subcommand — keeps the surface lean.)
```
