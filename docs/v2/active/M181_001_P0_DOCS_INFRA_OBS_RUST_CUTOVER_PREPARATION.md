...
- **Dimension 2.3** — the daemon serves from the distroless image, proven by the parity lane's single-target mode against a container-hosted daemon; the release job's runtime check covers the CI side → Test `test_runtime_on_production_base` (graded with §4's lane) — **IN_PROGRESS (parked, see Discovery 2026-08-31)**
