...
2. **The executable contract is `docs/metrics.census.tsv`, not the markdown table.** The TSV carries the full parity surface — name, kind (counter/updowncounter/histogram/gauge), number type (u64/i64/f64), unit, temporality, label keys, histogram bounds, series policy — and the Rust registry test `include_str!`s it. The human census in `docs/architecture/observability.md` stays exactly as the Zig suite already pins it; a name-parity test asserts TSV names ≡ markdown census names, so the two files cannot drift and no Zig line changes.
...
8. **Series budgets are const-asserted.** `SeriesPolicy { Fixed, Runner { slots }, Shared { Cost } }` replaces the boolean flags; a `const fn` sums the declared cost-family budget and `const _: () = assert!(…)` fails the BUILD past 256, the direct equivalent of the Zig comptime arithmetic. Runner families raise the SDK's per-stream default cap explicitly so 4,096 admitted slots plus `_other` fit.
...
- **Dimension 3.1** — every family the TSV census declares is emitted under that exact name, kind, number type, unit, temporality and label keys; a family on one side only is named and fails; TSV names ≡ markdown census names → Test `test_metric_family_registry_parity`
...
| 3.1 | unit | `test_metric_family_registry_parity` | every TSV-census family emits under the same name, kind, number, unit, temporality, label keys; TSV names ≡ markdown census names |
...
| 3.6 | unit (compile-fail) | `test_cost_budget_const_assert` | a declaration set past the 256 cost budget fails the build, not the runtime |
