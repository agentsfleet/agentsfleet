...
| `ui/packages/app/components/layout/ShellFrame.tsx`; `ShellControls.tsx`; `shell-sidebar-state.ts` | CREATE | Server-rendered frame, narrow controls, and one independently testable collapse-state owner that replace `Shell.tsx`. |
...
| `ui/packages/app/components/domain/island-dynamic/intent-module-loader.ts` | CREATE | One resettable, independently testable loader owns cached preload, capability checks, failure, and retry for every closed tool. |
...
- **Dimension 4.1** — transitions and eligible optimistic updates stay responsive and roll back exactly → Test `test_react19_transitions_and_optimistic_rollbacks_are_stable`
- **Dimension 4.2** — navigation creates no duplicate live subscription or analytics lifecycle → Test `test_navigation_does_not_duplicate_live_subscriptions`
