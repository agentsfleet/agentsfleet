...
# M139_004: One pinned telemetry schema on one namespace
...
| `src/agentsfleetd/observability/metrics_render.zig` | EDIT | Render the fixed attribute-omission family for Prometheus; normalize every family it writes onto the `agentsfleet_` namespace. |
| `src/agentsfleetd/observability/metrics_memory.zig` | EDIT | Normalize the nine durable-memory families onto the `agentsfleet_` namespace. |
| `src/agentsfleetd/observability/metrics_memory_test.zig` | EDIT | Follow the durable-memory family rename. |
| `src/agentsfleetd/observability/metrics_sensitive_memory.zig` | EDIT | Normalize the four process-memory and erasure families onto the `agentsfleet_` namespace. |
| `src/agentsfleetd/observability/metrics_runner.zig` | EDIT | Follow the durable-memory family rename in its render path and comments. |
| `src/agentsfleetd/observability/metrics_runner_test.zig` | EDIT | Follow the durable-memory family rename. |
| `src/agentsfleetd/observability/metrics.zig` | EDIT | Follow the rename in the live-family assertions; the removed-feature absence locks keep their historical spellings. |
| `src/agentsfleetd/observability/metrics_counters_test.zig` | EDIT | Follow the SSE family rename. |
| `src/agentsfleetd/queue/redis_pool_test.zig` | EDIT | Follow the Redis-pool family rename; this is the only test that renders those families against a live Pool. |
| `src/agentsfleetd/http/handlers/fleets/backpressure_integration_test.zig` | EDIT | Follow the API and SSE family rename in scrape assertions. |
| `src/agentsfleetd/http/handlers/runner/memory_loop_integration_test.zig` | EDIT | Follow the durable-memory family rename in scrape assertions. |
| `src/agentsfleetd/http/route_template.zig` | CREATE | Own route-template resolution so HTTP spans carry `http.route` without the raw request target. |
| `src/agentsfleetd/fleet/renewal_edge_test.zig` | EDIT | Follow the committed-renewal-debit return shape. |
| `src/agentsfleetd/fleet/renewal_integration_test.zig` | EDIT | Follow the committed-renewal-debit return shape. |
| `src/agentsfleetd/fleet/concurrency_renew_test.zig` | EDIT | Follow the committed-renewal-debit return shape. |
| `src/agentsfleetd/observability/otel_metrics_aggregate.zig` | EDIT | Carry the derived model-attribution budget into the aggregation ceiling. |
| `src/build/fixtures.zig` | EDIT | Drop the Grafana dashboard fixture registration with the dashboards themselves. |
...
| `deploy/grafana/runner_fleet.json` | DELETE | Its queries were live, but it moves with the rest of dashboard ownership rather than leaving one orphaned artefact behind. |
| `playbooks/operations/observability/03_dashboard.sh` | DELETE | Imported `deploy/grafana/*.json` and provisioned a PostgreSQL datasource for the nonexistent `usage_ledger`; both premises are gone. |
| `playbooks/operations/observability/001_playbook.md` | EDIT | Drop the dashboard import, panel verification, and PostgreSQL datasource steps; repoint the scrape check at an unconditionally-rendered family. |
| `playbooks/operations/observability/002_grafana_setup.md` | EDIT | Drop the dashboard import and `usage_ledger` datasource; state the one-namespace rule and the Collector dependency. |
| `playbooks/operations/observability/02_prometheus.sh` | EDIT | Probe a live family instead of the dead `agent_runs_created_total`, and fail rather than warn when it returns nothing. |
| `src/agentsfleetd/observability/semantic_schema_test.zig` | CREATE | Reject schema drift, stale names, and namespace splits as a real test. The former `audits/signal-routing.sh` was retired for pinning prose and source literals by exact string match; render the exposition body and check every family in it instead of grepping for text. |
...
| `docs/architecture/scaling.md` | EDIT | Four capacity-knob rows tell an operator to watch a renamed family; a runbook naming a dead metric fails the sweep exactly like a dashboard would. |
| `docs/architecture/data_flow.md` | EDIT | The SubscriptionHub redial narrative cites the renamed SSE reconnect family. |
| `src/agentsfleetd/config/runtime_loader.zig` | EDIT | Doc comment naming the renamed in-flight-streams gauge. |
...
| String Literals Are Always Constants (UFS) | yes | one registry owns every live wire name, unit, and fixed value |
...
- **Existing exporter:** `otel_metrics_payload.zig` — preserve bounded delta aggregation and replace its schema atomically.
...
- **Dimension 1.1** — ✅ **DONE** — every live name and unit is classified against the pinned source, while false aliases such as client-token usage for aggregate run totals are rejected → Test `test_semantic_registry_matches_pinned_sources`
- **Dimension 1.2** — ✅ **DONE** — logs, traces, and metrics serialize identical standard resource identity, valid JSON escaping, and no fabricated optional instance → Test `test_otlp_resources_share_semantic_identity`
...
The `/metrics` exposition carries two prefixes today: `metrics_render.zig` emits about twenty `fleet_*` families (API in-flight, Server-Sent Events (SSE), Redis pool, signup funnel, worker liveness, trigger totals) plus the durable-memory and sensitive-memory families, while `metrics_runner.zig`, `metrics_otel.zig`, and `metrics_trace.zig` emit `agentsfleet_*`. One process must expose one namespace. Normalize every family to the `agentsfleet_` prefix in the same atomic cutover as the OTLP schema, since both change the names an operator queries and neither has an external consumer before `2.0.0`. No dual emission and no recording-rule alias.

The rename is **family-scoped, not textual**: `fleet_id`, log event names, `EventKind` tags, PostHog event names, SSE frame types, and the Redis consumer group share the prefix and must not move. Only a name that reaches the Prometheus writer as a family is in scope. `fleet_triggered_total` becomes `agentsfleet_fleet_triggered_total` rather than a bare prefix swap, so the noun being triggered survives — matching the established `agentsfleet_runner_failures_total` shape.

The Grafana dashboards are **deleted** rather than repointed. `agent_run_breakdown.json` was dead in all six panels; `agent-observability.json` queried only superseded OTLP names; `runner_fleet.json` was live but moves with the rest of dashboard ownership. Assert the surviving drift properties in the semantic-schema test rather than a new make target — and do not reintroduce a prose-pinning shell audit. A dashboard-query drift test belongs to the dashboard workstream, against files that exist.

- **Dimension 4.1** — ✅ **DONE** — the attribute-omission family renders on every scrape with its fixed attribute and reason label sets, so bounded model attribution stays visible rather than silent → Test `test_attribution_omissions_are_visible_in_the_scrape_body`
- **Dimension 4.2** — ✅ **DONE** — source, fixture, and architecture contain no superseded live name or private metric key after the atomic cutover, under both the OTLP spelling and its Prometheus translation → Test `test_semantic_schema_has_no_live_legacy_aliases`
- **Dimension 4.3** — ✅ **DONE** — every Prometheus family rendered by the daemon shares the `agentsfleet_` prefix, and no renderer source declares a `fleet_`-prefixed family literal → Tests `test_prometheus_families_share_one_namespace` and `test_no_renderer_source_declares_a_superseded_family_name`
...
|------|-------|--------------------------------------------------------|
| Upstream convention moves | GenAI Development names change after the pinned commit | Live output remains on the pinned schema until a reviewed spec updates source, fixture, and architecture together. |
...
| Mixed namespace or schema | the exposition or payload carries an old and a new name together | The semantic-schema test rejects every superseded live name and every family outside the namespace; no compatibility alias ships. |
...
7. Superseded names have zero non-historical live hits, and every rendered family shares one namespace; no dual emission or compatibility alias exists.
...
| 4.1 | unit | `test_attribution_omissions_are_visible_in_the_scrape_body` | the omission family renders while every value is zero, carrying each fixed attribute key and each omission reason. |
| 4.2 | unit | `test_semantic_schema_has_no_live_legacy_aliases` | every rejected name is absent from the pinned OTLP fixture and, under its Prometheus translation, from the rendered scrape body. |
...
| 4.3 | unit | `test_no_renderer_source_declares_a_superseded_family_name` | no renderer source holds a `"fleet_`-prefixed literal, covering the Redis-pool families whose render needs a live Pool. |
...
Grep the four superseded OTLP metric names, private metric keys, and obsolete resource serializers across non-historical source, fixtures, audits, and architecture; every live hit must be replaced in the same commit.

Four artefacts are deleted outright, each because its every consumer or every source was already dead:

- `deploy/grafana/agent_run_breakdown.json` — four Prometheus panels named families no source emits; three PostgreSQL panels queried `billing.usage_ledger`, which no migration in `schema/` creates (the only repository hit is a comment in `002_vault_schema.sql`). Dead in 6 of 6 panels.
- `deploy/grafana/agent-observability.json` — every panel queried a superseded OTLP name.
- `deploy/grafana/runner_fleet.json` — live queries, deleted for ownership rather than deadness.
- `playbooks/operations/observability/03_dashboard.sh` — imported the deleted dashboards and provisioned a datasource for the nonexistent table.

An unused-migration sweep ran alongside the dashboard deletion: every table created across all 34 files in `schema/` has at least one non-test production reference in `src/`, so no migration is dropped.
...
- Parallel old/new emission, compatibility aliases, or recording-rule aliases.
- Provisioning a collector or changing Grafana credentials. Dashboard authoring is a separate workstream; this one deletes the dead artefacts and ships none.
...
1. **Successful user moment** — an operator queries one namespace and sees model-attributed token, duration, and credit data whose names and units match the schema documented in the payload.
...
3. **Optimal-way check** — one atomic schema registry is the direct path; renaming only the exposition would leave the payload's units and meanings false.
4. **Rebuild-vs-iterate** — refactor the schema boundary while retaining the bounded exporter and report-derived ownership.
5. **What we build** — one registry, shared resources, truthful metric descriptors, bounded attributes, semantic span keys, one Prometheus namespace, fixtures, audits, and architecture.
6. **What we do NOT build** — runner telemetry infrastructure, provider-call transport, workspace metric labels, dual emission, or replacement dashboards.
7. **Fit with existing features** — compounds M139_003's bounded exporter, accepted-report fence, and exact renewal metering without destabilizing money writes.
...
10. **Confused-user next step** — inspect the payload's schema URL and the omission counter, then use Postgres for exact workspace-level cost.

## Decomposition & alternatives (patch vs refactor)

- **Chosen shape:** one cross-cutting workstream owns registry, payloads, emit sites, fixtures, audits, and architecture because these are one wire schema and must switch atomically.
- **Alternatives considered:** an exposition-only rename was rejected because payload units and meanings would remain false; a generic OpenTelemetry Software Development Kit (SDK) dependency was rejected because the repository already has a bounded native Zig exporter and needs no second lifecycle owner.
...
- **Source finding (Jul 24, 2026, during §4)** — `agent_run_breakdown.json` was dead in **six** panels, not the four this spec recorded. Its three PostgreSQL panels query `billing.usage_ledger`, and no migration in `schema/` creates that table; the sole repository hit is a comment at `schema/002_vault_schema.sql:40`. The spec's repoint plan assumed that half was live, and the workspace dimension all four Prometheus panels wanted is forbidden on metrics by this spec's own Invariant 2 — so there was no live family to repoint them to. Surfaced to Indy rather than decided.
- **Decision (Jul 24, 2026)** — Indy directed deleting the dashboard artefacts outright and moving dashboard authoring to its own milestone:
  > Indy (2026-07-24): "Delete the dashboard and drop any unused migrations. I will create a separate milestone for the grafana dashbaord" — context: `agent_run_breakdown.json`, in answer to the six-dead-panels finding above.

  The agent then kept `agent-observability.json` in scope, reasoning that §1–§3 had already renamed the OTLP metrics and leaving it stale would ship the "Mixed namespace or schema" failure mode. Indy corrected that scope:
  > Indy (2026-07-24): "I thought the json … are something we will get rid off?" / "why are we still keeping it" — context: `deploy/grafana/agent-observability.json`.
