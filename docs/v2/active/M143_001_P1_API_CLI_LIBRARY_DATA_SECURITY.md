...
| `scripts/test-infra-ports.sh` | CREATE | Assigns this worktree three stable host ports from a hash of the Compose project name and writes them to `.env`, which Compose reads automatically so `make` and a bare `docker compose exec` cannot disagree. |
...
- **Dimension 3.2** — every path stays within the numeric table → Test `test_library_read_resource_bounds` — **IN_PROGRESS.** `observability/library_read_counters.zig` provides the tallies and names §3's maxima as constants in one place. **Not yet done:** the counters are not yet wired into the read paths, so nothing is measured. Only the tenant registry path exists to instrument; the global-models and Fleet rows of the table cannot be measured until those handlers are built, and writing the test first would assert vacuously.
