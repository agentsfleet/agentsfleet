...
- **Dimension 2.2** — approval webhook resolves the gate exactly as the M178 surface does (one continuation row) → Tests `afd_api/tests/webhook_approval_wall.rs` (10 tests — the wall: BOTH fail-closed paths, wrong key, tampered body, a proof bound to another instant, the stale replay, the window edge, and that no refusal reaches the store). The RESOLUTION half — gate resolved, one continuation row, identical to the bearer surface — needs a live Postgres → Test to write: `afd_api/tests/integration_approval_webhook.rs`
...
| 2.2 | unit (wall) + integration (resolution) | `webhook_approval_wall.rs` (10 tests) + `integration_approval_webhook.rs` (TO WRITE) | every refusal happens before the store is touched; then an accepted delivery → gate resolved + one continuation row, identical to the bearer surface |
...
| 3.1 | integration | `integration_store.rs` (14) + `integration_fleet_schedules.rs` (2) | create, bounds, fleet-scoped reads and the HTTP surface; `integration_sync.rs` (TO WRITE) carries the drifted-store-vs-upstream half |
...
| 3.3 | integration (negative) | `error_surface.rs` (9) + `integration_fence.rs` | upstream down during CRUD → typed retryable error that leaks no vendor status, its reason kept on the row and reclaimable; `integration_sync.rs` (TO WRITE) carries the repair |
...
| 4.2 | unit | `connector_callback_route.rs` (10) + `afd_connector/tests/connect_verify.rs` (7) | every route refusal before the secret read; and forged, foreign-secret, cross-connector, expired and BYSTANDER states each refused as their own reason. `integration_connector_callback.rs` (TO WRITE) carries the spent slot |
| 4.3 | unit + integration | `events.rs` (6 tests) + `connector_events_route.rs` (4 tests) + `integration_connector_events.rs` (TO WRITE) | the handshake decision; no inbound surface refused before the vault; the two cap bands answer differently; an unsigned handshake is not echoed; then valid event accepted, bad signature rejected |
