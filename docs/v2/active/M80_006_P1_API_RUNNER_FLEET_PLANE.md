...
- ✅ **§3 runner client `renew()`** (`91011f7e`): `control_plane_client.zig` `renew()` — 2xx → `renewed(deadline)`, definitive 4xx → `terminal(status)` (kill child), transport/5xx → error (retry next tick, fail-safe). Runner builds; fmt/lint clean.
...
- §3 runner-side driving (**the delicate core, not yet started**): the supervisor read-loop must wake periodically (a tick < `RENEWAL_WINDOW_MS`), call `cp.renew` inside the window, track a mutable renewed deadline, emit a synthetic keepalive on a quiet-but-alive child, and kill on `terminal`. Plan: first extract `child_supervisor` tests to a sibling (frees line budget — it's at 347/350), add a `RenewHook { ctx, tick(now, deadline) → keep|extend|terminate }` to `readResult` (supervisor stays HTTP-agnostic; window/HTTP logic lives in `loop.zig`, unit-testable with a fake client). `cp.renew()` is the building block, now in place.
