...
| `src/agentsfleetd/db/index_usage_integration_test.zig` | EDIT | Planner-fitness helper disables bitmap scans (deterministic among sibling fleet-prefixed indexes after 039); gains slot 040's shape-and-plan proof with the row-accumulation derivation |
...
| `schema/040_runner_leases_operator_read_indexes.sql` | CREATE | The §2 lease read's two access paths — `(runner_id, created_at, id)` for the page and `(fleet_id, event_id, fencing_token)` for the per-row reclaim probe (REVIEW, measured) |
