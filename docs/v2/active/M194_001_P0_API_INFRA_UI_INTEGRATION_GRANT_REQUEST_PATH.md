...
**Baseline evidence:** pending — measured before the Pull Request
...
| File | Action | Why |
|------|--------------|-----|
| `schema/836_fleet_approval_gates_grant_card.sql` · `schema/embed.zig` · `rustd/crates/afd_db/src/migration.rs` | CREATE/EDIT | One actionable card per nullable `active_grant_id`, using a unique index without status literals; the column is declared in slot 810 and released on resolution or expiry. Registered at the two sites `test_migration_list_matches_schema_directory_and_zig` pins — the version IS the slot number (RULE MIG). |
...
| `audits/doc-read.sh` · `AGENTS.orly.md` · `.oracle/orly.json` · `~/Projects/docs/changelog.mdx` | EDIT | `orly update` to 0.10.7, folded in on Indy's call — the same shape M193_001 recorded when a harness update rode a stream. The upgrade is this session's own finding shipped back: `check` now counts cited sections and names a bulk second, and it flags this stream's nine-in-one-second record on sight (Discovery, Sep 09). Run `--no-hooks`: `core.hooksPath` is shared across worktrees and points at the base checkout, and the two hooks are the repository's own committed files. A user-visible change: a fleet that needs a grant now asks for one. Own branch, per Operational defaults. |
| `playbooks/operations/acceptance/001_playbook.md` | EDIT | The walk the milestone changed. An install now always raises a grant card, so answering it becomes step 4 and the rest renumber; a killed fleet reads `KILLED` on the wall, so step 8 stops expecting `failed`. Amended because the expectations were stale against shipped behaviour, not to make this walk pass. |
| `playbooks/operations/acceptance/verdicts/3fbf3d9c…md` · `3fbf3d9c…/*.jpg` | CREATE | The signed verdict for the graded build and the four assertion-named images (§4.3). |
| `ui/packages/app/tests/e2e/acceptance/{approvals-page,operator-journey}.spec.ts` | EDIT | Both asserted `getByLabel("Pending approval gates")`; `approvals/page.tsx:78` deliberately renamed the region to `Approval gates` because it now holds every state. The stale assertions turned `deploy-dev / acceptance` red on this build. |
| `schema/837_fleet_approval_gates_inbox_page.sql`, `embed.zig`, `afd_db/src/migration.rs` | CREATE/EDIT | `(workspace_id, created_at, id)` in its OWN slot: the version IS the slot number and a recorded one never re-applies, so adding it to 810 would reach fresh databases only and every deployment already carrying gates would keep the sequential scan. Registered at the two sites `test_migration_list_matches_schema_directory_and_zig` pins. |
