...
- **Dimension 7.5** — DONE — measured on 50,075 seeded gates, the page read is 8 shared buffers and one index search; the read-time join it replaced was 157 buffers and fifty → Test `the_page_reads_newest_first` (the same read, re-measured; both plans in the PR body)
- **Dimension 7.6** — DONE — one file imports the identity provider's server SDK, down from twenty-seven, and the boundary is named after what the app consumes (a bearer) rather than after what the vendor supplies (a session) → Test `answers every caller in a request with the same token`, one of eight over `credential`, `requireCredential` and `claims`, each failing closed
...
| 7.5 | integration | `the_page_reads_newest_first` | The same read under `EXPLAIN (ANALYZE, BUFFERS)` on 50,075 gates → 8 buffers, one index search. The read-time `LEFT JOIN core.users` it replaced measured 157 buffers and fifty searches |
| 7.6 | unit | `answers every caller in a request with the same token` | Three concurrent `credential()` calls → one token, same value. Paired with `redirects to sign-in when there is not` and `answers null rather than throwing when the provider is unavailable`, so both guards fail closed |
