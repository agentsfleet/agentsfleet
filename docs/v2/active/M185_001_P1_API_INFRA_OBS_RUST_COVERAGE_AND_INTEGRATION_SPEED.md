...
| R2 | Equivalent live coverage runs are materially faster (§1, §3, §6) | the PR's `make test-integration-rustd` job duration, against the same job on the preceding PR | median wall ratio at most `0.80`, equal denominator | P0 | |
...
- **Benchmark venue — CI, not this machine (user decision).** R2's before-measurement needs the old lane on the baseline commit, which needs a second worktree with its own `target/`. The m185 worktree's target alone is 40 GB against 33 GB free, and the spec puts automatic deletion of normal developer Cargo caches out of scope, so the sibling worktrees' 20 GB is not available to reclaim. The user chose CI: measure `after` as this PR's own `make test-integration-rustd` job duration and take `before` from the same job lane on the preceding PR. Same runner shape, same reset contract, no local cache destroyed. `scripts/rustd_lane_benchmark.py` stays the comparison tool; only the venue moved.
