...
- **Benchmark venue — CI, not this machine (user decision).** R2's before-measurement needs the old lane on the baseline commit, which needs a second worktree with its own `target/`. The m185 worktree's target alone is 40 GB against 33 GB free, and the spec puts automatic deletion of normal developer Cargo caches out of scope, so the sibling worktrees' 20 GB is not available to reclaim. The user chose CI: measure `after` from this PR's own Rust coverage job and take `before` from the same job on the preceding PR. Same runner shape, same reset contract, no local cache destroyed.

  **Total job duration will not serve, and the reason is in this branch.** The job is named `test-integration-rustd` but runs `make test-coverage-rustd`, and this branch removed its `rustd/target` cache — correctly, because restoring instrumented binaries across revisions lets `cargo-llvm-cov` find two source revisions in one report. The baseline commit's workflow cached that directory. So `before` is a warm build and `after` is a cold one, and their ratio grades cache policy rather than this milestone; it would come out worse for a reason unrelated to the work.
