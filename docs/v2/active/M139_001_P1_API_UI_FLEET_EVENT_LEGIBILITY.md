...
| `ui/packages/{design-system,app}/**` pagination + connection surfaces | EDIT/CREATE | In-session Indy-directed expansions (Discovery §Scope expansions): unified paginator (`Pagination`, `DataTable*`, `EventsList`, `BillingUsageTab`), URL-cursor paging (`use-url-cursor-pages.ts`, `cursor-trail.ts`; paging Server Actions removed), §9 connection UX (`FleetConnectionIndicator`, `FleetConnectionNotice`), live-event cap. |
| `src/agentsfleetd/queue/redis_pool_test_support.zig` | EDIT | PingFake shutdown deadlock fix — Indy-directed in-session (Discovery). |
...
- **Dimension 3.1** — DONE — System-role rows render the compact variant; payload disclosure remains reachable → Test `test_system_rows_render_compact`
- **Dimension 3.2** — DONE — Operator sends, optimistic/failed states, and fleet replies render exactly as before (regression) → Test `test_conversation_rows_unchanged`
- **Dimension 3.3** — DONE — Change-proposal rows render an action `Badge` and link out only when the payload carries a link → Test `test_change_proposal_badge_and_conditional_link`
...
- **Harness defect diagnosed and fixed in-session (Jul 22, 2026, off-diff)** — one `make test-integration` run wedged for 23 minutes at 0% CPU with zero Postgres connections held. A `sample(1)` of the test binary named it exactly: `PingFake.shutdown` set `stop`, called `pokeAccept`, then `Thread.join`ed an accept-loop thread parked in a blocking `readv` on an already-accepted connection — `pokeAccept` only wakes the **accept** socket, so it could never interrupt an in-flight read. Indy directed the fix here rather than as a separate stream: > Indy: "fix it here. keep it driving". `shutdown` now half-closes the read side of the live connection (`stream.shutdown(io, .recv)`) under a mutex-guarded `live` handle, so the parked read returns EOF, the serve loop exits, and the serving thread's own `close` stays the only one. `src/agentsfleetd/queue/redis_pool_test_support.zig` added to Files-Changed under this direction.
...
- **Scope expansions (in-session, Indy-directed) (Jul 22–23, 2026)** — the diff grew well beyond the authored Files-Changed table; every expansion was Indy's live direction, not agent scope creep, so each is authorized and recorded here with its quote. (a) **Pagination consistency + dead-space + "Load more" removal** — > Indy: "Just provide a consistent user experience" / "The load more is even more bad, it just loads everything, and i need to scroll" / "the statement that says sorted: loaded must be removed". Unified all eight console tables on one numbered-page control; `DataTable`/`Pagination`/`DataTableModel`/`DataTableView` in the design system + `EventsList`/`BillingUsageTab` touched. (b) **URL-driven paging refactor** — > Indy: "I prefer the large refactor". Cursor state moved into the URL trail, fetch moved to the Server Components; `use-url-cursor-pages.ts` + `cursor-trail.ts` added, the two paging Server Actions + their tests removed (RULE NDC/ORP). (c) **§9 live-connection UX** — > Indy: "Yes agree continue". Added as a spec Section; `FleetConnectionIndicator` + working indicator, `FleetConnectionNotice` reduced to the offline-only band. (d) **Live-event cap** — > Indy: "Yes". `MAX_LIVE_EVENTS` cap in `fleet-stream-registry.ts`. React 19 / react-virtual / `useOptimistic` were evaluated and declined with measured reasons (chat seeds 20 rows, table pages at 25, reconcile is SSE-echo not action-return) — recorded so the non-adoption is a decision, not an omission.
