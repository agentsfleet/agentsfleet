...
- **Harness defect diagnosed (Jul 22, 2026, off-diff — NOT fixed here)** — one `make test-integration` run wedged for 23 minutes at 0% CPU with zero Postgres connections held. A `sample(1)` of the test binary named it exactly: `queue/redis_pool_test.zig:288` ("Pool hard max_active cap times out a saturated acquire") calls `PingFake.shutdown`, which sets `stop`, calls `pokeAccept`, then `Thread.join`s an accept-loop thread parked forever in a blocking `readv`. In `keep_open` mode `acceptLoop` only re-checks `stop` *between* reads, and `pokeAccept` wakes the **accept** socket — it cannot interrupt a read already in flight on an accepted connection. That test deliberately saturates the pool and parks conns, which is precisely the state that leaves a socket open with no further command coming, so the deadlock is seed-timing dependent, not rare. Plausibly the same root cause as the `--listen` note above, but that is **not** asserted — only the deadlock was directly observed. Left untouched: `src/agentsfleetd/queue/**` is outside this spec's Files-Changed scope. Owner call needed on whether it lands here or as its own fix (a read deadline, or closing accepted streams from `shutdown`).
