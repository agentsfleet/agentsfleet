...
- **§2 EXECUTE finding (agent-discovered, no ack needed — bug fix, not scope deferral):** `secretExistsForTenant` (§1, `state/tenant_model_entries.zig`) checked only the raw `secret_ref` against `vault.secrets.key_name`, but a dashboard-created secret (`POST /workspaces/{ws}/secrets`) is stored under the `fleet:`-prefixed key (`credential_key.allocKeyName`) — the same raw-vs-prefixed split `secret_probe.loadSelfManagedJson` already bridges for `tenant_model_selection`. Left as-is, POST `/v1/tenants/me/models` would 404 (`UZ-MODELS-002`) on every real dashboard-created secret. Fixed in the same diff (`secretExistsForTenant` now takes an allocator and tries raw-then-prefixed, mirroring the existing fallback); added a state-layer unit test for the prefixed case plus integration coverage via the real POST-secret path in Dimension 2.3.
