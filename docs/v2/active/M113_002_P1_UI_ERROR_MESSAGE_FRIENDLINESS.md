...
- **Dimension 4.6** — `CODE_MAP` shrinks to the entries that can never be (fully) backend-authored: `UZ-AUTH-401` (client-minted in `with-token.ts` when no Bearer token exists) and `UZ-AUTH-022` (ALSO client-minted, separately, by `lib/actions/require-scope.ts`'s fail-fast pre-check — caught mid-migration: removing it from `CODE_MAP` would have regressed that specific path back to a raw `"Operator scope required: {scope}"` string; it also got a backend `eu()` entry for the real-HTTP-403 path, which is harmless since `presentError` checks `CODE_MAP` first regardless). The two dead/unreachable entries found during migration (`UZ-VALIDATION-001`, `UZ-CRED-003` — no backend code with either string exists anywhere in `src/`) were **deleted, not left in place** — Indy's call on resume, reversing this Dimension's original "not in scope to remove" decision — with the matching test assertion updated in `lib/errors.test.ts` → Test `test_code_map_shrunk_to_unmappable_codes`

**Migration note (found during Dimension 4.1's audit, resolved on resume):** two of the 15 pre-existing `CODE_MAP` entries — `UZ-VALIDATION-001` and `UZ-CRED-003` — did not exist anywhere in the backend registry and were never client-minted; dead entries that could never fire. Originally left in place pending a scope decision; deleted this resume session per Indy's explicit go-ahead.
...
- **Resume-session consult:** asked whether to delete the two dead `CODE_MAP` entries (`UZ-VALIDATION-001`, `UZ-CRED-003`) flagged in Dimension 4.6/the migration note above, or leave them documented as intentional. Indy: delete them. Done — `ui/packages/app/lib/errors.ts` and the matching assertion in `lib/errors.test.ts`.
