...
6. `schema/020_fleet_metering_periods.sql` (per-slice `charged_nanos` + `created_at`, the accurate drain ledger) and `schema/011_fleet_execution_telemetry.sql` (the receive fee + the `(workspace_id, fleet_id, recorded_at DESC)` index the stage-scope join + prune ride). `renewal_settle.zig:127-134` is why the telemetry stage row cannot be timed: its `ON CONFLICT DO UPDATE` accumulates nanos but never advances `recorded_at`.
...
The pure half first: `dollarsToNanos(f64) i64` (× `tenant_billing.NANOS_PER_USD`, rounded, saturating — the parser already bounds inputs to `(0, 1000]` daily and `(0, 10000]` monthly, so the product cannot approach `i64` max, and the saturation is a belt-and-braces guard rather than a live path); `covers(FleetBudget, Spend) Verdict` returning `.ok` / `.day_exceeded` / `.month_exceeded`. Then `spendForFleet(pool, workspace_id, fleet_id, now_ms) ?Spend` — one statement that `UNION`s the per-slice stage drainage (`metering_periods.charged_nanos` by `created_at`, joined to telemetry for fleet scope + `recorded_at >= floor - MAX_RUNTIME` pruning) with the accurately-timed receive fee, then two `FILTER`ed sums; `null` on any DB fault so the caller can fail open explicitly rather than by coincidence. `startOfUtcMonthMillis` lands in `clock.zig` as a pure function of `now_ms` (RULE TIM — the tests pass a fixed `now`, never read the clock).
...
- **Dimension 1.8** — spend is timed by DRAIN, not by run start: a run started outside the rolling 24h that drained inside it IS counted; a multi-slice run counts each slice in the window it drained → Tests `integration: spend is timed by DRAIN, not by run start (the P1 regression)`, `integration: a multi-slice run counts each slice in the window it drained` → **DONE** (both fail on the pre-fix telemetry-`recorded_at` query — valid red-green)
...
- **Dimension 2.5** — a fleet whose `config_json` declares no `budget` renews normally; only a *declared-but-malformed* budget refuses → Tests `integration: fetch_budget_and_spend_admits_a_fleet_that_declares_no_budget`, `integration: a declared-but-malformed budget still refuses (fail closed)` → **DONE** (both green against live Postgres; this is the regression `service_token_splits_wire_test` caught)
...
- **Dimension 4.3** — the architecture doc names the per-fleet ceiling and its two gates → verified by `make lint-all` (`check_architecture_doc.sh`) and the R8 grep → **DONE** (billing_and_provider_keys.md §5.1 names both gates; `grep -c daily_dollars` → 3)
...
| Long run drains across a window boundary | a multi-renewal run drains money over up to `MAX_RUNTIME` (12h) | spend is summed per drain slice by `metering_periods.created_at`, not by the run's start — each slice counts in the window it actually drained in, so the daily/monthly ceilings measure real spend (the `/review` P1) |
...
- **Alternatives considered:**
  ...
  - *(d) Compute spend from `fleet.metering_periods` instead of `core.fleet_execution_telemetry`.* **Chosen — forced by the `/review` finding.** The first cut took the opposite call (sum the telemetry stage row, avoid the join) and it was wrong: the stage row's `recorded_at` is pinned at run start, so it cannot answer 'how much drained in the last 24h'. `metering_periods` carries the accurate per-slice `created_at`; the join to telemetry for `fleet_id` scope is the price of correctness, kept cheap by pruning the telemetry scan to `recorded_at >= floor - MAX_RUNTIME` (a slice cannot drain more than one run-length after its start).
...
- **Pre-CHORE(close) `/review` — a P1 the author missed (Jul 10 2026).** The mandatory adversarial review (Claude critical pass + an independent money-path subagent) found that the windowed spend query summed the accumulating `fleet_execution_telemetry` stage row, whose `recorded_at` is pinned at the first renewal and never advanced by the settle's `ON CONFLICT DO UPDATE` (`renewal_settle.zig:127-134`). So the windows bucketed a run's whole spend at its START time: a long run (up to `MAX_RUNTIME` = 12h) could age its spend out of the rolling-24h window up to 12h early (~2x daily breach), and a run straddling a UTC month boundary escaped the new month's `monthly_dollars` entirely. Near-zero skew for short runs; real for the long runaway a budget exists to stop. **Decision (Indy, Jul 10 2026): fix now — sum the per-slice ledger.** The rewrite sums `metering_periods.charged_nanos` by `created_at` + the accurately-timed receive fee, joined to telemetry for scope and pruned by `recorded_at >= floor - MAX_RUNTIME`. Two red-green regression tests (§1.8) pin it. The review also surfaced two cheaper hardenings, both fixed in the same pass: a `budget` key holding JSON `null`/a scalar was failing CLOSED (killing in-flight runs of a fleet that declared no ceiling) — now admits, distinct from a botched budget OBJECT which still fails closed (Dimension 2.5's twin); and `terminalReason` now gates on HTTP 402 so a stray `UZ-RUN-015` in another status can't be mislabeled `budget_breach`.
