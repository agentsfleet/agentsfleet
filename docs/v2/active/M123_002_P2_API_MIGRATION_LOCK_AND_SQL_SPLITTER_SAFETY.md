...
- **Skill-chain outcomes** —
  - `/write-unit-test`: diff ledger resolved; two coverage gaps it surfaced (the `applySqlStatements` validate call site and `applyOneMigration`'s success path were unreachable on the migrated shared DB) became the two fresh-bookkeeping integration tests; a Postgres-fidelity gap it surfaced (arbitrary dollar-tag length cap) was removed.
  - `/review` (local, pre-push): adversarial subagent found 6 issues; all fixed in the same branch with regression tests — (1) CONFIRMED stack-overflow via `next()` recursion on empty-statement floods (~50k `;` segfaulted `validate` itself) → loop; (2) CONFIRMED double-quoted identifiers unlexed (`"it's"` falsely rejected valid SQL; `"idx--tmp"` silently merged statements) → `"…"`/`""` tracking + `UnterminatedQuotedIdentifier`; (3) `E'…'` escape strings mis-lexed → standalone-`E` prefix detection with backslash escapes; (4) identifier-adjacent `$` diverged from Postgres maximal munch (`a$b$`) → preceding-byte guard; (5) correlated failures query would 42P01 when `schema_migration_failures` exists without `schema_migrations` → fall back to the uncorrelated probe (any row is then genuinely unresolved); (6) audit-stash tests could CASCADE-drop `audit` on a non-disposable database → `current_database()` guard (`agentsfleetdb` only) + `errdefer` fixture cleanup.
