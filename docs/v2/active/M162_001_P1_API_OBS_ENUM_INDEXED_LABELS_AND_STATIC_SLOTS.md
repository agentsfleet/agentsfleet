...
| `cli/test/acceptance/fixtures/cli.js` | EDIT | Scope addition approved mid-milestone: per-spawn credential store, so acceptance specs stop reading the developer's real login |
| `cli/test/json-contract.test.ts` | EDIT | Same isolation for the in-process `runCli` path, which `io.env` does not cover |
...
- **Review finding on this milestone's own diff, Aug 12, 2026** — case-folding provider normalization made two spellings export as one series while `cardinality.admitModel` still digested the caller's raw spelling, so one wire series spent two attribution slots and could starve a distinct model of its budget. Fixed by keying the budget on the canonical spelling; the regression test was verified to fail with the fix reverted. A second finding in the same pass: `PROVIDER_BASE` located its block by searching the literal table for a provider spelling, which would bind to the wrong index the day a runner label spelled the same string. Now derived from the block layout.
- **Scope addition, approved Aug 12, 2026** — Indy chose "Fix it here" over deferring, for the credential leak in the Command-Line Interface (CLI) acceptance harness. `composeEnv` allowlists the child environment and passed `HOME` through, so the credential store resolved to the developer's real login and every spec asserting unauthenticated behaviour failed on a machine where someone had logged in — while staying green on a clean Continuous Integration (CI) runner. Two harnesses needed isolating: the subprocess path through `composeEnv`, and the in-process `runCli` path, whose `io.env` never reaches `loadCredentials`. Verified at 1429 pass / 0 fail against the real credential store.
- **Deliberately not taken** — threading the environment through `loadCredentials` so `runCli`'s `io.env` covers the credential store. That is production code in `cli/src/`, well past the test-harness fix that was approved, and belongs with the CLI validation work.
