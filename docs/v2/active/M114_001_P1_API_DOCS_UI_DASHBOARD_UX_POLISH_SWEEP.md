...
| `ui/packages/app/components/layout/Shell.tsx` | EDIT (additional) | §16: the "Model rates" sidebar nav icon changes from `CpuIcon` to `CoinsIcon` so it no longer duplicates the tenant Models nav icon |
| `ui/packages/app/app/(dashboard)/admin/models/components/ModelsView.tsx` | EDIT | §16/§17: page/`PageTitle` "Models" → "Model rates"; adopts `PageHeader`(trimmed one-line subtitle) + a flex row with `<SectionLabel>Manage model rates</SectionLabel>` and the Create-model-rate trigger; `AddModelDialog` removed from inside `PageHeader` |
| `ui/packages/app/app/(dashboard)/admin/models/components/CatalogueList.tsx` | EDIT (additional) | §17/§18: custom "Model rates · N models" `<p>` removed (the `SectionLabel` replaces it); renders via the design-system `DataTable` (Provider / Model / Context / Rates / Actions) replacing the custom grid; the §10 delete `ConfirmDialog` gating preserved |
| `ui/packages/app/app/(dashboard)/admin/models/components/AddModelDialog.tsx` | EDIT | §19: trigger/submit/`DialogTitle` "Add model"/"Add model to catalogue" → "Create model rate"; description reworded to explain a model rate prices a model per-token for the team and makes it selectable as the platform default (rates per 1M tokens) |
...
| `ui/packages/app/tests/admin-models-ui.test.ts` | EDIT | §16-§20: asserts "Model rates" title, the `PageHeader` + `<SectionLabel>Manage model rates</SectionLabel>` structure, the `CatalogueList` `DataTable` (delete confirm preserved), the "Create model rate" dialog copy, and the reworded `PlatformDefaultCard` intro (16.1/17.1/18.1/19.1/20.1) |
| `ui/packages/app/tests/app-pages.test.ts` | EDIT (additional) | §16: adds the `CoinsIcon` lucide-react mock and asserts the "Model rates" nav entry uses it (not `CpuIcon`) |
...
`RunnerList` renders a custom `divide-y` grid with a standalone sort dropdown (`RUNNER_SORTS`/`SORT_LABELS` `Select`), out of step with API Keys/Secrets/Model-rates, which all render through the design-system `DataTable`. Unifying on `DataTable` aligns the operator surfaces and drops the bespoke grid. **Interactive column sorting is explicitly DEFERRED (Out of Scope) per Indy — this section only unifies the table structure;** the standalone sort dropdown is removed and newest-first (`-created_at`) stays the backend default order.
...
### §16 — Model-rates page identity + distinct nav icon (added mid-review, live request)

The operator Model-rates page (`/admin/models`) titles itself "Models" and its sidebar nav item reuses the same `CpuIcon` as the tenant-facing Models nav — so the two Models entries are visually indistinguishable. This section renames the page to "Model rates" and gives its nav entry a distinct rates icon.

- **Dimension 16.1** — the page and its `PageTitle` read "Model rates" (not "Models"); the "Model rates" sidebar nav icon in `components/layout/Shell.tsx` changes from `CpuIcon` to `CoinsIcon` so it no longer duplicates the Models nav icon → Test `test_model_rates_title_and_nav_icon`

### §17 — Model-rates page adopts the API-Keys/Secrets structure (added mid-review, live request)

Like §13 for Runners, `ModelsView` renders its `AddModelDialog` trigger inside `PageHeader` and `CatalogueList` carries its own custom "Model rates · N models" `<p>`. Adopting the shared PageHeader + SectionLabel pattern removes the bespoke count line and aligns with the sibling operator pages.

- **Dimension 17.1** — `ModelsView` renders a `PageHeader` (description a trimmed one-line subtitle, e.g. "Every model your team can run, priced per token — the platform default runs for users without their own key.") + `PageTitle` "Model rates", followed by a flex row pairing `<SectionLabel>Manage model rates</SectionLabel>` with the Create-model-rate trigger; `AddModelDialog` no longer renders inside `PageHeader`, and `CatalogueList`'s custom "Model rates · N models" `<p>` is removed (the `SectionLabel` replaces it) → Test `test_model_rates_manage_pattern`
...
### §19 — Create-model-rate dialog copy (added mid-review, live request)

The Create-model dialog reads "Add model" (trigger/submit) / "Add model to catalogue" (title) — the "Add" verb for a mint-a-new-resource action, and copy that never explains what a model rate is. This section renames it to "Create model rate" and rewords the description to explain the concept.

- **Dimension 19.1** — the trigger, submit button, and `DialogTitle` read "Create model rate" (was "Add model" / "Add model to catalogue"); the description explains that a model rate prices a model per-token for the team and makes it selectable as the platform default (rates per 1M tokens) → Test `test_create_model_rate_dialog_copy`
...
| 16.1 | unit | `test_model_rates_title_and_nav_icon` (`admin-models-ui.test.ts` + `app-pages.test.ts`) | the page/`PageTitle` read "Model rates" (no "Models" title); the "Model rates" sidebar nav entry uses `CoinsIcon`, not `CpuIcon` (asserted against the `app-pages.test.ts` nav mock) |
| 17.1 | unit | `test_model_rates_manage_pattern` (`admin-models-ui.test.ts`) | `ModelsView` renders `PageHeader`(trimmed subtitle) + `PageTitle` "Model rates" + `<SectionLabel>Manage model rates</SectionLabel>` alongside the Create-model-rate trigger; no `AddModelDialog` inside `PageHeader`; no "Model rates · N models" `<p>` in `CatalogueList` |
...
| 19.1 | unit | `test_create_model_rate_dialog_copy` (`admin-models-ui.test.ts`) | the trigger, submit button, and `DialogTitle` read "Create model rate" (no "Add model" / "Add model to catalogue"); the description mentions per-token pricing / platform default / rates per 1M tokens |
...
| R11 | §16-§20 Model-rates identity + nav icon, restructure, DataTable, create-dialog copy, platform-default card copy | `make test-coverage-all` (Dimensions 16.1-20.1) | exit 0 | P1 | ✅ app coverage EXIT 0, 1197 tests, 100% thresholds — new CatalogueList/ModelsView/AddModelDialog fully covered |
...
| custom "Model rates · N models" `<p>` + custom grid markup in `CatalogueList.tsx` (§17/§18 — replaced by `SectionLabel` + `DataTable`) | `grep -n "· .* models\|divide-y" ui/packages/app/app/"(dashboard)"/admin/models/components/CatalogueList.tsx` | 0 matches |
...
- **Deferrals:** §1-§12 are implemented and locally verified. §13-§20 (Runners + Model-rates B2 batch) were added mid-review and are IN_PROGRESS in this same tree/PR (#481) — R10/R11 grade them before CHORE(close). Interactive Runner-list column sorting (§15) is a Kishore-directed Out-of-Scope deferral, as is the http/loopback item. S4/acceptance-e2e is graded by PR CI, not deferred (see the rubric's S4 disposition — an environment constraint, not a dropped scope item).
