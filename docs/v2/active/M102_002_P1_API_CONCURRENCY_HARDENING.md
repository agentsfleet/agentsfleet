...
- **Use-after-free found → reclamation upgraded to hazard pointers (Indy + Orly/CTO, Jun 28 2026); §1 PARKED.** The §1.3 stress test (tight-loop publishes + preemptible readers under `std.testing.allocator`) **crashed with a real use-after-free**. Diagnosis: count-based bounded retention frees by *publish count*, but an OS-preempted reader stalls in *wall-clock time* — a reader parked mid-critical-section is lapped by `K` publishes and dereferences freed memory. Bounded retention is safe for the real workload (publishes ~6h apart) but **cannot** pass an adversarial no-UAF proof (the precondition "read critical section ≪ publish interval" is false under preemption). CTO ruling, Indy-approved: **switch reclamation to hazard pointers** — per-reader single-slot announce-then-validate; the writer scans all slots before freeing. Reads stay **contention-free** (per-core slot + a fence, no shared cache-line bounce), so the scaling win survives, and it is **preemption-safe** → passes the hammer test. **This amends Out-of-Scope**, which had ruled hazard/epoch reclamation out on the now-falsified timing argument. §1 is parked pending the hazard-pointer rebuild; the bounded-retention dead-end is stashed (`git stash`: "m102-002 §1 bounded-retention WIP"). §2–§6 + the perf benches follow the rebuilt §1.
- **Dashboard UX polish folded into this branch/PR (Indy, Jun 28 2026).** Indy surfaced v2-launch dashboard bugs + UX asks and directed they ship in THIS milestone/PR, not a separate spec/PR. Verbatim: _"I dont need a new milestone"_ · _"fix in this milestone/pr"_ · _"no new PR, it must be fixed in this"_. Shipped as six commits on `feat/m102-concurrency-hardening`, ahead of and independent of the parked concurrency work:
  - `5c68810d` copy/UX — billing "events"→"charges"; custom-secrets header/empty-state/helper/reference copy + a docs link; other-provider copy; install caption; **removed the install "Continue" skip-gate** (it created un-runnable fleets, server-424).
  - `ad1e5b0c` Clerk dark mode — `@clerk/themes` dark baseTheme so Account → Security device icons + Profile inputs are visible (app is dark-only; lib/theme.ts).
  - `22a6ad4a` route loading — shared `RouteLoading` (exact title + spinner) on every dashboard route; added the missing Billing loader (was flashing "Workspace").
  - `598f52a8` fleets empty state → full-width inline template gallery (reuses `InstallEntry`); **removed the duplicate "Install fleet" button**.
  - `4f8201eb` data-driven "why connect" — per-credential `reason` seeded on `core.fleet_bundle_templates` (schema/028 teardown-edit) → served by `list.zig` (passthrough `std.json.Value`) → rendered by the install gate when every unmet credential has a reason. Indy chose the **backend-data path over a UI map**, informed it was a schema migration: _"Full backend migration now"_.
