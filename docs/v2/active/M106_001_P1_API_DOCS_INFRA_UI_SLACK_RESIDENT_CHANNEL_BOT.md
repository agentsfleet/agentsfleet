...
| `schema/032_core_connector_channels_fleet_idx.sql` + `schema/embed.zig` + `src/agentsfleetd/cmd/common.zig` | CREATE/EDIT | index `core.connector_channels(fleet_id)` for the per-report reverse lookup; embed + register in `canonicalMigrations()`. |
| `src/agentsfleetd/cmd/serve.zig` + `src/agentsfleetd/http/handlers/common.zig` | EDIT | boot + graceful-stop the `connector:outbound` worker; `Context` Slack-API-base override (test seam, mirrors the OAuth `token_endpoint` override). |
