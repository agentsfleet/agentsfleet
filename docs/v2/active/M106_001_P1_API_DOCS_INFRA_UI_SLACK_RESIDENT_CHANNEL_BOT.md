...
- **Dimension 3.2** — ⏳ IN_PROGRESS — convergence to one fleet is **implemented** (name-unique 23505 → `resolveExistingByName`; binding ON CONFLICT), but the dedicated **concurrent** first-mention test is not yet written → Test `test_resident_fleet_concurrent_first_mention` (next step).
