...
`STAGE_PLATFORM_NANOS` becomes `RUN_NANOS_PER_SEC` — a single per-second rate, the **same value for both postures**. `STAGE_SELF_MANAGED_NANOS` retires entirely. A `runFee(elapsed_ms)` helper computes `(elapsed_ms / 1000) × RUN_NANOS_PER_SEC` (ms→s with the existing `@divTrunc` discipline). The charge is `RUN_FEE + TOKEN_COST` under platform, `RUN_FEE` only under self_managed. Why: time is billed at one neutral rate; tokens are the only posture-dependent axis. **Implementation default:** elapsed time is the **active** wall time of the slice (`now − last_metered_at`), never a flat per-renewal constant — the elapsed deltas sum to exactly total runtime.

- **Dimension 2.1** — a renewal at ~20s elapsed charges `20s × RUN_NANOS_PER_SEC`, not `30s × …` → Test `test_run_fee_is_elapsed_not_flat`
- **Dimension 2.2** — `RUN_NANOS_PER_SEC` is identical for platform and self_managed → Test `test_run_rate_same_both_postures`
- **Dimension 2.3** — platform charge = run fee + token cost → Test `test_platform_charges_run_plus_tokens`
- **Dimension 2.4** — self_managed charge = run fee only; tokens recorded, not charged → Test `test_self_managed_charges_run_only_records_tokens`
