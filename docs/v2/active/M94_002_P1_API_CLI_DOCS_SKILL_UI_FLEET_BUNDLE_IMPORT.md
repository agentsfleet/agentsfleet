...
| `src/agentsfleetd/http/handlers/fleets/create.zig` | EDIT | Accept optional `bundle_id`; generate default trigger when `TRIGGER.md` is absent; emit canonical Fleet response fields; accept an optional slug-safe `name` override (multi-instance — same bundle, many fleets per workspace). |
...
| `tests/fixtures/{fleetbundle,webhooks,runner}/**` and `build_fixtures.zig` | CREATE | Minimal in-repo fixtures for GitHub PR reviewer and Zoho Recruit outreach scenarios, wired via named `@embedFile` imports (replaces the retired `samples/` tree). |
...
- Jun 20, 2026 — Design (shotgun): a **four-variant** wireframe shotgun ran — A (Terminal Ledger), B (Guided Wizard), C (Split Workbench), D (Gallery-first) — hand-built in the real design-system tokens (dark, Commit Mono, mint pulse). Only the gstack designer's *image-generation* path was unavailable (no `OPENAI_API_KEY`); the variant exploration itself happened. **Variant D (Gallery-first) was approved.** Formal approval artifact: `~/.gstack/projects/agentsfleet-usezombie/designs/fleet-new-20260620/approved.json` (`approved_variant: "D — Gallery-first"`); the chosen markup is the `<!-- D — Gallery-first -->` block in the sibling `wireframes.html`. Decisions carried into the build: template cards lead `/fleets/new` as the hero; GitHub URL + Paste demoted to a secondary "or start from source" strip; Upload descoped; create sends `{bundle_id}`; the first-run Dashboard card surfaces the same template cards.
...
- Jun 21, 2026 — **§5.1 + §5.2 dashboard UI landed (gallery-first, Variant D).** `/fleets/new` now leads with template cards (`InstallSourceSelector` + `TemplateCard`); GitHub import + Paste are demoted to the "or start from source" strip; `?template=<id>` deep-link preselects a template (`InstallFleet` orchestrator, React 19 `useActionState`/`useTransition`). `BundlePreview` (§5.2) frames required credentials structurally as workspace service credentials — the preview copy keeps them distinct from the tenant model provider — and routes every missing one to the workspace credentials flow (`/credentials`), never `/settings/models`. The runtime credential allowlist/classifier was dropped as drift-prone dead code (Indy-flagged) in favour of a tested `missingCredentials` set-difference (`lib/fleet-credentials.ts`); create is gated until every required credential is present. Dashboard `FirstInstallCard` surfaces the same template cards (`Use template → /fleets/new?template=<id>`); the placeholder first-run illustration was retired (the approved wireframe leads with templates). `importBundleAction` added; create reuses `installFleetAction({bundle_id})`; catalog reads are deduped per render via React `cache()` (`listFleetTemplatesCached`). The unused `getBundleSnapshot` client method was removed (RULE NDC — its `?bundle_id=` resume deep-link is deferred). Stale app-side-fetch comments in `fleet-bundles.ts`/`types.ts` were corrected to the landed server-side fetch behaviour (`ImportBundleRequest.skill_markdown` is now optional). Tests: new `fleets-install-flow` (15 cases, incl. the §5.1 selector + §5.2 routing assertions), `fleet-credentials`, and `fleet-bundles` client; updated `fleets-actions`/`fleets-routes`/`dashboard-overview` and the e2e `install-ui` fixture (paste now sits behind "Paste SKILL.md instead"). Verified green: `tsc`, `oxlint --type-aware`, the design-token + UI-substitution audits, and `test:coverage` (934 pass / 0 fail, 100% statements/branches/functions/lines). Perf: workspace resolution is already deduped via React `cache()` (no change needed); React Compiler adoption and an app-wide fetch-revalidate strategy were each split into their own pending spec (Indy-decided).
- Jun 21, 2026 — **Multi-instance install (name override) — Indy-requested in-PR.** A single bundle should back many fleets in one workspace (each its own webhooks/cron, pointed at a different repo); the only blocker was `uq_fleets_workspace_id_name`. Architecture validated end-to-end: the runner leases by `content_hash` (name-agnostic — `service.zig:150`; SKILL.md rides the lease as `instructions`, TRIGGER.md as `policy`, support files pull via `GET /v1/runners/me/bundles/{hash}`), and nothing reads `config_json->>'name'` downstream (list/get read the `name` column), so overriding the persisted name is safe. `create.zig` `CreateBody` gains an optional `name`, validated via `validateSkillName` (slug-safe, ≤64) and applied after the SKILL/TRIGGER name-match check — absent ⇒ the SKILL.md name (today's behaviour). The gallery preview adds an optional Name field (`BundlePreview`); `InstallFleetRequest`'s bundle arm carries `name?`; `installFleetAction({bundle_id, name})`; OpenAPI create-request updated. Tests: UI `passes an operator-typed name override to create` (+ blank ⇒ no override); integration `fleet create name override enables same-source multi-instance` + `rejects an invalid name override` (DB-gated → CI). Verified green: `zig build`, both Linux cross-compiles, `make lint-zig` (fmt/zlint/pg-drain/test-depth/line-limit), `make check-openapi`, `make harness-verify`, daemon 1288 unit; app `tsc`/`oxlint`/design-token/100% coverage (937). Two adjacent runtime ideas Indy raised — (a) content-hash + provider prompt-cache the static SOUL so unchanged SKILL/TRIGGER isn't re-input per trigger, (b) Fleet Memory (durable per-fleet memory auto-applied to recurring problems; Indy notes NullClaw already flushes memory entries to Postgres) — are out of scope here and parked for their own specs.
