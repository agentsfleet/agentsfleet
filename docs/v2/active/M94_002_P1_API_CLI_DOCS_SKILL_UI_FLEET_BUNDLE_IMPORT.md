...
| `ui/packages/app/lib/github/**` and `ui/packages/app/lib/api/fleet-bundles.ts` | CREATE | App-side `github.com`-only bundle fetcher (SSRF guards, SHA pin, size/count caps, error taxonomy) + bundle snapshot/list API client. |
...
- Server-side GitHub fetching in `agentsfleetd`. The dashboard fetches `agentsfleet/skills` (template) or a public `github.com/owner/repo` URL app-side and posts assembled Markdown to the existing snapshot endpoint; moving the fetch into Zig is a later increment.
