...
| an HTTP handler, `public/openapi.json`, or a `route_*` file | `docs/REST_API_DESIGN_GUIDELINES.md` — Quick Checklist; §1–§5 (URL/method/body/response/error), §6 (the generated document and its gates), §7 (route registration), §8 (handler interface), §10 (pre-PR gates). |
